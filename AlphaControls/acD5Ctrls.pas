...
{$ENDIF}
  ...
  if Assigned(BoundLabel) then BoundLabel.HandleOwnerMsg(Message, Self);
end;
  ...
  with SkinData.SkinManager.ConstData.ComboBtn do begin
    ...
    if FCommonData.SkinManager.IsValidImgIndex(GlyphIndex) then
      DrawSkinGlyph(FCommonData.FCacheBmp,
                    ...
                    (Height - ButtonHeight) div 2), Mode, 1, FCommonData.SkinManager.ma[GlyphIndex],
  ...
  end;
{
  ...
  if glIndex > -1 then begin
    if ControlIsActive(FCommonData) then
      c.C := FCommonData.SkinManager.gd[FCommonData.SkinIndex].HotColor
    else
      c.C := FCommonData.SkinManager.gd[FCommonData.SkinIndex].Color;
