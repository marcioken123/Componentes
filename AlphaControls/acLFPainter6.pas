...
begin // +
  if Skinned then begin
    if DefaultManager.CommonSkinData.SmallCheckBox[cbChecked] >= 0
      then Ndx := DefaultManager.CommonSkinData.SmallCheckBox[cbChecked]
      else Ndx := DefaultManager.CommonSkinData.CheckBox[cbChecked];
    if Ndx > -1 then
...
begin // +
  if Skinned then begin
    if AIsDisabled then
      Result := BlendColors(DefaultManager.GetActiveEditFontColor, DefaultManager.GetActiveEditColor, 127)
    else
...
begin
  if Skinned then
    Result := BlendColors(DefaultManager.Palette[pcBorder], DefaultManager.GetActiveEditColor, 127)
  else
...
begin
  if Skinned then
    Result := BlendColors(DefaultManager.GetActiveEditColor, DefaultManager.GetActiveEditFontColor, 50)
  else
...
begin
  if Skinned then
    Result := BlendColors(DefaultManager.GetActiveEditColor, DefaultManager.GetActiveEditFontColor, 200)
  else
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      ...
      CI.Ready := False;
      CI.Bmp := nil;
      PaintItem(i, CI, True, State, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefaultManager.CommonSkinData);
...
begin
  if Skinned then begin
    ...
    case ACheckState of
      cbsUnchecked:
        if DefaultManager.CommonSkinData.SmallCheckBox[cbUnChecked] >= 0 then
          i := DefaultManager.CommonSkinData.SmallCheckBox[cbUnChecked]
        else
          i := DefaultManager.CommonSkinData.CheckBox[cbUnChecked];
      ...
      cbsGrayed:
        if DefaultManager.CommonSkinData.SmallCheckBox[cbGrayed] >= 0 then
          i := DefaultManager.CommonSkinData.SmallCheckBox[cbGrayed]
        else
          i := DefaultManager.CommonSkinData.CheckBox[cbGrayed];
    end;
...
begin
  if Skinned then begin
    case AButtonKind of
      cxbkComboBtn: begin
        i := DefaultManager.CommonSkinData.ComboBtn.SkinIndex;
        sSect := s_ComboBtn;
      end
      else begin
    ...
    case AButtonKind of
      ...
      cxbkSpinUpBtn: glIndex := DefaultManager.CommonSkinData.ScrollBtns[asTop].GlyphIndex;
      cxbkSpinDownBtn: glIndex := DefaultManager.CommonSkinData.ScrollBtns[asBottom].GlyphIndex;
      cxbkSpinLeftBtn: glIndex := DefaultManager.CommonSkinData.ScrollBtns[asLeft].GlyphIndex;
      cxbkSpinRightBtn: glIndex := DefaultManager.CommonSkinData.ScrollBtns[asRight].GlyphIndex;
    ...
    if glIndex > -1 then begin
      DrawSkinGlyph(TmpBmp, Point((WidthOf(R) - DefaultManager.ma[glIndex].Width) div 2,
        (HeightOf(R) - DefaultManager.ma[glIndex].Height) div 2), State, 1, DefaultManager.ma[glIndex], MakeCacheInfo(TmpBmp));
    end
    else
      case AButtonKind of
...
begin
  if Skinned then begin
    ...
    if DefManager.IsValidSkinIndex(i) then begin
      ...
      CI := MakeCacheInfo(TmpBmp);
      PaintItem(i, CI, True, State, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefManager.CommonSkinData);
...
begin
  if Skinned then begin
    ...
    ACanvas.FillRect(ACaptionRect, BlendColors(DefaultManager.GetGlobalColor, DefaultManager.Palette[pcBorder], 180));
    ACanvas.FrameRect(ACaptionRect, DefaultManager.Palette[pcBorder], 1, aBord);
    ACanvas.FrameRect(ACaptionRect, BlendColors(DefaultManager.GetGlobalColor, DefaultManager.Palette[pcBorder], 100), 1, cxBordersAll - aBord);
  end
  else inherited;
end;
...
begin
  if Skinned then begin
    if AState in [cxbsDefault, cxbsNormal, cxbsHot, cxbsPressed] then begin
      ...
      i := PaintSection(TmpBmp, s_ColHeader, s_Button, State, DefaultManager.CommonSkinData, ABounds.TopLeft, DefaultManager.GetGlobalColor);
  ...
  if Skinned then begin
    if AState in [cxbsDefault, cxbsNormal, cxbsHot, cxbsPressed] then begin
      ...
      if (w > 0) and (h > 0) then begin
        ...
        aBmp := CreateBmp32(R);
        PaintSection(aBmp, s_ColHeader, s_Button, State, DefaultManager.CommonSkinData, R.TopLeft, DefaultManager.GetGlobalColor);
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      ...
      CI.Ready := False;
      CI.Bmp := nil;
      PaintItem(i, CI, True, 0, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefaultManager.CommonSkinData);
      BitBlt(ACanvas.Handle, ARect.Left, ARect.Top, TmpBmp.Width, TmpBmp.Height, TmpBmp.Canvas.Handle, 0, 0, SRCCOPY);
      FreeAndNil(TmpBmp);
    end;
  end
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      ...
      PaintItem(i, MakeCacheInfo(BgBmp), True, 0, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefaultManager.CommonSkinData);
      BitBlt(ACanvas.Handle, ARect.Left, ARect.Top, TmpBmp.Width, TmpBmp.Height, TmpBmp.Canvas.Handle, 0, 0, SRCCOPY);
      FreeAndNil(TmpBmp);
      FreeAndNil(BgBmp);
    end;
...
begin
  if Skinned then begin
    ...
    if AIsNextButton then begin
      ...
      if Ndx >= 0 then PaintItem(Ndx, MakeCacheInfo(cBmp), True, GetState[AState], Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefaultManager.CommonSkinData) // Transparency is not needed
    end
    else begin
  ...
  begin
    with DefaultManager do
      if IsValidImgIndex(CommonSkinData.ScrollBtns[Side].GlyphIndex) then
        ...
        C := gd[CommonSkinData.ScrollBtns[Side].SkinIndex].Props[mini(State, ac_MaxPropsIndex)].FontColor.Color;
      ...
      if AHorizontal then begin
        ...
        if APart = sbpPageUp then
          with Scrolls[asLeft] do begin
            PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True,
                          State, Rect(0, 0, Bmp.Width, h), MkPoint, Bmp, DefaultManager.CommonSkinData);
            BitBlt(NewBmp.Canvas.Handle, 0, 0, w, h, Bmp.Canvas.Handle, BtnSize, 0, SRCCOPY);
          end
        else
          with Scrolls[asRight] do begin
            PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True,
                          State, Rect(0, 0, Bmp.Width, NewBmp.Height), MkPoint, Bmp, DefaultManager.CommonSkinData);
        ...
        if APart = sbpPageUp then
          with Scrolls[asTop] do begin
            PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True,
                          State, Rect(0, 0, w, Bmp.Height), MkPoint, Bmp, DefaultManager.CommonSkinData);
        ...
        sbpThumbnail: begin
          ...
          with Sliders[False] do begin
            PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                          CI, True, State, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), MkPoint, TmpBmp, DefaultManager.CommonSkinData);
            FreeAndNil(BGBmp);
  ...
  begin
    with DefaultManager do
      if DefaultManager.IsValidImgIndex(CommonSkinData.ScrollBtns[Side].GlyphIndex) then begin
        ...
        C := GetGlobalFontColor;// gd[CommonSkinData.ScrollBtns[Side].SkinIndex].Props[mini(State, ac_MaxPropsIndex)].FontColor.Color;
        gR := Rect(0, 0, TmpBmp.Width, TmpBmp.Height);
        if (State > 1) and ButtonsOptions.ShiftContentOnClick then
          OffsetRect(gR, 1, 1);
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      ...
      PaintItem(i, CI, True, State, R, MkPoint, ACanvas.Handle, DefaultManager.CommonSkinData);
      FreeAndNil(Bmp);
    end;
    ACanvas.Font.Assign(AFont);
    ACanvas.Font.Color := DefaultManager.gd[i].Props[integer(State <> 0)].FontColor.Color;
  ...
  if Skinned then begin
    ...
    if not AVertical then begin
      ...
      if DefaultManager.IsValidSkinIndex(i) then begin
        ...
        PaintItem(i, CI, False, 0, Rect(0, 0, Bmp.Width, Bmp.Height), MkPoint, Bmp, DefaultManager.CommonSkinData);
