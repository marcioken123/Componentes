...
{$IFDEF GDIPLUS_PNG}
  acgpUtils,
{$ENDIF} // GDIPLUS_PNG
  sGraphUtils, acntUtils, sAlphaGraph, sDefaults, sVCLUtils;
  ...
  if (Png.ImageProperties.ColorScheme in [csIndexed]) and (Length(Png.Transparency) > 0) then begin // Adding of indexed transparency
    ...
    if InitLine(Png, Pointer(S0), DeltaS) and InitLine(Bmp, Pointer(D0), DeltaD) then
      for Y := 0 to Png.Height - 1 do begin
        ...
        for X := 0 to min(Png.Width, Length(Png.Transparency)) - 1 do
            ...
            if InitLine(Png, Pointer(S0), DeltaS) then
              if Png.ColorManager.SourcePixelFormat <> pf24Bit then
                for Y := 0 to Png.Height - 1 do begin
                  ...
                  for X := 0 to Png.Width - 1 do begin
                    D[X].C := 0;
                    D[X].A := MaxByte - Png.ColorManager.FGammaTable[S[X]];
                  end;
                end
...
function PaethPredictor(a, b, c: Byte): Byte; {$IFDEF D2005}inline;{$ENDIF}
var
  p, pa, pb, pc: Integer;
begin
  p := a + b - c;
...
procedure TPNGGraphic.LoadFromResourceName(Instance: THandle; const ResName: String);
var
    ...
    with Stream, FImageProperties do begin
      ...
      try
        ...
        if Compression = ctLZ77 then begin
          ...
          GraphicExError(gesUnsupportedFeature, ['The compression scheme is', acPngExt]);
          ...
          if IsChunk(IDAT) then begin
            ...
            if IsChunk(PLTE) then begin
              if (FHeader.Length mod 3) <> 0 then
                GraphicExError(gesInvalidPalette, [acPngExt]);
...
{$ENDIF}
    ...
    if Stream.Size = 0 then
      GraphicExError(gesInvalidImage, [acPngExt])
    else begin
      Stream.Position := 0;
      Stream.ReadBuffer(Bmf, SizeOf(Bmf));
      if Bmf.bfType <> $4D42 then // Maybe bitmap?
        GraphicExError(gesInvalidImage, [acPngExt])
      else begin
        Stream.Position := 0;
  ...
  repeat
    ...
    if FDecoder.ZLibResult = Z_STREAM_END then begin
      if (FDecoder.AvailableOutput <> 0) or (FDecoder.AvailableInput <> 0) then
        GraphicExError(gesExtraCompressedData, [acPngExt]);

      Break;
    end;

    if FDecoder.ZLibResult <> Z_OK then
      GraphicExError(gesCompression, [acPngExt]);
    ...
    0:
      if BitDepth in [1, 2, 4, 8, 16] then
        with ColorManager do begin
          ...
          GraphicExError(gesInvalidColorFormat, [acPngExt]);
    2:
...
begin
  ...
  if FCurrentCRC <> FileCRC then
    GraphicExError(gesInvalidCRC, [acPngExt]);
end;
{$ENDIF} // GDIPLUS_PNG
        ...
        8: begin
          if CopyAlpha then begin
            ...
            if coApplyGamma in FTargetOptions then
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRunA8.R := FGammaTable[SourceR8^];
                  TargetRunA8.G := FGammaTable[SourceG8^];
                  TargetRunA8.B := FGammaTable[SourceB8^];
              ...
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRunA8.R := SourceR8^;
                  TargetRunA8.G := SourceG8^;
                  TargetRunA8.B := SourceB8^;
                  TargetRunA8.A := SourceA8^;
            ...
            if coApplyGamma in FTargetOptions then
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRun8.R := FGammaTable[SourceR8^];
                  TargetRun8.G := FGammaTable[SourceG8^];
                  TargetRun8.B := FGammaTable[SourceB8^];
              ...
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRun8.R := SourceR8^;
                  TargetRun8.G := SourceG8^;
                  TargetRun8.B := SourceB8^;
          ...
          if CopyAlpha then begin
            ...
            if coApplyGamma in FTargetOptions then
              if coNeedByteSwap in FSourceOptions then
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRunA16.R := Swap(MulDiv16(FGammaTable[SourceR8^], $FFFF, $FF));
                    TargetRunA16.G := Swap(MulDiv16(FGammaTable[SourceG8^], $FFFF, $FF));
                    TargetRunA16.B := Swap(MulDiv16(FGammaTable[SourceB8^], $FFFF, $FF));
                    TargetRunA16.A := Swap(MulDiv16(SourceA8^, $FFFF, $FF));
                ...
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRunA16.R := MulDiv16(FGammaTable[SourceR8^], $FFFF, $FF);
                    TargetRunA16.G := MulDiv16(FGammaTable[SourceG8^], $FFFF, $FF);
                    TargetRunA16.B := MulDiv16(FGammaTable[SourceB8^], $FFFF, $FF);
                    TargetRunA16.A := MulDiv16(SourceA8^, $FFFF, $FF);
              ...
              if coNeedByteSwap in FSourceOptions then
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRunA16.R := Swap(MulDiv16(SourceR8^, $FFFF, $FF));
                    TargetRunA16.G := Swap(MulDiv16(SourceG8^, $FFFF, $FF));
                    TargetRunA16.B := Swap(MulDiv16(SourceB8^, $FFFF, $FF));
                    TargetRunA16.A := Swap(MulDiv16(SourceA8^, $FFFF, $FF));
                ...
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRunA16.R := MulDiv16(SourceR8^, $FFFF, $FF);
                    TargetRunA16.G := MulDiv16(SourceG8^, $FFFF, $FF);
                    TargetRunA16.B := MulDiv16(SourceB8^, $FFFF, $FF);
                    TargetRunA16.A := MulDiv16(SourceA8^, $FFFF, $FF);
            ...
            if coApplyGamma in FTargetOptions then
              if coNeedByteSwap in FSourceOptions then
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRun16.R := Swap(MulDiv16(FGammaTable[SourceR8^], $FFFF, $FF));
                    TargetRun16.G := Swap(MulDiv16(FGammaTable[SourceG8^], $FFFF, $FF));
                    TargetRun16.B := Swap(MulDiv16(FGammaTable[SourceB8^], $FFFF, $FF));
                ...
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRun16.R := MulDiv16(FGammaTable[SourceR8^], $FFFF, $FF);
                    TargetRun16.G := MulDiv16(FGammaTable[SourceG8^], $FFFF, $FF);
                    TargetRun16.B := MulDiv16(FGammaTable[SourceB8^], $FFFF, $FF);
              ...
              if coNeedByteSwap in FSourceOptions then
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRun16.R := Swap(MulDiv16(SourceR8^, $FFFF, $FF));
                    TargetRun16.G := Swap(MulDiv16(SourceG8^, $FFFF, $FF));
                    TargetRun16.B := Swap(MulDiv16(SourceB8^, $FFFF, $FF));
                ...
                while Count > 0 do begin
                  if Boolean(Mask and BitRun) then begin
                    TargetRun16.R := MulDiv16(SourceR8^, $FFFF, $FF);
                    TargetRun16.G := MulDiv16(SourceG8^, $FFFF, $FF);
                    TargetRun16.B := MulDiv16(SourceB8^, $FFFF, $FF);
            ...
            if coNeedByteSwap in FSourceOptions then
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  ...
                  TargetRunA8.A := MulDiv16(Swap(SourceA16^), $FF, $FFFF);;
              ...
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  ...
                  TargetRunA8.G := Convert16_8(SourceG16^);
                  TargetRunA8.B := Convert16_8(SourceB16^);
                  TargetRunA8.A := MulDiv16(SourceA16^, $FF, $FFFF);;
        ...
        16: begin
          ...
          if CopyAlpha then begin
            ...
            if coNeedByteSwap in FSourceOptions then
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRunA16.R := Swap(SourceR16^);
                  TargetRunA16.G := Swap(SourceG16^);
                  TargetRunA16.B := Swap(SourceB16^);
                  TargetRunA16.A := Swap(SourceA16^);
              ...
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRunA16.R := SourceR16^;
                  TargetRunA16.G := SourceG16^;
                  TargetRunA16.B := SourceB16^;
                  TargetRunA16.A := SourceA16^;
            ...
            if coNeedByteSwap in FSourceOptions then
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRun16.R := Swap(SourceR16^);
                  TargetRun16.G := Swap(SourceG16^);
                  TargetRun16.B := Swap(SourceB16^);
              ...
              while Count > 0 do begin
                if Boolean(Mask and BitRun) then begin
                  TargetRun16.R := SourceR16^;
                  TargetRun16.G := SourceG16^;
                  TargetRun16.B := SourceB16^;
...
begin
  ...
  case DataFormat of
          ...
          if coApplyGamma in FTargetOptions then
            for I := 0 to LogPalette.palNumEntries - 1 do begin
              with LogPalette.palPalEntry[I] do begin
                peBlue  := FGammaTable[RunR8^]; Inc(RunR8);
                peGreen := FGammaTable[RunR8^]; Inc(RunR8);
                peRed   := FGammaTable[RunR8^]; Inc(RunR8);
              end;
              Inc(RunR8);
            end
          else
            for I := 0 to LogPalette.palNumEntries - 1 do begin
              with LogPalette.palPalEntry[I] do begin
                peBlue  := RunR8^; Inc(RunR8);
                peGreen := RunR8^; Inc(RunR8);
                peRed   := RunR8^; Inc(RunR8);
              end;
              Inc(RunR8);
            end
        else
    ...
    pfPlane8Triple, pfPlane8Quad: begin
      ...
      if coApplyGamma in FTargetOptions then
        for I := 0 to LogPalette.palNumEntries - 1 do begin
          LogPalette.palPalEntry[I].peRed   := FGammaTable[RunR8^]; Inc(RunR8);
          LogPalette.palPalEntry[I].peGreen := FGammaTable[RunG8^]; Inc(RunG8);
          LogPalette.palPalEntry[I].peBlue  := FGammaTable[RunB8^]; Inc(RunB8);
        end
      else
        for I := 0 to LogPalette.palNumEntries - 1 do begin
          LogPalette.palPalEntry[I].peRed   := RunR8^; Inc(RunR8);
          LogPalette.palPalEntry[I].peGreen := RunG8^; Inc(RunG8);
          LogPalette.palPalEntry[I].peBlue  := RunB8^; Inc(RunB8);
        end;
    end;
...
{$IFNDEF NOACPNG}
  {$IFDEF D2010}
    ...
    TPicture.RegisterFileFormat(acPngExt, 'Portable network graphics (AlphaControls)', TPNGGraphic);
    if not ACPngFirst then
      TPicture.RegisterFileFormat(acPngExt, 'Portable Network Graphics', TPngImage);
  {$ELSE}
    TPicture.RegisterFileFormat(acPngExt, 'Portable network graphics (AlphaControls)', TPNGGraphic);
  {$ENDIF}
{$ENDIF}
