...
uses sMessages, sStyleSimply, sVclUtils, sGraphUtils, acntUtils, sAlphaGraph, CommCtrl{$IFDEF DELPHI7UP}, Themes{$ENDIF};
...
function TsProgressBar.GetCount: integer;
var
  iSize: TSize;
  w, c, h: integer;
begin
  ...
  if (c > 1) and (Max > Min) and (Position > Min) and (Max - Min <> 0) then
    Result := Round(c / (Max - Min) * (Position - Min))
  else
    Result := 0;
end;
...
{$ENDIF}    
    begin
      PaintItem(si, ci, True, 0, MkRect(Bmp), p, Bmp, SkinData.CommonSkinData);
  ...
  if Max > Min then begin
    with FCommonData.SkinManager do
      if ProgressSkin <> '' then
        ...
        if Orientation = pbVertical then
          si := SkinCommonInfo.Sections[ssProgressV]
        else
          si := SkinCommonInfo.Sections[ssProgressH];
    ...
    case Message.Msg of
      ...
      PBM_SETPOS: begin
        if Style = pbstMarquee then begin
          ...
          FActPosition := Message.WParam;
          if FOldCount <> GetCount then
            Paint(0);
        end;
        Exit;
      end;
