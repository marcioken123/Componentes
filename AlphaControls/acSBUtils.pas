...
uses
  ...
  sSkinManager, sCommonData, sConst, acntTypes;
...
type
  ...
  protected
    ...
    function DoPrepareCache: boolean; virtual;
  ...
  protected
    ...
    function PaintContent: boolean; virtual;
    function PaintText(DoCalc: boolean): TRect;
    procedure acWndProc(var Message: TMessage); override;
  end;
  ...
  protected
    ...
    FBmp: TBitmap;
    procedure AfterCreation; override;
    function PaintContent: boolean; override;
  public
    destructor Destroy; override;
  end;
  ...
  protected
    ...
    procedure DrawCaption(const Coloring: TacColoring);
    procedure DrawGlyph; virtual;
    function GlyphSize: TSize; virtual;
    procedure DoDrawText(var Rect: TRect; const Flags: Longint; CanGlow: boolean);
...
{$ENDIF}
  ...
  protected
    ClientRect: TRect;
    BorderWidth: integer;
    function DoPrepareCache: boolean; override;
    procedure AfterCreation; override;
    procedure AC_WMPaint  (const Message: TWMPaint);
  ...
  protected
    ...
    procedure DrawBtn     (const Index: integer; R: TRect; const DC: hdc);
  ...
  protected
    ...
    procedure UpdateLeftScroll;
    procedure InitSkinData; override;
    procedure acWndProc(var Message: TMessage); override;
  public
    fThumbTracking: boolean; // To prevent calling original WindowProc in response to our own temporary style change (fixes TreeView problem)
...
{$ENDIF}
  ...
  TacCtrlState = set of (csPaintNeeded, csNCPaintNeeded);

  TacEditWnd = class(TacBaseWnd) // TCustomEdit, TCustomListbox compatible
  protected
    FClientRect: TRect;
    FrameColor: integer;
    CtrlState: TacCtrlState;
...
{$ENDIF}
  ...
  TacDBComboListWnd = class(TacComboListWnd) // ComboListBox
  protected
    procedure acWndProc(var Message: TMessage); override;
  public
    ComboBoxHandle: THandle;
  end;
  ...
  protected
    ...
    LBoxOpening, DropDownList: boolean;
  ...
  protected
    ...
    OldDrawBtnEvent: TTBAdvancedCustomDrawBtnEvent;
    procedure PrepareCache;
    function DisplayRect: TRect;
    procedure AfterCreation; override;
...
procedure ac_DrawScrollBtn(bRect: TRect; State: integer; Bmp: TBitmap; sd: TacSkinData; Side: TacSide);
procedure ac_DrawSlider   (bRect: TRect; State: integer; Bmp: TBitmap; sd: TacSkinData; Vert: boolean);
// Scroll Data
...
procedure SendScrollMessage(const Handle: hwnd; const scrMsg: integer; const scrId: integer; const pos: integer; DoSetScrollInfo: boolean = False);
procedure UpdateScrolls(const sw: TacScrollWnd; const Repaint: boolean = False);
function BtnDrawRect(R: TRect; sd: TacSkinData; Side: TacSide): TRect;
// Paint Data
...
//  {$IFDEF DELPHI_XE2} Spin, {$ENDIF}
  ...
  {$IFDEF DELPHI7UP} UxTheme, {$ENDIF}
  acntUtils, sGraphUtils, sStyleSimply, sSkinProps, sDefaults, sSkinProvider, acDials, sSkinMenus, acAnimation,
  sThirdParty, acGlow, sAlphaGraph, sMessages, sVclUtils, sMaskData, sMDIForm, acAlphaImageList, acPopupController, sComboBox;
...
function ac_GetSysFont(const fType: integer): TLogFontW;
var
...
begin
  ...
  if SystemParametersInfoW(SPI_GETNONCLIENTMETRICS, 0, @NonClientMetrics, 0) then
    with NonClientMetrics do
      case fType of
        FONT_CAPTION:   Result := lfCaptionFont;
        FONT_SMCAPTION: Result := lfSmCaptionFont;
        FONT_MENU:      Result := lfMenuFont;
        FONT_STATUS:    Result := lfStatusFont
        else            Result := lfMessageFont
      end
    else
      Result.lfHeight := 0;
end;
...
function SkinMainFontName: string;
begin
  if DefaultManager <> nil then
    case DefaultManager.Fonts.MainMode of
      fmCustom:   Result := DefaultManager.Fonts.MainFont;
      fmFromSkin: Result := DefaultManager.SkinCommonInfo.MainFont;
    end;
end;


procedure CheckSkinFont(var Font: TLogFontW); overload;
var
  i: integer;
  s: string;
begin
  s := SkinMainFontName;
  if s <> '' then begin
    FillMemory(@Font.lfFaceName[0], SizeOf(Font.lfFaceName), 0);
    for i := 0 to Length(s) - 1 do
      Font.lfFaceName[i] := WideChar(s[i + 1]);
  end;
end;


{$IFNDEF D2009}
procedure CheckSkinFont(var Font: TLogFont); overload;
var
  i: integer;
  s: string;
begin
  ...
  if s <> '' then begin
    FillMemory(@Font.lfFaceName[0], SizeOf(Font.lfFaceName), 0);
    for i := 0 to Length(s) - 1 do
      Font.lfFaceName[i] := AnsiChar(s[i + 1]);
  end;
end;
{$ENDIF}


procedure CheckSkinFont(Font: PFont); overload;
var
  s: string;
begin
  s := SkinMainFontName;
...
var
  f: TLogFontW;
begin
  f := ac_GetSysFont(aFontType);
  if f.lfHeight <> 0 then begin
    CheckSkinFont(f);
    aCanvas.Font.Handle := CreateFontIndirectW(f);
    SelectObject(aCanvas.Handle, aCanvas.Font.Handle);
  end
end;


procedure LoadCtrlFont(const ListSW: TacMainWnd; DoSetFont: boolean = False);
var
  hf: hFont;
{$IFDEF DELPHI_XE}
  f: TLogFontW;
...
begin
  with ListSW, SkinData.FCacheBMP do
    if SkinData.FOwnerControl <> nil then begin
      Canvas.Font.Assign(TacAccessControl(SkinData.FOwnerControl).Font);
      CheckSkinFont(PFont(@Canvas.Font));
      SelectObject(Canvas.Handle, Canvas.Font.Handle);
    end
    else begin
      hf := Cardinal(TrySendMessage(CtrlHandle, WM_GETFONT, 0, 0));
      if hf <> 0 then begin
        GetObject(hf, SizeOf(f), @f);
        CheckSkinFont(f);
{$IFDEF D2009}
        Canvas.Font.Handle := CreateFontIndirectW(f);
{$ELSE}
...
{$ENDIF}
        ...
        SelectObject(Canvas.Handle, Canvas.Font.Handle);
        if DoSetFont then
          TrySendMessage(CtrlHandle, WM_SETFONT, WParam(Canvas.Font.Handle), 0);
      end
      else
...
begin
  Result := False;
  with SkinData do
    if (SkinData <> nil) and SkinData.Skinned and (CommonSkinData.gd[SkinIndex].BorderIndex >= 0) then begin
      if not ByMouse or not (COC in sNoHotEdits) or ac_AllowHotEdits then
        with CommonSkinData do
          Result := (not FOwner.Options.NoMouseHover or not ByMouse) and ((ma[CommonSkinData.gd[SkinIndex].BorderIndex].ImageCount > 1) or (gd[SkinIndex].States > 1));
  end;
end;
...
var
  ...
  procedure CopyMasterCorner(dRect, sRect: TRect; sBmp: TBitmap);
  var
    S0, S: PRGBAArray_S;
    D0, D: PRGBAArray_D;
  ...
  begin
    if not SkinData.FCacheBmp.Empty then begin
      if CI.Ready then begin
        if dRect.Left + ci.X < 0 then
          dRect.Left := -ci.X;

        if dRect.Top + ci.Y < 0 then
          dRect.Top := -ci.Y;
      end;
      h := Min(HeightOf(dRect), HeightOf(sRect));
      h := Min(h, SkinData.FCacheBmp.Height - 1 - dRect.Top - ci.Y);
      h := Min(h, sBmp.Height - 1 - sRect.Top);
      if h >= 0 then begin
        w := Min(WidthOf(dRect), WidthOf(sRect));
        w := Min(w, SkinData.FCacheBmp.Width - 1 - dRect.Left - ci.X);
        w := Min(w, sBmp.Width - 1 - sRect.Left);

        if CI.Ready then begin
          if dRect.Left + w + CI.X > CI.Bmp.Width then
            w := CI.Bmp.Width - dRect.Left - CI.X;

          if dRect.Top + h + CI.Y > CI.Bmp.Height then
            h := CI.Bmp.Height - dRect.Top - CI.Y;
        end;
        if (w > 0) and (h > 0) then begin
          if dRect.Left < sRect.Left then begin
            if dRect.Left < 0 then begin
              inc(sRect.Left, - dRect.Left);
              dec(h, - dRect.Left);
              dRect.Left := 0;
            end;
          end
          else
            if sRect.Left < 0 then begin
              inc(dRect.Left, - sRect.Left);
              dec(h, - sRect.Left);
              sRect.Left := 0;
            end;

          if dRect.Top < sRect.Top then begin
            if dRect.Top < 0 then begin
              inc(sRect.Top, - dRect.Top);
              dec(h, - dRect.Top);
              dRect.Top := 0;
            end;
          end
          else
            if sRect.Top < 0 then begin
              inc(dRect.Top, - sRect.Top);
              dec(h, - sRect.Top);
              sRect.Top := 0;
            end;
          ...
          if not CI.Ready then begin
            if CI.FillColor <> clFuchsia then
              if InitLine(sBmp, Pointer(S0), DeltaS) and InitLine(SkinData.FCacheBmp, Pointer(D0), DeltaD) then
                for Y := 0 to h do begin
                  S := Pointer(PAnsiChar(S0) + DeltaS * (sRect.Top + Y));
                  D := Pointer(PAnsiChar(D0) + DeltaD * (dRect.Top + Y));
                  for X := 0 to w do
                    if S[sRect.Left + X].SC = sFuchsia.C then
                      D[dRect.Left + X].DC := col.C
                end;
          end
          else
            if InitLine(sBmp, Pointer(S0), DeltaS) and InitLine(SkinData.FCacheBmp, Pointer(D0), DeltaD) then
              for Y := 0 to h do begin
                ...
                for X := 0 to w do begin
                  if S[dX2].SC = sFuchsia.C then
                    D[dX1].DC := GetAPixel(ci.Bmp, dX1 + ci.X, dRect.Top + ci.Y + Y).C;
...
begin
  with Skindata do
    if Assigned(SkinManager) and not Wnd.DlgMode then begin
      if SkinManager.IsValidImgIndex(CommonSkinData.gd[SkinIndex].BorderIndex) and Assigned(FCacheBmp) and (Wnd.WndSize.cx > 1) and (Wnd.WndSize.cy > 1) then begin
...
procedure UpdateScrolls(const sw: TacScrollWnd; const Repaint: boolean = False);
begin
  if Assigned(sw) and not sw.inDestroying then
    if (sw.SkinData = nil) or not sw.SkinData.FUpdating and (sw.SkinData.FUpdateCount <= 0) then
...
var
  ...
  BGInfo: TacBGInfo;
  dk: TsDisabledKind;
  State, iSingle, BordWidth: integer;
begin
  if (SkinData <> nil) and SkinData.BGChanged and (CtrlHandle <> 0) then begin
    ...
    if DlgMode and (DefaultManager.Palette[pcBorder] <> clFuchsia) then begin
      ...
      if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then
        if (SkinData.FOwnerControl is TCustomEdit) and (TAccessEdit(SkinData.FOwnerControl).BorderStyle = bsNone) or
             (SkinData.FOwnerControl is TCustomTreeView) and (TTreeView(SkinData.FOwnerControl).BorderStyle = bsNone) then
          PaintItemBG(SkinData, CI, State, MkRect(SkinData.FCacheBmp), p, SkinData.FCacheBmp)
        else
          PaintItem(SkinData, CI, True, State, MkRect(SkinData.FCacheBmp), p, SkinData.FCacheBmp, not DlgMode);
    end;
...
{$ENDIF}
        if GetWindowLong(CtrlHandle, GWL_STYLE) and ES_MULTILINE = 0 then
          if Text <> '' then begin
            ...
            acWriteTextEx(SkinData.FCacheBMP.Canvas, PacChar(aText), True, R, Flags {$IFDEF D2009}or Cardinal(GetStringFlags(SkinData.FOwnerControl, Alignment)) and not DT_VCENTER{$ENDIF}, SkinData, ControlIsActive(SkinData));
          end
{$IFDEF D2009}
          else
            if not SkinData.FFocused then
              TextHintOut(SkinData, R, TextHint, Flags {$IFDEF D2009}or Cardinal(GetStringFlags(SkinData.FOwnerControl, Alignment)) and not DT_VCENTER{$ENDIF}, State);
{$ENDIF}
      end;
...
begin
  if ListSW <> nil then begin
    if not ListSW.Destroyed and (ListSW.SkinData <> nil) then
      if (ListSW.SkinData.FOwnerControl <> nil) then begin
        ...
        if Assigned(ListSW.OldWndProc) then begin
          ...
          if ACNativeInt(GetWindowLong(Handle, GWL_WNDPROC)) = ACNativeInt(ListSW.NewWndProcInstance) then
        ...
        if ListSW.OwnSkinData and (ListSW.SkinData <> nil) then begin
          ...
          if ListSW.SkinData.GlowID >= 0 then
            HideGlow(ListSW.SkinData.GlowID);
        end;
        ListSW.Destroyed := True;
{      end
      else
        if ListSW.SkinData.FOwnerObject is TsSkinProvider then begin
          FreeAndNil(TsSkinProvider(ListSW.SkinData.FOwnerObject).SystemMenu);
          ListSW.SkinData.WndProc := ListSW.OldWndProc;
          TsSkinProvider(ListSW.SkinData.FOwnerObject).Form.WindowProc := ListSW.OldWndProc;
          ListSW.Destroyed := True;}
        end;
...
begin
  with sw do begin
    ...
    SetProp(CtrlHandle, acPropStr, THandle(sw));
...
//        if ACNativeInt(GetWindowLong(ListSW.CtrlHandle, GWL_WNDPROC)) = ACNativeInt(ListSW.NewWndProcInstance) then begin
      ...
      if sBar.nBarType = SB_HORZ then begin
        if metric = SM_BTNSIZE then begin // Size of button
          if TsScrollWndData(sBar.sw.SkinData).HorzScrollData.ButtonsSize >= 0 then
            Result := sCommonData.ScaleInt(TsScrollWndData(sBar.sw.SkinData).HorzScrollData.ButtonsSize, sBar.sw.SkinData)
          else
...
//                with sBar.sw.SkinData.CommonSkinData.ScrollBtns[asLeft] do
                // Default from skin
                case sBar.sw.SkinData.CommonSkinData.ScrollArrows of
                  0: Result := 0;

                  1: Result := max(sCommonData.ScaleInt(16, sBar.sw.SkinData),  sBar.sw.SkinData.CommonSkinData.ScrollBtnLength);

                  else begin
                    Result := GetScrollSize(sBar.sw.SkinManager);
                    if Result < 0 then
                      Result := -sBar.nArrowLength * sBar.sw.SkinData.CommonSkinData.ScrollBtnLength
                    else
                      if IsWin10Up and (sBar.sw.SkinData.SkinManager.Options.ScaleMode = smVCL) then
                        Result := sCommonData.ScaleInt(Result, sBar.sw.SkinData);
                  end;
                end
        end
        else // Height of scrolllbar
          if TsScrollWndData(sBar.sw.SkinData).HorzScrollData.ScrollWidth >= 0 then
            Result := sCommonData.ScaleInt(TsScrollWndData(sBar.sw.SkinData).HorzScrollData.ScrollWidth, sBar.sw.SkinData)
          else begin
            Result := GetScrollSize(sBar.sw.SkinManager);
            if Result < 0 then // If system value needed
        ...
        if metric = SM_BTNSIZE then begin // Size of button
          if TsScrollWndData(sBar.sw.SkinData).VertScrollData.ButtonsSize >= 0 then
            Result := sCommonData.ScaleInt(TsScrollWndData(sBar.sw.SkinData).VertScrollData.ButtonsSize, sBar.sw.SkinData)
          else
            if ScrollsOptions.ButtonsSize >= 0 then
              Result := ScrollsOptions.ButtonsSize
            else
...
{
                with sBar.sw.SkinData.CommonSkinData.ScrollBtns[asTop] do
                begin
                  Result := GetScrollSize(sBar.sw.SkinManager);
                  if Result < 0 then
                    Result := -sBar.nArrowLength * sBar.sw.SkinData.CommonSkinData.ScrollBtnLength
        ...
        else // Width of scrolllbar
          if TsScrollWndData(sBar.sw.SkinData).VertScrollData.ScrollWidth >= 0 then
            Result := sCommonData.ScaleInt(TsScrollWndData(sBar.sw.SkinData).VertScrollData.ScrollWidth, sBar.sw.SkinData)
          else begin
            Result := GetScrollSize(sBar.sw.SkinManager);
            if Result < 0 then // If system value needed
              if sBar.nArrowWidth < 0 then
...
var
  ...
  parHandle: hwnd;
begin
  if sw.SkinData.COC in [COC_TsSkinProvider, COC_TacDialogWnd] then begin
...
procedure DrawCenterGlyph(b: TBitmap; Ndx: integer; State: integer; sd: TacSkinData; R: TRect);
var
  p: TPoint;
  w, h: integer;
begin
  with sd do
    if FOwner.IsValidImgIndex(Ndx) then begin
      w := ma[Ndx].Width;
      h := ma[Ndx].Height;
...
begin
  if not IsRectEmpty(bRect) then begin
    dRect := BtnDrawRect(bRect, sd, Side);
    b := CreateBmp32(dRect);
    with sd.ScrollBtns[Side] do begin
      Ci := MakeCacheInfo(Bmp);
      PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], Ci, True, State, MkRect(b), dRect.TopLeft, b, sd);
      ...
      if GlyphIndex >= 0 then
        DrawCenterGlyph(b, GlyphIndex, State, sd, gRect)
      else
        if SkinIndex >= 0 then begin
          State := mini(State, ac_MaxPropsIndex);
          C := sd.gd[SkinIndex].Props[State].FontColor.Color;
          if (State > 1) and CanClickShift(sd.ScrollBtns[Side].SkinIndex, sd) then
            OffsetRect(gRect, 1, 1);

          DrawArrow(b, C, clNone, gRect, Side, 0, 0, 0, sd.FOwner.Options.ActualArrowStyle, aScalePercents[sd.ScaleValue]);
        end;
    end;
...
var
  ...
  ci: TCacheInfo;
begin
  if sd.ThumbWidth > 0 then
    if Vert then
      InflateRect(bRect, -(WidthOf(bRect) - sd.ThumbWidth) div 2, 0)
    else
      InflateRect(bRect, 0, -(HeightOf(bRect) - sd.ThumbWidth) div 2);

  b := CreateBmp32(bRect);
  with sd.Sliders[Vert] do begin
    Ci := MakeCacheInfo(Bmp, bRect.Left, bRect.Top);
    PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], Ci, True, State, MkRect(b), MkPoint, b, sd);
    DrawCenterGlyph(b, GlyphIndex, State, sd, MkRect(b));
  end;
...
begin
  ...
  if not sb.sw.fThumbTracking then begin
    cd := sb.sw.SkinData.CommonSkinData;
    if not sb.fScrollVisible or (cd.FOwner = nil) or not cd.FOwner.IsValidSkinIndex(cd.Scrolls[asLeft].SkinIndex) or (WidthOf(R) < 2) then
      Exit;
    ...
    if scrollwidth > butwidth * 2 then begin
      ...
      with cd.Scrolls[asLeft] do
        PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                      ...
                      Point(R.Left - sb.sw.cxLeftEdge, R.Top - sb.sw.cyTopEdge), Bmp, cd);
      with cd.Scrolls[asRight] do
        PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                      CI, True, integer(uDrawFlags = HTSCROLL_PAGERIGHT) * 2, Rect(tdiv2, 0, Bmp.Width, Bmp.Height),
                      Point(R.Left + tdiv2 - sb.sw.cxLeftEdge, R.Top - sb.sw.cyTopEdge), Bmp, cd);
      // LEFT ARROW
      SetRect(ctrl, R.left, R.top, R.left + butwidth, R.bottom);
      ac_DrawScrollBtn(MkRect(WidthOf(Ctrl), HeightOf(Ctrl)), lbState, Bmp, cd, asLeft);
      // RIGHT ARROW
      SetRect(ctrl, R.right - butwidth, R.top, R.right, R.bottom);
      OffsetRect(ctrl, -R.Left, -R.Top);
      ac_DrawScrollBtn(ctrl, rbState, Bmp, cd, asRight);
      // MIDDLE PORTION
      // Getting real values
      ...
      if IsEnabled and (realthumbsize > 0) and (realthumbsize <= workingWidth) then begin
        ...
        ac_DrawSlider(thumb, integer((uHitTestPortion = HTSCROLL_THUMB) and fBarHot and IsEnabled and (Handle = hwndCurSB)), Bmp, cd, False);
      end;
    end
    // not enough room for the scrollbar, so just draw the buttons (scaled in size to fit)
      ...
      with cd.Scrolls[asLeft] do
        PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True, 0, MkRect(butWidth, Bmp.Height), MkPoint, Bmp, cd);
      with cd.Scrolls[asRight] do
        PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True, 0, Rect(butWidth, 0, Bmp.Width, Bmp.Height), MkPoint, Bmp, cd);

      ac_DrawScrollBtn(MkRect(butWidth, Bmp.Height), lbState, Bmp, cd, asLeft);
      ac_DrawScrollBtn(Rect(butWidth, 0, Bmp.Width, Bmp.Height), rbState, Bmp, cd, asRight);
    end;
    if not IsEnabled {and (sb.sw.SkinData.FOwnerControl = nil) and sb.sw.SkinData.FOwnerControl.Enabled} then begin
      BG.PleaseDraw := False;
      GetBGInfo(@BG, Handle);
      CI := BGInfoToCI(@BG);
...
begin
  ...
  if not sb.sw.fThumbTracking and not sb.sw.DontRepaint then begin
    sd := sb.sw.SkinData.CommonSkinData;
    if not sb.fScrollVisible or (sd.FOwner = nil) or not sd.FOwner.IsValidSkinIndex(sd.Scrolls[asTop].SkinIndex) or (HeightOf(R) < 2) then
      Exit;
    ...
    if scrollHeight > 0 then begin
      ...
      if scrollHeight > butheight * 2 then begin
        ...
        // Top part of scroll
        with sd.Scrolls[asTop] do
          PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                        CI, True, integer(uDrawFlags = HTSCROLL_PAGEUP) * 2, MkRect(Bmp.Width, tdiv2),
                        Point(R.Left - sb.sw.cxLeftEdge, R.Top - sb.sw.cyTopEdge), Bmp, sd);

        // Bottom part of scroll
        with sd.Scrolls[asBottom] do
          PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                        CI, True, integer(uDrawFlags = HTSCROLL_PAGEDOWN) * 2, Rect(0, tdiv2, Bmp.Width, Bmp.Height),
                        Point(R.Left - sb.sw.cxLeftEdge, R.Top + tdiv2 - sb.sw.cyTopEdge), Bmp, sd);
        // TOP ARROW
        SetRect(ctrl, R.left, R.top, R.Right, R.Top + butheight);
        OffsetRect(ctrl, -R.Left, -R.Top);
        ac_DrawScrollBtn(Ctrl, lbState, Bmp, sd, asTop);
        // Bottom ARROW
        SetRect(ctrl, R.Left, R.Bottom - butheight, R.Right, R.bottom);
        OffsetRect(ctrl, -R.Left, -R.Top);
        ac_DrawScrollBtn(ctrl, rbState, Bmp, sd, asBottom);
        //MIDDLE PORTION
        // Getting real values
        ...
        if IsEnabled and (thumbHeight > 0) and (realthumbsize <= workingHeight) then begin
          ...
          OffsetRect(thumb, -R.Left, -R.Top);
          ac_DrawSlider(thumb, integer((uHitTestPortion = HTSCROLL_THUMB) and fBarHot and IsEnabled and (Handle = hwndCurSB)), Bmp, sd, True);
        end;
      end
      //not enough room for the scrollbar, so just draw the buttons (scaled in size to fit)
      else begin
        ...
        with sd.Scrolls[asTop] do
          PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                        CI, True, 0, MkRect(Bmp.Width, butHeight), R.TopLeft, Bmp, sd);

        with sd.Scrolls[asBottom] do
          PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True, 0, Rect(0, butHeight, Bmp.Width, Bmp.Height), Point(R.Left, ButHeight), Bmp, sd);

        ac_DrawScrollBtn(MkRect(Bmp.Width, butheight), lbState, Bmp, sd, asTop);
        ac_DrawScrollBtn(Rect(0, Bmp.Height - butheight, Bmp.Width, Bmp.Height), rbState, Bmp, sd, asBottom);
      end;
      if not IsEnabled {and (sb.sw.SkinData.FOwnerControl = nil) and sb.sw.SkinData.FOwnerControl.Enabled} then begin
        BG.PleaseDraw := False;
        GetBGInfo(@BG, Handle);
  ...
  if sw <> nil then begin
    if sw.SkinData <> nil then
      with sw.SkinData do begin
        if not SkinManager.IsActive or InUpdating(sw.SkinData) then
          Exit;
    ...
    if sb <> nil then begin
      ...
      if sb <> nil then begin
        ...
        if sw.sbarHorz.fScrollVisible and sw.sbarVert.fScrollVisible then begin
          ...
          with sw.SkinData.CommonSkinData, Scrolls[asLeft] do
            if IsValidIndex(SkinIndex, Length(gd)) and (gd[SkinIndex].Props[0].Transparency <> 100) then begin
              if gd[SkinIndex].Props[0].Color <> clFuchsia then
                BGInfo.Color := gd[sw.SkinData.CommonSkinData.Scrolls[asLeft].SkinIndex].Props[0].Color
              else
                BGInfo.Color := FOwner.GetGlobalColor;

              if not Ac_IsScrollbarActive(sw.sBarVert) or not Ac_IsScrollbarActive(sw.sBarHorz) then begin
                State := integer(ControlIsActive(sw.SkinData));
                BGInfo.Color := BlendColors(gd[sw.SkinData.SkinIndex].Props[State].Color, BGInfo.Color, sDefaults.DefBlendDisabled);
              end;
              ...
              case BGInfo.BgType of
                ...
                else // If AlphaSkins are fully supported
                  FillDC(bGrip.Canvas.Handle, MkRect(bGrip), FOwner.GetGlobalColor);
              end;

          // Grip if exists
          if Ac_GripVisible(sw, Handle, R2) then begin
            LeftRight := sw.SkinData.CommonSkinData.GripRightBottom;
            if sw.SkinManager.IsValidImgIndex(LeftRight) then
              with sw.SkinData.CommonSkinData.ma[LeftRight] do
                DrawSkinGlyph(bGrip, Point(bGrip.Width - Width, bGrip.Height - Height),
                                  0, 1, sw.SkinData.CommonSkinData.ma[LeftRight], BGInfoToCI(@BGInfo));
          end;
...
begin
  with TNCCalcSizeParams(Pointer(lParam)^) do begin
    ...
    if (sw.SkinData.FOwnerControl is TCustomEdit) and not (sw.SkinData.FOwnerControl is TCustomMemo) then
      Result := sw.CallPrevWndProc(Handle, WM_NCCALCSIZE, wParam, lParam)
    else begin
      if dwStyle and (WS_VSCROLL or WS_HSCROLL) <> 0 then begin
        nwStyle := dwStyle and not (WS_VSCROLL or WS_HSCROLL);
      ...
      if not (sw.SkinData.COC in [COC_TacDialogWnd, COC_TsSkinProvider, COC_TsListView, COC_TVST]) then
        if sw.SkinData.SkinManager.IsActive then
          with sw.SkinData.CommonSkinData.gd[sw.SkinData.SkinIndex] do
            if (sw.SkinData.SkinManager.IsValidSkinIndex(sw.SkinData.SkinIndex) or (BorderRadius > 0)) then
              if (ScrollBorderOffset < 0) and (rgrc[0].left - oldrect.left > 1) then
                InflateRect(rgrc[0], -ScrollBorderOffset, -ScrollBorderOffset);
    end;
...
{$ENDIF}
      if sb <> nil then begin
        ...
        if (sb.fScrollFlags and CSBS_VISIBLE <> 0) and (HeightOf(R) >= GetScrollMetric(sb, SM_SCROLLWIDTH)) then begin
        ...
        if (sb.fScrollFlags and CSBS_VISIBLE <> 0) and (WidthOf(R) >= GetScrollMetric(sb, SM_SCROLLWIDTH)) then begin
          if sw.fLeftScrollbar then
...
//  sm := sbar.sw.SkinManager;
  sd := sbar.sw.SkinData.CommonSkinData;
  ...
  with sd.Scrolls[asLeft] do
    PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True, 0, MkRect(tdiv2, Bmp.Height),
                  Point(R.Left - sbar.sw.cxLeftEdge, R.Top - sbar.sw.cyTopEdge), Bmp, sd);

  with sd.Scrolls[asRight] do
    PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1], CI, True, 0, Rect(tdiv2, 0, Bmp.Width, Bmp.Height),
                  Point(R.Left + tdiv2 - sbar.sw.cxLeftEdge, R.Top - sbar.sw.cyTopEdge), Bmp, sd);

  if btnWidth <> 0 then begin
    ac_DrawScrollBtn(MkRect(BtnWidth, Bmp.Height), 0, Bmp, sd, asLeft);
    ac_DrawScrollBtn(Rect(Bmp.Width - BtnWidth, 0, Bmp.Width, Bmp.Height), 0, Bmp, sd, asRight);
  end;
  ac_DrawSlider(Rect(thumbpos - R.Left, 0, thumbpos + nThumbSize - R.Left, Bmp.Height), 2, Bmp, sd, False);
  dc := GetWindowDC(Handle);
  BitBlt(dc, R.Left, R.Top, Bmp.Width, Bmp.Height, Bmp.Canvas.Handle, 0, 0, SRCCOPY);
  ReleaseDC(Handle, dc);
  FreeAndNil(Bmp);
  ...
  if pos <> nLastSBPos then begin
    ...
    SendScrollMessage(Handle, uScrollTimerMsg, SB_THUMBTRACK, sbar.scrollInfo.nTrackPos, True);//not (sbar.sw is TacListViewWnd) and (sbar.sw.SkinData.COC <> COC_TsListView));
...
begin
  ...
  with sd.Scrolls[asBottom] do
    PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                  ...
                  Point(R.Left - sbar.sw.cxLeftEdge, R.Top + tdiv2 - sbar.sw.cyTopEdge), Bmp, sd);

  if btnHeight <> 0 then begin
    ac_DrawScrollBtn(MkRect(Bmp.Width, BtnHeight), 0, Bmp, sd, asTop);
    ac_DrawScrollBtn(Rect(0, Bmp.Height - BtnHeight, Bmp.Width, Bmp.Height), 0, Bmp, sd, asBottom);
  end;

  ac_DrawSlider(Rect(0, thumbpos - R.Top, Bmp.Width, thumbpos + nThumbSize - R.Top), 2, Bmp, sd, True);
  ...
  if pos <> nLastSBPos then begin
    sbar.scrollInfo.nTrackPos := sbar.scrollInfo.nMin + pos;
    sbar.sw.DontRepaint := True;
    SendScrollMessage(Handle, uScrollTimerMsg, SB_THUMBTRACK, sbar.scrollInfo.nTrackPos, not (sbar.sw is TacListViewWnd) and (sbar.sw.SkinData.COC <> COC_TsListView));
    sbar.sw.DontRepaint := False;
  end;
  nLastSBPos := pos;
  Result := 0;
...
var
  ...
  hrect, vrect: TRect;
begin
  if not bInHitTest then begin
    bInHitTest := True;
    if (sw.SkinData.SkinManager.ActiveControl <> sw.CtrlHandle) and
          not sw.SkinData.FMouseAbove and ((sw.SkinData.FOwnerControl = nil) or acMouseInControl(sw.SkinData.FOwnerControl)) and not (sw is TacMnuWnd) then
...
{$ENDIF}
  if SendAMessage(Handle, AC_BEFORESCROLL, MakeLParam(Word(ScrId), Word(ScrMsg))) <> 1 then begin // If using more then 64K is not forbidden (is not used in ListView)
    ...
    if DoSetScrollInfo then // Patch for DBGridEh
      case scrId of
        SB_THUMBTRACK: begin
          si.cbSize := SizeOf(TScrollInfo);
          si.fMask := SIF_POS;
...
var
  ...
  begin
    if Ac_IsScrollbarActive(sb) then begin
      ...
      SendScrollMessage(Handle, uScrollTimerMsg, uCurrentScrollPortion, 0, not (sw is TacListViewWnd));
      uScrollTimerPortion := uCurrentScrollPortion;
...
begin
  ...
  if uCurrentScrollPortion <> HTSCROLL_NONE then begin
    ...
    try
      ...
      lParam := 0; // Fixing of error on 4K screen
    end;
    GetWindowRect(Handle, winrect);
    pt.x := SmallInt(LOWORD(LongWord(lParam)));
    pt.y := SmallInt(HIWORD(LongWord(lParam)));
    ...
    case uCurrentScrollPortion of
      HTSCROLL_LEFT, HTSCROLL_RIGHT, HTSCROLL_PAGELEFT, HTSCROLL_PAGERIGHT, HTSCROLL_NONE: begin
        ...
        if sw.fThumbTracking then begin
          sw.DontRepaint := True;
          SendScrollMessage(Handle, uScrollTimerMsg, SB_THUMBPOSITION, nLastSBPos, not (sw is TacListViewWnd));
          sw.fThumbTracking := False;
        end;
        // send the SB_ENDSCROLL message now that scrolling has finished
        sw.DontRepaint := True;
        SendScrollMessage(Handle, uScrollTimerMsg, SB_ENDSCROLL, 0, not (sw is TacListViewWnd));
      ...
      HTSCROLL_THUMB: begin
        ...
        // send the SB_ENDSCROLL message now that scrolling has finished
        sw.DontRepaint := True;
        SendScrollMessage(Handle, uScrollTimerMsg, SB_ENDSCROLL, nLastSBPos, not (sw is TacListViewWnd));
        sw.DontRepaint := False;
        // adjust the total scroll area to become where the scrollbar
        // really is (take into account the inserted buttons)
        Ac_GetRealScrollRect(sb, R);
    ...
    if ACNativeInt(WParam) <> -1 then // Do not call default handler
  ...
  // send the scrollbar message repeatedly
    ...
    if wTimerID = COOLSB_TIMERID2 then begin
      ...
      if uScrollTimerPortion <> HTSCROLL_NONE then begin
        sw.DontRepaint := True;
        SendScrollMessage(Handle, uScrollTimerMsg, uScrollTimerPortion, 0, not (sw is TacListViewWnd));
        sw.DontRepaint := False;
      end;
      Result := 0;
    end
...
var
  ...
  CanRepaint: boolean;
begin
  if (SkinData <> nil) and (SkinData.FOwnerControl is TWinControl) and not (csDestroying in SkinData.FOwnerControl.ComponentState) and not Application.Terminated then
    Ctrl := TWinControl(SkinData.FOwnerControl)
  else
    Ctrl := nil;

  CanRepaint := (SkinData = nil) or not SkinData.FUpdating;
  FreeAndnil(sBarHorz);
  FreeAndnil(sBarVert);
  inherited Destroy;
  if (Ctrl <> nil) and CanRepaint and Ctrl.HandleAllocated {and IsWindowVisible(Ctrl.Handle)} then
    SetWindowPos(Ctrl.Handle, 0, 0, 0, 0, 0, SWPA_FRAMECHANGED);
end;
...
begin
{$IFDEF LOGGED}
  if (SkinData.FOwnerControl = nil) {and (SkinData.FOwnerControl.Tag = 2) }then
    AddToLog(Message);
{$ENDIF}
  case Message.Msg of
    // UniSoft::end
{$IFNDEF FPC}
    WM_CONTEXTMENU:
      if (SkinData <> nil) and (SkinData.FOwnerControl <> nil) and (TacAccessControl(SkinData.FOwnerControl).PopupMenu <> nil) then
        if SkinData.SkinManager <> nil then
          SkinData.SkinManager.SkinableMenus.HookPopupMenu(TacAccessControl(SkinData.FOwnerControl).PopupMenu, SkinData.SkinManager.Active);
{$ENDIF}
    ...
    end;
{
    WM_WINDOWPOSCHANGING:
      if TWindowPos(Pointer(Message.LParam)^).Flags and (SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOZORDER) = (SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOZORDER) then begin
        SkinData.BGChanged := True; // If scrolled by changing of ScrollBar.Pos
    ...
    WM_DESTROY, WM_NCDESTROY:
      if not Destroyed then begin
        ...
        if (SkinData <> nil) and (SkinData.FCacheBmp <> nil) then
          ...
          try
            if SkinData.FOwnerControl <> nil then begin
              ...
              if Message.Result = 0 then
                if (Self is TacComboListWnd) and
                      (TacComboListWnd(Self).OwnerCtrl is TsComboBox) and
                         not (csDestroying in TacComboListWnd(Self).OwnerCtrl.ComponentState) then begin
                  UninitializeACScroll(CtrlHandle, True, False, ListSW); // Destroy ListSW if parent of ComboBox is changed
                  Exit;
                end
                else
                  UninitializeACScroll(CtrlHandle, False, False, ListSW);
        ...
        case Message.WParamHi of
          ...
          AC_GETPROVIDER: begin
            if SkinData.FOwnerObject is TsSkinProvider then
              Message.Result := LRESULT(SkinData.FOwnerObject); // Used for menuline init
...
{$ENDIF}
    ...
    //                  PacPaintInfo(Message.LParam).FontIndex := SkinIndex;
            ...
            AC_PREPARECACHE: begin
              Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
              if SkinData.BGChanged and not InUpdating(SkinData) and (Message.Result <> -2) then begin
            ...
            AC_CHILDCHANGED: begin
              ...
              if (Message.WParamLo = 1) and (SkinData.FOwnerControl is TWinControl) and TWinControl(SkinData.FOwnerControl).HandleAllocated then
                if SkinData.FOwnerControl.ComponentState * [csDestroying] = [] then
          ...
          end;
{
        ...
        WM_NCPAINT:
          if IsWindowVisible(CtrlHandle) then begin
            ...
            if not InUpdating(SkinData) and not DontRepaint and ScrollsInitialized then
              Message.Result := LRESULT(Ac_NCDraw(Self, CtrlHandle));
          end;
...
begin
{$IFDEF LOGGED}
//  if (SkinData <> nil) and (SkinData.FOwnerControl.Tag = 11) then
//    AddToLog(Message);
{$ENDIF}
  case Message.Msg of
    SM_ALPHACMD:
      case Message.WParamHi of
        ...
        AC_MOUSELEAVE: begin
          inherited;
          if not acMouseInControlDC(CtrlHandle) then
        ...
        AC_BUFFEREDPRINTCLIENT:
          if FInBufferedPrintClient then begin
{            R := ClientRect;
            OffsetRect(R, cxLeftEdge, cyTopEdge);
            R.Left := cxLeftEdge;
            R.Top := cyTopEdge;
            R.Right := WndSize.cx - cxRightEdge;
            R.Bottom := WndSize.cy - cyBottomEdge;}
            R := MkRect(WndSize);
            BufferedPrintClient(CtrlHandle, R, ColorToRGB(Color));
      ...
      end;
{
      CM_BUFFEREDPRINTCLIENT:
        if FInBufferedPrintClient then begin
          ...
          R.Right := WndSize.cx - cxRightEdge;
          R.Bottom := WndSize.cy - cyBottomEdge;
          BufferedPrintClient(CtrlHandle, R, SwapRedBlue(Color));
          FInBufferedPrintClient := False;
        end;
}
    WM_SIZE{, WM_WINDOWPOSCHANGED}: begin
      if IsWindowVisible(CtrlHandle) or
           (GetWindowLong(CtrlHandle, GWL_STYLE) and WS_VISIBLE <> 0) or
             (SkinData.FOwnerControl = nil) or
               not SkinData.FOwnerControl.Visible or
                 (SkinData.COC <> COC_TsAdapterEdit) or
                   (Self is TacVirtualTreeViewWnd) then // Prevent errors in some controls when not visible
        if SkinData.GlowID >= 0 then begin
          inherited;
          RepaintGlow(SkinData.GlowID);
        end
        else
          inherited;
...
//      if SkinData.Updating then Exit;
    ...
    WM_PRINT: begin
      if (SkinData.COC = COC_TsMemo) and (SkinData.TranspMode = TM_PARTIAL) and not IsFluentOS and (SkinData.FOwnerControl <> nil) and not TWinControl(SkinData.FOwnerControl).DoubleBuffered and IsWinVistaUp {$IFNDEF FLUENTCOMP}and Assigned(BeginBufferedPaint){$ENDIF} then begin
        TWinControl(SkinData.FOwnerControl).DoubleBuffered := True; // Avoiding a problem of painting when bluring on form used and memo is opaque under old Windows
      end;
      ...
      if PRF_CLIENT and Message.LParam <> 0 then begin // Patch for using with BE
        MoveWindowOrg(DC, -cxLeftEdge, -cyTopEdge);
        IntersectClipRect(DC, 0, 0, SkinData.FCacheBmp.Width - cxRightEdge, SkinData.FCacheBmp.Height - cyBottomEdge);
      end
      else begin
        BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, cxLeftEdge, cyTopEdge, cxRightEdge, cyBottomEdge);
      ...
      if (sBarVert <> nil) and (sBarHorz <> nil) then begin
        VSWidth := 0;
        if sBarVert.fScrollVisible then begin
          VSWidth := GetScrollMetric(sbarVert, SM_SCROLLWIDTH);
          if fLeftScrollbar then
            MoveWindowOrg(DC, VSWidth, 0);
        end;
        IntersectClipRect(DC, 0, 0,
                          SkinData.FCacheBmp.Width  - cxLeftEdge - cxRightEdge  - VSWidth,
                          SkinData.FCacheBmp.Height - cyTopEdge - cyBottomEdge  - integer(sBarHorz.fScrollVisible) * GetScrollMetric(sBarHorz, SM_SCROLLWIDTH));
      end;

      if (SkinData.TranspMode = TM_PARTIAL) and not (not IsFluentOS and (SkinData.COC in [COC_TsTreeView, COC_TsListView])) and IsWinVistaUp {$IFNDEF FLUENTCOMP}and Assigned(BeginBufferedPaint){$ENDIF} and (not (SkinData.FOwnerControl is TWinControl) or not TWinControl(SkinData.FOwnerControl).DoubleBuffered) then
        PaintBuffer := BeginBufferedPaint(DC, MkRect(SkinData.FCacheBmp), BPBF_TOPDOWNDIB, nil, DC)
      else
        PaintBuffer := 0;
      ...
      with SkinData.FCacheBmp do
        if PRF_CLIENT and Message.LParam <> 0 then // Patch for using with BE
          ...
          BitBlt(DC, 0, 0, Width, Height, Canvas.Handle, cxLeftEdge, cyTopEdge, SRCCOPY);
...
//        MakeRectOpaque(PaintBuffer);

      if (PaintBuffer <> 0) and IsWinVistaUp {$IFNDEF FLUENTCOMP}and Assigned(EndBufferedPaint){$ENDIF} then
    ...
    WM_SETTEXT, CM_ENABLEDCHANGED, CM_FONTCHANGED: begin
      ...
      if SkinData.CtrlSkinState and ACS_CHANGING = 0 then
        if not SkinData.FUpdating and (SkinData.FUpdateCount <= 0) and not InAnimationProcess then
    ...
    WM_ERASEBKGND:
      if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
        ...
        CtrlState := CtrlState - [csPaintNeeded];
        if not ParamsChanged then
          SetSkinParams;
        ...
        if SingleLineEdit and not IsWindowEnabled(CtrlHandle) then begin
          ...
          if IsWinVistaUp and InAnimationProcess and (TWMPaint(Message).DC <> SkinData.PrintDC) then begin
            Message.Result := ERASE_OK;
            Exit;
          end;
      end;
    ...
    WM_PAINT:
      if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
        ...
        if InUpdating(SkinData) or InAnimationProcess and (SkinData.PrintDC = 0) then begin
          ...
          if GetWindowLong(CtrlHandle, GWL_EXSTYLE) and WS_EX_RTLREADING = 0 {SysLocale.MiddleEast }then begin
            Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, -1, hdc(Message.wParam));
            DC := GetWindowDC(CtrlHandle);
            if SkinData.COC <> COC_TVST then
              BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, cxLeftEdge, min(3, cyTopEdge) {do not paint over header}, cxRightEdge, cyBottomEdge);
...
//      Skindata.FOwnerControl.Perform(WM_SETREDRAW, 0, 0); // Desc 2
      inherited acWndProc(Message);
...
//{$ENDIF}
        ...
        CtrlState := CtrlState - [csNCPaintNeeded];
        Exit;
      end;
...
//        Skindata.FOwnerControl.Perform(WM_SETREDRAW, 0, 0); // Desc 2
        inherited acWndProc(Message);
//        Skindata.FOwnerControl.Perform(WM_SETREDRAW, 1, 0); // Desc 2
        SetSkinParams;
        if csPaintNeeded in CtrlState then
          RedrawWindow(CtrlHandle, nil, 0, RDW_UPDATENOW or RDW_INVALIDATE or RDW_FRAME)
        else
          if csNCPaintNeeded in CtrlState then
            TrySendMessage(CtrlHandle, WM_NCPAINT, 0);
      end
      else
        Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
  ...
  case Message.Msg of
    ...
    WM_SIZE:
      if IsWindowVisible(CtrlHandle) and (SkinData.FUpdateCount <= 0) then
  ...
  if aHandle <> 0 then begin
    if (Repaint or not (Self is TacComboListWnd)) and
      ...
      if ASkinData = nil then begin
        if not IsWinVistaUp or (SkinParams.SkinSection = s_ComboBox) then
          Ac_SetWindowTheme(AHandle, s_Space, s_Space) // Don't touch win themes in dialogs
      end
      else
        if not ((ASkinData.COC in [COC_TsListView, COC_TsTreeView]) and IsWinVistaUp) then
    ...
    if OwnSkinData and not (SkinData.COC in [COC_TVST] {exceptions}) then
        ...
        with PacBGInfo(Message.LParam)^ do begin
          if BgType = btUnknown then
            if SkinData.CustomColor then begin // Used in ComboListWnd and similar windows
              BgType := btFill;
              if SkinData.FOwnerControl <> nil then
                Color := TacAccessControl(SkinData.FOwnerControl).Color
              else
                Color := Self.Color;
            end
            else
...
begin
  if not (SkinData.FOwnerControl is TCustomEdit) or sBarHorz.fScrollVisible or sBarVert.fScrollVisible then
    ...
    Result := ((WndSize.cx - 2 * cxLeftEdge) <> WidthOf(cR)) or ((WndSize.cy - 2 * cyTopEdge) <> HeightOf(cR));
  end;
end;
...
{$ENDIF}
      if Assigned(FOwnerControl) and not (csDestroying in FOwnerControl.ComponentState) then begin
        if not SkinData.CustomColor then begin
          TWinControl(FOwnerControl).Brush.Handle := 0;
          TacAccessControl(FOwnerControl).Color := clWindow;	
    		end;
        CtrlSkinState := CtrlSkinState or ACS_CHANGING;
        TempOnChange := TacAccessControl(FOwnerControl).Font.OnChange;
//        if not (Self is TacGridEhWnd) then
          TacAccessControl(FOwnerControl).Font.OnChange := nil; // Prevent a recreating of some controls

        if not SkinData.CustomFont then
          TacAccessControl(FOwnerControl).Font.Color := clWindowText;

        TacAccessControl(FOwnerControl).Font.OnChange := TempOnChange;
      ...
      if (FOwnerControl <> nil) and not (csDestroying in FOwnerControl.ComponentState) then // Scrollbars updating if exists
        if ((SkinData = nil) or not SkinData.FUpdating or (SkinData.SkinIndex < 0)) {and IsWindowVisible(Ctrl.Handle)} then
          SetWindowPos(CtrlHandle, 0, 0, 0, 0, 0, SWPA_FRAMECHANGED);
    end;
end;
...
{$ENDIF}
  if Assigned(SkinData) and Assigned(SkinData.FOwnerControl) then begin
    StdColor := TacAccessControl(SkinData.FOwnerControl).Color;
    StdFontColor := TacAccessControl(SkinData.FOwnerControl).Font.Color;
    ...
    if Assigned(SkinData) and SkinData.Skinned then begin
      if Assigned(SkinData.FOwnerControl) then begin // If VCL used
        if ControlIsActive(SkinData) then begin
          b := SkinData.CommonSkinData.gd[SkinData.SkinIndex].States > 1;
          if b then
            CtrlState := CtrlState + [csNCPaintNeeded];
        end
        else
        ...
        if not SkinData.CustomColor then begin
          ...
          if HasProperty(SkinData.FOwnerControl, acFocusColor) then begin
            if TacAccessControl(SkinData.FOwnerControl).Color <> C then begin
            ...
            if TacAccessControl(SkinData.FOwnerControl).Color <> C then begin
              TacAccessControl(SkinData.FOwnerControl).Color := C;
              CtrlState := CtrlState + [csPaintNeeded];
            end;
        ...
        if not SkinData.CustomFont then begin
          ...
          if (TacAccessControl(SkinData.FOwnerControl).Font.Color <> C) {and not (csRecreating in SkinData.FOwnerControl.ControlState)} then begin
            SkinData.CtrlSkinState := SkinData.CtrlSkinState or ACS_CHANGING;
//            if not (Self is TacGridEhWnd) then
//              TacAccessControl(SkinData.FOwnerControl).Font.OnChange := nil; // Do not change a size

            if not (SkinData.COC in [COC_TsTreeView, COC_TsListView]) then begin // Avoiding of font scaling problem
              TacAccessControl(SkinData.FOwnerControl).Font.Color := C;
              SkinData.FOwnerControl.Perform(CM_FONTCHANGED, 0, 0);
            end;
        ...
        if not SkinData.CustomColor then
          if not DlgMode then
            Color := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[0].Color
          else
...
begin
  case Message.Msg of
    ...
    WM_PRINT: begin
      ...
      if (SkinData.FOwnerControl is TCustomListView) and (TListView(SkinData.FOwnerControl).BorderStyle = bsNone) then begin
  ...
  case Message.Msg of
    WM_SIZE: begin
      SkinData.BGChanged := True;
      if SkinData.Skinned and not InUpdating(SkinData) and IsWindowVisible(CtrlHandle) then begin
        PrepareCache(SkinData, CtrlHandle, DlgMode);
        InvalidateRect(CtrlHandle, nil, True);
      end;
    end;
    ...
    WM_PRINT:
      if ViewStyle = vsReport then begin
        ...
        MoveWindowOrg(TWMPaint(Message).DC, cxLeftEdge, cyTopEdge);
        IntersectClipRect(TWMPaint(Message).DC, 0, 0,
                          ...
                          SkinData.FCacheBmp.Height - 2 * cyTopEdge  - integer(sBarHorz.fScrollVisible) * GetScrollMetric(sBarHorz, SM_SCROLLWIDTH));
...
begin
  ...
  inherited;
  HoverColIndex := -1;
  FhHeaderProc := {$IFDEF DELPHI6UP}Classes.{$ENDIF}MakeObjectInstance(HeaderWndProc);
...
var
  ...
  State, Flags, gWidth, si, ArrowIndex: integer;
{$IFDEF TNTUNICODE}
  Item: THDItemW;
{$ELSE}
  Item: THDItem;
...
begin
  try
    ...
    with SkinData.SkinManager.SkinCommonInfo do
      si := PaintSection(TempBmp, Sections[ssColHeader], Sections[ssButton], State, SkinData.CommonSkinData, ControlRect.TopLeft, CI.FillColor);
    ...
    CheckSkinFont(@TempBmp.Canvas.Font);
    TextRC := R;
...
{$ENDIF}
      ...
      if item.fmt and HDF_SORTDOWN = HDF_SORTDOWN then begin
        ArrowSide := asBottom;
        ArrowIndex := SkinData.CommonSkinData.ScrollBtns[ArrowSide].GlyphIndex;
      end
      else
      ...
      if ArrowIndex <> -2 then
        if ArrowIndex >= 0 then
          ArrowSize := MkSize(SkinData.CommonSkinData.ma[ArrowIndex].Width + 6, SkinData.CommonSkinData.ma[ArrowIndex].Height)
        else
...
{$ENDIF}
      ...
      if ArrowIndex >= 0 then
        DrawSkinGlyph(TempBmp, Point(TextRc.Right + 6, (HeightOf(TextRc) - ArrowSize.cy) div 2), State, 1, SkinData.CommonSkinData.ma[ArrowIndex], MakeCacheInfo(TempBmp))
      else
        if ArrowIndex = -1 then begin
          ...
          gR.Bottom := gR.Top + 3;
          State := mini(State, ac_MaxPropsIndex);
          C := SkinData.CommonSkinData.gd[si].Props[State].FontColor.Color;
          DrawColorArrow(TempBmp, C, gR, ArrowSide, GetPPI(SkinData));
        end;

      if (State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
...
begin
  if not Destroyed and (ViewStyle = vsReport) then begin
    try
      with Message do begin
        case Msg of
          WM_NCHITTEST:
            if HotTrack then begin
              SkinData.SkinManager.ActiveControl := CtrlHandle;
...
begin
  ...
  if FhHeaderProc <> nil then begin
    {$IFDEF DELPHI6UP}Classes.{$ENDIF}FreeObjectInstance(FhHeaderProc);
    FhHeaderProc := nil;
  end;
  ...
  if SkinData.FOwnerControl <> nil then
    TacAccessControl(SkinData.FOwnerControl).Color := StdColor;
...
begin
  if SkinData.FOwnerControl <> nil then
    ...
    St := ACNativeInt(SendMessage(CtrlHandle, LVM_FIRST + 143 {LVM_GETVIEW}, 0, 0));
    case St of
      0:   Result := vsIcon;
      1:   Result := vsReport;
    ...
    WM_VSCROLL:
      with TWMVScroll(Message), sBarVert.ScrollInfo do begin
        ...
        if (ScrollCode = SB_ENDSCROLL) and HasProperty(SkinData.FOwnerControl, acDataSource) and (SkinData.FOwnerControl.ClassName <> acTwwDBGrid) then
          SendScrollMessage(CtrlHandle, WM_VSCROLL, SB_THUMBPOSITION, sBarVert.ScrollInfo.nTrackPos, not (sBarVert.sw is TacListViewWnd));
      end;

    WM_PRINT:
      with TWMPaint(Message) do begin
        InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
        if (sBarVert <> nil) and sBarVert.fScrollVisible then
          VSWidth := GetScrollMetric(sbarVert, SM_SCROLLWIDTH)
        else
          VSWidth := 0;

        if SkinData.BGChanged or SkinData.FCacheBmp.Empty then begin
          if not ParamsChanged then
            SetSkinParams;

          PrepareCache(SkinData, CtrlHandle, False);
        end;
        ...
        Bmp := CreateBmp32(WndSize.cx + VSWidth, WndSize.cy);
        NewDC := Bmp.Canvas.Handle;
        Bmp.Canvas.Lock;

        SavedDC := SaveDC(NewDC);
        try
          MoveWindowOrg(NewDC, cxLeftEdge, cyTopEdge);
          if (sBarVert <> nil) or (sBarHorz <> nil) then
            IntersectClipRect(NewDC, 0, 0,
                              SkinData.FCacheBmp.Width  - cxLeftEdge - cxRightEdge  - VSWidth,
                              SkinData.FCacheBmp.Height - cyTopEdge - cyBottomEdge  - integer(sBarHorz.fScrollVisible) * GetScrollMetric(sBarHorz, SM_SCROLLWIDTH));


          Message.Result := CallPrevWndProc(CtrlHandle, WM_PAINT, WParam(NewDC), Message.LParam);
        finally
          RestoreDC(NewDC, SavedDC);
        end;
        BitBltBorder(NewDC, 0, 0, WndSize.cx, WndSize.cy, SkinData.FCacheBmp.Canvas.Handle, 0, 0, cxLeftEdge, cyTopEdge, cxRightEdge, cyBottomEdge);
        Ac_NCDraw(Self, CtrlHandle, -1, NewDC); // Scrolls painting
        Bmp.Canvas.Unlock;

        BitBlt(DC, OffsX, OffsY, Bmp.Width, Bmp.Height, Bmp.Canvas.Handle, 0, 0, SRCCOPY);
        MoveWindowOrg(DC, cxLeftEdge, cyTopEdge);
        Bmp.Free;
        Exit;
      end;
      ...
      case Message.WParamHi of
        ...
        AC_SETSCALE:
          ...
          if HasProperty(SkinData.FOwnerControl, acTitleFont) then begin
            ...
            if f <> nil then
              TFont(f).Height := TacAccessControl(SkinData.FOwnerControl).Font.Height;
          end;
      end;
...
begin
  ...
  with SkinData, SkinManager, SkinData.CommonSkinData do begin
    ...
    if HasProperty(FOwnerControl, acControlLook) then begin
      ...
      TryChangeIntProp(FOwnerControl, acGridLineColor, Palette[pcGrid]);
    ...
    with SkinCommonInfo do begin
      TryChangeIntProp(FOwnerControl, acFooterColor,     gd[IndexGlobalInfo].Props[0].Color);
      TryChangeIntProp(FOwnerControl, acFooterCellColor, gd[IndexGlobalInfo].Props[0].Color);
      TryChangeIntProp(FOwnerControl, acTitleColor,      gd[IndexGlobalInfo].Props[0].Color);
            ...
            if SkinData.Skinned then begin
              if InUpdating(SkinData) then begin
                ...
                try
                  if not SkinData.CustomColor then begin
                    State := integer(SkinData.FFocused or SkinData.FMouseAbove and MayBeHot(SkinData));
                    C := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[State].Color
                  end
                  else
                    if SkinData.FOwnerControl <> nil then
                      C := TacAccessControl(SkinData.FOwnerControl).Color
                    else
    ...
    if SkinData <> nil then
      with SkinData, CommonSkinData do begin
        ...
        if not CustomColor then
          C := gd[SkinIndex].Props[State].Color
        else
          if FOwnerControl <> nil then
            C := TacAccessControl(FOwnerControl).Color;

        if not CustomFont then
          fC := gd[SkinIndex].Props[State].FontColor.Color
        else
          if FOwnerControl <> nil then
            fC := TacAccessControl(FOwnerControl).Font.Color;

        if (FOwnerControl <> nil) {and not (csRecreating in FOwnerControl.ControlState) }then
          with TacAccessControl(FOwnerControl) do begin
//            TreeView_SetTextColor(CtrlHandle, acColorToRGB(fC));
            if Font.Color <> fC then begin
              Font.OnChange := nil; // Do not change a size
              Font.Color := fC;
            end;
...
begin
  case DefaultManager.Fonts.MainMode of
    fmCustom:   sName := DefaultManager.Fonts.MainFont;
    fmFromSkin: sName := DefaultManager.SkinCommonInfo.MainFont;
  end;
  if sName <> '' then begin
    f := TFont.Create;
    try
...
{$ENDIF}
  case Message.Msg of
    ...
    CB_SETCURSEL, CM_CHANGED, CM_TEXTCHANGED:
      if EasyComboBox and (SkinData.FUpdateCount <= 0) then begin
...
{$ENDIF}
    ...
    WM_SETREDRAW: begin
      ...
      if Message.WParam = 0 then begin
        inc(SkinData.FUpdateCount);
        if ListSW <> nil then
          inc(ListSW.SkinData.FUpdateCount);
      end
      else begin
        dec(SkinData.FUpdateCount);
        if ListSW <> nil then
          dec(ListSW.SkinData.FUpdateCount);
      end;
    end;
    ...
    WM_CTLCOLOREDIT: begin
      SetTextColor(hdc(Message.wParam), acColorToRGB(clWindowText));
      SetBkColor(hdc(Message.wParam), acColorToRGB(clWindow));
      if DefaultManager <> nil then
        Message.Result := LResult(DefaultManager.Brushes[pcEditBG]);
//      Message.Result := LResult(CreateSolidBrush(acColorToRGB(clWindow)));
    end;
...
{$ENDIF}
        if FListHandle = 0 then begin
          ...
          if (i >= 0) and not DlgMode then
            with SkinData.CommonSkinData.gd[i].Props[0] do begin
...
begin
  ...
  if not DlgMode or (Style and CBS_DROPDOWNLIST = CBS_DROPDOWNLIST) then
    ...
    if Style and CBS_DROPDOWN = CBS_DROPDOWN then
      w := GetComboBtnSize(Skindata.SkinManager, SkinData.CommonSkinData.PPI)
    else
      w := 0;

  GetWindowRect(CtrlHandle, r);
  if GetWindowLong(CtrlHandle, GWL_EXSTYLE) and WS_EX_RTLREADING <> 0 {SysLocale.MiddleEast }then
    Result.Left := SkinData.CommonSkinData.ComboBoxMargin
  else
    Result.Left := WidthOf(r) - w - SkinData.CommonSkinData.ComboBoxMargin;

  Result.Top := SkinData.CommonSkinData.ComboBoxMargin;
  Result.Right := Result.Left + w;
  Result.Bottom := HeightOf(r) - SkinData.CommonSkinData.ComboBoxMargin;
end;
...
begin
  ...
  if odComboBoxEdit in State then begin
    ...
    if (ControlIsActive(SkinData) or (GetFocus <> 0) and ComboBox.Focused or SkinData.FFocused) and (SkinData.CommonSkinData.gd[SkinData.SkinIndex].States > 1) then
      aState := 1
    else
      aState := 0;
  end
    ...
    CI.Ready := False;
    CI.FillColor := Color;
    aState := integer(SkinData.CommonSkinData.gd[SkinData.SkinIndex].States > 1);
  end;
  aState := mini(aState, ac_MaxPropsIndex);
  ...
  if odSelected in State then begin
    ...
    if sNdx < 0 then
      ...
      PaintItem(sNdx, CI, True, 1, Rect, MkPoint, Bmp, SkinData.CommonSkinData);
    ...
    ComboBox.Canvas.Font.Color := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[aState].FontColor.Color;
    if CI.Ready then
  ...
  if odSelected in State then
    ...
    Bmp.Canvas.Font.Color := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[aState].FontColor.Color;
...
var
  ...
  TmpBmp: TBitmap;
begin
  if (SkinData <> nil) and SkinData.SkinManager.IsActive then begin
    ...
    with SkinData.CommonSkinData, ComboBtn do begin
      if SkinIndex >= 0 then begin
        BitBlt(TmpBmp.Canvas.Handle, 0, 0, TmpBmp.Width, TmpBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY);
        PaintItem(SkinIndex, MakeCacheInfo(TmpBmp),
                  True, Mode, R, MkPoint, TmpBmp, SkinData.CommonSkinData, BGIndex[0], BGIndex[1]);
      end
      else
        BitBlt(TmpBmp.Canvas.Handle, 0, 0, TmpBmp.Width, TmpBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY);
      ...
      if GlyphIndex >= 0 then begin
        ...
        DrawSkinGlyph(TmpBmp, Point(R.Left + (WidthOf(R) - ma[GlyphIndex].Width) div 2,
                      (HeightOf(wR) - ButtonHeight) div 2), Mode, 1, ma[GlyphIndex], MakeCacheInfo(TmpBmp));
      end
      else begin // Paint without glyph
        Mode := mini(Mode, ac_MaxPropsIndex);
        if SkinIndex >= 0 then
          C := gd[SkinIndex].Props[Mode].FontColor.Color
        else
          C := clBlack;

        DrawArrow(TmpBmp, C, clNone, R, asBottom, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData));
//        DrawColorArrow(TmpBmp, C, R, asBottom, GetPPI(SkinData));
      end;
    end;
    BitBlt(DC, R.Left, R.Top, WidthOf(R), HeightOf(R), TmpBmp.Canvas.Handle, R.Left, R.Top, SRCCOPY);
    FreeAndNil(TmpBmp);
  end;
  if not acMouseInControlDC(CtrlHandle) then
...
begin
  if SkinData.FOwnerControl <> nil then
    SkinData.FCacheBMP.Canvas.Font.Assign(TacAccessControl(SkinData.FOwnerControl).Font);
...
var
  DC: hdc;
begin
  if acMouseInControlDC(CtrlHandle) and (GetWindowLong(CtrlHandle, GWL_STYLE) and CBS_DROPDOWNLIST <> CBS_SIMPLE) then
    ShowGlowingIfNeeded(SkinData, False, CtrlHandle, MaxByte, SkinData.SkinManager.Effects.AllowAnimation)
  else
    if SkinData.GlowID >= 0 then
      HideGlow(SkinData.GlowID);
...
{$ENDIF}
  ...
  case Message.Msg of
    WM_WINDOWPOSCHANGED:
      if (Message.LParam <> 0) and (TWindowPos(Pointer(Message.LParam)^).Flags and SWP_FRAMECHANGED <> 0) and IsWindowVisible(CtrlHandle) then
        AddToAdapter(TWinControl(SkinData.FOwnerControl));
...
{$ENDIF}
    ...
    if obj <> nil then begin
      ...
      TryChangeIntProp(obj, acDataHorzColor, Palette[pcGrid]);
      TryChangeIntProp(obj, acDataVertColor, Palette[pcGrid]);
...
{$ENDIF}
  case Message.Msg of
    WM_NCHITTEST: begin
      Message.Result := Ac_NCHitTest(Self, CtrlHandle, Message.WParam, Message.LParam);
      if Message.Result = 0 then
        Message.Result := CallPrevWndProc(CtrlHandle, WM_NCHITTEST, Message.WParam, Message.LParam);
    ...
    LB_ADDSTRING{For faster adding of items}, WM_VSCROLL, WM_HSCROLL: begin // Direct calling of standard scrolling
      Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
      Exit;
    end;
    ...
    WM_PRINT, WM_PRINTCLIENT:
      if (Message.WParam <> 0) and IsWindowVisible(CtrlHandle) then
        with Message do begin
          PrepareCache;
          Result := CallPrevWndProc(CtrlHandle, Msg, WParam, LParam);
          if (Msg <> WM_PRINTCLIENT) or (ProcessMessage(LB_GETCOUNT, 0, 0) = 0) then begin
            DC := hdc(WParam);
            OffsetRect(wR, -wR.Left, -wR.Top);
            if not Shown then begin
              if uCurrentScrollPortion = HTSCROLL_NONE then
                if (ProcessMessage(LB_GETCOUNT, 0, 0) = 0) then
                  BitBlt(DC, wR.Left, wR.Top, WidthOf(wR), HeightOf(wR), SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY)
                else
...
begin
  ...
  if IsWinVistaUp then
    DoLayered(CtrlHandle, True, MaxByte);
...
begin
  if not bPreventStyleChange then begin
    ...
    bPreventStyleChange := True;
    with SkinData, SkinData.CommonSkinData do
      if Assigned(SkinData) and Assigned(SkinManager) and FOwner.IsValidSkinIndex(SkinIndex) then begin
        if not CustomColor then
        ...
        FrameColor := FOwner.Palette[pcBorder];
        if SkinData <> nil then begin
          BGChanged := True;
          Updating := False;
        end;
      ...
      WM_NCPAINT:
        if IsWindowVisible(CtrlHandle) then begin
          ...
          if not InUpdating(SkinData) then begin
            ...
            SavedDC := SaveDC(DC);
            try
              bIndex := SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex;
              if (bIndex >= 0) then
                if GetWindowLong(CtrlHandle, GWL_EXSTYLE) and WS_EX_CLIENTEDGE = WS_EX_CLIENTEDGE then
                  bWidth := 2
                else
                  bWidth := integer(GetWindowLong(CtrlHandle, GWL_STYLE) and WS_BORDER = WS_BORDER)
              else
                bWidth := SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderWidth;
...
{$IFDEF WIN64}
  {$O-} // Solving issue with invisible header
{$ENDIF}
...
begin
  ...
  if HasProperty(SkinData.FOwnerControl, acVETColors) then begin
    obj := GetObjProp(SkinData.FOwnerControl, acVETColors);
    if obj <> nil then begin
      C := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[0].FontColor.Color;
  ...
  if HasProperty(SkinData.FOwnerControl, acColors) then begin
    obj := GetObjProp(SkinData.FOwnerControl, acColors);
    if obj <> nil then begin
      if FocusedSelectionColor = clHighlight then
        SetIntProp(obj, acFocusedSelectionColor, SkinData.SkinManager.GetHighLightColor);

      if UnfocusedSelectionColor = clHighlight then
        SetIntProp(obj, acUnfocusedSelectionColor, SkinData.SkinManager.GetHighLightColor(False));

      if FocusedSelectionBorderColor = clHighlight then
        SetIntProp(obj, acFocusedSelectionBorderColor, SkinData.SkinManager.GetHighLightColor);

      if UnfocusedSelectionBorderColor = clHighlight then
        SetIntProp(obj, acUnfocusedSelectionBorderColor, SkinData.SkinManager.GetHighLightColor(False));

     if SelectionTextColor = clHighlightText then
...
begin
  if Assigned(Self) and not DontRepaint and Assigned(SkinData) and SkinData.Skinned and (PaintInfo.TargetCanvas <> nil) then
    with SkinData.SkinManager, SkinData.CommonSkinData do begin
      if IsValidSkinIndex(SkinCommonInfo.Sections[ssColHeader]) then
        si := SkinCommonInfo.Sections[ssColHeader]
      else
        si := SkinCommonInfo.Sections[ssButton];

      R := PaintInfo.PaintRectangle;
      Bmp := CreateBmp32(R);
      if PaintInfo.Column = nil then begin
        PaintItem(si, MakeCacheInfo(SkinData.FCacheBmp), True, 0, MkRect(Bmp), R.TopLeft, Bmp, SkinData.CommonSkinData);
        BitBlt(PaintInfo.TargetCanvas.Handle, R.Left, R.Top, Bmp.Width, Bmp.Height, Bmp.Canvas.Handle, 0, 0, SRCCOPY);
        Bmp.Free;
      end
      else begin
        ...
        if hpeBackground in Elements then begin
          PaintItem(si, MakeCacheInfo(SkinData.FCacheBmp), True, State, MkRect(Bmp), R.TopLeft, Bmp, SkinData.CommonSkinData);
          BitBlt(PaintInfo.TargetCanvas.Handle, R.Left, R.Top, Bmp.Width, Bmp.Height, Bmp.Canvas.Handle, 0, 0, SRCCOPY);
        end
        else begin
          BitBlt(Bmp.Canvas.Handle, 0, 0, Bmp.Width, Bmp.Height, PaintInfo.TargetCanvas.Handle, R.Left, R.Top, SRCCOPY);
...
{$ENDIF}
            if Text <> '' then begin
              ...
              OffsetRect(TextRC, -R.Left, -R.Top);
              HeaderProp := GetObjProp(SkinData.FOwnerControl, acHeader);
              if HeaderProp <> nil then begin
          ...
          if (hpeSortGlyph in Elements) and PaintInfo.ShowSortGlyph then begin
            ...
            if HeaderProp <> nil then begin
              ...
              if GetOrdProp(HeaderProp, acSortDirection) = 0 then
                Index := ScrollBtns[asTop].GlyphIndex
              else
                Index := ScrollBtns[asBottom].GlyphIndex;

              if Index >= 0 then
                DrawSkinGlyph(Bmp, Point(PaintInfo.SortGlyphPos.x - R.Left,
                              (HeightOf(R) - ma[Index].Height) div 2), State, 1, ma[Index], MakeCacheInfo(Bmp))
              else begin
                State := mini(State, ac_MaxPropsIndex);
                C := gd[CommonSkinData.ScrollBtns[Side].SkinIndex].Props[State].FontColor.Color;
                gR.Left := PaintInfo.SortGlyphPos.x - R.Left;
                gR.Top := (HeightOf(R) - ma[Index].Height) div 2;
                gR.Bottom := gR.Top + 3;
                gR.Right := gR.Left + 6;
                if (State > 1) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
                  OffsetRect(gR, 1, 1);

                DrawColorArrow(Bmp, C, gR, Side, GetPPI(SkinData));
              end;
            end;
          end;
...
procedure TacVirtualTreeViewWnd.InitSkinData;
begin
  inherited;
  SkinData.COC := COC_TVST;
end;


function TacMainWnd.DoPrepareCache: boolean;
begin
...
{$IFNDEF FPC}
    WM_CONTEXTMENU:
      if (SkinData <> nil) and (SkinData.FOwnerControl <> nil) then
        with SkinData, TacAccessControl(FOwnerControl) do
...
{$ENDIF}
  ...
  if not Assigned(SkinData) or not Assigned(SkinData.SkinManager) or not SkinData.SkinManager.IsActive then begin
    if Message.Msg = SM_ALPHACMD then
      case Message.WParamHi of
        AC_GETFONTINDEX:
          with SkinData do begin
...
//                  PacPaintInfo(Message.LParam).FontIndex := SkinIndex;
    ...
    if Message.Msg = SM_ALPHACMD then
      case Message.WParamHi of
        ...
        AC_GETBG:
          if (SkinData <> nil) and not InUpdating(SkinData) then begin
            if SkinData.BGChanged then
//SkinData.BGChanged := True;
              DoPrepareCache;
...
begin
  ...
  OldProc := nil;
  if uxthemeLib <> 0 then
    if (ASkinData = nil) or not (ASkinData.COC in [COC_TsSkinProvider]) then
      if not (Self is TacPanelWnd) and not (Self is TacSpinWnd) then
  ...
  if SkinData <> nil then begin
    if SkinManager <> nil then
      if not Destroyed then begin
        ...
        if not ItemsRemoving or (msSkinRemoving in SkinManager.ManagerState) or (Adapter <> nil) and (msSkinRemoving in TacCtrlAdapter(Adapter).Provider.SkinData.SkinManager.ManagerState) then begin
          if Assigned(OldWndProc) then begin
            ...
            if OldProc <> nil then begin
              if ACNativeInt(GetWindowLong(CtrlHandle, GWL_WNDPROC)) = ACNativeInt(NewWndProcInstance) then
                SetWindowLong(CtrlHandle, GWL_WNDPROC, LONG_PTR(OldProc));

              OldProc := nil;
              if NewWndProcInstance <> nil then begin
                {$IFDEF DELPHI6UP}Classes.{$ENDIF}FreeObjectInstance(NewWndProcInstance);
                NewWndProcInstance := nil;
              end;
            end;
        ...
        if (uxthemeLib <> 0) and (SkinData <> nil) then
          if SkinData.FOwnerObject is TsSkinProvider then begin
            if not (csDestroying in TsSkinProvider(SkinData.FOwnerObject).ComponentState) and TsSkinProvider(SkinData.FOwnerObject).DrawNonClientArea then
              Ac_SetWindowTheme(CtrlHandle, nil, nil);
          end
          else
            if not ((SkinData.COC in [COC_TsListView, COC_TsTreeView]) and AeroIsEnabled) then
              Ac_SetWindowTheme(CtrlHandle, nil, nil);
...
begin
  if SkinData = nil then
    if Self is TacBtnWnd then begin
      SkinData := TsCtrlSkinData.Create(nil, True);
      SkinData.FSWHandle := CtrlHandle;
...
begin
  ...
  RemoveProp(CtrlHandle, acPropStr);
  SetClassLong(CtrlHandle, GCL_HBRBACKGROUND, Integer(GetSysColorBrush(COLOR_BTNFACE)));
end;
...
begin
  ParamsChanged := True;
  SetClassLong(CtrlHandle, GCL_HBRBACKGROUND, Integer(SkinData.SkinManager.Brushes[pcMainColor]));
end;
...
begin
  ...
  inherited;
  Caption := GetWndText(CtrlHandle);
  if (Caption <> '') and not (Self is TacIconWnd) and not (Self is TacEdgeWnd) then begin
    GetWindowRect(CtrlHandle, WndRect);
    WndSize := MkSize(WndRect);
    rText := PaintText(True);
    WndSize.cx := WidthOf(rText);
    WndSize.cy := HeightOf(rText);
    if (WndSize.cx > WidthOf(WndRect)) or (WndSize.cy > HeightOf(WndRect)) then
      SetWindowPos(CtrlHandle, 0, 0, 0, WndSize.cx, WndSize.cy, SWP_NOMOVE or SWP_NOACTIVATE or SWP_NOZORDER or SWP_NOCOPYBITS or SWP_NOSENDCHANGING or SWP_NOREPOSITION);
  end;
end;


function TacStaticWnd.PaintContent: boolean;
begin
  if Caption <> '' then
    PaintText(False);
...
begin
  ...
  if not DoCalc then begin
    gwl := GetWindowLong(CtrlHandle, GWL_STYLE);
    gwl_ex := GetWindowLong(CtrlHandle, GWL_EXSTYLE);
    if gwl and SS_NOPREFIX <> 0 then
      Flags := Flags or DT_NOPREFIX;

    if gwl_ex and WS_EX_RIGHT <> 0 then
      Flags := Flags or DT_RIGHT;

    if Application.BiDiMode = bdRightToLeft then
      Flags := Flags or DT_RTLREADING;

    SkinData.FCacheBmp.Canvas.Font.Color := GetFontColor(CtrlHandle, SkinData.SkinIndex, SkinData.SkinManager);
    SkinData.FCacheBmp.Canvas.Brush.Style := bsClear;
    acWriteTextEx(SkinData.FCacheBmp.Canvas, PacChar(Caption), True, Result, Flags, -1, False);
  end
  else
    acDrawText(SkinData.FCacheBmp.Canvas.Handle, PacChar(Caption), Result, Flags or DT_CALCRECT);
end;
...
function SearchID(Child: HWND; Data: LParam): BOOL; stdcall;
type
  PHWND = ^HWND;
begin
  if GetDlgCtrlId(Child) = Integer(Pointer(Data)^) then begin
    Integer(Pointer(Data)^) := 0; // ID is found
    Result := False;
  end
  else
    Result := True;
...
function HaveID(CtrlHandle: THandle; aID: integer; WndData: TacBtnWnd): boolean;
var
  ChildID: HWND;
{$IFDEF D2009}
  i: integer;
...
begin
{$IFDEF D2009}
  if (WndData is TacButtonWnd) and (TacButtonWnd(WndData).Btn <> nil) then begin
    Result := False;
    for i := 0 to TacButtonWnd(WndData).Btn.Parent.ControlCount - 1 do
      if (TacButtonWnd(WndData).Btn.Parent.Controls[i] is TButton) and (TButton(TacButtonWnd(WndData).Btn.Parent.Controls[i]).ModalResult = aID) then begin
        Result := True;
        Exit;
    end
  end
...
{$ENDIF}
  begin
    ChildID := aID;
    EnumChildWindows(GetParent(CtrlHandle), @SearchID, LPARAM(@ChildID));
    Result := ChildID = 0;
  end;
end;
...
const               // blue     active
  BlueRedText: array [boolean, boolean] of TColor = ((slBtnRedText, slBtnRedTextActive), (slBtnBlueText, slBtnBlueTextActive));

procedure TacBtnWnd.acWndProc(var Message: TMessage);
var
  R: TRect;
  IsBlueCancel, OkIsFound: boolean;
begin
{$IFDEF LOGGED}
//  AddToLog(Message);
{$ENDIF}
  case Message.Msg of
    SM_ALPHACMD:
      case Message.WParamHi of
        AC_SETNEWSKIN:
          if ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager) then begin
            FreeAndNil(TsCtrlSkinData(SkinData).LightMask);
            FreeAndNil(TsCtrlSkinData(SkinData).HotCache);
          end;

        AC_REMOVESKIN:
          if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) and not (csDestroying in SkinData.FOwnerControl.ComponentState) then begin
            FreeAndNil(TsCtrlSkinData(SkinData).LightMask);
            FreeAndNil(TsCtrlSkinData(SkinData).HotCache);
          end;
        ...
        AC_GETCOLORTONE: begin
          if SkinData.Skinned then
            if ID in [idOK, idYes, idCancel, idNo] then begin
              if ID = idCancel then begin
                OkIsFound := HaveID(CtrlHandle, idOK, Self) or HaveID(CtrlHandle, idYes, Self);
                IsBlueCancel := HaveID(CtrlHandle, idNo, Self);
              end
              else begin
                IsBlueCancel := False;
                OkIsFound := True;
              end;
              if OkIsFound then begin
                if bcBackground in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
                  case ID of
                    idOK, idYes:    PacColoring(Message.LParam)^.ColorToneBG := iff(CurrentState = 0, slBtnGreen, slBtnGreenActive);
                    idCancel, idNo: PacColoring(Message.LParam)^.ColorToneBG := BlueRedText[IsBlueCancel, CurrentState <> 0];
{                      if IsBlueCancel then
                        PacColoring(Message.LParam)^.ColorToneBG := iff(CurrentState = 0, slBtnBlue, slBtnBlueActive)
                      else
                        PacColoring(Message.LParam)^.ColorToneBG := iff(CurrentState = 0, slBtnRed, slBtnRedActive)}
                  end;

                if bcGlyph in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
                  case ID of
                    idOK, idYes:    PacColoring(Message.LParam)^.ColorText := iff(CurrentState = 0, slBtnGreenText, slBtnGreenTextActive);
                    idCancel, idNo: PacColoring(Message.LParam)^.ColorToneBG := BlueRedText[IsBlueCancel, CurrentState <> 0];
{                      if IsBlueCancel then
                        PacColoring(Message.LParam)^.ColorText := iff(CurrentState = 0, slBtnBlueText, slBtnBlueTextActive)
                      else
                        PacColoring(Message.LParam)^.ColorText := iff(CurrentState = 0, slBtnRedText, slBtnRedTextActive)}
                  end;
    ...
    WM_SIZE{, WM_WINDOWPOSCHANGED}: begin
      ...
      if IsWindowVisible(CtrlHandle) or
           (GetWindowLong(CtrlHandle, GWL_STYLE) and WS_VISIBLE <> 0) or
             (SkinData.FOwnerControl = nil) or
               not SkinData.FOwnerControl.Visible or
                 (SkinData.COC <> COC_TsAdapterEdit) then // Prevent errors in some controls when not visible
        if SkinData.GlowID >= 0 then begin
          inherited;
          RepaintGlow(SkinData.GlowID);
        end
    ...
    WM_MOVE: begin
      if IsValidIndex(SkinData.SkinIndex, Length(SkinData.CommonSkinData.gd)) then
      ...
      if IsWindowVisible(CtrlHandle) or
           (GetWindowLong(CtrlHandle, GWL_STYLE) and WS_VISIBLE <> 0) or
             (SkinData.FOwnerControl = nil) or
               not SkinData.FOwnerControl.Visible then // Prevent errors in some controls when not visible
        if SkinData.GlowID >= 0 then begin
          inherited;
          HideGlow(SkinData.GlowID);
        end
    ...
    WM_LBUTTONUP:
      if SkinData <> nil then begin
        if beClick in SkinData.SkinManager.AnimEffects.Buttons.Events then
...
begin
  ...
  SkinData.FCacheBMP.Width := WndSize.cx;
  SkinData.FCacheBMP.Height := WndSize.cy;
  if (CurrentState = 3 {Def/Focused}) and (SkinData.CommonSkinData.gd[SkinData.SkinIndex].States < 4) then
    iState := 1
  else
    iState := min(CurrentState, SkinData.CommonSkinData.gd[SkinData.SkinIndex].States - 1);
  ...
  PaintItem(SkinData, CI, True, iState, MkRect(WndSize), p, SkinData.FCacheBmp, False{True}, integer(Down), integer(Down));

  HandleLighting(TsCtrlSkinData(SkinData), iState);

  GetColoring(Coloring);
//  if Coloring.ColorToneBG <> clNone then
//    ChangeBitmapPixels(SkinData.FCacheBmp, ChangeColorTone, acColorToRGB(Coloring.ColorToneBG), clFuchsia);

  UpdateBmpColors(SkinData.FCacheBmp, SkinData, True, iState, Coloring.ColorToneBG);

  DrawCaption(Coloring);
  DrawGlyph;
...
begin
  ...
  DrawStyle := DT_EXPANDTABS or DT_CENTER or DT_ALPHATEXT * Cardinal(SkinData.TranspMode = 2);
  ...
  with SkinData, CommonSkinData do begin
    ...
    if Coloring.ColorText <> clNone then begin
      SkinData.FCacheBMP.Canvas.Font.Color := acColorToRGB(Coloring.ColorText);
    ...
    if IsWindowEnabled(CtrlHandle) and SkinManager.ButtonsOptions.ShowFocusRect and (ProcessMessage(BM_GETSTATE, 0, 0) and BST_FOCUS <> 0) and (Caption <> '') then begin
      ...
      State := min(1, gd[SkinIndex].States);
      FocusRect(SkinData.FCacheBMP.Canvas, R, gd[SkinData.SkinIndex].Props[State].FontColor.Color, clNone);
    end;
  end;
end;
...
begin
  ...
  if Down and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
    OffsetRect(Result, 1, 1);
end;


procedure TacBtnWnd.DoDrawText(var Rect: TRect; const Flags: Integer; CanGlow: boolean);
begin
  acWriteTextEx(SkinData.FCacheBMP.Canvas, PacChar(Caption), True, Rect, Flags, SkinData, CurrentState <> 0, SkinData.SkinManager, CanGlow);
end;
...
procedure TacBtnWnd.UpdateLight(Sender: TObject; Data: PacAnimEventData);
begin
  if IsWindowVisible(CtrlHandle) and IsWindowEnabled(CtrlHandle) and (beLighting in SkinData.SkinManager.AnimEffects.Buttons.Events){EventEnabled(aeLighting, FAnimatEvents)} then
    DoUpdateLight(TsCtrlSkinData(SkinData), Data.DoOutput)
  else
    TsCtrlSkinData(SkinData).FDoLighting := False;

  TsCtrlSkinData(SkinData).DestroyNeedlessLight;
  Data.DoLighting := TsCtrlSkinData(SkinData).DoLighting;
end;
...
procedure TacBtnWnd.GetColoring(var Coloring: TacColoring);
var
  IsBlueCancel, OkIsFound: boolean;
begin
  Coloring.ColorToneBG := clNone;
  Coloring.ColorText := clNone;
  if (SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring <> []) and (ID in [idOK, idYes, idCancel, idNo]) then begin
{$IFDEF D2009}
    if (Self is TacButtonWnd) and (TacButtonWnd(Self).Btn <> nil) then begin
      OkIsFound := HaveID(CtrlHandle, idOK, Self) or HaveID(CtrlHandle, idYes, Self);
      IsBlueCancel := (TacButtonWnd(Self).Btn.ModalResult = mrCancel) and HaveID(CtrlHandle, idNo, Self);
    end
    else
{$ENDIF}
      if ID = idCancel then begin
    ...
    if OkIsFound then begin
      if bcBackground in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
        case ID of
          idOK, idYes: Coloring.ColorToneBG := iff(CurrentState = 0, slBtnGreen, slBtnGreenActive);
          idCancel, idNo:
            if IsBlueCancel then
              Coloring.ColorToneBG := iff(CurrentState = 0, slBtnBlue, slBtnBlueActive)
            else
      ...
      if bcGlyph in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
        case ID of
          idOK, idYes:    Coloring.ColorGlyph := iff(CurrentState = 0, slBtnGreenText, slBtnGreenTextActive);
          idCancel, idNo: Coloring.ColorGlyph := BlueRedText[IsBlueCancel, CurrentState <> 0];
        end;

      if bcText in SkinData.SkinManager.ButtonsOptions.ModalButtonsColoring then
        case ID of
          idOK, idYes:    Coloring.ColorText := iff(CurrentState = 0, slBtnGreenText, slBtnGreenTextActive);
          idCancel, idNo: Coloring.ColorGlyph := BlueRedText[IsBlueCancel, CurrentState <> 0];
        end;
    end;
  end;
...
begin
  TsCtrlSkinData(SkinData).OnUpdateLight := UpdateLight;
  FMouseClicked := False;
  if SkinData.SkinSection = '' then
    SkinData.SkinSection := s_Button;

  SkinData.COC := COC_TsButton;
  if (SkinData <> nil) and (SkinData.FOwnerControl is TButton) then
    if TButton(SkinData.FOwnerControl).ModalResult in [mrOk, mrYes] then
      ID := idOk
    else
      if TButton(SkinData.FOwnerControl).ModalResult in [mrCancel, mrNo] then
        ID := idNo
      else
        ID := 0
  else
    ID := GetDlgCtrlID(CtrlHandle);
  inherited;
end;
...
begin
  case Message.Msg of
    ...
    WM_SETFOCUS: begin // Suppress this message for avoiding of anvir problem
      GetWindowRect(CtrlHandle, R);
...
function TacToolBarWnd.DoPrepareCache: boolean;
begin
  InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
  if SkinData.BGChanged then begin
...
procedure TacToolBarWnd.DrawBtn(const Index: integer; R: TRect; const DC: hdc);
const
  Margin = 2;
var
  pt: TPoint;
  ...
  if ProcessMessage(TB_GETBUTTON, Index, LPARAM(@Btn)) <> 0 then begin
    ...
    if Btn.fsStyle = TBSTYLE_SEP then begin // Separator
      ...
      if DefaultManager.IsValidImgIndex(mi) then begin
        ...
        PaintItem(si, MakeCacheInfo(SkinData.FCacheBmp), True, 0, nRect, Point(R.Left + nRect.Left, R.Top + nRect.Top), BtnBmp, SkinData.CommonSkinData);
    ...
    cy := 0;
    cx := 0;
    BtnIndex := SkinData.SkinManager.SkinCommonInfo.Sections[ssToolButton];
    pOffset := 0;
    ...
    PaintItem(BtnIndex, MakeCacheInfo(SkinData.FCacheBmp), True, State, MkRect(BtnBmp), r.TopLeft, BtnBmp, SkinData.CommonSkinData);
{$IFDEF TNTUNICODE}
    ...
    if Btn.iBitmap >= 0 then begin
      ...
      if i <> 0 then begin
        ...
        if Btn.fsStyle and $0080 = $0080 then begin // BTNS_WHOLEDROPDOWN
          ArrowIndex := SkinData.CommonSkinData.ScrollBtns[asBottom].GlyphIndex;
          if ArrowIndex >= 0 then begin
            ArrowSize := MkSize(SkinData.CommonSkinData.ma[ArrowIndex]);
            if s <> '' then begin
              DrawSkinGlyph(BtnBmp, Point((BtnBmp.Width - ArrowSize.cx) div 2 + pOffset, (BtnBmp.Height - ArrowSize.cy) div 2 + pOffset), State, 1, SkinData.CommonSkinData.ma[ArrowIndex], MakeCacheInfo(BtnBmp));
              ImageList_Draw(i, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(R) - cx - ArrowSize.cx) div 2 + pOffset, 4 + pOffset, 0);
            end
            else begin
              DrawSkinGlyph(BtnBmp, Point(BtnBmp.Width - 1 - ArrowSize.cx - pOffset, (BtnBmp.Height - ArrowSize.cy) div 2 + pOffset), State, 1, SkinData.CommonSkinData.ma[ArrowIndex], MakeCacheInfo(BtnBmp));
              ImageList_Draw(i, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(R) - cx - ArrowSize.cx) div 2 + pOffset, 4 + pOffset, 0);
            end;
          end
          else
...
{
          ico := ImageList_GetIcon(i, Btn.iBitmap, 0);
          GetIconInfo(ico, IconInf);
          icoBmp := TBitmap.Create;
          icoBmp.Handle := IconInf.hbmColor;
          cx := icoBmp.Width;
          cy := icoBmp.Height;
          icoBmp.Free;
          ...
          ildp.himl := i;
          ildp.i := Btn.iBitmap;
          ildp.hdcDst := BtnBmp.Canvas.Handle;
          ildp.x := (WidthOf(R) - cx) div 2 + pOffset;
          ildp.y := (HeightOf(R) - cy) div 2 + pOffset;
          ildp.cx := cx;
          ildp.cy := cy;
          ildp.xBitmap := 0;
          ildp.yBitmap := 0;
    ...
    if s <> '' then begin
      ...
      CheckSkinFont(@BtnBmp.Canvas.Font);
      ...
      Ndx := iff(State = 0, SkinData.SkinIndex, BtnIndex);

      if NeedParentFont(SkinData.SkinManager, Ndx, State) then begin
        pi.SkinManager := SkinData.SkinManager;
        pi.R := WndRect;
        pi.State := State;
        pi.FontIndex := Ndx;
        if GetFontIndex(GetParent(CtrlHandle), @pi) <> 0 then
          Ndx := pi.FontIndex;
      end;
...
begin
  ...
  if not InUpdating(SkinData) then begin
    ...
    if DlgMode then begin
      ...
      BordIndex := SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex;
      cRect.Left   := SkinData.CommonSkinData.ma[BordIndex].WL;
      cRect.Right  := WndSize.cx - SkinData.CommonSkinData.ma[BordIndex].WR;
      cRect.Bottom := WndSize.cy - SkinData.CommonSkinData.ma[BordIndex].WB;
...
begin
{$IFDEF LOGGED}
//  if SkinData.FOwnerControl.Tag = 2 then
//    AddToLog(Message);
{$ENDIF}
  case Message.Msg of
    SM_ALPHACMD:
...
begin
  ...
  SkinData.FCacheBmp.Canvas.Font.Color := GetFontColor(CtrlHandle, SkinData.SkinIndex, SkinData.SkinManager);
  SkinData.FCacheBmp.Canvas.Brush.Style := bsClear;
  acWriteTextEx(SkinData.FCacheBmp.Canvas, PacChar(Caption), True, R, Flags, -1, False);
end;
...
begin
  ...
  if SkinData.FOwnerControl is TWinControl then begin
    ...
    AlphaBroadCast(TWinControl(SkinData.FOwnerControl), m);
  end;
  inherited;
end;
...
begin
  ...
  if not TimerIsActive(SkinData) then begin
    InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
    if not InUpdating(SkinData) and SkinData.SkinManager.IsActive then
    ...
    DC := GetDC(CtrlHandle);
    if (SkinData.AnimTimer <> nil) and (SkinData.AnimTimer.BmpOut <> nil) then
      BitBlt(DC, 0, 0, SkinData.AnimTimer.BmpOut.Width, SkinData.AnimTimer.BmpOut.Height, SkinData.AnimTimer.BmpOut.Canvas.Handle, 0, 0, SRCCOPY)
    else begin
      PrepareCache;
      BitBlt(DC, 0, 0, WndSize.cx, WndSize.cy, SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY);
...
{$ENDIF}
  case Message.Msg of
    SM_ALPHACMD:
      case Message.WParamHi of
        AC_MOUSEENTER:
          if (SkinData.FOwnerControl <> nil) and not SkinData.FMouseAbove and (DefaultManager.ActiveControl = CtrlHandle) and not SkinData.SkinManager.Options.NoMouseHover then begin
            GetWindowRect(CtrlHandle, R);
            if PtInRect(R, acMousePos) then begin
              SkinData.FMouseAbove := True;
              SkinData.BGChanged := False;
              SkinData.Updating := False;
              if CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then begin
        ...
        AC_MOUSELEAVE:
          if SkinData.FMouseAbove and (SkinData.FOwnerControl <> nil) then begin
            SkinData.FMouseAbove := False;
            SkinData.Updating := False;
            SkinData.BGChanged := False;
            if CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then
              MouseInGlyph := False
            else
    ...
    CM_TEXTCHANGED, WM_ENABLE, BM_SETSTATE, BM_SETCHECK:
      if(SkinData.FOwnerControl <> nil) and IsWindowVisible(CtrlHandle) then begin
        SetRedraw(CtrlHandle, 0);
//        if Message.Msg in [] then begin
          if not CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then
            StopTimer(SkinData)
          else
            if SkinData.AnimTimer <> nil then begin
  //              SkinData.BGChanged := True;
              SkinData.Updating := False;
            end;
{        end
...
//        if (WM_KEYUP = Message.Msg) and (beClick in SkinData.SkinManager.AnimEffects.Buttons.Events) then
//          DoClickAnimation(SkinData);
    ...
    WM_LBUTTONDBLCLK, WM_LBUTTONDOWN:
      if (SkinData.FOwnerControl <> nil) and SkinData.FOwnerControl.Enabled then begin
        SkinData.FOwnerControl.ControlState := SkinData.FOwnerControl.ControlState + [csLButtonDown];
        if CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then begin
          ...
          DoChangePaint(SkinData, 2, UpdateWindow_CB, EventEnabled(aeMouseDown, [aeGlobalDef]), True);
      end;
    ...
    WM_LBUTTONUP:
      if SkinData.FOwnerControl <> nil then begin
        ...
        if CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then begin
          ...
          if EventEnabled(aeClick, [aeGlobalDef]) then
    ...
    WM_MOUSEMOVE:
      if (SkinData.FOwnerControl <> nil) and CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then
        MouseInGlyph := PtInRect(GlyphScreenRect, acMousePos);
  end;
end;
...
begin
  if Caption <> '' then begin
    ...
    Fmt := Fmt or TextWrapping[WordWrap] or DT_TOP;

    SkinData.FCacheBmp.Canvas.Font.Color := GetFontColor(CtrlHandle, SkinData.SkinIndex, SkinData.SkinManager, CtlState);
    SkinData.FCacheBmp.Canvas.Brush.Style := bsClear;
    acWriteTextEx(SkinData.FCacheBmp.Canvas, PacChar(Caption), True, rText, Fmt, -1, ControlIsActive(SkinData));
    SkinData.FCacheBmp.Canvas.Pen.Style := psClear;
    SkinData.FCacheBmp.Canvas.Brush.Style := bsSolid;
    if Focused then begin
      InflateRect(rText, 2, 0);
      State := min(1, SkinData.CommonSkinData.gd[SkinData.SkinIndex].States);
...
begin
  ...
  with SkinData{.SkinManager} do
    if (Style and BS_RADIOBUTTON <> BS_RADIOBUTTON) and (Style and BS_AUTORADIOBUTTON <> BS_AUTORADIOBUTTON) or
      ...
      Result := CommonSkinData.CheckBox[AState]
    else
      Result := CommonSkinData.RadioButton[AState = cbChecked];
end;


function TacCheckBoxWnd.GlyphScreenRect: TRect;
var
  i: integer;
  R: TRect;
begin
...
procedure TacCheckBoxWnd.SetMouseInGlyph(const Value: boolean);
begin
  if FMouseInGlyph <> Value then begin
    FMouseInGlyph := Value;
    if Value then begin
      if (SkinData.GlowID < 0) and SkinData.Skinned and CircleClickMode(SkinData.FOwnerControl, SkinData, [aeGlobalDef]) then
...
begin
  Result := SkinData.CommonSkinData.ma[i].Height;
end;


function TacCheckBoxWnd.SkinGlyphWidth(const i: integer): integer;
begin
  Result := SkinData.CommonSkinData.ma[i].Width;
end;
...
begin
  if (SkinData.FCacheBmp.Width > 0) and (i >= 0) then begin
    R := SkinCheckRect(i);
    sAlphaGraph.DrawSkinGlyph(SkinData.FCacheBmp, R.TopLeft, CtlState, 1, SkinData.CommonSkinData.ma[i], MakeCacheInfo(SkinData.FCacheBmp))
  end;
end;
...
function TacLinkWnd.PaintContent: boolean;
var
  s: acString;
  rText: TRect;
  Flags: Cardinal;
...
function TacIconWnd.PaintContent: boolean;
var
  hi: hIcon;
begin
  ...
  if IsVertical then begin
    ...
    if not PtInRect(WndRect, acMousePos) then begin
      ...
      if PtInRect(Rect(WndRect.Left, WndRect.Top, WndRect.Right - WndSize.cx div 2, WndRect.Bottom), acMousePos) then begin
        Btn1State := (1 + integer(bMousePressed)) * integer(not RTL);
        Btn2State := (1 + integer(bMousePressed)) * integer(RTL);
      end
      else begin
        Btn1State := (1 + integer(bMousePressed)) * integer(RTL);
        Btn2State := (1 + integer(bMousePressed)) * integer(not RTL);
      end;
...
{$ENDIF}
  if Skinned then begin
    case Message.Msg of
      ...
      WM_PAINT: begin
        ...
        if OwnerWnd <> nil then
          SendAMessage(OwnerWnd.Handle, AC_UPDATEPAGEBTN);
    ...
    case Message.Msg of
      ...
      WM_WINDOWPOSCHANGING: if OwnerWnd <> nil then
        if OwnerWnd.BiDiMode <> bdLeftToRight then
          TWMWindowPosChanging(Message).WindowPos.x := 0;
    end;
  end
  else begin
    inherited;
...
//    with SkinData.SkinManager do
    begin
      SkinData.SkinSection := SkinData.CommonSkinData.UpDownBtns[asTop].SkinSection;
      ...
      with SkinData.CommonSkinData, ScrollBtns[asTop] do
        if GlyphIndex >= 0 then begin
          ...
          R := Btn1Rect;
          if (Btn1State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
            OffsetRect(R, 1, 1);

          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn1State > 0), ac_MaxPropsIndex);
            DrawColorArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, R, asTop, GetPPI(SkinData))
          end
          else
            DrawColorArrow(SkinData.FCacheBMP, 0, R, asTop, GetPPI(SkinData));
        end;

      SkinData.SkinSection := SkinData.CommonSkinData.UpDownBtns[asbottom].SkinSection;
      ...
      with SkinData.CommonSkinData, ScrollBtns[asBottom] do
        if GlyphIndex >= 0 then begin
          ...
          R := Btn2Rect;
          if (Btn2State = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
            OffsetRect(R, 1, 1);

          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn2State > 0), ac_MaxPropsIndex);
            DrawColorArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, R, asBottom, GetPPI(SkinData));
          end
          else
            DrawColorArrow(SkinData.FCacheBMP, 0, R, asBottom, GetPPI(SkinData));
        end;
    end;
  end
  else begin
...
//    with SkinData.SkinManager do
    begin
      SkinData.SkinSection := SkinData.CommonSkinData.UpDownBtns[asLeft].SkinSection;
      PaintItem(SkinData, CI, True, Btn1State, Btn1Rect, Point(WndPos.X, WndPos.Y), SkinData.FCacheBMP, True);
      with SkinData.CommonSkinData, ScrollBtns[asLeft] do
        if IsValidIndex(GlyphIndex, Length(gd)) then begin
          p.x := Btn1Rect.Left + (WidthOf(Btn1Rect) - ma[GlyphIndex].Width) div 2;
          p.y := Btn1Rect.Top + (HeightOf(Btn1Rect) - ma[GlyphIndex].Height) div 2;
          DrawSkinGlyph(SkinData.FCacheBMP, p, Btn1State, 1, ma[GlyphIndex], MakeCacheInfo(SkinData.FCacheBmp));
        end
          ...
          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn1State > 0), ac_MaxPropsIndex);
            DrawArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, clNone, R, asLeft, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData));
          end
          else
            DrawArrow(SkinData.FCacheBMP, 0, clNone, R, asLeft, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData));
        end;

      SkinData.SkinSection := SkinData.CommonSkinData.UpDownBtns[asRight].SkinSection;
      PaintItem(SkinData, CI, True, Btn2State, Btn2Rect, Point(WndPos.X, WndPos.Y), SkinData.FCacheBMP, True);
      with SkinData.CommonSkinData, ScrollBtns[asRight] do
        if IsValidIndex(GlyphIndex, Length(gd)) then begin
          p.x := Btn2Rect.Left + (WidthOf(Btn2Rect) - ma[GlyphIndex].Width) div 2;
          p.y := Btn2Rect.Top + (HeightOf(Btn2Rect) - ma[GlyphIndex].Height) div 2;
          DrawSkinGlyph(SkinData.FCacheBMP, p, Btn2State, 1, ma[GlyphIndex], MakeCacheInfo(SkinData.FCacheBmp));
          ...
          if sArrowMask >= 0 then begin
            State := math.min(integer(Btn2State > 0), ac_MaxPropsIndex);
            DrawArrow(SkinData.FCacheBMP, gd[sArrowMask].Props[State].FontColor.Color, clNone, R, asRight, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData));
          end
          else
            DrawArrow(SkinData.FCacheBMP, 0, clNone, R, asRight, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData));
        end;
    end;
  end;
end;
...
procedure TacSpinWnd.SetRTL;
begin
  RTL := True;
  SetWindowLong(CtrlHandle, GWL_EXSTYLE, GetWindowLong(CtrlHandle, GWL_EXSTYLE) or WS_EX_LAYOUTRTL);
end;
...
begin
  if SkinData.FOwnerControl is TBitBtn then
    with TBitBtn(SkinData.FOwnerControl) do begin
      ...
      if DrawData.Images is TsCharImageList then begin
        C := GetFontColor(SkinData.FOwnerControl, SkinData.SkinIndex, DrawData.CommonSkinData.FOwner, DrawData.CurrentState);
...
begin
  ...
  if SkinData.FOwnerControl is TBitBtn then begin
    ...
    if (CurrentState = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then begin
      inc(x);
      inc(y);
    end;
...
begin
  if not HaveImage then
    ...
    if DrawData.Images is TsCharImageList then begin
      C := GetFontColor(Btn, SkinData.SkinIndex, DrawData.CommonSkinData.FOwner, DrawData.CurrentState);
      if DrawData.Canvas <> nil then
        DrawData.Canvas.Font.Color := C;
...
var
  ...
  begin
{$IFDEF AC_NOSHADOW}
    if ShadowForm <> nil then
      if not TacShadowForm(ShadowForm).Locked and ((CtrlHandle = 0) or not IsWindowVisible(CtrlHandle)) then begin
        if TacShadowForm(ShadowForm).HandleAllocated and IsWindowVisible(TacShadowForm(ShadowForm).Handle) then begin
          TacShadowForm(ShadowForm).HideWnd;
          FreeAndNil(ShadowForm);
        end;
      end
      else
        TacShadowForm(ShadowForm).UpdateShadowPos;
{$ENDIF}
  end;
  ...
  if Message.Msg = SM_ALPHACMD then
    case Message.WParamHi of
      ...
      AC_DROPPEDDOWN: begin
        ...
        if SkinData.SkinManager.MenuSupport.SkinnedShadows and (ShadowForm = nil) then begin
          SetClassLong(CtrlHandle, GCL_STYLE, GetClassLong(CtrlHandle, GCL_STYLE) and not NCS_DROPSHADOW);
{$IFDEF AC_NOSHADOW}
          ShadowForm := AttachShadowWnd(CtrlHandle, nil, SkinData, SkinData.SkinManager.MenuSupport.AlphaBlend, True);
          TacShadowForm(ShadowForm).UpdateShadowPos;
{$ENDIF}
        end;
      end;
{$IFDEF AC_NOSHADOW}
      AC_UPDATESHADOW:
        if ShadowForm <> nil then begin
          TacShadowForm(ShadowForm).Locked := False;
          with PacLayerPos(Message.LParam)^ do
            if Bounds.BHeight > 0 then
              TacShadowForm(ShadowForm).SetNewPos(Bounds.BLeft, Bounds.BTop, Bounds.BWidth, Bounds.BHeight, LayerBlend);
...
{$IFDEF AC_NOSHADOW}
        ...
        WM_SHOWWINDOW:
          if (Message.WParam = 0) and (ShadowForm <> nil) then begin
            TacShadowForm(ShadowForm).HideWnd;
            FreeAndNil(ShadowForm);
          end;
{$ENDIF}
        ...
        WM_NCPAINT: begin
          if not IsNT then
            ...
            mi := SkinData.SkinManager.SkinableMenus.GetMenuInfo(nil, nil, CtrlHandle);
...
begin
  if AeroIsEnabled then
  	SetClassLong(CtrlHandle, GCL_HBRBACKGROUND, ACNativeInt(SkinData.SkinManager.Brushes[pcMainColor]));
...
var
  ...
  DC, SaveIndex: HDC;
begin
  if ((SkinData.FOwnerControl = nil) or not (csDestroying in SkinData.FOwnerControl.ComponentState)) and {IsWindowVisible(CtrlHandle) and }not InUpdating(SkinData) then begin
    if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl.Parent <> nil) and (csCreating in SkinData.FOwnerControl.Parent.ControlState) then
      Exit;
    ...
    if not InUpdating(SkinData) then begin
      ...
      try
        ...
        b := SkinData.HalfVisible or SkinData.BGChanged or (Panel = nil);
        if SkinData.RepaintIfMoved and (Panel <> nil) then begin
          GetClipBox(DC, R);
...
begin
{$IFDEF LOGGED}
//  if (Panel <> nil) and (Panel.Tag = 2) then
//    AddToLog(Message);
{$ENDIF}
  if (SkinData.FOwnerControl <> nil) and (csAcceptsControls in TWinControl(SkinData.FOwnerControl).ControlStyle) or (SkinData.FOwnerControl = nil) then begin
    if Message.Msg = SM_ALPHACMD then
      case Message.WParamHi of
        AC_CTRLHANDLED: begin
          Message.Result := 1;
        ...
        AC_INVALIDATE:
          if (SkinData.FOwnerControl is TWinControl) and TWinControl(SkinData.FOwnerControl).HandleAllocated then begin
            SkinData.BGChanged := True;
            RedrawWindow(TWinControl(SkinData.FOwnerControl).Handle, nil, 0, iff(Message.WParamLo <> 0, RDWA_ALLNOW, RDWA_ALL));
          end;
      ...
      if Assigned(SkinData) and SkinData.Skinned then
        case Message.Msg of
          ...
          WM_PRINT: begin
            ...
            GetClientRect(CtrlHandle, ClRect);
            w := (WidthOf(WndRect) - WidthOf(ClRect)) div 2;
            MoveWindowOrg(TWMPaint(Message).DC, w, w);
            AC_WMPaint(TWMPaint(Message).DC);
            Exit;
          end;

          CM_SHOWINGCHANGED:
            if SkinData.FOwnerControl.Visible and (Message.WParam = 1) and (SkinData.FOwnerControl.ComponentState * [csDestroying] = []) then begin
              if SkinData.SkinManager.Options.OptimizingPriority = opMemory then
                SkinData.Updating := False;

              AddToAdapter(TWinControl(SkinData.FOwnerControl));
              M := MakeMessage(SM_ALPHACMD, AC_SETNEWSKIN_HI, ACNativeInt(SkinData.SkinManager), 0);
              AlphaBroadCastCheck(SkinData.FOwnerControl, CtrlHandle, M);
              M := MakeMessage(SM_ALPHACMD, AC_REFRESH_HI, ACNativeInt(SkinData.SkinManager), 0);
              AlphaBroadCastCheck(SkinData.FOwnerControl, CtrlHandle, M);
            end;
      ...
      SendMessage(CtrlHandle, WM_NCPAINT, 0, 0);
{
    WM_SIZE: begin
      SkinData.BGChanged := True;
      RedrawWindow(CtrlHandle, nil, 0, RDW_ERASE or RDW_UPDATENOW);
    end;}
  end;
end;


function TacPanelWnd.DoPrepareCache: boolean;
var
  Panel: TacAccessPanel;
  CI: TCacheInfo;
  w: integer;
...
begin
  ...
  if not CI.Ready and (CI.FillColor = clFuchsia) then begin
    ...
    InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
    PaintItem(SkinData, CI, False, 0, MkRect(SkinData.FCacheBMP), WndPos, SkinData.FCacheBMP, False);
    if Panel <> nil then begin
      R := Panel.ClientRect;
      ...
      GetClientRect(CtrlHandle, R);

    if Panel <> nil then begin
      SkinData.PaintOuterEffects(Panel, MkPoint);
    end;
...
begin
  if not InUpdating(SkinData) then begin
    ...
    try
      if not DoPrepareCache then begin
        SkinData.Updating := True;
        Exit;
      end;
      w := (WidthOf(WndRect) - WidthOf(ClRect)) div 2;
...
begin
  if (Panel = nil) or (Panel.Tag <> 999) then begin // If SilentPanel
    if SkinData.FOwnerControl <> nil then begin
{$IFDEF D2010}
      if not (SkinData.FOwnerControl is TCustomPanel) or not TAccessPanel(SkinData.FOwnerControl).ShowCaption then
        Exit;
{$ENDIF}
...
begin
  if SkinData.FOwnerControl <> nil then begin
    ...
    Panel := TacAccessPanel(SkinData.FOwnerControl);
    if SkinData.SkinSection = '' then
      if SkinData.FOwnerControl is TCustomPanel then
  ...
  if not InUpdating(SkinData) then begin
    ...
    try
      ...
      if SkinData.BGChanged then begin
        ...
        R := PageRect;
        PaintItem(SkinData.SkinIndex, CI, False, 0, R, Point(WndPos.X, WndPos.Y), SkinData.FCacheBmp, SkinData.CommonSkinData);
        SkinData.BGChanged := False;
      end;
    ...
    if Message.Msg = SM_ALPHACMD then
      case Message.WParamHi of
        ...
        AC_SETNEWSKIN: begin
          ...
          if (SkinData.FOwnerControl is TWinControl) and TWinControl(SkinData.FOwnerControl).HandleAllocated then begin // Be sure that Tabs handles allocated already
            with TWinControl(SkinData.FOwnerControl) do
              for i := 0 to ControlCount - 1 do
...
var
  ...
  i, h, w, iHeight, iWidth, TabsCovering, TabIndex, {TabMask, }TabState, aState: integer;

  function ColorTone: TColor;
  begin
    if State = 0 then
      if TabIndex >= 0 then
        Result := SkinData.CommonSkinData.gd[TabIndex].Props[0].Color
      else
        Result := clBtnFace
    else
      Result := clNone;
  ...
  begin
    ...
    i := integer(State <> 0);
    Bmp.Canvas.Font.Color := SkinData.CommonSkinData.gd[TabIndex].Props[i].FontColor.Color;
  end;
...
{$ENDIF}
    ...
    // Tabs drawing
    with SkinData{.SkinManager} do begin
      if CommonSkinData.Tabs[tlSingle][asTop].SkinIndex > 0 then begin // new style
        TabState := State;
        TabIndex := CommonSkinData.Tabs[tlSingle][acTabSides[TabPosition]].SkinIndex;
        case Style of
          tsTabs:
            with CommonSkinData.Tabs[tlSingle][acTabSides[TabPosition]] do
        ...
        try
          if (State = 2) and (Index = ActiveTabIndex) then begin
            ...
            PaintItem(TabIndex, CI, True, TabState, MkRect(TempBmp), MkPoint, Bmp, SkinData.CommonSkinData);
          end
          else begin
            ...
            PaintItem(TabIndex, CI, True, TabState, MkRect(TempBmp), aRect.TopLeft, TempBmp, SkinData.CommonSkinData);
...
{$IFNDEF FPC}
    if (PageControl <> nil) and TPageControl(SkinData.FOwnerControl).OwnerDraw and Assigned(TPageControl(SkinData.FOwnerControl).OnDrawTab) then begin
      ...
      PageControl.Canvas.Font.Assign(TacAccessControl(SkinData.FOwnerControl).Font);
      Bmp.Canvas.Font.Assign(PageControl.Canvas.Font);
      if TabIndex >= 0 then begin
        aState := mini(State, ac_MaxPropsIndex);
        Bmp.Canvas.Font.Color := SkinData.CommonSkinData.gd[TabIndex].Props[aState].FontColor.Color;
...
{$ENDIF}
    ...
    if (SkinData.FOwnerControl <> nil) and not bFontChanged then
      Bmp.Canvas.Font.Assign(TacAccessControl(SkinData.FOwnerControl).Font);
    ...
    case TabPosition of
          ...
          if ImageList <> nil then
            if (ImageList is TsAlphaImageList) and SkinData.SkinManager.Effects.DiscoloredGlyphs then
              DrawAlphaImgList(ImageList, Bmp,
                               ...
                               ItemData.iImage, 0, ColorTone, 0, 1, False, GetPPI(SkinData))
            else
      ...
      tpLeft: begin
        ...
        if (ImgList <> 0) and (ItemData.iImage >= 0) then begin
          ...
          if ImageList <> nil then
            if (ImageList is TsAlphaImageList) and SkinData.SkinManager.Effects.DiscoloredGlyphs then
              DrawAlphaImgList(ImageList, Bmp, rText.Left + (WidthOf(rText) - iWidth) div 2, i, ItemData.iImage, 0, ColorTone, 0, 1, False, GetPPI(SkinData))
            else
  ...
  if (SkinData <> nil) and SkinData.Skinned then
    case Message.Msg of
      ...
      WM_PAINT:
        if IsWindowVisible(CtrlHandle) then begin
          if Message.WParam = 0 then
            Message.WParam := WParam(BeginPaint(CtrlHandle, {$IFDEF FPC}@ps{$ELSE}ps{$ENDIF}))
          else
            BeginPaint(CtrlHandle, {$IFDEF FPC}@ps{$ELSE}ps{$ENDIF});
  ...
  if not InUpdating(SkinData) then begin
    ...
    if ParentBG.BgType = btCache then begin
      if ParentBG.Bmp.Empty then
        SendMessage(ParentWnd, WM_PAINT, ACNativeInt(ParentBG.Bmp.Canvas.Handle), 0); // Prepare image
  ...
  if (SkinData <> nil) and SkinData.Skinned then
    case Message.Msg of
      ...
      WM_PRINT: begin
        ...
        SkinData.Updating := False;
        AC_WMPaint(TWMPaint(Message));
        if BtnSW <> nil then begin
          SavedDC := SaveDC(hdc(Message.WParam));
          try
            MoveWindowOrg(hdc(Message.WParam), BtnSW.WndPos.X, BtnSW.WndPos.Y);
            IntersectClipRect(hdc(Message.WParam), 0, 0, BtnSW.WndSize.cx, BtnSW.WndSize.cy);
            BtnSW.SkinData.Updating := False;
            SendMessage(BtnSW.CtrlHandle, WM_PAINT, Message.WParam, 0);
          finally
...
{$IFNDEF FPC}
          ...
          ToolBar.OnAdvancedCustomDrawButton := OldDrawBtnEvent;
{$ENDIF}
          DroppedButton := nil;
        end;
  ...
  with Control do begin
    ...
    try
      for i := 0 to Control.ControlCount - 1 do begin
        if (Controls[i] is TToolButton) and Designing then
          Continue;
...
{$IFNDEF FPC}
    OldDrawBtnEvent := ToolBar.OnAdvancedCustomDrawButton;
    ToolBar.OnAdvancedCustomDraw := OurAdvancedCustomDraw;
    ToolBar.OnAdvancedCustomDrawButton := OurAdvancedCustomDrawButton;
{$ENDIF}
    if SkinData.SkinSection = '' then
...
begin
  Result := ToolBar.Buttons[Index].BoundsRect;
...
var
  RC, RW: TRect;
begin
  if (SkinData <> nil) and SkinData.Skinned(True) then begin
    if not InUpdating(SkinData) then
...
var
  ...
  var
    ...
    begin
      ...
      if (Mode = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
        OffsetRect(Result, 1, 1);
    end;
  ...
  begin
    if ToolBar.ShowCaptions then begin
      cRect := CaptionRect;
{$WARNINGS OFF}
      acWriteTextEx(BtnBMP.Canvas, PacChar(Button.Caption), True, cRect, DT_CENTER or DT_VCENTER or DT_SINGLELINE, pInfo.FontIndex, Mode > 0);
    end;
  end;
  ...
  begin
    with SkinData.CommonSkinData do begin
      ...
      BorderIndex := FOwner.GetMaskIndex(SkinIndex, s_BordersMask, SkinData.CommonSkinData);
      if FOwner.IsValidImgIndex(BorderIndex) then
        DrawSkinRect(BtnBmp, R, ci, ma[BorderIndex], Mode, True);

      gR.Left := IntButtonWidth;
      gR.Top := 0;
      gR.Right := WidthOf(iR);
      gR.Bottom := HeightOf(iR);
      if (Mode = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then
        OffsetRect(R, 1, 1);

//      i := math.min(integer(Mode > 0), ac_MaxPropsIndex);
      sGraphUtils.DrawArrow(BtnBmp, BtnBmp.Canvas.Font.Color, clNone, gR, asBottom, 0, 0, 0, SkinData.SkinManager.Options.ActualArrowStyle, GetPPI(SkinData))
    end;
  end;
...
begin
  if SkinData.Skinned then begin
    DefaultDraw := False;
    if (Stage in [cdPrePaint]) and (Button.Index >= 0) then begin
      ...
      if (bWidth <> 0) and (bHeight <> 0) then begin
        SkinIndex := SkinData.SkinManager.SkinCommonInfo.Sections[ssToolButton];
        ...
        bRect := R;
        OffsetRect(bRect, ToolBar.Left, ToolBar.Top);

        pInfo.State := Mode;
        pInfo.R := bRect;
        pInfo.SkinManager := SkinData.SkinManager;

        if not ToolBar.Flat or (Mode <> 0) then begin
          pInfo.FontIndex := SkinIndex;
          GetFontIndex(Button, @pInfo);
        end
        else begin
          pInfo.FontIndex := SkinData.SkinIndex; // Index of ToolBar section
          GetFontIndex(ToolBar, @pInfo);
        end;

        BtnBmp.Canvas.Font.Assign(ToolBar.Font);
        if SkinData.SkinManager.IsValidSkinIndex(pInfo.FontIndex) then
          BtnBmp.Canvas.Font.Color := SkinData.SkinManager.gd[pInfo.FontIndex].Props[Mode].FontColor.Color
        else
          BtnBmp.Canvas.Font.Color := clWindowText;
        ...
        PaintItemBg(SkinIndex, ci, Mode, R, MkPoint(Button), BtnBmp, SkinData.CommonSkinData);
        R.Right := bWidth - AddedWidth;

        ci.X := ci.X + Button.Left;
        ci.Y := ci.Y + Button.Top;
        BorderIndex := SkinData.SkinManager.GetMaskIndex(SkinIndex, s_BordersMask, SkinData.CommonSkinData);
        if BorderIndex >= 0 then
          DrawSkinRect(BtnBmp, R, ci, SkinData.CommonSkinData.ma[BorderIndex], Mode, True);
        ...
        if Assigned(OldDrawBtnEvent) then
          OldDrawBtnEvent(Sender, Button, State, cdPostPaint, Flags, DefaultDraw);
      end;
    end;
  end
  else begin
  ...
  if Assigned(SkinData.SkinManager) then
    with SkinData.SkinManager, SkinData.CommonSkinData do begin
      ...
      if IsValidSkinIndex(SkinData.SkinIndex) then begin
        SkinIndex := GetMaskIndex(SkinData.SkinIndex, s_StatusPanelMask, SkinData.CommonSkinData);
        if IsValidImgIndex(SkinIndex) then
          ...
          if not (SimplePanel or (Index = PartsCount - 1)) then begin
            ...
            if IsValidskinIndex(si) then begin
              mi := GetMaskIndex(si, s_BordersMask, SkinData.CommonSkinData);
              if IsValidImgIndex(mi) then begin
                ...
                CI := MakeCacheInfo(SkinData.FCacheBmp);
                PaintItem(si, CI, True, 0, MkRect(TempBmp.Width, TempBmp.Height), Point(Rect.Right - TempBmp.Width, 1), TempBmp, SkinData.CommonSkinData);
...
begin
  if (SkinData.SkinManager <> nil) and (SkinData.FOwnerControl is TStatusBar) then
    with SkinData{.SkinManager}, TStatusBar(SkinData.FOwnerControl), CommonSkinData do
      if SizeGrip and SkinData.SkinManager.IsValidImgIndex(GripRightBottom) then begin
...
type
  TAccessButton = class({$IFDEF TNTUNICODE}TTntSpeedButton{$ELSE}TSpeedButton{$ENDIF});
...
begin
  with TSpeedButton(Ctrl) do begin
    Size := TextRectSize;
    DoOffset := (CurrentState = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData);
...
begin
  ...
  IsUpDown := (Ctrl.ClassName = 'TTimerSpeedButton');
...
begin
  try
    if not Destroyed and Assigned(Ctrl) and Assigned(PCtrl) then begin
      Destroyed := True;
      with Ctrl do
...
{$IFDEF DELPHI_XE2}
const
  Sides: array [boolean{IsUp}] of TacSide = (asBottom, asTop);
{$ENDIF}
var
  DrawData: TacDrawGlyphData;
{$IFDEF DELPHI_XE2}
...
{$IFDEF DELPHI_XE2}
  if IsUpDown then
    with SkinData.CommonSkinData do begin
      BtnSkinNdx := SkinData.SkinIndex;
      if NeedParentFont(SkinData.SkinManager, BtnSkinNdx, CurrentState) then
        C := GetFontColor(Ctrl.Parent.Parent.Handle, SkinData.SkinIndex, SkinData.SkinManager)
      else
        if IsValidIndex(BtnSkinNdx, Length(gd)) then
          C := ColorToRGB(gd[BtnSkinNdx].Props[CurrentState].FontColor.Color)
        else
          C := clBlack;

      DrawArrow(SkinData.FCacheBmp, C, clNone, MkRect(Ctrl), Sides[Ctrl.Top < 4], 0, 0, SmallArrowSize, SkinData.SkinManager.Options.ActualArrowStyle, aScalePercents[PPIToIndex(GetControlPPI(Ctrl))])
    end
  else
{$ENDIF}
...
begin
  ...
  with TSpeedButton(Ctrl) do begin
    ...
    case Layout of
      blGlyphLeft: begin
        x := iff(Margin <> -1, Margin, dw);
        y := (Height - gh) div 2;
      end;

      blGlyphRight: begin
        x := Width - gw - iff(Margin <> -1, Margin, gw);
        y := (Height - GlyphHeight) div 2;
      end;

      blGlyphTop: begin
        x := (Width - gw) div 2;
        y := iff(Margin <> -1, Margin, dh);
      end;

      blGlyphBottom: begin
        x := (Width - gw) div 2;
        y := Height - gh - iff(Margin <> -1, Margin, dh);
      end;
    end;
    if (CurrentState = 2) and CanClickShift(SkinData.SkinIndex, SkinData.CommonSkinData) then begin
      inc(x);
...
begin
  Result := True;
  InitCacheBmp(SkinData);
  SkinData.FCacheBmp.Canvas.Font.Assign(TacAccessControl(Ctrl).Font);
  ParentBG.BgType := btUnknown;
  ParentBG.DrawDC := 0;
  ParentBG.PleaseDraw := False;
  if IsUpDown then begin
    GetBGInfo(@ParentBG, Ctrl.Parent.Parent);
    SkinData.SkinIndex := SkinData.SkinManager.SkinCommonInfo.Sections[ssSpeedButton];
  end
  else
  ...
  if not CI.Ready or (CI.Bmp.Width <> 0) then begin
    if IsUpDown then
      PaintItem(SkinData, CI, True, CurrentState, MkRect(Ctrl), Point(Ctrl.Parent.Left + Ctrl.Left, Ctrl.Parent.Top + Ctrl.Top), SkinData.FCacheBmp, True, integer(CurrentState = 2), integer(CurrentState = 2))
    else
...
begin
  ...
  if SkinData.SkinManager.SkinCommonInfo.Sections[ssTrackBar] >= 0 then begin
    PaintBar;
...
begin
  aRect := ChannelRect;
  i := SkinData.SkinManager.GetMaskIndex(SkinData.SkinManager.SkinCommonInfo.Sections[ssTrackBar], s_SliderChannelMask, SkinData.CommonSkinData);
  if SkinData.SkinManager.IsValidImgIndex(i) then begin
    if IsVertical then begin
      h := SkinData.CommonSkinData.ma[i].Width - 1;
      w := WidthOf(aRect);
      aRect.Left := aRect.Left + (w - h) div 2;
      aRect.Right := aRect.Left + h;
    end
    else begin
      h := SkinData.CommonSkinData.ma[i].Height - 1;
      w := HeightOf(aRect);
      aRect.Top := aRect.Top + (w - h) div 2;
      aRect.Bottom := aRect.Top + h;
    end;
    CI := MakeCacheInfo(SkinData.FCacheBmp);
    DrawSkinRect(SkinData.FCacheBmp, aRect, CI, SkinData.CommonSkinData.ma[i], integer(ControlIsActive(SkinData)), True);
  end;
  if IsVertical then
    PaintTicksVer
  else
...
begin
  ...
  with SkinData.SkinManager, SkinData.CommonSkinData do begin
    if IsVertical then
      i := GetMaskIndex(SkinCommonInfo.Sections[ssTrackBar], s_SliderVertMask, SkinData.CommonSkinData)
    else
      i := -1;

    if i < 0 then
      i := GetMaskIndex(SkinCommonInfo.Sections[ssTrackBar], s_SliderHorzMask, SkinData.CommonSkinData);
...
begin
  with SkinData.SkinManager, SkinData.CommonSkinData do begin
    if Horz then
      GlyphIndex := GetMaskIndex(SkinCommonInfo.Sections[ssTrackBar], s_TickHorz, SkinData.CommonSkinData)
    else
      GlyphIndex := GetMaskIndex(SkinCommonInfo.Sections[ssTrackBar], s_TickVert, SkinData.CommonSkinData);
...
begin
  Result := integer(ProcessMessage(TBM_GETTICPOS, ACNativeInt(i), 0));
end;
...
{$ENDIF}
  ...
  if Assigned(SkinData) then begin
    case Message.Msg of
      ...
      WM_PRINT:
        if {(DefaultManager <> nil) and DefaultManager.IsActive and} SkinData.Skinned then begin
...
begin
  Result := R;
  with sd, sd.ScrollBtns[Side] do
    if (SkinIndex >= 0) and (Length(gd) > 0) and gd[SkinIndex].ReservedBoolean and (MaskIndex >= 0) then begin
      w := 17;
      case Side of
...
begin
  ...
  ac_DrawSlider(R, SliderState, b, SkinData.CommonSkinData, Control.Kind = sbVertical);
end;
...
begin
  if Control <> nil then begin
    ...
    SavedDC := SaveDC(DC);
    try
      if SkinData.Skinned {and (SkinData.SkinManager <> nil) and SkinData.SkinManager.IsActive} and ControlIsReady(Control) and ([csDestroying, csLoading] * Control.ComponentState = []) and not SkinData.Updating then begin
        if Control.Width <> Control.Height then begin // If not grip
          ...
          with SkinData, CommonSkinData do begin
            ...
            if not IsRectEmpty(FBar1Rect) then begin
              ...
              with Scrolls[Side] do
                if SkinManager.IsValidSkinIndex(SkinIndex) then
                  PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                              lCi, True, LocalState, FBar1Rect, MkPoint(Control), FCacheBmp, SkinData.CommonSkinData);
            end;
            Bar2Rect;
            if not IsRectEmpty(FBar2Rect) then begin
              ...
              with Scrolls[Side] do
                if SkinManager.IsValidSkinIndex(SkinIndex) then
                  PaintItemFast(SkinIndex, MaskIndex, BGIndex[0], BGIndex[1],
                                lCi, True, LocalState, FBar2Rect, Point(Control.Left + FBar2Rect.Left, Control.Top + FBar2Rect.Top), FCacheBmp, SkinData.CommonSkinData)
            end;
          end;
          BitBlt(bmp.Canvas.Handle, 0, 0, bmp.Width, bmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY);
          CI.Bmp := bmp;
          CI.Ready := bmp <> nil;
          if Control.Kind = sbHorizontal then begin
            ac_DrawScrollBtn(Btn1Rect, Btn1State, Bmp, SkinData.CommonSkinData, asLeft);
            ac_DrawScrollBtn(Btn2Rect, Btn2State, Bmp, SkinData.CommonSkinData, asRight);
          end
          else begin
            ac_DrawScrollBtn(Btn1Rect, Btn1State, Bmp, SkinData.CommonSkinData, asTop);
            ac_DrawScrollBtn(Btn2Rect, Btn2State, Bmp, SkinData.CommonSkinData, asBottom);
          end;
          if Control.Enabled then
            DrawSlider(bmp);
          ...
          if SkinData.CommonSkinData.Scrolls[asLeft].SkinIndex >= 0 then
            FillDC(DC, MkRect(Control), SkinData.CommonSkinData.gd[SkinData.CommonSkinData.Scrolls[asLeft].SkinIndex].Props[0].Color)
          else
  ...
  if Message.Msg = SM_ALPHACMD then
    case Message.WParamHi of
      ...
      AC_REFRESH: if RefreshNeeded(SkinData, Message) then begin
        ...
        CommonMessage(Message, SkinData);
        AlphaBroadCastCheck(SkinData.FOwnerControl, CtrlHandle, Message);
        if (AC_REFRESH = Message.WParamHi) {and IsWindowVisible(CtrlHandle)} then
          SetWindowPos(CtrlHandle, 0, 0, 0, 0, 0, SWPA_FRAMECHANGED);
...
//            AC_WMPaint(TWMPaint(Message).DC);
          ...
          WM_PARENTNOTIFY:
            if Message.WParamLo in [WM_CREATE, WM_DESTROY] then begin
              inherited;
              {$IFNDEF VER185} // If not D2007, incorrect unicode there
              if Message.WParamLo = WM_CREATE then
                AddToAdapter(TWinControl(SkinData.FOwnerControl));
              {$ELSE}
                {$IFNDEF TNTUNICODE}
...
begin
  if IsWindowVisible(CtrlHandle) then begin
    ...
    FillAlphaRect(SkinData.FCacheBmp, MkRect(SkinData.FCacheBmp.Width - 2 * BorderWidth, SkinData.FCacheBmp.Height - 2 * BorderWidth), MaxByte);
...
function TacContainerWnd.DoPrepareCache: boolean;
begin
  InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
  if SkinData.BGChanged then begin
    GetClientRect(CtrlHandle, ClientRect);
...
procedure TacScrollWnd.UpdateLeftScroll;
begin
...
begin
  inherited;
  R := ArrowRect;
  i := mini(CurrentState, ac_MaxPropsIndex);
  C := SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[i].FontColor.Color;
  DrawColorArrow(SkinData.FCacheBmp, C, R, asBottom, GetPPI(SkinData));
end;


procedure TacDBComboListWnd.acWndProc(var Message: TMessage);
begin
  inherited;
  case Message.Msg of
    WM_MOUSEWHEEL:
      SendMessage(ComboBoxHandle, Message.Msg, Message.WParam, Message.LParam);
  end;
end;
