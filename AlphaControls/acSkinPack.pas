...
uses
  Windows, SysUtils, {$IFDEF DELPHI_XE2}UITypes, {$ENDIF}Classes;
...
procedure PackDir(const SrcPath, FileName: string); {$IFDEF WARN_DEPRECATED} deprecated; {$ENDIF}
procedure UnpackSkinFile(const Filename, DestDirectory: String); {$IFDEF WARN_DEPRECATED} deprecated; {$ENDIF}
...
begin
  ...
  if Status <> INVALID_HANDLE_VALUE then
    repeat
      if (FindData.cFileName[0] <> s_Dot) and (FindData.dwFileAttributes and FILE_ATTRIBUTE_DIRECTORY = 0) then
