...
begin
  result := deflateInit_(stream,level,acZLIB_VERSION,SizeOf(TZStreamRec));
end;


function DeflateInit2(var stream: TZStreamRec; level, method, windowBits, memLevel, strategy: Integer): Integer;
begin
  result := deflateInit2_(stream,level,method,windowBits,memLevel,strategy, acZLIB_VERSION,SizeOf(TZStreamRec));
end;


function InflateInit(var stream: TZStreamRec): Integer;
begin
  result := inflateInit_(stream,acZLIB_VERSION,SizeOf(TZStreamRec));
end;


function InflateInit2(var stream: TZStreamRec; windowBits: Integer): Integer;
begin
  result := inflateInit2_(stream,windowBits,acZLIB_VERSION,SizeOf(TZStreamRec));
end;
...
procedure ZInternalCompress(var zstream: TZStreamRec; const inBuffer: Pointer; inSize: Integer; out outBuffer: Pointer; out outSize: Integer);
const
  delta = 256;
begin
  outSize := ((inSize + (inSize div 10) + 12) + $FF) and not $FF;
  ...
  try
    try
      ...
      while ZCompressCheck(deflate(zstream,Z_FINISH)) <> Z_STREAM_END do begin
        Inc(outSize,delta);
...
procedure ZInternalDecompress(zstream: TZStreamRec; const inBuffer: Pointer;
  inSize: Integer; out outBuffer: Pointer; out outSize: Integer; outEstimate: Integer);
var
  delta: Integer;
begin
  delta := (inSize + $FF) and not $FF;
  ...
  try
    try
      ...
      zstream.next_out := outBuffer;
      zstream.avail_out := outSize;
      while ZDecompressCheck(inflate(zstream,Z_NO_FLUSH)) <> Z_STREAM_END do begin
        Inc(outSize,delta);
        ReallocMem(outBuffer,outSize);
        zstream.next_out := PChar(Integer(outBuffer) + zstream.total_out);
        zstream.avail_out := delta;
...
procedure ZCompress2(const inBuffer: Pointer; inSize: Integer; out outBuffer: Pointer; out outSize: Integer; level: TZCompressionLevel;
  windowBits, memLevel: Integer; strategy: TZStrategy);
var
  zstream: TZStreamRec;
begin
  FillChar(zstream, SizeOf(TZStreamRec),0);
  ZCompressCheck(DeflateInit2(zstream, ZLevels[level], Z_DEFLATED, windowBits, memLevel, ZStrategies[strategy]));
  ZInternalCompress(zstream, inBuffer, inSize, outBuffer, outSize);
end;
...
function ZCompressStr2(const s: String; level: TZCompressionLevel; windowBits, memLevel: Integer; strategy: TZStrategy): String;
var
  buffer: Pointer;
  size: Integer;
begin
  ZCompress2(PChar(s), Length(s), buffer, size, level, windowBits, memLevel, strategy);
  SetLength(result, size);
  Move(buffer^, result[1], size);
  FreeMem(buffer);
end;
...
begin
  ...
  while zresult <> Z_STREAM_END do begin
    zstream.next_out := outBuffer;
    zstream.avail_out := bufferSize;
...
begin
  ...
  FZStream.next_out := FBuffer;
  FZStream.avail_out := SizeOf(FBuffer);
  ZCompressCheck(DeflateInit2(FZStream, ZLevels[compressionLevel], Z_DEFLATED, windowBits, memLevel, ZStrategies[strategy]));
end;
...
begin
  ...
  try
    ...
    while ZCompressCheck(deflate(FZStream,Z_FINISH)) <> Z_STREAM_END do begin
...
begin
  ...
  while FZStream.avail_in > 0 do begin
    ...
    if FZStream.avail_out = 0 then begin
...
begin
  ...
  zresult := Z_OK;
  while (FZStream.avail_out > 0) and (zresult <> Z_STREAM_END) do begin
  ...
  if (zresult = Z_STREAM_END) and (FZStream.avail_in > 0) then begin
...
var
  ...
  i: Integer;
begin
  if (offset = 0) and (origin = soFromBeginning) then begin
    ...
    if ((offset >= 0) and (origin = soFromCurrent)) or (((offset - FZStream.total_out) > 0) and (origin = soFromBeginning)) then begin
      ...
      if (offset = 0) and (origin = soFromEnd) then
        while Read(buf, SizeOf(buf)) > 0 do Continue
      else
        raise EZDecompressionError.Create(SZInvalid);
