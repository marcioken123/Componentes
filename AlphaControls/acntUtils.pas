...
function GetCaptionFontSize(DPI: integer = 96): integer;
function acGetTitleFont(DPI: integer = 96): hFont;
function HexToInt(HexStr: AnsiString): Int64;
function AnyStrToCurr(var S: string; var Value: Extended): boolean; overload;
function AnyStrToCurr(var S: string): Extended; overload;
function ScaleInteger(Value: integer; SysScale: integer): integer;
function ScalePPI(Value: integer; PPI: integer): integer;
function PPIToIndex(PPI: integer): integer;
...
function SumTrans(const i1, i2: integer): integer; // Doesn't used
...
function RotateRect(R: TRect; Clockwise: boolean = True; ParentWidth: integer = 0): TRect;
function RectsAnd(const R1, R2: TRect): TRect;
function SumRects(const R1, R2: TRect): TRect;
procedure AddPadding(var R: TRect; Padding: TRect);
function PaddingToRect(Padding: TacPadding; PPI: integer): TRect;
...
function MkRect(const Size: TSize): TRect; overload;

procedure ReflectRect(var R: TRect; Width, Height: integer; Horz: boolean); overload;
procedure ReflectRect(var R: TRect; const MainRect: TRect; Horz: boolean); overload;

function MkSize(Width: integer = 0; Height: integer = 0): TSize; overload;
function MkSize(const Bmp: TBitmap): TSize; overload;
function MkSize(const R: TRect): TSize; overload;
...
function iff(const L: boolean; const c1, c2: acChar): acChar; overload;
{ Returns position of word number N in string S. WordDelims - chars, word delimiters}
function acWordPosition(const N: Integer; const S: acString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True {old behavior}): Integer;
function WordPosition(const N: Integer; const S: AnsiString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True {old behavior}): Integer;
...
procedure WaitTicks(ABeginTicks: DWORD; ADelay: integer = acTimerInterval);
...
function ExtractWord(const N: Integer; const S: AnsiString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True {old behavior}): AnsiString;
function acExtractWord(const N: Integer; const S: acString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True): acString;
// Properties
function  HasProperty    (const Component: TObject; const PropName: String): Boolean;
function  GetIntProp     (const Component: TObject; const PropName: String): acNativeInt;
...
procedure InitWinVer;
function WinMajorVer: integer;
function IsWinVistaUp: boolean;
function IsWin8Up: boolean;
function IsWin10Up: boolean;

{$IFNDEF D2009}
{$IFDEF D2007}
function CharInSet(const C: WideChar; const CharSet: TSysCharSet): Boolean; overload;
function CharInSet(const C: Char; const CharSet: TSysCharSet): Boolean; overload;
{$ELSE}
function CharInSet(const C: acChar; const CharSet: TSysCharSet): Boolean; overload;
  {$IFDEF TNTUNICODE}
...
procedure AddObjLink(AObj: TObject; ACtlContainer: TControl);
procedure DelObjLink(ACtl: TObject; ACtlContainer: TControl); overload;
procedure DelObjLink(ACtl: TObject; AList: TList); overload;
procedure AddObjToList(AObj: TObject; AList: TList);
procedure DelLinkFromList(ACtl: TObject; AList: TList);
{$ENDIF}
...
uses
  ...
  sVCLUtils, acPng, sSkinProvider, sSkinManager, sMessages;


var
  IsDebuggerPresent: function: Boolean; stdcall;
  IsWow64Process: function (hProc: THandle; out Is64: Windows.bool): Windows.Bool; stdcall;
  acWinMajorVer: byte = 0;
  acWinMinorVer: byte = 0;

function SysReadyForDPI: boolean;
begin
  Result := (Win32MajorVersion > 6) or (Win32MajorVersion = 6) and (Win32MinorVersion >= 3);
end;
...
function NetWkstaGetInfo(ServerName: LPWSTR; Level: DWORD; BufPtr: PBYTE): Longint; stdcall; external 'netapi32.dll' name 'NetWkstaGetInfo';
...
begin
  ...
  acWinMajorVer := LoByte(Ver);
  acWinMinorVer := HiByte(Ver);
  if IsWin8Up then
    if NetWkstaGetInfo(nil, 100, PBYTE(@LBuf)) = 0 then begin
      acWinMajorVer := LBuf^.wki100_ver_major;
      acWinMinorVer := LBuf^.wki100_ver_minor;
    end;
end;
...
function WinMajorVer: integer;
begin
  if acWinMajorVer = 0 then
    InitWinVer;

  Result := acWinMajorVer;
end;


function IsWinVistaUp: boolean;
begin
  Result := WinMajorVer >= 6;
end;


function IsWin8Up: boolean;
begin
  Result := (WinMajorVer > 6) or ((WinMajorVer = 6) and (acWinMinorVer > 1));
end;


function IsWin10Up: boolean;
begin
  Result := WinMajorVer >= 10;
end;


var
  ac_SystemParametersInfoForDpi: function(uiAction, uiParam: UINT; pvParam: Pointer; fWinIni: UINT; DPI: UINT): BOOL; stdcall;

type
  TacNonClientMetrics = {$IFDEF TNTUNICODE}TNonClientMetricsW{$ELSE}TNonClientMetrics{$ENDIF};

function acSysParamInfo(uiAction, uiParam: UINT; pvParam: Pointer; DPI: integer = 96): boolean;
begin
  if SysReadyForDPI and Assigned(ac_SystemParametersInfoForDpi) then
    Result := ac_SystemParametersInfoForDpi(uiAction, uiParam, pvParam, 0, DPI)
  else
    Result := False;

  if not Result then
{$IFDEF TNTUNICODE}
    Result := SystemParametersInfoW(uiAction, uiParam, pvParam, 0);
{$ELSE}
...
function GetCaptionFontSize(DPI: integer = 96): integer;
var
  NonClientMetrics: TacNonClientMetrics;
begin
  NonClientMetrics.cbSize := {$IFDEF D2010}TacNonClientMetrics.SizeOf{$ELSE}SizeOf(NonClientMetrics){$ENDIF};
  if acSysParamInfo(SPI_GETNONCLIENTMETRICS, 0, @NonClientMetrics, DPI) then
    Result := min(-11, NonClientMetrics.lfCaptionFont.lfHeight)
  else
    Result := 0
end;
...
begin
  ...
  if acSysParamInfo(SPI_GETNONCLIENTMETRICS, 0, @NonClientMetrics, DPI) then
    Result := {$IFDEF TNTUNICODE}CreateFontIndirectW{$ELSE}CreateFontIndirect{$ENDIF}(NonClientMetrics.lfCaptionFont)
  else
    Result := 0;
end;
...
function AnyStrToCurr(var S: string; var Value: Extended): boolean;
var
  p1, p2: integer;
  DS: Char;
begin
  ...
  if S <> '' then begin
    ...
    p1 := Pos(',', S);
    p2 := Pos('.', S);
    if (p1 > 0) and (p2 > 0) then // is here thousand separator and decimal separator
      if p2 > p1 then begin
        DS := '.';
        S := StringReplace(S, ',', '', [rfReplaceAll]);
      end
    ...
    if DS <> {$IFDEF DELPHI_XE}FormatSettings.{$ENDIF}DecimalSeparator then
      S := StringReplace(S, DS, {$IFDEF DELPHI_XE}FormatSettings.{$ENDIF}DecimalSeparator, [rfReplaceAll]);
    try
{$IFDEF DELPHI7UP}
...
function AnyStrToCurr(var S: string): Extended;
begin
  Result := 0;
  AnyStrToCurr(S, Result);
end;
...
begin
  case PPI of
      0..110  {96}: Result := 0; // 100%
    111..134 {120}: Result := 1; // 125%
    135..170 {144}: Result := 2; // 150%
    else            Result := 3; // 200% //  171..260 {192}: Result := 3; // 200%
//  261..350 {288}: Result := 4; // 300%
//  else            Result := 5; // 400%
  end;
end;
...
begin
  ...
  r.RootKey := Key;
  try
    if r.KeyExists(Section) and r.OpenKey(Section, False) and r.ValueExists(Named) then
...
function iff(const L: boolean; const s1, s2: string): string;
begin
  if l then
    Result := s1
  else
...
function iff(const L: boolean; const o1, o2: TObject): TObject;
begin
  if l then
    Result := o1
  else
...
function iff(const L: boolean; const i1, i2: integer): integer;
begin
  if l then
    Result := i1
  else
...
function iff(const L: boolean; const b1, b2: boolean): boolean;
begin
  if l then
    Result := b1
  else
...
begin
  ...
  if FullInclude then
    ...
    Result := (SmallRect.Left < BigRect.Right) and (SmallRect.Right > BigRect.Left) and (SmallRect.Top < BigRect.Bottom) and (SmallRect.Bottom > BigRect.Top);
end;
...
begin
  if Clockwise then begin
    ...
    Result.Left := R.Top;
    Result.Right := R.Bottom;
    Result.Bottom := ParentWidth - R.Left;
    Result.Top := ParentWidth - R.Right;
  end;
end;
...
procedure AddPadding(var R: TRect; Padding: TRect);
begin
  inc(R.Left,   Padding.Left);
  inc(R.Top,    Padding.Top);
  dec(R.Right,  Padding.Right);
  dec(R.Bottom, Padding.Bottom);
end;


function PaddingToRect(Padding: TacPadding; PPI: integer): TRect;
begin
  with TDstRect(Result), Padding do begin
    DLeft     := Left * PPI div 96;
    DTop       := Top * PPI div 96;
    DRight   := Right * PPI div 96;
    DBottom := Bottom * PPI div 96;
  end;
end;


function MkRect(Right: integer = 0; Bottom: integer = 0; Left: integer = 0; Top: integer = 0): TRect;
begin
  Result := Rect(Left, Top, Right, Bottom);
end;


function MkRect(Bmp: TBitmap): TRect;
begin
  Result := MkRect(Bmp.Width, Bmp.Height);
end;


function MkRect(Ctrl: TControl): TRect;
begin
  Result := MkRect(Ctrl.Width, Ctrl.Height);
end;


function MkRect(const Size: TSize): TRect;
begin
  Result := MkRect(Size.cx, Size.cy);
end;


procedure ReflectRect(var R: TRect; Width, Height: integer; Horz: boolean);
var
  i: integer;
begin
  if Horz then begin
...
procedure ReflectRect(var R: TRect; const MainRect: TRect; Horz: boolean);
var
  i: integer;
begin
  if Horz then begin
    i := R.Left - MainRect.Left;
    R.Left := MainRect.Left + MainRect.Right - R.Right;
    R.Right := MainRect.Right - i;
  end
  else begin
    i := R.Top - MainRect.Top;
    R.Top := MainRect.Top + MainRect.Bottom - R.Bottom;
    R.Bottom := MainRect.Bottom - i;
  end;
end;


function MkSize(const Bmp: TBitmap): TSize;
begin
  Result.cx := Bmp.Width;
...
function MkSize(const R: TRect): TSize;
begin
...
function MkPoint(Control: TControl): TPoint;
begin
...
function MkSize(Width: integer = 0; Height: integer = 0): TSize;
begin
  Result.cx := Width;
  Result.cy := Height;
end;
...
function OffsRect(const aRect: TRect; aOffsetX, aOffsetY: integer): TRect;
begin
...
begin
  ...
  if not SkipEmpty then begin
    if (N > 0) and (L > 0) then begin
      ...
      Count := 1;
      while I <= L do
        if CharInSet(S[I], WordDelims) then begin
          Inc(Count);
          Inc(I);
    ...
    Count := 0;
    Result := 0;
    while (I <= L) and (Count <> N) do begin
      while (I <= L) and acCharIn(S[I], WordDelims) do
        Inc(I);
      ...
      if Count <> N then
        while (I <= L) and not acCharIn(S[I], WordDelims) do
          Inc(I)
      else
        Result := I;
    end;
...
function WordPosition(const N: Integer; const S: AnsiString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True): Integer;
var
  Count, I, L: Integer;
begin
  I := 1;
  ...
  if not SkipEmpty then begin
    ...
    Result := 0;
    while (I <= L) and (Count <> N) do begin
      while (I <= L) and CharInSet(S[I], WordDelims) do
        Inc(I);
      ...
      if Count <> N then
        while (I <= L) and not CharInSet(S[I], WordDelims) do
          Inc(I)
      else
        Result := I;
    end;
...
function ExtractWord(const N: Integer; const S: AnsiString; const WordDelims: TSysCharSet; SkipEmpty: boolean = True): AnsiString;
var
  I: Integer;
  Len: Integer;
begin
  Len := 0;
  I := WordPosition(N, S, WordDelims, SkipEmpty);
  if I <> 0 then
...
begin
  Len := 0;
  I := acWordPosition(N, S, WordDelims, SkipEmpty);
  if I <> 0 then
...
procedure WaitTicks(ABeginTicks: DWORD; ADelay: integer = acTimerInterval);
begin
  while ABeginTicks + ADelay > GetTickCount do Sleep(0);
end;
...
begin
  ...
  PropInfo := GetPropInfo(Component.ClassInfo, PropName);
  if PropInfo <> nil then begin
    I := acNativeInt(GetOrdProp(Component, PropInfo));
...
begin
  ...
  if ptrPropInfo = nil then
    ...
    Result := acNativeInt(GetOrdProp(Component, ptrPropInfo));
end;
...
{$IFDEF TNTUNICODE}
  ...
  Result := (integer(Code) <> -1{INVALID_FILE_ATTRIBUTES}) and (FILE_ATTRIBUTE_DIRECTORY and Code <> 0);
{$ELSE}
  {$IFDEF DELPHI7UP}
...
function acSameText(const S1, S2: ACString): Boolean;
begin
  Result := {$IFDEF TNTUNICODE}WideSameText{$ELSE}SameText{$ENDIF}(S1, S2);
end;
...
{$ELSE}
function CharInSet(const C: acChar; const CharSet: TSysCharSet): Boolean;
begin
  {$IFDEF TNTUNICODE}
...
begin
  ...
  if hShell32 <> 0 then
    if not Assigned(acSHGetFileInfo) then
      acSHGetFileInfo := GetProcAddress(hShell32, {$IFDEF D2010}'SHGetFileInfoW'{$ELSE}'SHGetFileInfo'{$ENDIF});
end;


procedure AddObjLink(AObj: TObject; ACtlContainer: TControl);
begin
  TControl(ACtlContainer).Perform(SM_ALPHACMD, MakeWParam(1, AC_LINKOBJ), LPARAM(AObj));
end;
...
var
  i: integer;
begin
  // Search if added already
  for i := 0 to AList.Count - 1 do
    if AList[i] = AObj then
      Exit;
...
begin
  for i := 0 to AList.Count - 1 do
    DelObjLink(ACtl, TControl(AList[i]));
end;


procedure DelObjLink(ACtl: TObject; ACtlContainer: TControl);
begin
  TControl(ACtlContainer).Perform(SM_ALPHACMD, MakeWParam(0, AC_LINKOBJ), LPARAM(ACtl));
end;


procedure DelObjLink(ACtl: TObject; AList: TList);
var
  i: integer;
begin
  for i := 0 to AList.Count do // Send msg to all controls from list
    TControl(AList[i]).Perform(SM_ALPHACMD, MakeWParam(0, AC_LINKOBJ), LPARAM(ACtl));
end;
...
initialization
  ...
  if hUser32 <> 0 then
    ac_SystemParametersInfoForDpi := GetProcAddress(hUser32, 'SystemParametersInfoForDpi'{$IFDEF TNTUNICODE} + 'W'{$ENDIF});
