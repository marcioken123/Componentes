...
const
  ...
  MaxCompSkinVersion = 16.99;

  // Followed colors are changed according to the skin palette if used with acColorToRGB function
...
{$ENDIF}
  ...
  slBtnRed             = TColor(clSystemColor or 10395391);//$D5D5FF);
  slBtnRedActive       = TColor(clSystemColor or 8290047);//$BDBDFF);
  slBtnRedText         = TColor(clSystemColor or $0000B4);
  slBtnRedTextActive   = TColor(clSystemColor or $0000A2);

  slBtnGreen           = TColor(clSystemColor or 8114328);//$AEE194);
  slBtnGreenActive     = TColor(clSystemColor or 4121205);//$CDFFB4);
  ...
  slBtnYellowText       = TColor(clSystemColor or $0079B5);//$00A5D7);
  ...
  BGT_STRETCHHORZ   = BGT_STRETCH or BGT_TEXTUREHORZ;
{
  ...
  acAlignments: array [TAlignment] of DWord = (ES_LEFT, ES_RIGHT, ES_CENTER);
...
type
  ...
  TacOuterVisibility = (ovNone, ovAlways, ovFocused, ovMouseHovered);
  TacGrayedMode = (gmAlways, gmInactive);
  TacSide = (asLeft, asTop, asRight, asBottom);
  TacTabLayout = (tlFirst, tlLast, tlMiddle, tlSingle);
{
  TsAccessControl = class(TControl) // Will be removed in the Beta. Use acntTypes.TacAccessControl
  public
    property AutoSize;
...
{$ENDIF}
  ...
  TacBtnEvent = (beMouseEnter, beMouseLeave, beMouseDown, beMouseUp, beClick, beLighting);
  TacBtnEvents = set of TacBtnEvent;

//  TacAnimClickMode = (cmWaveOut, cmFillLight, cmLightAndWave);

  TacAnimatEvent = (aeMouseEnter, aeMouseLeave, aeMouseDown, aeMouseUp, aeClick, aeLighting, aeGlobalDef);
...
const
  CharBullet = acChar({$IFDEF WIDETEXT}#$25CF{$ELSE}'*'{$ENDIF});
...
//  ASE_UPDATE = 8;                          // Section updated, Next Byte - LengthOf Name, Next Word - Size of section

  ASE_MSG    = $A06A + 918; // $A400;      //
//  ASE_UPD    = $A06A + 919; // $A401;      // Section updated, Next Byte - LengthOf Name, Next Word - Size of section
{$ENDIF}
  ...
  acBtnEvents: array [TacAnimatEvent] of TacBtnEvent = (beMouseEnter, beMouseLeave, beMouseDown, beMouseUp, beClick, beLighting, beMouseUp);
...
//  COC_TsCustomDateEdit   = 137;
  ...
  ssLightCtrls: TsCodes    = [COC_TsButton, COC_TsBitBtn, COC_TsSpeedButton, COC_TsComboBox, COC_TsComboBoxEx, COC_TsSlider, COC_TsBarTitle,
                              COC_TsFileDirEdit..COC_TsDateEdit, COC_TsColorBox];
...
var
  ...
  acSysDPIAware: boolean = False;
  acIsDPIAware: boolean = False;
  acDebugMode: boolean = False;
  acCheckScale: boolean = False; // Do checking of scale in the TsRichEdit
  acDialogPosIsCenter: boolean = False; // System dialogs coordinates means a center of dialog (used for centering of dialogs over the main form)
...
type
  ...
  TacHorzAlign = (haLeft, haCenter, haRight);
{$IFNDEF FPC}
...
var
  ...
  ac_OldGlyphsMode:      boolean = True;   // Drawing of glyphs in comboedits from old resources
...
procedure InitSysVariables;
begin
  if acScrollBtnLength < 0 then
    acScrollBtnLength := GetSystemMetrics(SM_CXHSCROLL);
end;


procedure acLoadResStr(var AValue: acString; ALib: HModule; AIdent, DefValue: integer; const Suffix: acString = ''); overload;
var
...
procedure acLoadResStr(var AValue: acString; ALib: HModule; AIdent: integer; const DefValue: acString; const Suffix: acString = ''); overload;
var
  ResStringRec: TResStringRec;
begin
  if Lib <> 0 then begin
...
{
function acColorToIdent(Int: Integer; var Ident: string): Boolean;
var
  i: integer;
begin
  for i := 0 to High(acSkinColorNames) do
    if acSkinColorNames[i].Value = Int then begin
      Ident := acSkinColorNames[i].Name;
      Result := True;
      Exit;
    end;
...
function acIdentToColor(const Ident: string; var Int: Integer): Boolean;
var
  i: integer;
begin
  for i := 0 to High(acSkinColorNames) do
    if SameText(acSkinColorNames[i].Name, Ident) then begin
      Int := Integer(acSkinColorNames[i].Value);
      Result := True;
      Exit;
    end;
...
//  RegisterIntegerConsts(TypeInfo(TColor), acIdentToColor, acColorToIdent);
