...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Menus, ImgList, ExtDlgs, StdCtrls, Buttons, ExtCtrls, ComCtrls,
  sHintManager, sEdit, sBitBtn, sSpeedButton, sPanel, sCheckBox, sGroupBox, sTrackBar, sComboBox, sLabel, sSkinProvider, sColorSelect, sDialogs;
...
uses
  ShellApi, IniFiles,
  sConst, sGraphUtils, acntUtils, sGradient, sGradBuilder, sStoreUtils, sStyleSimply, sMessages, sSkinManager, sStrings, sVCLUtils;
...
procedure KillForm;
begin
  if Assigned(HintDesigner) then
    HintDesigner.Release;
end;


procedure THintDesigner.sComboBox4Change(Sender: TObject);
begin
  if sComboBox4.ItemIndex < sComboBox4.Items.Count - 1 then
...
begin
  ...
  if sComboBox4 <> Sender then
    sComboBox4.ItemIndex := ord(Manager.Predefinitions);
...
begin
  nc := ColorToRGB(C);
  Panel.Color := nc;
  if (GetRValue(nC) + GetGValue(nC) + GetBValue(nC)) / 3 > 129 then
...
begin
  ...
  if sHintManager1.HintKind.Style in [hsNone, hsStandard] then
...
begin
  ...
  case sHintManager1.HintKind.Style of
    hsBalloon:
      FTempHint := TsBalloonHintWindow.Create(Self);

    hsComics:
      FTempHint := TsComicsHintWindow.Create(Self);

    hsEllipse:
      FTempHint := TsEllipseHintWindow.Create(Self);

    hsSimply:
      FTempHint := TsSimplyHintWindow.Create(Self)

    else begin
      if Assigned(DefaultManager) and DefaultManager.CommonSkinData.Active then
        BitBlt(PaintBox1.Canvas.Handle, 0, 0, PaintBox1.Width, PaintBox1.Height, Panel4.SkinData.FCacheBmp.Canvas.Handle, 4, 4, SRCCOPY)
      else
        FillDC(PaintBox1.Canvas.Handle, PaintBox1.ClientRect, ColorToRGB(clBtnFace));
  ...
  FTempHint.Caption := acs_PreviewHint;
  ...
  if sHintManager1.FCacheBmp = nil then
    sHintManager1.FCacheBmp := CreateBmp32(FTemphint.Width, FTemphint.Height);
  ...
  if sHintManager1.HintKind.ShadowEnabled then
    FTempHint.PaintShadow;
  ...
  try
    if Assigned(BodyBmp) and Assigned(MaskBmp) then
      SumByMask(sHintManager1.FCacheBmp, BodyBmp, MaskBmp, FTempHint.ClientRect);
...
begin
  if TrackBar2.Position > 100 - TrackBar1.Position then
    TrackBar2.Position := 100 - TrackBar1.Position;

  sHintManager1.HintKind.TexturePercent := TrackBar1.Position;
  if sHintManager1.HintKind.TexturePercent = 0 then
    sHintManager1.HintKind.Texture.Assign(nil);
...
begin
  if TrackBar1.Position > 100 - TrackBar2.Position then
    TrackBar1.Position := 100 - TrackBar2.Position;
...
begin
  if sHintManager1.HintKind.GradientData <> '' then begin
    Bmp := CreateBmp32(PaintBox2.Width, PaintBox2.Height);
...
var
  ...
  b: boolean;
begin
  if Panel4.SkinData.Updating and Panel4.SkinData.Skinned then
    Exit;
  // Prepare of the PaintBox BG
  MainBmp := CreateBmp32(PaintBox1.Width, PaintBox1.Height);
  MainBmp.Canvas.Brush.Color := clSilver;
  MainBmp.Canvas.Font.Name := 'Small fonts';
  MainBmp.Canvas.Font.Size := 7;
  if Panel4.SkinData.Skinned then
    BitBlt(MainBmp.Canvas.Handle, 0, 0, PaintBox1.Width, PaintBox1.Height, Panel4.SkinData.FCacheBmp.Canvas.Handle, 4, 4, SRCCOPY)
  else
    FillDC(MainBmp.Canvas.Handle, PaintBox1.ClientRect, ColorToRGB(clBtnFace));
  ...
  for y := 0 to 30 do begin
    b := not b;
    for x := 0 to 30 do
      MainBmp.Canvas.TextOut(x * w - integer(b) * wd2, y * h, s);
  end;
end;
...
begin
  inherited;
  if (Message.Msg = cardinal(SM_ALPHACMD)) and (Message.WParamHi = AC_REFRESH) then
    PrepareMainBmp;
end;
...
begin
  ...
  if FileExists(sHintManager1.HintKind.TextureFile) then
    sHintManager1.HintKind.Texture.LoadFromFile(sHintManager1.HintKind.TextureFile);
...
begin
  Caption         := acs_HintDsgnTitle;
  Label1.Caption  := acs_HintDsgnPreserved;
  Label2.Caption  := acs_HintDsgnStyle;
  Label3.Caption  := acs_HintDsgnBevelWidth;
  Label5.Caption  := acs_HintDsgnArrowLength;
  Label6.Caption  := acs_HintDsgnHorizMargin;
  Label7.Caption  := acs_HintDsgnVertMargin;
  Label8.Caption  := acs_HintDsgnRadius;
  Label9.Caption  := acs_HintDsgnMaxWidth;
  Label10.Caption := acs_HintDsgnPauseHide;
  Label11.Caption := acs_Percent;
  Label12.Caption := acs_Blur;
  Label13.Caption := acs_HintDsgnOffset;
  Label14.Caption := acs_HintDsgnTransparency;
  Label15.Caption := acs_Percent;
  Label17.Caption := acs_HintDsgnTransparency;

  sBitBtn1.Caption      := acs_Font;
  sBitBtn2.Caption      := acs_Texture;
  BitBtn1.Caption       := acs_CloseStr;
  BitBtn2.Caption       := acs_HintDsgnLoad;
  BitBtn3.Caption       := acs_HintDsgnSave;
  sColorSelect2.Caption := acs_HintDsgnBorderTop;
  sColorSelect3.Caption := acs_HintDsgnBorderBottom;
  sColorSelect4.Caption := acs_HintDsgnColor;
  GroupBox1.Caption     := acs_Shadow;
  GroupBox2.Caption     := acs_Background;
  GroupBox3.Caption     := acs_Gradient;
end;
