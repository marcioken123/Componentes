...
{$IFNDEF NOTFORHELP}
    ...
    procedure SetAutoMouseWheel(const Value: boolean);
  protected
    acTrackPos: integer;
...
{$IFNDEF NOTFORHELP}
    ...
    property Align;
    property Anchors;
    property AutoMouseWheel: boolean read FAutoMouseWheel write SetAutoMouseWheel default False;
...
{$ENDIF}
    ...
    property OnUnDock;
{$ENDIF} // NOTFORHELP
    property WheelDelta: integer read FWheelDelta write FWheelDelta default 120;
...
function TsScrollBox.BorderOffset: integer;
begin
  Result := BorderWidth + ActBorderWidth * integer(BorderStyle = bsSingle)
end;
...
begin
  ...
  if not SkinData.CustomColor or Assigned(FOnPaint) then begin
...
procedure TsScrollBox.SetAutoMouseWheel(const Value: boolean);
begin
  FAutoMouseWheel := Value;
  if Value then begin
    ...
    with Message do
      if not InUpdating(SkinData) then
        if not (csPaintCopy in ControlState) and (TMessage(Message).WParam <> WParam(TMessage(Message).LParam) {PerformEraseBackground, TntSpeedButtons}) then begin
          ...
          if (DC <> 0) and not FCommonData.CustomColor then begin // From PaintTo
            if FCommonData.BGChanged or FCommonData.FCacheBmp.Empty then
              PrepareCache;
    ...
    with Message do
      if ControlIsReady(Self) then begin
        ...
        if BiDiMode = bdRightToLeft then
          if (ListSW <> nil) and ListSW.sBarVert.fScrollVisible then
            MoveWindowOrg(DC, GetScrollMetric(ListSW.sBarVert, SM_SCROLLWIDTH), 0);
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then
    case Message.WParamHi of
      ...
      AC_PREPARECACHE: begin
        if SkinData.Skinned and not InUpdating(SkinData) then
          PrepareCache;
      ...
      AC_GETDEFINDEX: begin
        if FCommonData.SkinManager <> nil then
          with FCommonData.SkinManager.SkinCommonInfo do
            Message.Result := iff(BorderStyle = bsNone, Sections[ssTransparent], Sections[ssPanelLow]) + 1;
  ...
  case Message.Msg of
    ...
    CM_BIDIMODECHANGED: if SysLocale.MiddleEast then RecreateWnd;
  end;
  ...
  if not ControlIsReady(Self) then
            ...
            AC_GETBG: begin
              ...
              with PacBGInfo(Message.LParam)^ do
                if (BgType = btCache) and not PleaseDraw then begin
                  inc(Offset.X, 2 * integer(BorderStyle = bsSingle) + BorderWidth);
                  inc(Offset.Y, 2 * integer(BorderStyle = bsSingle) + BorderWidth);
                end;
    ...
    if (Message.Result >= 0) {Avoiding an issue when ScrollBox is killed by child control already} and (FCommonData <> nil) and FCommonData.Skinned then
      case Message.Msg of
        ...
        WM_LBUTTONDOWN:
          if FAutoMouseWheel and not (csDesigning in ComponentState) and CanFocus and Parent.Visible then
            SetFocus();
  ...
  if FAutoMouseWheel then
    if not bacFlag then begin
      ...
      if Form <> nil then begin
        ...
        m.Result := 0;
        WheelDelta := WheelDelta * FWheelDelta div WHEEL_DELTA;
        if c <> nil then begin
        ...
        if m.Result = 0 then begin
          newPos := min(max(0, VertScrollBar.Position - (WheelDelta * VertScrollBar.Increment div WHEEL_DELTA)), VertScrollBar.Range);
          if newPos <> VertScrollBar.Position then begin
            if Assigned(FOnBeforeScroll) then
              FOnBeforeScroll(Self);

            SendScrollMessage(Handle, WM_VSCROLL, SB_THUMBTRACK, newPos);
            if Assigned(FOnAfterScroll) then
              FOnAfterScroll(Self);
