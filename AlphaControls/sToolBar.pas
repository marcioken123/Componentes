...
{$IFNDEF NOTFORHELP}
  ...
  {$ELSE}
    procedure StdPaint(Button: TToolButton; State: TCustomDrawState; Bmp: TBitmap);
  {$ENDIF}
  public
...
uses
  ...
  sAlphaGraph, sSkinManager, sThirdParty;
...
//  si: integer;
  ...
  BGInfo: TacBGInfo;
begin
//  if ToolBar.SkinData.SkinManager.IsValidImgIndex(mi) then
  begin
    Bmp := CreateBmp32(R);
    ...
    if BGInfo.Bmp <> nil{BGInfo.BgType = btCache} then
    ...
    if IsDivider then
      PaintItem(ToolBar.SkinData.SkinManager.SkinCommonInfo.Sections[ssDivider],
                BGInfoToCI(@BGInfo), True, 0, nRect, Point(R.Left + ToolBar.BorderWidth, R.Top + ToolBar.BorderWidth), Bmp, ToolBar.SkinData.CommonSkinData);
...
begin
  ...
  if not (csDesigning in ComponentState) and Assigned(FCommonData) and FCommonData.Skinned then
    if (AComponent is TPopupMenu) and Assigned(DefaultManager) and (Operation = opInsert) then
      DefaultManager.SkinableMenus.HookPopupMenu(TPopupMenu(AComponent), True);
end;

{$ELSE}

procedure TsToolBar.StdPaint(Button: TToolButton; State: TCustomDrawState; Bmp: TBitmap);
const
  DownStyles: array[Boolean] of Integer = (BDR_RAISEDINNER, BDR_SUNKENOUTER);
  FillStyles: array[Boolean] of Integer = (BF_MIDDLE, 0);
var
  PaintRect: TRect;
  Mode, DrawFlags: Integer;
begin
  ...
  PaintRect := Button.ClientRect;
  if not Flat then begin
    DrawFlags := DFCS_BUTTONPUSH or DFCS_ADJUSTRECT;
    if Mode = 2 then
      DrawFlags := DrawFlags or DFCS_PUSHED;

    DrawFrameControl(Bmp.Canvas.Handle, PaintRect, DFC_BUTTON, DrawFlags);
  end
  else begin
    if (Mode > 0) or (csDesigning in ComponentState) then
      DrawEdge(Bmp.Canvas.Handle, PaintRect, DownStyles[Mode = 2], FillStyles[Transparent] or BF_RECT)
    else
      if not Transparent then begin
        Bmp.Canvas.Brush.Color := Color;
        Bmp.Canvas.FillRect(PaintRect);
      end;
...
{$IFDEF FPS}
  if FState in [bsDown, bsExclusive] then
    if (FState = bsExclusive) and (not Flat or not MouseInControl) then begin
      Bmp.Canvas.Brush.Bitmap := AllocPatternBitmap(clBtnFace, clBtnHighlight);
      Bmp.Canvas.FillRect(PaintRect);
    end;
{$ENDIF}
end;
...
procedure TsToolBar.CancelMenu;
var
  i: integer;
begin
...
procedure TsToolBar.ChangeScale(M, D: Integer{$IFDEF DELPHI_10BERLIN}; isDpiChange: Boolean{$ENDIF});
begin
...
{$IFDEF TNTUNICODE}
      if Button is TTntToolButton then
        pc := PacChar(TTntToolButton(Button).Caption)

      else
{$ENDIF}
{$WARNINGS OFF}
        pc := PacChar(Button.Caption);
{$WARNINGS ON}

      if NeedParentFont(SkinData.SkinManager, iSkinIndex, Mode) then
        Ndx := GetFontIndex(Self, SkinData.SkinIndex, SkinData.SkinManager)
      else
        Ndx := iSkinIndex;

      acWriteTextEx(BtnBMP.Canvas, pc, Enabled and (DisabledImages = nil), cRect, DT_CENTER or DT_VCENTER or DT_SINGLELINE, Ndx, Mode > 0)
    end;
  end;
  ...
  begin
    i := integer(Mode > 0);
    BtnBmp.Canvas.Font.Color := FCommonData.CommonSkinData.gd[iSkinIndex].Props[i].FontColor.Color;
  ...
  var
    i, Mode, x, y: integer;
  begin
    with FCommonData.SkinManager, FCommonData.CommonSkinData, gd[iSkinIndex] do begin
      ...
      with ScrollBtns[asBottom] do
        if IsValidIndex(GlyphIndex, High(ma)) then begin
          ...
          i := integer(Mode > 0);
          DrawColorArrow(BtnBmp, gd[iSkinIndex].Props[i].FontColor.Color, R, asBottom, GetPPI(SkinData));
        end;
    end;
  end;

begin
  if FCommonData.Skinned {$IFNDEF DELPHI6UP} or True {$ENDIF} then begin
    DefaultDraw := False;
    ...
    if not (Stage in [cdPrePaint]) then
      ...
      if Stage in [cdPrePaint] then begin
        if not (csDesigning in ComponentState) then begin
          if not Flat and (HotButtonIndex = Button.Index) then
            State := State + [cdsHot];

          if DroppedButton = Button then
            State := State + [cdsHot];
        end;
...
{$IFDEF DELPHI6UP}
            if Menu <> nil then
              iSkinIndex := FCommonData.SkinManager.SkinCommonInfo.Sections[ssMenuItem]
            else
{$ENDIF}
              if Flat then
                iSkinIndex := FCommonData.SkinManager.SkinCommonInfo.Sections[ssToolButton]
              else
                iSkinIndex := FCommonData.SkinManager.SkinCommonInfo.Sections[ssSpeedButton]
{$IFNDEF DELPHI6UP}
          else begin
            iSkinIndex := -1;
            ci := EmptyCI;
...
{$ENDIF}
          ...
          if iSkinIndex >= 0 then begin
            ...
            PaintItemBg(iSkinIndex, ci, Mode, R, Point(Button.Left, Button.Top), BtnBmp, FCommonData.CommonSkinData);
            R.Right := bWidth - AddedWidth;

            ci.X := ci.X + Button.Left;
            ci.Y := ci.Y + Button.Top;
            if FCommonData.CommonSkinData.gd[iSkinIndex].BorderIndex >= 0 then
              DrawSkinRect(BtnBmp, R, ci, FCommonData.CommonSkinData.ma[FCommonData.CommonSkinData.gd[iSkinIndex].BorderIndex], Mode, True);
{$IFNDEF DELPHI6UP}
          end
          else begin
            FillDC(BtnBmp.Canvas.Handle, MkRect(BtnBmp), clBtnFace);
            StdPaint(Button, State, BtnBmp);
{$ENDIF}
          end;
          DrawBtnCaption;
          DrawBtnGlyph;
...
begin
  if SkinData.FOwnerControl <> nil then begin
    ...
    if (CurrentBounds.Right = SkinData.FCacheBmp.Width) or
         (SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[State].GradientPercent <> 0) and
           (SkinData.BGType and BGT_GRADIENTHORZ <> 0) then
      // If Rect is not used
      SkinData.InvalidRectH.Right := 0
    else begin
      if SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex < 0 then
        bWidth := 0
      else
        bWidth := SkinData.CommonSkinData.ma[SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex].WR;
    ...
    if (CurrentBounds.Bottom = SkinData.FCacheBmp.Height)
         or (SkinData.CommonSkinData.gd[SkinData.SkinIndex].Props[State].GradientPercent <> 0)
      ...
      if SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex < 0 then
        ...
        bWidth := SkinData.CommonSkinData.ma[SkinData.CommonSkinData.gd[SkinData.SkinIndex].BorderIndex].WB;
...
begin
  ...
  if ParentBG.BgType = btNotReady then begin
    ...
    PaintItem(FCommonData, BGInfoToCI(@ParentBG), True, 0, MkRect(Self), Point(Left, Top), FCommonData.FCacheBmp, True);
  ...
  if HandleAllocated and IsWindowVisible(Handle) then
    if FCommonData.Skinned then begin
      if not InUpdating(FCommonData) and not (csDestroying in ComponentState) then begin
        if not (FCommonData.BGChanged or FCommonData.FCacheBmp.Empty) or PrepareCache then begin
          DC := GetWindowDC(Handle);
          SavedDC := SaveDC(DC);
          try
            Windows.GetClientRect(Handle, RC);
  ...
  if Message.Msg = SM_ALPHACMD then begin
    case Message.WParamHi of
      ...
      AC_GETDEFINDEX: begin
        if FCommonData.SkinManager <> nil then
          Message.Result := FCommonData.SkinManager.SkinCommonInfo.Sections[ssToolBar] + 1;
      ...
      case Message.Msg of
        WM_PRINT:
          with TWMPaint(Message) do begin
            ...
            if SkinIsChanging(SkinData) then begin // Preventing of blinking when skin is changed
        ...
        WM_ERASEBKGND:
          if ([csDesigning, csDestroying] * ComponentState = []) and not InUpdating(FCommonData) then begin
            FCommonData.Updating := FCommonData.Updating;
            Message.Result := 1;
            Exit;
          end;
...
{$ENDIF}
      ...
      WM_PAINT:
        if not InAnimationProcess then begin
          ...
          w := SkinData.SkinManager.SkinCommonInfo.Sections[ssDivider];
      ...
      CN_DROPDOWNCLOSED:
        if (DroppedButton <> nil) and (DroppedButton.Style = tbsDropDown) then begin
          HotButtonIndex := -1;
          i := DroppedButton.Index;
          DroppedButton := nil;
          RepaintButton(i);
