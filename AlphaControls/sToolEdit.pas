...
//    property Text stored DateIsStored;
  ...
  published
    ...
    property TitleHeight: integer read FTitleHeight write FTitleHeight default 18;
    property TitleBtnsWidth: integer read FTitleBtnsWidth write FTitleBtnsWidth default 18;
...
{$IFNDEF NOTFORHELP}
  ...
  published
    ...
    property Text stored False;//DateIsStored;
    property TextHint;
    property Weekends;
...
uses
  ...
  sMessages, sVCLUtils, acPathDialog, acShellCtrls, sSkinManager, sGlyphUtils, sGraphUtils, sCommonData;
...
begin
  ...
  if Flag then begin
    ...
    if Flag then
      if ofOldStyleDialog in DialogOptions then
        sFileName := FDialog.Files[0]
      else
...
begin
  ...
  if Flag then begin
    ...
    with PathDialogForm do begin
      ...
      sScrollBox1.Visible := ShowRootBtns;
...
//    D := Date;
    if (FMaxDate <> 0) and (Date > FMaxDate) then
      ...
      if (FMinDate <> 0) and (Date < FMinDate) then
        ...
        D := StrToDateFmtDef(FDateFormat, Text, Date);
...
begin
  if (FPopupWindow <> nil) and (TsPopupCalendar(FPopupWindow).FCalendar <> nil) then
    with TsPopupCalendar(FPopupWindow) do begin
      sMonthCalendar1.Initializing := True;
      ...
      sMonthCalendar1.Initializing := False;
    end;
end;
...
procedure TsCustomDateEdit.PopupWindowClose;
begin
  inherited;
  dec(SysPopupCount);
end;
...
begin
  // Destroy own popup calendar if exists
  FreeAndNil(FPopupWindow);
  // Check for another visible calendar
  if sConst.sPopupCalendar <> nil then
    sConst.sPopupCalendar.Close;
  // Create new popup calendar
  FPopupWindow := TsPopupCalendar.Create(Self);
  // Define a global variable with current visible calendar
  sConst.sPopupCalendar := TForm(FPopupWindow);
  with TsPopupCalendar(FPopupWindow) do begin
    BorderStyle := bsNone;
    if (SkinData.SkinManager <> nil) and (SkinData.SkinManager.Options.ScaleMode = smVCL) then begin
      sMonthCalendar1.TitleHeight := ScaleInt(FTitleHeight, SkinData);
      sMonthCalendar1.TitleBtnsWidth := ScaleInt(FTitleBtnsWidth, SkinData);
    end
    else begin
      sMonthCalendar1.TitleHeight := FTitleHeight;
      sMonthCalendar1.TitleBtnsWidth := FTitleBtnsWidth;
    end;
    ...
    FCalendar.DimUnacceptedCells := FDimUnacceptedCells;
    ...
    FCalendar.OnAcceptDate := OnAcceptDate;
    FCalendar.CalendarChanged := True;
//    FCalendar.FGrid.UpdateStates;
...
begin
  ...
  if IsRightToLeft then begin
    ...
    for i := l downto 1 do begin
      ...
      if X + GlyphSpace(True) > pos - w then begin
        Result := i;
        Exit;
      end
      else
