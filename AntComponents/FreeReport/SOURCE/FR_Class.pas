...
{$ENDIF}
    ...
    property OnReportLoaded: TNotifyEvent read FOnReportLoaded write FOnReportLoaded;
  published
  ...
  private
    FTemplatesDir: TFileName;
  protected
    procedure SetTemplatesDir(const ADir: TFileName); virtual;
  public
    Page: TfrPage;
    Modified: Boolean;
    ...
    property TemplatesDir: TFileName read FTemplatesDir write SetTemplatesDir;
    procedure SetSecondHelpFile(const AFile: TFileName; const AProc: TNotifyEvent); virtual; abstract;
  end;
...
const
  ...
  frSpecFuncs: Array[0..frSpecCount - 1] of String = ('PAGE#', '',
    ...
    'RECORD#', 'TEMPLATEPAGE#', 'TOTALPAGES', 'TOTALRECORDSTEMPLATEPAGE',
    'TOTALRECORDS', 'TOTALTEMPLATEPAGES'
    );
...
var
  ...
  RecNo: Integer;                      // current record number in Building mode
  frCharset: 0..255;
...
var
  ...
  SavedPageRecords: TList;      // number of records by page in entire report
  SavedAllRecords: Integer;     // number of records in entire report
...
function Create180Font(Font: TFont): HFont;
var
  F: TLogFont;
begin
  GetObject(Font.Handle, SizeOf(TLogFont), @F);
  F.lfEscapement := 1800;
  F.lfOrientation := 1800;
  Result := CreateFontIndirect(F);
end;

function Create270Font(Font: TFont): HFont;
var
  F: TLogFont;
begin
  GetObject(Font.Handle, SizeOf(TLogFont), @F);
  F.lfEscapement := 2700;
  F.lfOrientation := 2700;
  Result := CreateFontIndirect(F);
end;
...
begin
  ...
  if DocMode = dmDesigning then
    Canvas.FillRect(Rect(DRect.Left, DRect.Top,
      DRect.Right, DRect.Bottom))
  // use calculating coords instead of dx, dy - for best view
  else
    Canvas.FillRect(Rect(x, y,
      Round((SaveX + SaveDX) * ScaleX + OffsX),
      Round((SaveY + SaveDY) * ScaleY + OffsY)));
end;
...
var
  ...
  procedure Line1(x, y, x1, y1: Integer; L, R, T, B: Boolean);
  begin
    if x = x1 then
    begin
      if T then Canvas.MoveTo(x, y)
      else Canvas.MoveTo(x, y + Canvas.Pen.Width div 2);
      if B then Canvas.LineTo(x1, y1)
      else Canvas.LineTo(x1, y1 - Canvas.Pen.Width div 2);
    end else
    begin
      if L then Canvas.MoveTo(x, y)
      else Canvas.MoveTo(x + Canvas.Pen.Width div 2, y);
      if R then Canvas.LineTo(x1, y1)
      else Canvas.LineTo(x1 - Canvas.Pen.Width div 2, y1);
    end;
  end;
  procedure Line2(x, y, x1, y1: Integer; L, R, T, B: Boolean);
  var
    i, w: Integer;
  begin
    w := Canvas.Pen.Width;
    ...
    if x = x1 then
      for i := 0 to w - 1 do
        Line1(x - w div 2 + i, y, x - w div 2 + i, y1, L, R, T, B)
    else
      for i := 0 to w - 1 do
        Line1(x, y - w div 2 + i, x1, y - w div 2 + i, L, R, T, B);
    Canvas.Pen.Width := w;
  end;
  procedure Line3(x, y, x1, y1: Integer; TopLeft, L, R, T, B: Boolean);
  var
    iL, IR, IT, iB: Integer;
  begin
    iL := 0; iR := 0; iT := 0; iB := 0;
    if L then iL := Canvas.Pen.Width;
    if R then iR := Canvas.Pen.Width;
    if T then iT := Canvas.Pen.Width;
    if B then iB := Canvas.Pen.Width;
    if x = x1 then
    begin
      if TopLeft then
      begin
        Line1(x - Canvas.Pen.Width, y - iT,
          x1 - Canvas.Pen.Width, y1 + iB, L, R, T, B);
        Line1(x + Canvas.Pen.Width, y + iT,
          x1 + Canvas.Pen.Width, y1 - iB, L, R, T, B);
      end else
      begin
    ...
    begin
      if TopLeft then
      begin
        Line1(x - iL, y - Canvas.Pen.Width,
          x1 + iR, y1 - Canvas.Pen.Width, L, R, T, B);
        Line1(x + iL, y + Canvas.Pen.Width,
          x1 - iR, y1 + Canvas.Pen.Width, L, R, T, B);
      end else
      begin
...
begin
  ...
  begin
    ...
    begin
      Pen.Color := clBlack;
      Pen.Width := 1;
      Line(x, y + 3, x, y); Line(x, y, x + 4, y);
      Line(x, y + dy - 3, x, y + dy); Line(x, y + dy, x + 4, y + dy);
      Line(x + dx - 3, y, x + dx, y); Line(x + dx, y, x + dx, y + 4);
      Line(x + dx - 3, y + dy, x + dx, y + dy); Line(x + dx, y + dy, x + dx, y + dy - 4);
    end;
    if (Round(FrameWidth) = 0) or ((FrameTyp and $F) = 0) then Exit;
    // use calculating coords instead of dx, dy - for best view
    x1 := Round((SaveX + SaveDX) * ScaleX + OffsX);
    y1 := Round((SaveY + SaveDY) * ScaleY + OffsY);
    Pen.Width := Round(FrameWidth);
    ...
    begin
      ...
      T := (FrameTyp and $8) <> 0;
      B := (FrameTyp and $2) <> 0;
      if (FrameStyle = 0) or (FillColor <> clNone) or (DocMode = dmDesigning) then
      begin
        ...
        if L then Line1(x, y, x, y1, L, R, T, B);
        if R then Line1(x1, y, x1, y1, L, R, T, B);
        if T then Line1(x, y, x1, y, L, R, T, B);
        if B then Line1(x, y1, x1, y1, L, R, T, B);
        if FrameStyle = 0 then Exit;
        Pen.Color := FrameColor;
        if Pen.Color = clNone then
          Pen.Color := clBlack;
        if FrameStyle = 5 then
      ...
      begin
        Pen.Style := TPenStyle(FrameStyle);
        if L then Line2(x, y, x, y1, L, R, T, B);
        if R then Line2(x1, y, x1, y1, L, R, T, B);
        if T then Line2(x, y, x1, y, L, R, T, B);
        if B then Line2(x, y1, x1, y1, L, R, T, B);
      end else
      begin
        if L then Line3(x, y, x, y1, true, L, R, T, B);
        if R then Line3(x1, y, x1, y1, false, L, R, T, B);
        if T then Line3(x, y, x1, y, true, L, R, T, B);
        if B then Line3(x, y1, x1, y1, false, L, R, T, B);
      end;
    end;
  end;
end;
...
begin
  ...
  AdjustUnused := TfrMemoView(From).AdjustUnused;
  AdjustOrient := TfrMemoView(From).AdjustOrient;
...
var
  ...
  begin
    h := 0; oldh := 0;
    if AdjustOrient = 2 then
    begin
      h := Create180Font(WCanvas.Font);
  ...
  var
    ...
    h, oldh: HFont;
  begin
    if AdjustOrient = 3 then
      h := Create270Font(WCanvas.Font)
    else
...
{$ELSE}
          if (aw <> nw) and not Exporting then
            GetCharacterPlacement(Canvas.Handle, PChar(str), LongBool(Length(str)),
              LongBool(nw), GCP, GCP_JUSTIFY + GCP_MAXEXTENT)
          else
            GCP.lpDx := nil;
{$ENDIF}
          ...
          begin
            if (AdjustOrient = 2) then
              ExtTextOut(Canvas.Handle, DR.Right + DR.Left - curx,
                DR.Bottom + DR.Top - cury, ETO_CLIPPED, @DR,
        ...
        if Exporting then
          if AdjustOrient = 2 then
            CurReport.InternalOnExportText(DR.Right + DR.Left - curx,
              DR.Bottom + DR.Top - cury, str, Self)
          else
  ...
  var
    ...
    var
      ...
      ParaEnd: Boolean;
    begin
      if not Streaming or (curx + th <= DR.Right) then
      begin
        ...
        if Exporting then
          if AdjustOrient = 3 then
            CurReport.InternalOnExportText(ox + DR.Right + DR.Left - curx,
              DR.Bottom + DR.Top - cury, str, Self)
          else
            CurReport.InternalOnExportText(curx, cury, str, Self);
        Inc(CurStrNo);
...
begin
  ...
  begin
    ...
    Read(AdjustUnused, 1);
    Read(AdjustOrient, 1);
    if (AdjustOrient = 0) and ((Adjust and 4) = 4) then
      AdjustOrient := 1;
    Read(w, 2);
    if frVersion < 23 then
      w := frCharset;
...
begin
  ...
  begin
    ...
    Write(i, 4);
    Write(Adjust, 2);
    Write(AdjustUnused, 1);
    Write(AdjustOrient, 1);
{$IFDEF Delphi2}
    w := frCharset;
{$ELSE}
    w := Font.Charset;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        v := RecNo + 1
      else if s = frSpecFuncs[9] then
        v := FCurPageIdx + 1
      else if s = frSpecFuncs[10] then
        v := SavedAllPages
      else if s = frSpecFuncs[11] then
      begin
        if FCurPageIdx < SavedPageRecords.Count then
          v := Integer(SavedPageRecords.Items[FCurPageIdx])
        else
          v := 0;
      end
      else if s = frSpecFuncs[12] then
        v := SavedAllRecords
      else if s = frSpecFuncs[13] then
        v := Pages.Count
      else
      begin
...
{Soulsnake Fix}
        begin
          MessageBox(0, PChar('Undefined symbol "' + SubValue + '"'),
            PChar('Warning'), mb_Ok + mb_IconWarning);
          v := '';
          frVariables[SubValue] := v;
        end
{End Soulsnake Fix}
      end;
    end;
...
{$ENDIF}
    ...
    MessageBox(0, PChar(SFRFError), PChar(LoadStr(SError)),
      mb_Ok + mb_IconError);
  if Assigned(FOnReportLoaded) then
    FOnReportLoaded(Self);
end;
...
begin
  ...
  Application.HelpFile := ExtractFilePath(Application.ExeName) + 'FRuser.hlp';
  if frDesigner <> nil then
  begin
    frDesigner.TemplatesDir := TemplatesDir;
...
begin
  ...
  begin
    ...
    SavedPageRecords.Clear;
    for i := 0 to Pages.Count-1 do
      SavedPageRecords.Add(Pointer(Pages.Pages[i].CurPos - 1));
    SavedAllRecords := RecNo;
    DoublePass := False;
    FirstTime := False;
...
begin
  ...
  frCompressor.Free;
  HookList.Free;
  SavedPageRecords.Free;
end;


procedure TfrReportDesigner.SetTemplatesDir(const ADir: TFileName);
begin
  FTemplatesDir := ADir;
end;
