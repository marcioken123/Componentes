...
type
  ...
  protected
    procedure SetTemplatesDir(const ADir: TFileName); override;
  public
    ...
    procedure SetSecondHelpFile(const AFile: TFileName; const AProc: TNotifyEvent); override;
  end;
...
begin
  ...
  ComboOrientation.Hint := 'Rotation';
...
begin
  SetEnabled([FrB1, FrB2, FrB3, FrB4, FrB5, FrB6, ClB1, ClB3, E1, SB1, SB2, StB1],
    RectTypEnabled);
  SetEnabled([ClB2, C2, C3, FnB1, FnB2, FnB3, AlB1, AlB2, AlB3, ComboOrientation, AlB5, AlB6, AlB7, AlB8, HlB1],
    FontTypEnabled);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ComboOrientation.ItemIndex := AdjustOrient;
    ...
    begin
      ...
      with t as TfrMemoView do
        case b of
         ...
         14: begin
               Adjust := (Adjust and $FB) + Word(ComboOrientation.ItemIndex in [1,3]) * 4;
               AdjustOrient := ComboOrientation.ItemIndex;
               LastAdjust := Adjust;
             end;
         15: begin
...
begin
  ...
  begin
    ...
    {Soulsnake Fix}
    E3.Text := IntToStr(Round(pgMargins.Left * 5 / 18));
    E4.Text := IntToStr(Round(pgMargins.Top * 5 / 18));
    E5.Text := IntToStr(Round(pgMargins.Right * 5 / 18));
    E6.Text := IntToStr(Round(pgMargins.Bottom * 5 / 18));
    E7.Text := IntToStr(Round(ColGap * 5 / 18));
    {End Soulsnake Fix}
    ...
    begin
      ...
      try
        {Soulsnake Fix}
        pgMargins := Rect(Round(StrToInt(E3.Text) * 18 / 5),
                          Round(StrToInt(E4.Text) * 18 / 5),
                          Round(StrToInt(E5.Text) * 18 / 5),
                          Round(StrToInt(E6.Text) * 18 / 5));
        ColGap := Round(StrToInt(E7.Text) * 18 / 5);
        {End Soulsnake Fix}
      except
        on exception do
        begin
...
var
  path: string;
begin
  path := ChangeFileExt(Application.HelpFile, '.pdf');
...
procedure TfrDesignerForm.SetTemplatesDir(const ADir: TFileName);
begin
...
procedure TfrDesignerForm.SetSecondHelpFile(const AFile: TFileName; const AProc: TNotifyEvent);
var
  NewItem: TMenuItem;
begin
  ...
  begin
    ...
    NewItem.Name := 'TutorialMenu';
    NewItem.Caption := 'Tutorial from AMC help file';
    NewItem.ShortCut := TextToShortCut('Ctrl+F1'); 
    NewItem.OnClick := AProc;
    HelpMenu.Insert(1, NewItem);
  end;
end;
