...
procedure DoFuncId; forward;
  ...
  begin
    ...
    begin
{Soulsnake Fix}
      if MemoFrom.Count = i then break;
{End Soulsnake Fix}
      s1 := MemoFrom[i];
      j := 1;
...
begin
  ...
  begin
    ...
    else if s[1] = ttIf then
    begin
{Original}
      //if FParser.CalcOPZ(Remain(s, 4)) = 0 then
{End Original}
{Soulsnake Fix}
      if FParser.Calc(Remain(s, 4)) = 0 then
{End Soulsnake Fix}
      begin
        i := Ord(s[2]) + Ord(s[3]) * 256;
        continue;
...
{End Original}
{Soulsnake Fix}
      SetValue(s1, FParser.Calc(Remain(s, j)));
{End Soulsnake Fix}
    end;
    Inc(i);
  end;
