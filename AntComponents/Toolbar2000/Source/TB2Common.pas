...

{
  ...
  $jrsoftware: tb2k/Source/TB2Common.pas,v 1.43 2007/06/20 02:21:35 jr Exp $
}
...
{function GetSystemParametersInfoBool(const Param: UINT; const Default: BOOL): BOOL;} {vb-}
function GetSystemParametersInfoBool(const Param: UINT; const Default: BOOL = False): BOOL; {vb+}
...
function InflateRect(var Rect: TRect; DX, DY: Integer): Boolean; {vb+}
...
function Min(A, B: Integer): Integer;} {vb-}
{vb+}
function Max(A, B: Integer): Integer; {$IFDEF JR_D9} inline; {$ENDIF}
function Min(A, B: Integer): Integer; {$IFDEF JR_D9} inline; {$ENDIF}
{vb+end}
{$IFNDEF CLR}
...
function SetRectEmpty(var Rect: TRect): Boolean; {vb+}
...
uses
  ...
  {MMSYSTEM, TB2Version;} {vb-}
  MMSYSTEM, TB2Version {$IFDEF JR_D9}, Types {$ENDIF}; {vb+}
