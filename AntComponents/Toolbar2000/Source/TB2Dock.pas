...

{
  ...
  $jrsoftware: tb2k/Source/TB2Dock.pas,v 1.118 2007/06/20 02:29:56 jr Exp $
}
...
type
  ...
  private
    ...
    {FArrangeNeeded, FMoved: Boolean;} {vb- FMoved don't used}
    FArrangeNeeded: Boolean; {vb+}
...
uses
  ...
  {Registry, Consts, Menus,} {vb-}
  Registry, Consts, Menus {$IFDEF JR_D9}, Types {$ENDIF}, {vb+}
...
var
  ...
  begin
    ...
    if PreventFloating and not Assigned(MouseOverDock) then begin
      ...
      if FDragSplitting then
        ...
        {SetCursor(OldCursor);} {vb-}
        SetCursor(LoadCursor(0, IDC_SIZEALL)); {vb+ indicate opportunity to drag}
    end;
  ...
  procedure BuildDockList;
    ...
    begin
      {Result := D.FAllowDrag and (D.Position in DockableTo);} {vb-}
      Result := D.Accepts(Self) and CanDockTo(D); {vb+}
    end;
...
begin
  ...
  SendMessage(Handle, WM_CONTEXTMENU, WPARAM(Handle), Word(PosX) or (Word(PosY) shl 16));
  {$ELSE}
  CheckMenuPopup;
  {$ENDIF}
end;
