...
{
  Toolbar2000
  Copyright (C) 1998-2007 by Jordan Russell
  All rights reserved.
  ...
  $jrsoftware: tb2k/Source/TB2Item.pas,v 1.298 2007/04/02 19:28:41 jr Exp $
}
...
type
  ...
  TTBAdjustImageIndexEvent = procedure(Sender: TTBCustomItem; var AImageIndex: TImageIndex) of object; {vb+}
  ...
  TTBOrientation = (tboHorizontal, tboVertical); {vb+}
  ...
  private
    ...
    FPopupMenu: TPopupMenu; {vb+}
    FInheritOptions: Boolean;
    FNotifyList: TList;
    FOnAdjustImageIndex: TTBAdjustImageIndexEvent; {vb+}
    ...
    FRadioItem: Boolean;
    FShortCut: TShortCut;
    FSubitemsPopupMenu: TPopupMenu; {vb+}
    ...
    procedure InternalSetPopupMenu(var Menu: TPopupMenu; NewMenu: TPopupMenu); {vb+}
    {$IFDEF JR_D6}
    ...
    procedure SetPopupMenu(Value: TPopupMenu); {vb+}
    procedure SetRadioItem(Value: Boolean);
    procedure SetSubitemsPopupMenu(Value: TPopupMenu); {vb+}
  ...
  protected
    ...
    function GetChevronPopupPosition(var PopupPositionRec: TTBPopupPositionRec): Boolean; virtual; {vb+}
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
    function GetImageIndex: TImageIndex; virtual; {vb+}
  ...
  public
    ...
    {property ImageIndex: TImageIndex read FImageIndex write SetImageIndex stored IsImageIndexStored default -1;} {vb-}
    ...
    property PopupMenu: TPopupMenu read FPopupMenu write SetPopupMenu; {vb+}
    ...
    property SubitemsPopupMenu: TPopupMenu read FSubitemsPopupMenu write SetSubitemsPopupMenu; {vb+}
    property Visible: Boolean read FVisible write SetVisible stored IsVisibleStored default True;
    property OnAdjustImageIndex: TTBAdjustImageIndexEvent read FOnAdjustImageIndex write FOnAdjustImageIndex; {vb+}
  ...
  protected
    ...
    {procedure MouseWheel(WheelDelta: Integer; X, Y: Integer); virtual;} {vb-}
    procedure MouseWheel(WheelDelta: Integer; X, Y: Integer; var Handled: Boolean); virtual; {vb+}
  ...
  public
    ...
    function GetPopupMenu: TPopupMenu; dynamic; {vb+}
  ...
  protected
    ...
    procedure KeyDown(var Key: Word; Shift: TShiftState); virtual;
    procedure MouseWheel(WheelDelta: Integer; X, Y: Integer); dynamic; {vb+}
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  ...
  published
    ...
    property OnAdjustImageIndex; {vb+}
    property OnClick;
    property OnSelect;
  end;
  ...
  protected
    ...
    property AnimationDirection: TTBAnimationDirection read FAnimationDirection; {vb+}
  public
  ...
  private
    ...
    function GetItems: TTBCustomItem;
    function GetItemsPopupMenu: TPopupMenu; {vb+}
    procedure SetImages(Value: TCustomImageList);
    procedure SetItemsPopupMenu(Value: TPopupMenu); {vb+}
  protected
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
  public
    constructor Create(AOwner: TComponent); override;
    ...
    {property Items: TTBRootItem read FItem;} {vb- why not publish it for Object Inspector?}
  published
  ...
  published
    property Images: TCustomImageList read GetImages write SetImages;
    property Items: TTBRootItem read FItem;
    property ItemsPopupMenu: TPopupMenu read GetItemsPopupMenu write SetItemsPopupMenu; {vb+}
...
uses
  ...
  {MMSYSTEM, TB2Consts, TB2Common, IMM, TB2Acc;} {vb-}
  MMSYSTEM, TB2Consts, TB2Common, IMM, TB2Acc {$IFDEF JR_D9}, Types {$ENDIF}, Math; {vb+}
  ...
  try
    ...
    if Assigned(ParentView) then begin
      ...
      if not IsRectEmpty(ParentView.FMonitorRect) then
        ...
        {MonitorRect := GetRectOfMonitorContainingRect(ParentItemRect, False);} {vb-}
        MonitorRect := GetRectOfMonitorContainingRect(ParentItemRect, True); {vb+}
    end
    else begin
...
begin
  ...
  {if (Result <> '') and Application.HintShortCuts and
    ...
    Result := Format('%s (%s)', [Result, ShortCutToText(Item.ShortCut)]);} {vb-}
  {vb+}
...
{procedure TTBItemViewer.MouseWheel(WheelDelta, X, Y: Integer);} {vb-}
...
var
  ...
  Margins: TRect;
  PopupRec: TTBPopupPositionRec; {vb+}
...
var
  ...
  {SaveIndex, SaveIndex2: Integer;} {vb-}
  SaveIndex: Integer; {vb+}
  BkColor: TColor;
  OldBmp: HBITMAP; {vb+}
begin
  ValidatePositions;
...
begin
  if Assigned(FSelected) and FSelected.Item.Enabled then begin
    ...
    EndModal;
  {$IFNDEF CLR}
  Exit; asm db 0,'Toolbar2000 (C) 1998-2007 Jordan Russell',0 end;
  {$ENDIF}
end;
                ...
                if not MouseIsDown then begin
                  ...
                  if Marshal.SystemDefaultCharSize = 1 then
                    ConvertedKey := Encoding.GetEncoding(GetInputLocaleCodePage).
                      GetChars([Byte(Msg.wParam)])[0]
                  else
                  {$ENDIF}
                    ConvertedKey := Chr(Msg.wParam);
        ...
        WM_MOUSEFIRST..WM_MOUSELAST:
          case Msg.message of
            ...
            WM_LBUTTONUP, WM_RBUTTONUP:
              {if (Msg.message = WM_LBUTTONUP) or TrackRightButton then begin} {vb-}
              {vb+}
...
procedure TTBPopupWindow.WMNCCalcSize(var Message: TWMNCCalcSize);
  ...
  begin
    {InflateRect(R, -PopupMenuWindowNCSize, -PopupMenuWindowNCSize);} {vb-}
    with GetNCSize do InflateRect(R, -X, -Y); {vb+}
  end;
