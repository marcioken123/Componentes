...
{ Lista de Objetos do tipo TACBrCHQModelo
  A Procedure CARREGABANCOSINI, consegue carregar a definicição de Cheques de
  ...
  utilizados (Mais explicações no Implementação da CARREGABANCOSINI abaixo) }
...
TACBrCHQClass = class
  ...
  public
    ...
    procedure ImprimirLinha( AString : String ) ; Virtual ;
...
procedure TACBrCHQClass.ImprimirLinha( AString : String );
begin
...
begin
  ...
  begin
     MessageDlg('Arquivo '+fsArquivoBemaFiINI+' não encontrado. '+
                'Valores padrões serão utilizados.',mtWarning,[mbOk],0);
     fsArquivoBemaFiINI := '' ;
  end ;

  if fsArquivoBemaFiINI = '' then
     ArqTemp := ExtractFilePath( Application.ExeName )+'BemaFi32.INI'
  else
...
{    linha da cidade/data ------------------------------------------------+
 ...
 coluna do valor numerico ------------+  |  |  |  |  |  |  |  |  |  |  |  |
     ...
     if (Count = 0) and (fsArquivoBemaFiINI <> '') then
     begin
        MessageDlg('Arquivo '+fsArquivoBemaFiINI+' não possui nenhum modelo válido'+
