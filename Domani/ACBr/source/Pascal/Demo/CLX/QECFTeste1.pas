...
uses ACBrECF, ACBrBase, ACBrECFClass, ACBrECFBematech, 
  StrUtils, SysUtils, Classes, QTypes, QGraphics,
  QControls, QForms, QDialogs, QStdCtrls,  QComCtrls, QButtons, QExtCtrls,
  QMenus;
...
uses ACBrUtil, QVendeItem, QEfetuaPagamento, QRelatorio, DateUtils,
  TypInfo;
...
begin
  ...
  StatusBar1.Panels[0].Text :=  Estados[ ACBrECF1.Estado ] ;
                               { GetEnumName(TypeInfo(TACBrECFEstado),
                                 interger( ACBrECF1.Estado ) ) ;}
end;
...
begin
  ...
  begin
     ...
     mResp.Lines.Add( 'Subtotaliza Cupom: '+FloatToStr( Desc ) );
     AtualizaMemos ;
  end ;
end;
