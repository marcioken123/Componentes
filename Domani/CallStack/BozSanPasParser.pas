...
type
  ...
  TBozSanPasParser = class;

  TBozSanPasToken = class(TObject)
  private
  ...
  TBozSanPasParser = class(TObject)
  private
    FToken: TBozSanPasToken;
  ...
  public
    ...
    property Token: TBozSanPasToken read FToken;
...
constructor TBozSanPasToken.Create;
begin
  inherited Create;
  ID := tkNone;
  LineNumber := 0;
...
function TBozSanPasToken.GetData;
begin
  SetString(Result, (Origin + Position), Length);
end; { GetData }


destructor TBozSanPasParser.Destroy;
begin
  Token.Free;
  inherited Destroy;
end; { Destroy }

constructor TBozSanPasParser.Create;
begin
  inherited Create;
  FComment := csNo;
  FEndCount := 0;
  FImplementationsPos := 0;
  FToken := TBozSanPasToken.Create;
  Visibility := tkUnknown;
end; { Create }

function TBozSanPasParser.GetSubString(StartPos, EndPos: LongInt): String;
var
  SubLen: Integer;
begin
...
procedure TBozSanPasParser.SetOrigin(value: PChar);
begin
  FOrigin := value;
  Run := 0;
  FToken.Origin := value;
...
procedure TBozSanPasParser.SetRunPos(NewPos: LongInt);
begin
  Run := NewPos;
  NextToken;
end; { SetRunPos }

procedure TBozSanPasParser.HandleComments;
begin
  Case FComment of
    csAnsi:
      begin
...
function TBozSanPasParser.IdentKind: TTokenKind;
var
  HashKey: Integer;
  aToken: String;
...
procedure TBozSanPasParser.NextToken;
begin
  Case FOrigin[Run] of
    #0:
      begin
...
procedure TBozSanPasParser.NextID(ID: TTokenKind);
begin
  NextToken;
...
function TBozSanPasParser.GetIsJunk: Boolean;
begin
  Case FToken.ID of
...
procedure TBozSanPasParser.NextNonComment;
begin
  repeat
    NextToken;
...
procedure TBozSanPasParser.NextNonJunk;
begin
  repeat
    NextToken;
...
procedure TBozSanPasParser.NextNonSpace;
begin
  NextToken;
  while FToken.ID = tkSpace do NextToken;
end; { NextNonSpace }

procedure TBozSanPasParser.ToLineStart;
begin
  Run := FToken.LinePos;
  NextToken;
end; { ToLineStart }

procedure TBozSanPasParser.NextClassLine;
begin
...
procedure TBozSanPasParser.NextObjectLine;
begin
  while FToken.ID <> tkNull do
  begin
    if FToken.ID in BigIdentDirect then
...
function TBozSanPasParser.GetMethodImpLine(ClassName: String; MethodName: String): LongInt;
begin
  Result := -1;
  while FToken.ID <> tkNull do
  begin
