...
//    function GetDisplayText(ARecordIndex, AItemIndex: Integer): string; virtual;
  ...
  TCompareItems = function (AItem1, AItem2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer of object;

procedure QuickSortList(ASortList: {$IFNDEF CLR}PPointerList{$ELSE}ArrayList{$ENDIF};
...
function CompareByLinkObject(AItem1, AItem2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
...
begin
{$IFDEF CLR}
  if (ANewCount >= 0) and (ANewCount < AList.Count) then
  begin
    ACount := AList.List.Count;
    AList.List.RemoveRange(ANewCount, ACount - ANewCount);
  end
  else
    if AList.Count <> ANewCount then
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FEditingRecordIndex <= ARecordIndex then
    ...
    try
      ...
      begin
        ...
        if DataController.IsFocusedSelectedMode and
          ([soSelectedRecords, soMultipleSelectedRecords] * DataController.Summary.Options <> []) then
          FChanges := FChanges + [dcicSummary];
...
begin
  ...
  if ARecordCount = 0 then
    ...
    if (not DataController.FInDeleteSelection or DataController.FInLoadStorage) and
      (not DataController.IsKeyNavigation or DataController.IsSmartRefresh) then
    begin
...
begin
  ...
  Add(fviAll, Null, cxSFilterString({$IFNDEF CLR}@{$ENDIF}cxSFilterBoxAllCaption), False);
  Add(fviCustom, Null, cxSFilterString({$IFNDEF CLR}@{$ENDIF}cxSFilterBoxCustomCaption), False);
  if DataController.IsGridMode then
    ANullExists := True // custom loading
  else
    LoadRecords(AUseFilteredValues, ANullExists);
  if ANullExists then
    Add(fviBlanks, Null, cxSFilterString({$IFNDEF CLR}@{$ENDIF}cxSFilterBoxBlanksCaption), False);
  if ANullExists or AUseFilteredValues and NonBlanksFilterApplied then
    Add(fviNonBlanks, Null, cxSFilterString({$IFNDEF CLR}@{$ENDIF}cxSFilterBoxNonBlanksCaption), False);
...
begin
  ...
  begin
    ...
    for I := 0 to AChildCount - 1 do
      AList.Add({$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(ChildRecordIndex[ADataGroupIndex, I]));
end;
...
begin
  ...
  begin
    ...
    Result := (ASummaryItems <> nil) and (ASummaryValues <> nil) and
      not VarIsEmpty(ASummaryValues{$IFNDEF CLR}^{$ENDIF});
  end;
end;
...
begin
  ...
  if ARowIndex1 <> -1 then
    FRowIndexes.Add({$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(ARowIndex1));
  if (ARowIndex2 <> -1) and (ARowIndex2 <> ARowIndex1) then
    FRowIndexes.Add({$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(ARowIndex2));
end;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if AValueDef.ValueTypeClass.ClassName <> S then
              InvalidOperation(cxSDataInvalidStreamFormat);
          end;
        end;
        BeginUpdate;
    ...
    begin
      ...
      try
        ...
        begin
          AValueDef := DataStorage.ValueDefs[I];
          if AValueDef.StreamStored then
            AWriter.WriteString(AValueDef.ValueTypeClass.ClassName);
        end;    
        // RecordCount
...
begin
  ...
  try
    ...
    begin
      ...
      ARelationIndexes.Insert(0, {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(AMasterRelation.Index));
...
begin
  ...
  // TODO: check
  if Provider.IsSyncMode and SyncDetailsFocusWithMaster and
