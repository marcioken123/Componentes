...
uses
  ...
  cxDataUtils{$IFDEF CLR}, System.Reflection{$ENDIF};
...
type
  ...
  protected
    ...
    procedure DataEvent(Event: TDataEvent; Info: {$IFDEF CLR}TObject{$ELSE}Longint{$ENDIF}); override;
...
{$IFDEF CLR}
  ...
  private
    class var ActiveRecord_PropertyInfo: PropertyInfo;
    class var BookmarkAvailable_MethodInfo: MethodInfo;
    class var BufferCount_PropertyInfo: PropertyInfo;
    class procedure CheckMethodInfo(const AName: string; var AMethodInfo: MethodInfo);
    class procedure CheckPropertyInfo(const AName: string; var APropertyInfo: PropertyInfo);
  public
    class function _ActiveRecord(Instance: TObject): Integer;
    class function _BookmarkAvailable(Instance: TObject): Boolean;
    class function _BufferCount(Instance: TObject): Integer;
  end;
{$ENDIF}
...
uses
  cxVariants{$IFDEF CLR}, Contnrs{$ENDIF};

{$IFDEF CLR}

class function TDataSetAccess._ActiveRecord(Instance: TObject): Integer;
begin
  CheckPropertyInfo('ActiveRecord', ActiveRecord_PropertyInfo);
  Result := Integer(ActiveRecord_PropertyInfo.GetValue(Instance, []));
end;

class function TDataSetAccess._BookmarkAvailable(Instance: TObject): Boolean;
begin
  CheckMethodInfo('BookmarkAvailable', BookmarkAvailable_MethodInfo);
  Result := Boolean(BookmarkAvailable_MethodInfo.Invoke(Instance, []));
end;

class function TDataSetAccess._BufferCount(Instance: TObject): Integer;
begin
  CheckPropertyInfo('BufferCount', BufferCount_PropertyInfo);
  Result := Integer(BufferCount_PropertyInfo.GetValue(Instance, []));
end;

class procedure TDataSetAccess.CheckMethodInfo(const AName: string; var AMethodInfo: MethodInfo);
var
  AType: System.Type;
...
class procedure TDataSetAccess.CheckPropertyInfo(const AName: string; var APropertyInfo: PropertyInfo);
var
  AType: System.Type;
  AFlags: System.Reflection.BindingFlags;
begin
...
function CanModifyLookupField(AField: TField): Boolean;
var
  AMasterFields: {$IFDEF CLR}TObjectList{$ELSE}TList{$ENDIF};
  I: Integer;
begin
  Result := False;
  if Assigned(AField.DataSet) then
  begin
    AMasterFields := {$IFDEF CLR}TObjectList.Create(False){$ELSE}TList.Create{$ENDIF};
    try
...
procedure TcxCustomFieldDataLink.DataEvent(Event: TDataEvent;
  Info: {$IFDEF CLR}TObject{$ELSE}Longint{$ENDIF});
begin
...
var
  AFieldList: {$IFDEF CLR}TObjectList{$ELSE}TList{$ENDIF};
  I: Integer;
begin
      ...
      begin
        if (*(*)AValueSource = evsText(*) or Assigned(Field.OnSetText)*) then
          ...
          if (AValueSource = evsKey) and (Field.KeyFields <> '') then
            ...
            begin
              AFieldList := {$IFDEF CLR}TObjectList.Create(False){$ELSE}TList.Create{$ENDIF};
              try
