...
type
  ...
  protected
    ...
    {$IFDEF DELPHI7}
    procedure DataEvent(Event: TDataEvent; Info: {$IFNDEF CLR}Integer{$ELSE}TObject{$ENDIF}); override;
    {$ENDIF}
    procedure DataSetChanged; override;
    procedure DataSetScrolled(Distance: Integer); override;
    procedure EditingChanged; override;
    procedure FocusControl({$IFNDEF CLR}Field: TFieldRef{$ELSE}const Field: TField{$ENDIF}); override;
...
uses
  ...
  {$IFDEF CLR}, Contnrs, System.Runtime.InteropServices, System.Reflection{$ENDIF};
...
procedure TcxDBDataLink.DataEvent(Event: TDataEvent;
  Info: {$IFNDEF CLR}Integer{$ELSE}TObject{$ENDIF});
begin
...
procedure TcxDBDataLink.FocusControl({$IFNDEF CLR}Field: TFieldRef{$ELSE}const Field: TField{$ENDIF});
...
var
  ...
  AItemIndex: Integer;
begin
  if Assigned(Field) {$IFNDEF CLR}and Assigned(Field^){$ENDIF} then
  begin
    ADone := False;
    AItemIndex := FindItemIndex(Field{$IFNDEF CLR}^{$ENDIF});
...
{$ELSE}
  Result := TDataSetAccess._ActiveRecord(DataSet);
{$ENDIF}
end;
...
{$ELSE}
  Result := TDataSetAccess._BookmarkAvailable(DataSet);
{$ENDIF}
end;
...
{$ELSE}
  Result := TDataSetAccess._BufferCount(DataSet);
{$ENDIF}
end;
...
function TcxDBDataProvider.BookmarkValid(const ABookmark: TBookmarkStr): Boolean;
{$IFDEF CLR}
var
  P: IntPtr;
{$ENDIF}
...
{$ELSE}
  P := Marshal.StringToHGlobalAnsi(ABookmark);
  try
    Result := Dataset.BookmarkValid(P);
  finally
...
function TcxDBDataProvider.CompareBookmarks(const AItem1, AItem2: TBookmarkStr): Integer;
{$IFDEF CLR}
var
  I1, I2: IntPtr;
{$ENDIF}
begin
  ...
  {$ELSE}
  begin
    I1 := Marshal.StringToHGlobalAnsi(AItem1);
    try
      I2 := Marshal.StringToHGlobalAnsi(AITem2);
      try
        Result := Dataset.CompareBookmarks(I1, I2);
      finally
  ...
  begin
    ...
    try
      ...
      {$IFNDEF CLR}
        New(P);
      {$ENDIF}
        P{$IFNDEF CLR}^{$ENDIF} := DataController.GetInternalRecordId(Integer(AList[I]), AFieldList);
        AKeyList.Add(P);
      end;
      for I := AKeyList.Count - 1 downto 0 do
      begin
        P := PVariant(AKeyList[I]);
        if DataSet.Locate(DataController.KeyFieldNames, P{$IFNDEF CLR}^{$ENDIF}, []) then
      ...
      begin
        ...
        try
          ...
          while (not EOF or ANext or AStartEOF) and
            ((AStartPos = '') or (Self.CompareBookmarks(Bookmark,  AStartPos) <> 0)) do
          begin
            AStartEOF := False;
...
begin
  ...
  begin
    ...
    {$IFNDEF CLR}
      ...
      Result := V;
    {$ELSE}
      Result := VarArrayCreate([0, ADBField.FieldCount - 1], varVariant);
      for I := 0 to ADBField.FieldCount - 1 do
        Result[I] := inherited GetGroupValue(ARecordIndex, ADBField[I]);
    {$ENDIF}
    end;
  end
  else
...
begin
  ...
  begin
    ...
    {$ELSE}
      AList := TObjectList.Create(False);
      try
        Provider.DataSet.GetFieldList(AList, MasterDetailKeyFieldNames);
        FMasterDetailKeyFields.Assign(AList);
      finally
        AList.Free;
      end;
    {$ENDIF}
