...
function VarBetweenArrayCreate(const AValue1, AValue2: Variant): Variant;
begin
  Result := VarArrayCreate([0, 1], {$IFDEF CLR}varObject{$ELSE}varVariant{$ENDIF});
  Result[0] := AValue1;
  Result[1] := AValue2;
end;
...
function VarListArrayCreate(const AValue: Variant): Variant;
begin
  Result := VarArrayCreate([0, 0], {$IFDEF CLR}varObject{$ELSE}varVariant{$ENDIF});
  Result[0] := AValue;
end;
...
begin
  ...
  V := VarArrayCreate([0, C - 1], {$IFDEF CLR}varObject{$ELSE}varVariant{$ENDIF});
...
const
  ValTtoVarT: array[TValueType] of Integer = (varNull, varError,
    ...
    varCurrency, varDate, {$IFDEF CLR}varString{$ELSE}varOleStr{$ENDIF},
...
var
  ...
  begin
    ...
    C := ReadInteger;
    // read values
    Value := VarArrayCreate([0, C - 1], {$IFDEF CLR}varObject{$ELSE}varVariant{$ENDIF});
    for I := 0 to C - 1 do
    begin
...
begin
  ...
  {$IFDEF DELPHI6}
    vaInt64:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VInt64{$ENDIF} := ReadInteger;
  {$ENDIF}
    vaExtended:
