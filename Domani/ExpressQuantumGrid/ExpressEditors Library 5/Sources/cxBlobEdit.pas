...
{$IFDEF DELPHI6}
  Variants,
{$ENDIF}
  Math, cxEditUtils, cxDrawTextUtils, cxVariants;
...
procedure TcxBlobEditViewData.Calculate(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      Result.Left := Left + (Right - Left - imgBlobImages.Width) div 2;
      Result.Top := Top + (Bottom - Top - imgBlobImages.Height) div 2;
    end;
    Result.Right := Result.Left + imgBlobImages.Width;
    Result.Bottom := Result.Top + imgBlobImages.Height;
  end;
...
procedure TcxCustomBlobEdit.CreatePopupControls;
  ...
  procedure GetButton(var AButton: TcxButton; const ACaption: string; AHeight, AWidth: Integer);
  begin
    if AButton = nil then
    begin
  ...
  begin
    ...
    FButtonWidth := Max(Canvas.TextWidth(ACancelCaption + '00'), Canvas.TextWidth(AOkCaption + '00'));
    AButtonHeight := MulDiv(Canvas.TextHeight('Wg'), 20, 13);

    GetButton(FCancelButton, ACancelCaption, AButtonHeight, FButtonWidth);
    FCancelButton.Cancel := True;
    FCancelButton.ModalResult := mrCancel;

    if (not ActiveProperties.ReadOnly) and DataBinding.IsDataAvailable then
    begin
      GetButton(FOkButton, AOkCaption, AButtonHeight, FButtonWidth);
