...
{ TcxMonthListBox - Bug in Delphi5: Must be in the interface part }
  ...
  private
    ...
    function GetYearByIndex(AMonthDelta: Integer): WORD; overload;
    function GetYearByIndex(AMonthDelta: Integer; out AMonth: WORD): WORD; overload;
...
{$ENDIF}
    ...
    function CanResize(var NewWidth, NewHeight: Integer): Boolean;
      {$IFDEF VCL}override;{$ELSE}virtual;{$ENDIF}
...
function IsDateValid(ADate: Double): Boolean;
begin
...
procedure TcxClock.Paint;
  ...
  begin
    if AHourDot then
    begin
      Canvas.Pixels[X, Y] := clTeal;
      InternalPolyLine(Canvas, [Point(X - 1, Y + 1), Point(X - 1, Y - 1),
        Point(X, Y - 1)], clAqua, False);
      InternalPolyLine(Canvas, [Point(X, Y + 1), Point(X + 1, Y + 1),
        Point(X + 1, Y - 1)], clBtnText, False);
      Canvas.ExcludeClipRect(Rect(X - 1, Y - 1, X + 2, Y + 2));
    end;
  end;
  ...
  begin
    DecodeTime(TDateTime(FTime), AHour, AMin, ASec, AMSec);
    ACenter := Point(Width div 2, Height div 2);
    AHandRadiusX := Width / 2 - 2;
    AHandRadiusY := Height / 2 - 2;
    with Canvas.Canvas do
    begin
...
begin
  RX := Width / 2 - 2;
  RY := Height / 2 - 2;
  ACenter.X := Width div 2;
  ACenter.Y := Height div 2;
  for I := 0 to 59 do
  begin
    AAngle := 2 * Pi * I / 60;
...
begin
  ...
  Font := Calendar.Font;
  FontChanged;
  DecodeDate(Calendar.FirstDate, FCurrentYear, FCurrentMonth, ADay);
  if ShowYears then
    TopMonthDelta := -3
  else
    TopMonthDelta := 1 - FCurrentMonth;
...
var
  ATopYear, ABottomYear: Word;
begin
  ATopYear := GetYearByIndex(TopMonthDelta + FSign);
  ABottomYear := GetYearByIndex(TopMonthDelta + FSign + FItemCount - 1);
  if (FSign < 0) and (ATopYear >= MinYear) or (FSign > 0) and (ABottomYear <= MaxYear) then
...
begin
  ...
  begin
    AYear := GetYearByIndex(TopMonthDelta + ItemIndex, AMonth);
...
function TcxMonthListBox.GetYearByIndex(AMonthDelta: Integer): WORD;
var
  AMonth: WORD;
begin
  Result := GetYearByIndex(AMonthDelta, AMonth);
end;

function TcxMonthListBox.GetYearByIndex(AMonthDelta: Integer; out AMonth: WORD): WORD;
begin
  Result := FCurrentYear;
  AMonth := FCurrentMonth;
  ChangeMonth(Result, AMonth, AMonthDelta);
end;
...
begin
  ...
  begin
    AYear := GetYearByIndex(FTopMonthDelta + Value);
    if (AYear < MinYear) or (AYear > MaxYear) then
      Value := -1;
    if FItemIndex <> Value then
    begin
      begin
...
begin
  ...
  CloseUp;
  if ItemIndex <> -1 then
    Calendar.SetFirstDate(EncodeDate(FCurrentYear, ItemIndex + 1, 1));
end;
...
begin
  ...
  AYear := GetYearByIndex(TopMonthDelta, AMonth);
  ...
  begin
    ...
    Canvas.FillRect(R);
    if (AYear >= MinYear) and (AYear <= MaxYear) then
    begin
...
procedure TcxCustomCalendar.CalculateViewInfo;
  ...
  var
    ABorderWidth: Integer;
    AHeaderOffset: TRect;
    AMaxMonthNameWidth, AMonthNameWidth, ASpaceWidth, AYearTextWidth, I: Integer;
  ...
  begin
    ...
    for I := 1 to 12 do
    begin
      AMonthNameWidth := Canvas.TextWidth(LongMonthNames[I]);
      if AMonthNameWidth > AMaxMonthNameWidth then
        AMaxMonthNameWidth := AMonthNameWidth;
    end;
    AYearTextWidth := Canvas.TextWidth('9999');
    ASpaceWidth := FDateRegionWidth - ABorderWidth * 2 - AHeaderOffset.Left -
      AHeaderOffset.Right - AMaxMonthNameWidth - AYearTextWidth - 4 * (FColWidth - 2);
    with FViewInfo do
    begin
      ArrowRects[caPrevMonth] := Rect(AHeaderOffset.Left + ABorderWidth,
        AHeaderOffset.Top + ABorderWidth,
    ...
    AMonthRegionWidth := AMaxMonthNameWidth + ASpaceWidth * AMaxMonthNameWidth div
      (AMaxMonthNameWidth + AYearTextWidth);
...
begin
  ...
  if ArrowsForYear then
  begin
    DrawHeaderText(LongMonthNames[AMonth], FViewInfo.MonthRegion, HotTrackRegion = chrMonth);
    DrawHeaderText(IntToStr(AYear), FViewInfo.YearRegion, HotTrackRegion = chrYear);
  end
  else
    DrawHeaderText(LongMonthNames[AMonth] + ' ' + IntToStr(AYear),
...
var
  ...
  begin
    ...
    SelectDate := cxEncodeDate(AYear, AMonth, ADay);
  end;
...
procedure TcxCustomCalendar.Paint;  
  ...
  begin
    ...
    S := DateTimeToStr(SelectDate + cxSign(SelectDate) * FTimeEdit.Time);
    Canvas.Font := Font;
    Canvas.Font.Color := clBtnText;
    Canvas.Brush.Color := GetHeaderColor;
    ATextSize := Canvas.TextExtent(S);
  ...
  begin
    ...
    for I := 0 to 5 do
    begin
      if not IsDateValid(GetDateFromCell(0, I)) then
        Continue;
      ...
      Canvas.DrawTexT(IntToStr(GetWeekNumber(GetDateFromCell(0, I),
...
{$IFNDEF VCL}
procedure TcxCustomCalendar.ChangeBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
begin
  ...
  FirstDate := cxEncodeDate(AYear, AMonth, 1);
end;
...
var
  R: TRect;
begin
  if (FSelectDate <> Value) and IsDateValid(Value) then
  begin
    FSelectDate := Value;
        ...
        if Kind = ckDate then
          if TimeOf(ADate) <> 0 then
            ...
            begin
              ...
              ADateEdit.FSavedTime := 0;
        ADisplayValue := ADate;
//        ADisplayValue := DateToDisplayValue(ADate(*, True*));
      end;
    except
      AError := True;
    end;
...
//  ActiveProperties.PrepareDisplayValue(ADate, ADisplayValue, True);
  ...
  begin
    ...
    try
      if ADate = NullDate then
        ...
        InternalEditValue := TDateTime(ADate);
...
begin
  ...
  begin
    ...
    ActiveProperties.PrepareDisplayValue(ADate, Result, True);
end;
...
procedure TcxCustomDateEdit.InternalSetEditValue(const Value: TcxEditValue;
  ...
  begin
    if not ActiveProperties.SaveTime or not(VarIsStr(Value) or VarIsNumericEx(Value) or VarIsDate(Value)) then
      ...
      begin
        cxStrToDateTime(Value, not cxFormatController.UseDelphiDateTimeFormats,
          ADate);
        if ADate = NullDate then
          FSavedTime := 0
        else
