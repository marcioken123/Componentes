...
{$ENDIF}
  ...
  public
    function CanResizeVisualArea(var NewSize: TSize;
      AMaxHeight: Integer = 0): Boolean; override;
...
uses
  dxThemeConsts, dxThemeManager, dxUxTheme, cxEditConsts, cxEditPaintUtils,
  cxEditUtils, cxScrollBar;
...
{$IFDEF VCL}
  ...
  begin
    APainter.DrawSizeGrip(ACanvas, ARect);
    Exit;
  end;
...
{$ENDIF}
    ...
    SetWindowShadowRegion(APopupWindow.Handle, R,
      {$IFDEF VCL}NativeStyle, {$ENDIF}Shadow, AExcludeRect);
  end;
end;
    ...
    if CanResize(ANewWidth, ANewHeight) then
      ...
      begin
        ...
        begin
          ...
          Edit.FPopupSizeChangedByMouse := True;
...
function TcxCustomEditPopupWindow.CanResize(var NewWidth, NewHeight: Integer): Boolean;
  ...
  function GetMaxVisualAreaHeight: Integer;
  var
    ADesktopWorkArea, AEditBounds: TRect;
    AMaxHeight: Integer;
  begin
    ADesktopWorkArea := GetDesktopWorkArea(Point(Left, Top));
    if ViewInfo.SizeGripCorner in [ecoBottomLeft, ecoBottomRight] then
      AMaxHeight := ADesktopWorkArea.Bottom - Top
    else
    begin
      ...
      AMaxHeight := AEditBounds.Top - ADesktopWorkArea.Top;
    end;
    with ViewInfo.GetClientExtent do
      Result := AMaxHeight - Top - Bottom;
  end;
  ...
  if (NewHeight <> Height) or (NewWidth <> Width) then
    with Edit do
      ...
      begin
        ...
        if not ILookupData.CanResizeVisualArea(ANewVisualAreaSize, GetMaxVisualAreaHeight) then
        begin
          NewWidth := Self.Width;
...
{$ELSE}
    for I := 1 to ABorderWidth do
    begin
      ScreenCanvas.Canvas.DrawFocusRect(AFrameRect);
      InflateRect(AFrameRect, -1, -1);
    end;
{$ENDIF}
  end;
...
begin
  ...
  Result := Result or (Edit <> nil) and (HasPopupWindow or IsHotAndPopup) and
    ...
    Properties.KeepArrowButtonPressedWhenDroppedDown;
end;
...
begin
  ...
  else if QEvent_type(Event) = QEventType(QEventType_CMDropDownByPaste) then
  begin
    Result := True;
...
begin
  if ActiveProperties.PopupSizeable and FPopupSizeChangedByMouse then
    if IsInplace then
    begin
      if EditData <> nil then
        with TcxCustomDropDownEditData(EditData) do
...
begin
  ...
  FPopupSizeChangedByMouse := False;
end;
...
begin
  ...
  begin
    ...
    if (AListSize.cy = NewSize.cy) and (AListSize.cy <= AMaxHeight) then
      Break;
...
var
  AClientExtent, AEditBounds, R, RTop, RBottom: TRect;
...
begin
  ...
  if IsChildClassWindow(Handle) then
  begin
  ...
  with ActiveProperties do
    ...
    begin
      R := GetDesktopWorkArea(AEditBounds.TopLeft);
      AWidth := R.Right - R.Left - (AClientExtent.Left + AClientExtent.Right);
    end;
  ...
  if (GetCaptureControl = Self) and (AActiveControl is TWinControl) then
    ...
    begin
      ...
      begin
        ...
        SendMessage(Handle, WM_LBUTTONDOWN, 0,
          LPARAM(Word(AMousePos.X) or (Word(AMousePos.Y) shr 16)));
      end;
    end;
end;
...
var
  AMsg: TMsg;
begin
  Result := not FLookupDataTextChangedLocked and (HasPopupWindow or
    not ActiveProperties.CanIncrementalSearch and PeekMessage(AMsg, PopupWindow.Handle, CM_SHOWPOPUPWINDOW, CM_SHOWPOPUPWINDOW, PM_NOREMOVE) or
...
begin
  if ActiveProperties.PopupControl <> nil then
    ...
    begin
      Visible := False;
      PopupWindow.SetFocusedControl(PopupWindow);
...
begin
  ...
  begin
    ...
    begin
      ...
      TCustomForm(APopupControl).BorderStyle := {$IFDEF VCL}bsNone{$ELSE}fbsNone{$ENDIF};
    end;
