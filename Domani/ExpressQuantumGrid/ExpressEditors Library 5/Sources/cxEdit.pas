...
{$ENDIF}
  ...
  Classes, Controls, Forms, Graphics, Menus, StdCtrls, SysUtils,
...
const
  ...
  cxEditStyleValueCount = cxContainerStyleValueCount + 4;
...
{$ENDIF}
  ...
  TcxCustomEdit = class(TcxContainer, IUnknown, IcxEditRepositoryItemListener, IdxSkinSupport)
  private
    FAutoSize: Boolean;
    ...
    function GetActiveStyle: TcxCustomEditStyle;
  ...
  {$ENDIF}
    ...
    procedure HandleValidationError(const ErrorText: string); virtual;
...
{$ENDIF}
    ...
    property ActiveStyle: TcxCustomEditStyle read GetActiveStyle;
...
function IsSpaceChar(C: Char): Boolean;
begin
  Result := (C = ' ') or (C = #0) or (C = #9) or (C = #10) or (C = #12) or
    (C = #13);
end;
...
begin
  ...
  if not DoDrawButtonBorder(ACanvas, AButtonVisibleIndex, ARect, AContentRect) then
    ...
    begin
      ...
      if AButtonViewInfo.Data.State = ebsDisabled then
        ...
        if FEdit <> nil then
          ABrushColor := FEdit.Style.Color
        else
          ABrushColor := clWindow;
    end
    else
    begin
...
var
  ...
  procedure DrawTextButtonContent;
    ...
    const
      AAlingMap: array[TAlignment] of Integer = (cxAlignLeft, cxAlignRight, cxAlignHCenter);
    begin
      ACanvas.Font.Color := AColor;
      ACanvas.DrawText(AButtonViewInfo.Data.Caption, R, AAlingMap[AButtonViewInfo.Data.ContentAlignment] or
...
begin
  ...
  try
    ...
    begin
      ...
      if AButtonViewInfo.Data.Kind = bkText then
...
procedure TcxCustomEditViewInfo.DrawEditButton(ACanvas: TcxCanvas;
  ...
  begin
    Result := (ButtonsInfo[AButtonVisibleIndex].Data.State = ebsPressed) and
      (ecoOffsetButtonContent in Edit.ContentParams.Options);
  end;
...
begin
  ...
  begin
    DrawComplexFrameEx(ACanvas, ARect, {$IFDEF LINUX}clBtnHighlight{$ELSE}cl3DLight{$ENDIF}, {$IFDEF VCL}cl3DDkShadow{$ELSE}clShadow{$ENDIF});
    DrawComplexFrameEx(ACanvas, ARect, {$IFDEF LINUX}clBtnFace{$ELSE}clBtnHighlight{$ENDIF}, clBtnShadow);
  end;
...
procedure TcxCustomEditViewInfo.DrawNativeButtonBackground(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect);
  ...
  procedure DrawButtonThemeBackground(AThemeObject: TdxThemedObjectType; APart: Integer);
  var
    ATheme: TTheme;
  begin
    ATheme := OpenTheme(AThemeObject);
    with AButtonViewInfo do
      DrawThemeBackground(ATheme, ACanvas.Handle, APart, Data.NativeState, ARect);
  end;

begin
  if AButtonViewInfo.Data.ComboBoxStyle then
    DrawButtonThemeBackground(totComboBox, CP_DROPDOWNBUTTON)
  else
    DrawButtonThemeBackground(totButton, BP_PUSHBUTTON);
end;
...
procedure TcxCustomEditViewData.CalculateButtonsViewInfo(ACanvas: TcxCanvas;
  ...
  procedure CheckButtonsOnly(APrevButtonsWidth, AButtonsWidth: Integer);
  var
    ...
    begin
      if AButtonViewInfo.Data.LeftAlignment then
      begin
        for J := 0 to AButtonVisibleIndex - 1 do
      ...
      begin
        for J := 0 to AButtonVisibleIndex - 1 do
          with AViewInfo.ButtonsInfo[J] do
            if not Data.LeftAlignment then
            begin
  ...
  begin
    ...
    begin
      ...
      if not AAllButtonsAreStretchable and not AButtonViewInfo.Stretchable then
        Continue;
  ...
  begin
    ...
    CheckButtonsOnly(APrevButtonsRect.Right - APrevButtonsRect.Left,
...
begin
  ...
  begin
    ...
    if AStyle.TransparentBorder and (Style.LookAndFeel.SkinPainter = nil) then
        ...
        if ANativeStyle then
          ...
          begin
            if Style.LookAndFeel.SkinPainter <> nil then
              ...
              ABorderWidth := GetContainerBorderWidth(TcxContainerBorderStyle(GetBorderStyle));
            Result := cxEmptyRect;
            if bLeft in AStyle.Edges then
              Result.Left := ABorderWidth;
            if bTop in AStyle.Edges then
              Result.Top := ABorderWidth;
            if bRight in AStyle.Edges then
              Result.Right := ABorderWidth;
            if bBottom in AStyle.Edges then
...
procedure TcxCustomEditViewData.CalculateViewInfo(AViewInfo: TcxCustomEditViewInfo; AIsMouseEvent: Boolean);
  ...
  function GetEditNativeState: Integer;
  begin
    if not Enabled then
      Result := ETS_DISABLED
...
begin
  ...
  if AViewInfo.NativeStyle then
    AViewInfo.NativeState := GetEditNativeState
  else
...
begin
  ...
  if (Edit <> nil) and (Edit.DefaultParentColor) then
    ...
    begin
      GetThemeColor(OpenTheme(totEdit), EP_EDITTEXT, AViewInfo.NativeState, TMT_FILLCOLOR, AColor);
      FillColor := AColor;
    end
    else
      FillColor := Style.Color;
...
begin
  Result := [svButtonTransparency, svEdges, svFont, svGradientButtons,
    svHotTrack, svPopupBorderStyle, svShadow, svTransparentBorder];
end;
...
function TcxCustomEdit.InternalGetNotPublishedStyleValues:
  TcxEditStyleValues;
begin
  Result := [svButtonStyle, svButtonTransparency, svGradientButtons,
    svPopupBorderStyle];
end;
...
function TcxCustomEdit.WantKey(Key: Integer; Shift: TShiftState;
  const KeyText: WideString): Boolean;
begin
  Result := CheckButtonShortCuts(Key) or inherited WantKey(Key, Shift, KeyText);
end;
{$ENDIF}
...
procedure TcxCustomEdit.DoFocusChanged;
  ...
  begin
    Result := AWnd;
    while (Result <> 0) and IsChildClassWindow(Result) do
  ...
  begin
    ...
    Result := AParentForm <> nil;
    if Result then
    begin
      with AParentForm do
        Result := not Visible and (fsModal in FormState) and (ModalResult <> mrNone) and (ModalResult <> mrCancel);
      Result := Result or not FFocused and HandleAllocated and (Parent is TCustomForm) and
        (TCustomForm(Parent).ControlCount = 1) and (GetRootParent(Handle) = GetRootParent(GetFocus));
    end;
  end;
...
procedure TcxCustomEdit.HandleValidationError(const ErrorText: string);
var
  AControl: TWinControl;
begin
...
function TcxCustomEdit.GetActiveStyle: TcxCustomEditStyle;
begin
...
begin
  ...
  begin
    ...
    AViewData := TcxCustomEditViewData(CreateViewData);
    try
      ASize := AViewData.GetEditSize(ScreenCanvas, EditValue, AEditSizeProperties);
    finally
      FreeAndNil(AViewData);
    end;

    if CanAutoWidth and not (Align in [alTop, alBottom, alClient{$IFDEF DELPHI6}, alCustom{$ENDIF}]) then
      ANewWidth := ASize.cx;
