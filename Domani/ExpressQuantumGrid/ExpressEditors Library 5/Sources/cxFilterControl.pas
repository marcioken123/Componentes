...
type
  ...
  TcxCustomFilterControl = class(TcxControl, IcxMouseTrackingCaller, IcxFormatControllerListener)
  private
  ...
  protected
    ...
    procedure SetParent({$IFNDEF VCL}const {$ENDIF}AParent: TWinControl); override;
    // overrided cxControl
...
begin
  if (AID >= 0) and (AID < Control.FilterLink.Count) then
    Result := Control.FilterLink.ItemLinks[AID]
  else
    Result := nil;
end;
...
procedure TcxCustomFilterControl.SetEnabled({$IFNDEF VCL}const {$ENDIF}Value: Boolean);
begin
  inherited;
  LayoutChanged;
end;

procedure TcxCustomFilterControl.SetParent({$IFNDEF VCL}const {$ENDIF}AParent: TWinControl);
begin
...
begin
  ...
  // editors pool
  FInplaceEditors := TcxInplaceEditList.Create;
...
begin
  ...
  SetTextColor(Canvas.Canvas.Handle, ColorToRGB(Control.Color));
