...
type
  ...
  private
    ...
    function FindHintControllerOnParents: Boolean;
    function GetAnimationStyle: TcxHintAnimate;
    procedure SetIcon(Value: TIcon);
...
begin
  ...
  ACaption := '';
  if Assigned(FOnShowHintEx) then FOnShowHintEx(Self, ACaption, HintStr, CanShow, HintInfo);
...
begin
  ...
  if (Length(AHint) < 100) and
    Assigned(AnimateWindowProc) then
  begin
    FAnimStyle := GetAnimationStyle;
    if FAnimStyle <> cxhaNone then
    begin
      ...
      if not IsWindowsXPOrLater then
        DisableRegion;
...
begin
  ...
  if FindHintControllerOnParents then
    Exit;

  AHintInitialized := False;
  ...
  begin
    ...
    begin
      ...
      AHintInitialized := True;
      Break;
    end;
  end;
  if not AHintInitialized then
    ResetToStandardHint;
end;

function TcxCustomHintWindow.FindHintControllerOnParents: Boolean;

  function FindHintController(AControl: TWinControl): TcxCustomHintStyleController;
  var
    AController: TcxCustomHintStyleController;
    I: Integer;
  begin
...
begin
  ...
  begin
    AController := FindHintController(AParent);
    if (AController <> nil) or not AParent.HandleAllocated then
      Break;
...
function TcxCustomHintWindow.GetAnimationStyle: TcxHintAnimate;
var
