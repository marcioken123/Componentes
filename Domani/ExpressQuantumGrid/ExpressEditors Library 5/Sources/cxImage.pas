...
{$ENDIF}
  ...
  cxControls, cxDataUtils, cxEdit, cxEditConsts, cxGraphics, cxLookAndFeels;
...
type
  ...
  public
    ...
    function GetGraphicClass(AItem: TObject;
      ARecordIndex: Integer{$IFDEF VCL};
      APastingFromClipboard: Boolean = False{$ENDIF}): TGraphicClass; virtual;
  ...
  private
    FClipboardFormat: {$IFDEF VCL}Word{$ELSE}string{$ENDIF};
...
{$ENDIF}
    ...
    function GetGraphicClass{$IFDEF VCL}(APastingFromClipboard: Boolean = False){$ENDIF}: TGraphicClass; virtual;
  ...
  public
    ...
    property ClipboardFormat: {$IFDEF VCL}Word{$ELSE}string{$ENDIF}
...
begin
  ...
  Width := 140;
  Height := 100;
  FClipboardFormat := {$IFDEF VCL}CF_PICTURE{$ELSE}'image/delphi.picture'{$ENDIF};
...
{$ELSE}
  if AGraphicClass = TBitmap then
    Result := Clipboard.Provides('image/delphi.bitmap')
  else if AGraphicClass = nil then
    Result := Clipboard.Provides('image/delphi.picture')
  else
    Result := Clipboard.Provides(ClipboardFormat);
{$ENDIF}
end;
    ...
    try
      ...
      begin
        if ActiveProperties.CustomFilter <> '' then
          Filter := ActiveProperties.CustomFilter
        else
        begin
          AGraphicClass := ActiveProperties.GraphicClass;
          if AGraphicClass <> nil then
...
begin
  with TcxImageViewInfo(AViewInfo) do
    if VarIsStr(AEditValue) and (AEditValue <> '') then
    begin
