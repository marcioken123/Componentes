...
{$ENDIF}
  ...
  cxEditDataRegisteredRepositoryItems, cxGeometry;
...
type
  ...
  public
    ...
    property Control: TcxEditingControl read GetControl;
    property LookAndFeelPainterClass: TcxCustomLookAndFeelPainterClass read GetPainterClass;
...
begin
  ...
  begin
    ...
    begin
      ...
      if AIsNeedOffsetHint then Inc(P.Y, GetCursorHeightMargin);
...
{$IFNDEF VCL}
  FDragImage.Canvas.Start;
  cxRectOffset(AOrg, -AOrg.Left. -AOrg.Top);
{$ENDIF}
...
begin
  if (Key in EditStartChars) and (Controller.FocusedItem <> nil) then
  begin
...
function TcxCustomViewInfoItem.GetPainterClass: TcxCustomLookAndFeelPainterClass;
begin
...
begin
  GetViewParams(AStyleIndexes[Control.Controller.Focused or Control.Focused], nil, nil, Result);
end;
...
{$ELSE}
  Result := Application.MessageBox(AText, ACaption,
    [smbOK, smbCancel], smsWarning) = smbOK
{$ENDIF}
end;
