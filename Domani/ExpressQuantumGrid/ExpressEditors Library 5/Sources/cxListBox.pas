...
{$ELSE}
function TcxInnerListBox.DrawItem(Index: Integer; Rect: TRect;
  State: TOwnerDrawState): Boolean;
begin
  Result := Container.DrawItem(Canvas, Index, Rect, State);
  if not Result then
...
begin
  Result := Assigned(FOnDrawItem){$IFNDEF VCL} and
    (ListStyle in [lbOwnerDrawFixed, lbOwnerDrawVariable]){$ENDIF};
  if Result then
