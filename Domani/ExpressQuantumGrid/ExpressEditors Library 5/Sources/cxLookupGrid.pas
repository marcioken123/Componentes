...
function TcxLookupGridColumn.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
function TcxLookupGridColumn._AddRef: Integer;
begin
  Result := -1;   // -1 indicates no reference counting is taking Position
end;

function TcxLookupGridColumn._Release: Integer;
begin
  Result := -1;   // -1 indicates no reference counting is taking Position
end;
...
begin
  ...
  FOptions := TcxLookupGridOptions.Create(Self);
  CreateHandlers;
  CreateSubClasses;
  Color := clWindow;
  ParentColor := False;
...
{$IFDEF LINUX}
function TcxCustomLookupGrid.CanFocusOnClick: Boolean;
begin
  Result := False;
end;
{$ENDIF}
