...
uses
  ...
  StdCtrls, SysUtils, cxClasses, cxContainer, cxControls, cxDataUtils, cxEdit,
  cxExtEditConsts, cxExtEditUtils, cxGraphics, cxHeader, cxLookAndFeelPainters,
...
//    procedure AdjustScrollWidth;
  ...
  private
    ...
    property HeaderSectionRect[Index: Integer]: TRect read GetHeaderSectionRect;
  protected
...
begin
  ...
  FVScrollBarVisible := IsVScrollBarVisible;
...
procedure TcxMCInnerPanel.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.WindowClass.style := Params.WindowClass.style and not (CS_HREDRAW or CS_VREDRAW);
end;

procedure TcxMCInnerPanel.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  Message.Result := 1;
end;
...
procedure TcxMCListBox.DrawCellTextEx(var ARect: TRect; AFlags, AItemIndex, AColumnIndex: Integer);
  ...
  const
    AlignmentsHorzArray: array[TAlignment] of Integer = (DT_LEFT, DT_RIGHT, DT_CENTER);
  ...
  begin
    Result := AStartFlag or
      AlignmentsHorzArray[HeaderSections[AColumnIndex].Alignment] or
...
var
  I: Integer;
begin
  Result := Rect(HeaderSectionRect[AColumnIndex].Left + 2, ATop,
    HeaderSectionRect[AColumnIndex].Right - 2, ABottom);
  if AVScrollBarVisible and MultiLines and
    (AColumnIndex = FInnerHeader.Sections.Count - 1) and
    FInnerHeader.Sections[AColumnIndex].AutoSize then
      Result.Right := Result.Right - GetScrollBarSize.cx;
  if OverflowEmptyColumn then
    for I := AColumnIndex + 1 to HeaderSections.Count - 1 do
      if GetTextPart(AItemIndex, I) = '' then
        Result.Right := Result.Right + RectWidth(HeaderSectionRect[I])
      else
        Break;
end;
...
function TcxMCListBox.GetHeaderSectionRect(Index: Integer): TRect;
begin
  Result := FInnerHeader.GetSectionRect(Index);
end;
...
procedure TcxMCListBox.DrawItem(Control: TWinControl; Index: Integer;
  ...
  begin
    ATextRect := GetCellRect(Index, AColumnIndex, ARect.Top, ARect.Bottom,
      InnerListBox.FVScrollBarVisible);
  ...
  begin
    ...
    FInnerListBox.Canvas.MoveTo(HeaderSectionRect[AColumnIndex].Right, ARect.Top);
    FInnerListBox.Canvas.LineTo(HeaderSectionRect[AColumnIndex].Right, ARect.Bottom);
  end;
...
{$ENDIF}
begin
  Height := CalcItemHeight(Index, InnerListBox.FVScrollBarVisible);
end;
