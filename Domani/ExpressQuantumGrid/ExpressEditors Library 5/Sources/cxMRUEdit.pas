...
begin
  if Value = '' then
    Exit;
  I := ActiveProperties.LookupItems.IndexOf(Value);
  if I <> 0 then
    if I = -1 then
    begin
      DoOnNewLookupDisplayText(Value);
      ActiveProperties.LookupItems.Insert(0, Value);
      ActiveProperties.CheckItemCount;
    end
    else
      ActiveProperties.LookupItems.Move(I, 0);
end;
