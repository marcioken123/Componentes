...
type
  ...
  TcxCustomNavigator = class(TcxControl, IUnknown,
    IcxNavigatorOwner, IcxMouseTrackingCaller)
  private
    FButtons: TcxCustomNavigatorButtons;
...
uses
  cxEditPaintUtils, cxEditUtils, cxEditConsts, dxThemeConsts, dxUxTheme;
...
begin
  ...
  UpdateSelected;
  if (AButtonViewInfo <> nil) and PtInRect(AButtonViewInfo.Bounds, Point(X, Y)) then
...
{$ENDIF}
    ...
    Navigator.GetNavigatorLookAndFeel.GetAvailablePainter({$IFDEF VCL}totButton{$ENDIF}).DrawButtonGroupBorder(Canvas,
...
begin
  ...
  ABitmap := TBitmap.Create;
  ABitmap.Width := AButtonBounds.Right - AButtonBounds.Left;
  ABitmap.Height := AButtonBounds.Bottom - AButtonBounds.Top;
  ABitmap.PixelFormat := pf32bit;
  ACanvas := TcxCanvas.Create(ABitmap.Canvas);
  try
    R1 := Rect(0, 0, ABitmap.Width, ABitmap.Height);
    cxDrawTransparentControlBackground(Navigator.GetNavigatorControl, ACanvas, R1);
    APainter.DrawButtonInGroup(ACanvas, R1, AButtonState, FButtons.Count,
