...
type
  ...
  TcxRadioButton = class(TRadioButton, IUnknown,
    IcxMouseTrackingCaller, IcxLookAndFeelContainer)
  private
    FButtonRect: TRect;
    FCanvas: TcxCanvas;
    FColumn: Integer;
...
uses
  cxEditConsts, cxEditPaintUtils, cxEditUtils, cxLookAndFeelPainters, cxVariants,
  dxThemeConsts, dxThemeManager, Math;
...
begin
  ...
  if not Enabled and not Supports(Self, IcxContainerInnerControl) then
  ...
  ACanvas.DrawText(Caption, R, AFlags,
  ...
  begin
    ...
    {$IFDEF MSWINDOWS}InflateRect(R, 1, 1);{$ENDIF}
    Inc(R.Bottom);
    if IsInplace then
      CheckFocusRect(R);
...
function TcxCustomRadioGroupViewInfo.ThemeHandle: TTheme;
begin
...
begin
  ...
  begin
    ...
    if not Checked and not IsInplace and DoEditing then
      Checked := True;
  end;
end;
...
procedure TcxCustomRadioGroupButton.Paint;
var
  ABitmap: TBitmap;
...
begin
  ...
  begin
    APainter := LookAndFeel.SkinPainter;
    ARect :=  ClientRect;
    ...
    try
      cxDrawTransparentControlBackground(Self, ABitmapCanvas, ARect);
      APainter.DrawGroupBoxBackground(ABitmapCanvas, ARect,
        Rect(0, 0, ABitmap.Width, ABitmap.Height));
      ARadioButtonSize := APainter.RadioButtonSize;
      FButtonRect := GetRadioButtonRect(ARadioButtonSize, IsNativeStyle);

      with FButtonRect do
        APainter.DrawRadioButton(ABitmapCanvas, Left, Top, AButtonStateMap[State],
          Checked, Focused, clNone, csDesigning in ComponentState);

      DrawCaption(ABitmapCanvas, IsNativeStyle);    
