...
type
  ...
  public
    constructor Create(AParentIFolder: IShellFolder;
      AParentPIDL, APIDL: PItemIDList; AFast: Boolean); virtual;
    destructor Destroy;override;
  ...
  private
    ...
    property IsFetchQueueEmpty: Boolean read GetIsFetchQueueEmpty;
  ...
  public
    ...
    procedure StopFetch;
    property IsItemUpdating: Boolean read GetIsItemUpdating;
  end;
...
uses
  cxControls, cxContainer, cxEdit, dxUxTheme;
...
//  if (AEnumSettings and SHCONTF_NONFOLDERS = 0) and AFast then
  ...
  begin
    ...
    if Succeeded(AParentIFolder.BindToObject(GetLastPidlItem(AFullPIDL), nil, IID_IShellFolder,
      {$IFDEF LESS_THAN_130}Pointer({$ENDIF}ATempIFolder{$IFDEF LESS_THAN_130}){$ENDIF})) then
...
begin
  try
    ...
    if Result = S_OK then
       Exit;
    Result:=pFolder.GetUIObjectOf(0,Integer(pidl<>nil)(*1*),pidl,IID_IShellDetails,nil,{$IFDEF LESS_THAN_130}Pointer({$ENDIF}sd{$IFDEF LESS_THAN_130}){$ENDIF});
  finally
    if sd = nil then
...
begin
  ...
  try
    if Failed(AIFolder.GetUIObjectOf(AWnd, AItemPIDLList.Count,
      PItemIDList(APIDLList^), IID_IContextMenu, nil,
      {$IFDEF LESS_THAN_130}Pointer(AContextMenu){$ELSE}AContextMenu{$ENDIF})) then
        Exit;
...
function cxShellItemsInfoGathererFetchThreadFunction(
  ...
  begin
    ...
    SetEvent(AItemsInfoGatherer.FIsItemUpdatingEvent);
    try
      AItemProducer.NotifyUpdateItem(AItem);
    finally
      ResetEvent(AItemsInfoGatherer.FIsItemUpdatingEvent);
    end;
  end;
  ...
  var
    AFetchQueue: TList;
  begin
    AFetchQueue := AItemsInfoGatherer.FetchQueue.LockList;
    try
      ...
      AItemsInfoGatherer.FetchQueue.UnlockList;
    end;
  end;
  ...
  try
    repeat
      ...
      begin
        ...
        if not AItemsInfoGatherer.IsFetchQueueEmpty then
...
var
  LockpFolder:THandle;
  pFolder:IShellFolder;
  FSortingItemProducer: TcxCustomItemProducer;
...
var
  ...
  begin
    if FSortingItemProducer.DoCompareItems(TcxShellItemInfo(Item1).Folder,
      TcxShellItemInfo(Item2).Folder, Result) then
        Exit;
    Result := 0;
    if (Item1 = nil) or (Item2 = nil) then
      Exit;
    Result := R[TcxShellItemInfo(Item2).IsFolder] - R[TcxShellItemInfo(Item1).IsFolder];
    if Result = 0 then
      Result := SmallInt(pFolder.CompareIDs(0, TcxShellItemInfo(Item1).pidl, TcxShellItemInfo(Item2).pidl));
  end;
    ...
    try
      ...
      repeat
        ...
        if (currentCelt=0) or (rPidl=nil) then
           Break;
        try
          Item:=TcxShellItemInfo.Create(ShellFolder, FFolderPidl, rPidl, False);
...
begin
  ...
  FIsItemUpdatingEvent := CreateEvent(nil, True, False, nil);
  CreateFetchThread;
end;
...
begin
  DestroyFetchThread;
  InternalCloseHandle(FIsItemUpdatingEvent);
...
procedure TcxShellItemsInfoGatherer.ClearFetchQueue(
  ...
  function CheckFetchQueue: Boolean;
  var
  ...
  begin
    Result := True;
    AList := FetchQueue.LockList;
    try
      I := 0;
      while I < AList.Count do
      begin
        AItem := AList[I];
        if AItem.ItemProducer = AItemProducer then
          if not AItem.Locked then
          begin
            AList.Remove(AItem);
            Dispose(AItem);
            Continue;
...
begin
  ...
  try
    repeat
      AAllItemsDeleted := CheckFetchQueue;
//      Application.ProcessMessages; // TODO
    until(AAllItemsDeleted);
  finally
...
begin
  StopFetch;
  try
    AFetchQueue := FetchQueue.LockList;
    try
      AItemIndex := GetFetchQueueItemIndex(AFetchQueue, AItemProducer, AIndex);
      if AItemIndex = -1 then
      begin
        if APriority then
          AFetchQueue.Insert(0, CreateRequestItem(AItemProducer, AIndex, True))
        else
          AFetchQueue.Add(CreateRequestItem(AItemProducer, AIndex, False));
      end
      else
        if APriority and not PcxRequestItem(AFetchQueue[AItemIndex]).Locked then
          AFetchQueue.Move(AItemIndex, 0);
    finally
      FetchQueue.UnlockList;
    end;
  finally
...
function TcxShellItemsInfoGatherer.GetIsFetchQueueEmpty: Boolean;
begin
  Result := FetchQueue.LockList.Count = 0;
  FetchQueue.UnlockList;
end;
...
function TcxShellItemsInfoGatherer.GetIsItemUpdating: Boolean;
begin
  Result := WaitForSingleObject(FIsItemUpdatingEvent, 0) = WAIT_OBJECT_0;
end;
  ...
  begin
    ...
    try
      GetDesktopIShellFolder.BindToObject(AParentPIDL, nil, IID_IShellFolder,
        {$IFDEF LESS_THAN_130}Pointer({$ENDIF}FParentShellFolder{$IFDEF LESS_THAN_130}){$ENDIF});
    finally
...
begin
  if GetPIDLItemsCount(AbsolutePIDL) = 0 then
    ...
    GetDesktopIShellFolder.BindToObject(AbsolutePIDL, nil, IID_IShellFolder,
      {$IFDEF LESS_THAN_130}Pointer({$ENDIF}Result{$IFDEF LESS_THAN_130}){$ENDIF});
end;
...
constructor TcxShellItemInfo.Create(AParentIFolder: IShellFolder;
  AParentPIDL, APIDL: PItemIDList; AFast: Boolean);
var
  AWithoutAV: Boolean;
begin
...
begin
  // Processing details
  Details.Clear;
  if Succeeded(ShellFolder.QueryInterface({$IFDEF LESS_THAN_D6}cxIID_IShellFolder2{$ELSE}cxIShellFolder2{$ENDIF}, AShellFolder2)) then
  begin
...
begin
  ...
  AColumnIndex := 0;
  Clear;
  if Succeeded(AShellFolder.QueryInterface({$IFDEF LESS_THAN_D6}cxIID_IShellFolder2{$ELSE}cxIShellFolder2{$ENDIF}, SF2)) then
  begin
    ...
    begin
      ...
      begin
        ...
        if not IsWindowsXPOrLater and ADefaultColumns and
