...
type
  ...
  TcxShellCompareEvent = procedure(Sender: TObject;
    AItem1, AItem2: TcxShellFolder; out ACompare: Integer) of object;
...
begin
  if Source is TcxShellListViewOptions then
    FAutoNavigate := TcxShellListViewOptions(Source).FAutoNavigate;
...
begin
  ...
  begin // There are no items selected, so drop target is current opened folder
    ...
    TryReleaseDropTarget;
    New := True;
    if Failed(ItemProducer.ShellFolder.CreateViewObject(Handle,IDropTarget, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempDropTarget{$IFDEF LESS_THAN_130}){$ENDIF})) then
       Exit;
  ...
  begin // Use one of Items as Drop Target
    ...
    TryReleaseDropTarget;
    New := True;
    tempPidl := GetPidlCopy(TcxShellItemInfo(ItemProducer.Items[Item.Index]).pidl);
    try
      if Failed(ItemProducer.ShellFolder.GetUIObjectOf(Handle, 1, tempPidl, IDropTarget, nil, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempDropTarget{$IFDEF LESS_THAN_130}){$ENDIF})) then
         Exit;
    finally
...
procedure TcxCustomInnerShellListView.WMContextMenu(
  ...
  function IsPointInvalid(const P: TPoint): Boolean;
  begin
...
begin
  ...
  if IsPointInvalid(P) then
    ATempPoint := P
  else
  begin
    ATempPoint := ScreenToClient(P);
    if not PtInRect(ClientRect, ATempPoint) then
    begin
      inherited;
      Exit;
    end;
  ...
  AHandled := False;
  DoContextPopup(ATempPoint, AHandled);
...
begin
  ...
  begin // Root object selected
    ...
    if ItemProducer.ShellFolder = nil then
       Exit;
    Res:=ItemProducer.ShellFolder.CreateViewObject(Handle, IDropTarget, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempDropTarget{$IFDEF LESS_THAN_130}){$ENDIF});
    if Failed(Res) then
       Exit;
  end
  else
    ...
    try
      ...
      begin
        if Failed(ItemProducer.ShellFolder.BindToObject(tempPidl, nil, IID_IShellFolder, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempShellFolder{$IFDEF LESS_THAN_130}){$ENDIF})) then
           Exit;
        if Failed(tempShellFolder.CreateViewObject(Handle, IDropTarget, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempDropTarget{$IFDEF LESS_THAN_130}){$ENDIF})) then
           Exit;
      end
      else
      begin
        Res := ItemProducer.ShellFolder.GetUIObjectOf(Handle, 1, tempPidl, IDropTarget, nil, {$IFDEF LESS_THAN_130}Pointer({$ENDIF}tempDropTarget{$IFDEF LESS_THAN_130}){$ENDIF});
        if Failed(Res) then
           Exit;
      end;
    finally
      ...
      repeat
        ...
        try
          ...
          if currentCelt = 0 then
             Break;
          Item := TcxShellItemInfo.Create(ShellFolder, FolderPidl, rPidl, False);
          if Item.Name = '' then
          begin
            Item.Free;
            Continue;
