...
type
  ...
  protected
    ...
    procedure DrawSplitLine(AFullRedraw: Boolean = True; ADelta: Integer = 0;
      AErase: Boolean = True); virtual;
    ...
    procedure InternalMouseUp(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer); virtual;
...
procedure TcxCustomSplitter.DrawSplitLine(AFullRedraw: Boolean = True;
  ADelta: Integer = 0; AErase: Boolean = True);
var
  APoint: TPoint;
  ADrawRect, ARect: TRect;
begin
  if not AFullRedraw and (ADelta = 0) then
    Exit;
  FLineVisible := not FLineVisible;
  APoint := Point(Left, Top);
  if Align in [alLeft, alRight] then
    APoint.X := Left + FSplit + 1
  else
    APoint.Y := Top + FSplit + 1;
  APoint := Parent.ClientToScreen(APoint);
  ADrawRect.TopLeft := APoint;
  ADrawRect.Right := ADrawRect.Left + Width - 1;
  ADrawRect.Bottom := ADrawRect.Top + Height - 1;
  if not AFullRedraw then
  begin
    ARect := ADrawRect;
    if not AErase then
      ADelta := - ADelta;
    if Align in [alLeft, alRight] then
      OffsetRect(ARect, ADelta, 0)
    else
      OffsetRect(ARect, 0, ADelta);
    SubtractRect(ADrawRect, ADrawRect, ARect);
  end;
  with ADrawRect do
...
procedure TcxCustomSplitter.InternalMouseUp(Button: TMouseButton;
...
begin
  if (Button <> mbLeft) or not Assigned(FControl) then
    Exit;
  FLastPatternDrawPosition := -1;
...
var
  ACursor: TCursor;
  ADelta, ALocalNewSize, ALocalSplit: Integer;
...
begin
  ...
  begin
    CalcSplitSize(X, Y, ALocalNewSize, ALocalSplit);
    case FSplitterState of
      sstResizing:
      begin
        if DoCanResize(ALocalNewSize) then
        begin
          ADelta := ALocalSplit - FSplit;
          if not ResizeUpdate then
            DrawSplitLine(False, ADelta);//(False);
