...
{$ENDIF}
  ...
  public
    ...
    function CanResizeVisualArea(var NewSize: TSize;
      AMaxHeight: Integer = 0): Boolean; virtual;
    procedure CloseUp;
    procedure Deinitialize;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ATextRows := nil;
          cxMakeTextRows(ACanvas.Canvas, PcxCaptionChar(AText), R, ATextParams, ATextRows, ARowCount, AEditSizeProperties.MaxLineCount);
...
begin
  if Edit.Container.ActiveProperties.UseLeftAlignmentOnEditing then
    Value := taLeftJustify;
  with Edit do
...
begin
  ...
  try
    AKey := Word(Key);
    if (AKey = VK_TAB){$IFNDEF VCL} or (AKey = Key_Backtab){$ENDIF} then
      Key := #0;
...
begin
  ...
  if FInternalUpdating then
    Exit;
  if not CheckTextEditState(Helper, APrevState) and (Message.CharCode <> 0) then
  begin
...
function TcxCustomTextEditLookupData.InternalLocate(var AText, ATail: string;
  ...
  begin
    case ActiveProperties.EditingStyle of
      esEditList:
        ACorrectCurrentKey := ASynchronizeWithText or (AText = '') and not Find('');
      esFixedList:
...
function TcxCustomTextEditViewInfo.NeedShowHint(ACanvas: TcxCanvas;
  ...
  begin
    Result := DrawTextFlags and not(CXTO_CENTER_VERTICALLY or CXTO_BOTTOM) or
      CXTO_TOP or CXTO_CALCROWCOUNT;
  end;
  ...
  begin
    ...
    AFlags := GetTextFlags;
    R := TextRect;
    if not IsMultiLine(AFlags) then
      R.Right := MaxInt div 2;
    R.Bottom := MaxInt div 2;
    ATextRows := nil;
    Result := not cxMakeTextRows(ACanvas.Canvas,
      PcxCaptionChar(Text),
      R, cxCalcTextParams(ACanvas.Canvas, AFlags), ATextRows, ARowCount, MaxLineCount);
    if not Result then
    begin
      L := R.Left;
      ...
      begin
        ...
        if ARowText = '' then
          ASize := Size(0, ACanvas.TextHeight('Zg'))
        else
          GetTextExtentPoint32(ACanvas.Handle, PAnsiChar(ARowText), Length(ARowText), ASize);
        Inc(ATextBottom, ASize.cy);
        if (ASize.cx > ATextWidth) or (L + ASize.cx > ARealVisibleBounds.Right)
          or (ATextBottom > ARealVisibleBounds.Bottom) then
        begin
          Result := True;
          Break;
        end;
...
begin
  ...
  if Result then
  begin
    AIsMultiLine := IsMultiLine(GetTextFlags);
    AText := Text;
    ATextRect := TextRect;
...
begin
  ...
  try
    ATextHeight := ACanvas.FontHeight(Font);
    case EditProperties.Alignment.Vert of
      taTopJustify:
        Result := TextRect.Top + ATextHeight - 1;
      taBottomJustify:
        Result := TextRect.Bottom - 1;
    else
      Result := TextRect.Top + (TextRect.Bottom - TextRect.Top - ATextHeight) div 2 + ATextHeight - 1;
    end;
  finally
    ACanvas.Free;
  end;
...
begin
  ...
  Result := (ActiveProperties.EditingStyle in [esEdit, esEditList])
    ...
    (ActiveProperties.Alignment.Horz = taLeftJustify));
end;
...
begin
  ADisplayValue := {$IFNDEF VCL}string(PChar(Value)){$ELSE}Value{$ENDIF};
...
begin
  Result := ScreenCanvas.FontHeight(TControlAccess(InnerTextEdit.Control).Font)
end;
...
{$IFNDEF VCL}
function TcxCustomTextEdit.GetText: string;
begin
  Result := InternalGetText;
end;

procedure TcxCustomTextEdit.SetText(const Value: string);
begin
