...
type
  ...
  {$ENDIF}
    ...
    class function GroupBoxTextColor: TColor; virtual;
    // TrackBar
    class procedure DrawTrackBar(ACanvas: TcxCanvas; ARect: TRect; AEnabled: Boolean;
...
begin
  ...
  begin
    ...
    case AArrowDirection of
      adUp:
        begin
          P[cxArrowTopPointIndex] := Point((Left + Right - 1) div 2, MulDiv(Top + Bottom - ASize.cy, 1, 2){$IFDEF VCL}{ - 1}{$ENDIF});
      ...
      adLeft:
        begin
          P[cxArrowTopPointIndex] := Point((Left + Right - ASize.cx) div 2, (Top + Bottom (*{$IFNDEF VCL}- 1{$ENDIF}*)) div 2);
      ...
      adRight:
        begin
          ...
          P[cxArrowTopPointIndex] := Point(P[cxArrowLeftBasePointIndex].X + ASize.cx - 1, (Top + Bottom (*{$IFNDEF VCL}- 1{$ENDIF}*)) div 2);
...
begin
  if AImageList <> nil then
    AImageList.Draw(ACanvas.Canvas, AGlyphRect.Left, AGlyphRect.Top,
          AImageIndex{$IFNDEF VCL}, itImage{$ENDIF}, AEnabled); // ??? itMask TODO
end;
...
class function TcxCustomLookAndFeelPainter.GroupBoxTextColor: TColor;
begin
  Result := clDefault;
end;

class procedure TcxCustomLookAndFeelPainter.DrawTrackBar(ACanvas: TcxCanvas;
  ARect: TRect; AEnabled: Boolean; AHorizontal: Boolean);
begin
end;
