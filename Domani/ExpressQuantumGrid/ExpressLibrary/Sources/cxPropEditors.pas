...
{$IFDEF CLR}
  ...
  TcxCollectionProperty = class(TCollectionProperty)
  public
    procedure Edit; override;
  end;
{$ENDIF}
...
{$IFDEF CLR}
  APropList := GetPropList(AInstance.ClassInfo, [tkMethod]);
...
{$IFDEF CLR}
  APropList := GetPropList(AInstance.ClassInfo, [tkClass]);
  ACount := Length(APropList);
{$ELSE}
...
{$ENDIF}
  for I := 0 to ACount - 1 do
  begin
    AClassType := GetTypeData(APropList[I].PropType{$IFNDEF CLR}^{$ENDIF}).ClassType;
    if not AClassType.InheritsFrom(TComponent) and AClassType.InheritsFrom(TPersistent) and
    {$IFDEF CLR}
      (Length(GetPropList(AClassType.ClassInfo, [tkMethod])) <> 0) then
    {$ELSE}
...
begin
  Designer.GetMethods(GetTypeData(FPropInfo.PropType{$IFNDEF CLR}^{$ENDIF}), Proc);
end;
...
begin
  ...
  begin
    ...
    SetMethodProp(Instance, FPropInfo,
      Designer.CreateMethod(AValue, GetTypeData(FPropInfo.PropType{$IFNDEF CLR}^{$ENDIF})));
    if NewMethod then
    begin
      {$IFDEF DELPHI5}
...
begin
  ...
  ACollection := TCollection(GetObjValue);
  ShowCollectionEditorClass(Designer, GetEditorClass,
    TComponent(APersistent), ACollection, GetName, GetColOptions);
end;
{$ENDIF}
