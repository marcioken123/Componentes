...
type
  ...
  TfrmPropertiesStoreEditor = class(TForm)
    ...
    {$ELSE}
    procedure TreeCustomDrawItem(Sender: TCustomViewControl; Item: TCustomViewItem;
  ...
  public
    ...
    Property Designer: {$IFDEF DELPHI6}IDesigner{$ELSE}IFormDesigner{$ENDIF} read FDesigner write FDesigner;
  end;
...
procedure ShowPropertiesStoreEditor(APropertiesStore: TcxPropertiesStore;
  AOwnerComponent: TComponent; ADesigner: {$IFDEF DELPHI6}IDesigner{$ELSE}IFormDesigner{$ENDIF});
...
procedure TfrmPropertiesStoreEditor.TreeCustomDrawItem(Sender: TCustomViewControl;
    Item: TCustomViewItem; Canvas: TCanvas; const Rect: TRect; State: TCustomDrawState;
    Stage: TCustomDrawStage; var DefaultDraw: Boolean);
begin
  with Canvas do
  begin
    if TTreeNode(Item).Data <> nil then
    begin
      if PfrmPropertiesStoreRecord(TTreeNode(Item).Data)^.Stored then
      begin
        if TTreeNode(Item).Selected and Tree.Focused then
          Font.Color := RGB(255, 255, 255)
        else
          Font.Color := RGB(0, 0, 255);
        Font.Style := [fsBold];
