...
{$IFDEF CLR}
  ...
  AStream.ReadBuffer(S[0], AStream.Size);
  Result := S;
{$ELSE}
...
{$IFNDEF CLR}
  AInt := Integer(ASet);
{$ELSE}
  AInt := IConvertible(Enum(ASet)).ToInt32(nil); // TODO
{$ENDIF}
...
begin
  ...
  {$ELSE}
    ASet := Enum.ToObject(TypeOf(ASet), AInt);
  {$ENDIF}
  end
  else
    raise EPropertyConvertError.Create('');
...
begin
  ...
  {$ENDIF}
    try
    {$IFDEF CLR}
      APropList := GetPropInfos(ATypeInfo);
    {$ELSE}
      GetPropInfos(ATypeInfo, APropList);
    {$ENDIF}
      for I := 0 to ATypeData.PropCount - 1 do
        if APropList[I].PropType{$IFNDEF CLR}^{$ENDIF}.Kind = tkClass then
...
begin
  ...
  {$ENDIF}
    ...
    {$ENDIF}
      for I := 0 to ATypeData.PropCount - 1 do
        if APropList[I].PropType{$IFNDEF CLR}^{$ENDIF}.Kind <> tkMethod then
          AProperties.Add(APropList[I].Name);
    finally
    {$IFNDEF CLR}
      FreeMem(APropList, SizeOf(PPropInfo) * ATypeData.PropCount);
...
begin
  ...
  if ATypeInfo = nil then
    Exit;
  APropInfo := GetPropInfo(ATypeInfo, AName{$IFDEF CLR}, []{$ENDIF});
  if APropInfo <> nil then
    if APropInfo{$IFNDEF CLR}^{$ENDIF}.PropType{$IFNDEF CLR}^{$ENDIF}.Kind = tkClass then
...
begin
  ...
  begin
    ...
    begin
      ...
      if APropInfo <> nil then
      begin
        case APropInfo{$IFNDEF CLR}^{$ENDIF}.PropType{$IFNDEF CLR}^{$ENDIF}.Kind of
...
begin
  ...
  //  raise ERegistryException.CreateFmt(cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxCantOpenRegistryKey), [AStorageName]);
...
begin
  ...
  begin
    if not FRegistry.CreateKey(GenRegistryPath(FStorageName)) then
      raise ERegistryException.CreateFmt(cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxCantCreateRegistryKey), [FStorageName]);
  ...
  begin
    if not FRegistry.OpenKey(GenRegistryPath(FStorageName), False) then
      raise ERegistryException.CreateFmt(cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxCantOpenRegistryKey), [FStorageName]);
