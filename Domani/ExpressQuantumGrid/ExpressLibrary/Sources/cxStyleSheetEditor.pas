...
begin
  ...
  try
    ...
    for I := 0 to ASelected.Count - 1 do
      AListBox.Selected[Integer({$IFDEF CLR}Pointer{$ENDIF}(ASelected[I]))] := True;
  finally
    ASelected.Free;
  end;
end;
...
{$IFDEF CLR}
  APropList := GetPropList(FStyleSheet.GetStyles.ClassInfo, [tkClass]);
  ...
  try
    ...
    for I := 0 to ACount - 1 do
      ...
      begin
        ...
        lbStyles.Items.Add(APropList[I].Name);
      end;
  finally
...
begin
  ...
  DrawText(lbStyles.Canvas.Handle, {$IFNDEF CLR}PChar{$ENDIF}(AText), Length(AText), Rect, AFlags);
...
var
  I: Integer;
  AOpenPictureDialog: {$IFDEF VCL}TOpenPictureDialog{$ELSE}TOpenDialog{$ENDIF};
begin
  AOpenPictureDialog := {$IFDEF VCL}TOpenPictureDialog{$ELSE}TOpenDialog{$ENDIF}.Create(nil);
  try
    ...
    {$ELSE}
      Filter := 'All (*.bmp;*.jpg;*.jpeg;*.gif;*.png;*.tif;*.tiff)|*.bmp;*.jpg;*.jpeg;*.gif;*.png;*.tif;*.tiff' +
        '|Bitmaps (*.bmp)|*.bmp' +
        '|JPEG Images (*.jpg)|*.jpg' +
