...
{$ELSE}
  ...
  BinToHex(AStream.Memory, 0, AText, 0, AStream.Size);
  Result := StringOf(AText);
{$ENDIF}
end;
...
{$ELSE}
  AText := BytesOf(S);
  HexToBin(AText, 0, AStream.Memory, 0, AStream.Size);
{$ENDIF}
end;
...
begin
  if FListeners.IndexOf({$IFNDEF CLR}Pointer{$ENDIF}(AListener)) = -1 then
    FListeners.Add({$IFNDEF CLR}Pointer{$ENDIF}(AListener));
end;
...
begin
  if FListeners <> nil then
    FListeners.Remove({$IFNDEF CLR}Pointer{$ENDIF}(AListener));
end;
...
var
  ...
  var
    I: Integer;
    AIndex: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF};
  begin
    ...
    for I := 0 to AStyleSheet.GetStyles.Count - 1 do
    begin
      AIndex := {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(AStyleSheet.GetStyles.Items[I].Index);
...
procedure TcxStyles.GetViewParams(Index: Integer; AData: TObject; AStyle: TcxStyle;
  out AParams: TcxViewParams);
var
  AAssignedStyle: TcxStyle;
...
{$IFDEF CLR}
  APropList := GetPropList(ADestStyleSheet.GetStyles.ClassInfo, [tkClass]);
  ACount := Length(APropList);
{$ELSE}
  ACount := GetPropList(ADestStyleSheet.GetStyles.ClassInfo, [tkClass], @APropList);
{$ENDIF}
  for I := 0 to ACount - 1 do
    if GetTypeData(APropList[I].PropType{$IFNDEF CLR}^{$ENDIF}).ClassType = TcxStyle then
    begin
...
var
  ...
  begin
  {$IFDEF CLR}
    APropList := GetPropList(AStyleSheet.GetStyles.ClassInfo, [tkClass]);
    ACount := Length(APropList);
  {$ELSE}
...
procedure LoadStyleSheetsFromIniFile(const AIniFileName: string;
  ...
  begin
    ...
    if (APropInfo <> nil) and (GetTypeData(APropInfo.PropType{$IFNDEF CLR}^{$ENDIF}).ClassType = TcxStyle) then
    begin
