...
type
  ...
  protected
    ...
    procedure DrawTabImageAndText(TabVisibleIndex: Integer); virtual;
    ...
    function GetNativeContentOffset: TRect;
...
{$IFDEF VCL}
    ...
    function IsClientVisible: Boolean; virtual;
{$ELSE}
...
{$ENDIF}
  ...
  protected
    ...
    function IsClientVisible: Boolean; override;
    procedure Loaded; override;
...
uses
  Math, cxPCConsts, cxPCPainters, cxPCPaintersFactory, dxThemeManager, dxUxTheme;
...
function TextSize(ATab: TcxTab; const AText: string; AFont: TFont = nil): TSize;
var
  AScreenCanvas: TcxScreenCanvas;
begin
  AScreenCanvas := TcxScreenCanvas.Create;
  try
    if AFont = nil then
...
begin
  ...
  if not IsTooSmallControlSize then SetLongitudinalExtendedTabsRectsBounds;
end;
...
var
  I: Integer;
  dRow: Integer;
  RelativeTopPartRowCount: Integer;
begin
  ConvertRowNumbersToNumbersRelativeToTabPosition;
  RelativeTopPartRowCount := RowCount;

  if FMainTabVisibleIndex <> -1 then
    if ScrollOpposite then
      RelativeTopPartRowCount := FVisibleTabList[FMainTabVisibleIndex].FVisibleRow + 1
    else
  ...
  FTopOrLeftPartRowCount := RelativeTopPartRowCount;
...
begin
  if not FPainter.AllowRotate and Value then
    raise Exception.Create(Format(scxPCAllowRotateError, [FPainter.GetStyleName]));
...
{$ELSE}
procedure TcxCustomTabControl.SetHotTrackColor(const Value: TColor);
begin
  if Value <> FHotTrackColor then
  begin
    FHotTrackColor := Value;
    if FHotTrackNavigatorButton <> [] then
      FPainter.RepaintButton(GetNavigatorButton(FHotTrackNavigatorButton), nbsHotTrack)
    else if FHotTrackTabVisibleIndex <> -1 then
      FPainter.RepaintTab(FHotTrackTabVisibleIndex, tpcNotSpecified);
  end;
end;
{$ENDIF}
...
const
  StandardStyleNameMap: array[TcxPCStandardStyle] of string = (
    'tsTabs', 'tsButtons', 'tsFlatButtons'{$IFNDEF VCL}, 'tsNoTabs'{$ENDIF});
var
  NewPainterClass: TcxPCPainterClass;
begin
...
procedure TcxCustomTabControl.MouseDown(Button: TMouseButton;
  ...
  procedure NavigatorButtonDown(NavigatorButton: TcxPCNavigatorButton);
  var
    OldButtonState: TcxPCNavigatorButtonState;
  begin
    OldButtonState := FNavigatorButtonStates[NavigatorButton];
    if OldButtonState in [nbsPressed, nbsDisabled] then Exit;

    FNavigatorButtonStates[NavigatorButton] := nbsPressed;
    FPressedNavigatorButton := [NavigatorButton];
    FPainter.RepaintButton(NavigatorButton, OldButtonState);

    if NavigatorButton in [nbTopLeft, nbBottomRight] then
    begin
      ArrowButtonClick(NavigatorButton);
      if FPressedNavigatorButton = [NavigatorButton] then
        CreateTimer;
      Exit;
    end;
  end;

  procedure TabDown(TabVisibleIndex: Integer);
  begin
    with FVisibleTabList[TabVisibleIndex] do
    begin
      ...
      begin
        FPressedTabVisibleIndex := TabVisibleIndex;
...
{$IFNDEF VCL}
  Shift := Shift + MouseButtonToShift(Button);
  FShift := Shift;
{$ENDIF}
  ...
  if FPainter.IsOverButton(X, Y, NavigatorButton) then
    NavigatorButtonDown(NavigatorButton)
  else
  begin
    PressedTabVisibleIndex := IndexOfTabAt(X, Y);
    if PressedTabVisibleIndex <> -1 then TabDown(PressedTabVisibleIndex);
  end;
end;
...
begin
  ...
  begin
    ...
    ADistanceBetweenTabs := DistanceGetter(FPainter.GetTabsNormalDistance, not Rotate);
    if IsTooSmallControlSize then FFirstVisibleTab := TabVisibleIndex
    else
    begin
...
begin
  ...
  case Key of
    ...
    VK_RETURN{$IFNDEF VCL}, Key_Enter{$ENDIF}, VK_SPACE:
      if FTracking <> -1 then
      begin
...
begin
  raise Exception.Create('TcxCustomTabControl.' + SourceMethodName + ': ' + Msg);
end;
...
begin
  if (Style <> cxPCDefaultStyle) and (LookAndFeel.SkinPainter = nil) then
    Exit;
...
function TcxCustomTabControl.IsClientVisible: Boolean;
begin
  Result := True;
end;
{$ELSE}
procedure TcxCustomTabControl.CreateWidget;
begin
  inherited CreateWidget;
end;

function TcxCustomTabControl.DesignEventQuery(Sender: QObjectH;
  ...
  case QEvent_type(Event) of
    ...
    QEventType_MouseButtonDblClick:
      if Sender = Handle then
        Result := PassDesignMouseEvent(QMouseEvent_x(QMouseEventH(Event)), QMouseEvent_y(QMouseEventH(Event)));
  end;
end;

procedure TcxCustomTabControl.EnabledChanged;
begin
  inherited EnabledChanged;
  RequestLayout;
end;

function TcxCustomTabControl.WantKey(Key: Integer; Shift: TShiftState;
...
begin
  ...
  if ssAlt in Shift then
    Result := HandleDialogChar(Key);
  if not Result then
  begin
    InternalKey := Key;
    Result := InternalKeyDown(InternalKey, Shift);
  end;
  if not Result then
...
begin
  raise Exception.Create('TcxPCImageList.' + SourceMethodName + ': ' + Msg);
end;
...
begin
  ...
  begin
    ...
    BaseImages.Draw(Canvas, 0, 0, Index{$IFNDEF VCL}, itImage{$ENDIF}, Enabled);
...
begin
  if FBaseImages = Value then exit;
  if FBaseImages <> nil then
  begin
    FBaseImages.UnRegisterChanges(FBaseImageChangeLink);
    FFreeNotificator.RemoveSender(FBaseImages);
  end;
  FBaseImages := Value;
  if FBaseImages <> nil then
  begin
    FBaseImages.RegisterChanges(FBaseImageChangeLink);
    FFreeNotificator.AddSender(FBaseImages);
  end;
  BaseImageListChange(FBaseImages);
end;
...
function TcxPageControl.FindNextPageEx(ACurrentPage: TcxTabSheet;
  ...
  function IncrementIndex(var I: Integer): Boolean;
  var
    APrevI: Integer;
  begin
    if not ACircular and
      (not AGoForward and (I = 0) or AGoForward and (I = PageCount - 1)) then
    begin
      Result := False;
      Exit;
    end;
    ...
    Result := (I <> APrevI) and (I <> FPages.IndexOf(ACurrentPage));
  end;
...
begin
  ...
  if I = -1 then
    ...
    if not IncrementIndex(I) then
      Exit;
  repeat
    if not ACheckTabAccessibility or Pages[I].TabVisible and Pages[I].Enabled then
    begin
      Result := Pages[I];
      Break;
    end;
  until not IncrementIndex(I);
end;
...
function TcxPageControl.IsClientVisible: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  FSavedClipRgns := TList.Create;
end;

destructor TcxPCCustomPainter.Destroy;
begin
  DestroySavedClipRgns;
  FreeAndNil(FSavedClipRgns);
...
procedure TcxPCCustomPainter.DrawTabImageAndText(TabVisibleIndex: Integer);
var
  Tab: TcxTab;
  ...
  procedure InitializeBitmap(ABitmap: TBitmap; S: TCaption; R: TRect;
    ...
    procedure InternalDrawText(ATextRect: TRect);
    var
      APartId, AStateId: Integer;
    begin
      if IsNativePainting and AEnabled then
      begin
        GetTabNativePartAndState(TabVisibleIndex, APartId, AStateId);
        DrawThemeText(OpenTheme(totTab), ABitmap.Canvas.Handle, APartId,
          AStateId, S, -1, DT_SINGLELINE or DT_END_ELLIPSIS, 0, ATextRect);
      end
      else
        DrawText(ABitmap.Canvas.Handle, PChar(S), -1,
          ATextRect, DT_SINGLELINE or DT_END_ELLIPSIS);
    end;
  ...
  var
    ABitmap: TBitmap;
  begin
    ABitmap := GetTabVerticalTextBitmap(TabVisibleIndex);
    if ABitmap = nil then
    begin
      ABitmap := CreateNewTabVerticalTextBitmap(TabVisibleIndex);
    ...
    if IsNativePainting then
      Tab.PaintBitmap.Canvas.Draw(R.Left, R.Top, ABitmap)
    else
  ...
  begin
    if IsNativePainting then
      InitializeBitmap(Tab.PaintBitmap, ACaption, R, AEnabled, ABackgroundColor)
    else
    begin
...
begin
  FOutTabImageAndTextData.TabVisibleIndex := TabVisibleIndex;
  Tab := ParentInfo.VisibleTabs[TabVisibleIndex];
  PrepareTabCanvasFont(Tab, ParentInfo.Canvas);
  StandardPainterPrepareOutTabImageAndTextData(TabVisibleIndex);
  ABackgroundColor := GetTabBodyColor(TabVisibleIndex);

  with FOutTabImageAndTextData do
  begin
    IsTabEnabled := Tab.RealEnabled;
    TabImageIndex := ParentControl.GetImageIndex(Tab.Index);
    if TabImageIndex <> -1 then
      if IsNativePainting then
        ParentControl.FImages.Draw(Tab.PaintBitmap.Canvas, TabImageRect.Left,
    ...
    begin
      if InternalIsVerticalText(ParentControl) then
        OutVerticalText(Tab.Caption, TabTextRect, IsTabEnabled, ABackgroundColor)
      else
        OutHorizontalText(Tab.Caption, TabTextRect, IsTabEnabled, ABackgroundColor);
    end;
  end;
...
begin
  if ParentInfo.MainTabVisibleIndex <> -1 then
    Result := ParentInfo.PageColors[ParentInfo.MainTabVisibleIndex]
  else
...
procedure TcxPCCustomPainter.DestroySavedClipRgns;
var
  I: Integer;
begin
  for I := 0 to FSavedClipRgns.Count - 1 do
    DeleteObject(HRGN(FSavedClipRgns[I]));
end;
...
begin
  ...
  begin
    if IntersectWithCurrentClipRegion then
    begin
      CurrentClipRegion := QPainter_clipRegion(Handle);
      if not QRegion_contains(CurrentClipRegion, PRect(@ClipR)) then Exit;
    end;
    SaveClipRgn;
    if IntersectWithCurrentClipRegion then
      IntersectClipRect(ClipR)
    else
      QPainter_setClipRect(Handle, @ClipR);
  end;
  Result := True;
end;
{$ENDIF}
...
var
  ARgn: HRGN;
begin
  if FSavedClipRgns.Count = 0 then
    Exit;
  ARgn := HRGN(FSavedClipRgns.Last);
  SelectClipRgn(ParentInfo.Canvas.Handle, ARgn);
  if ARgn <> 0 then
    DeleteObject(ARgn);
  FSavedClipRgns.Delete(FSavedClipRgns.Count - 1);
end;
...
begin
  ...
  if GetClipRgn(ParentInfo.Canvas.Handle, ARgn) = 1 then
    FSavedClipRgns.Add(Pointer(ARgn))
  else
  begin
    DeleteObject(ARgn);
    FSavedClipRgns.Add(nil);
  end;
end;
...
begin
  raise Exception.Create('TcxVisibleTabList.' + SourceMethodName + ': ' + Msg);
end;
...
const
  APaintingPositionIndexMap: array [TcxTabPosition, TcxRotationAngle] of Integer = (
    (1, 2, 3{$IFDEF VCL}, 0{$ENDIF}),
    (10, 11, 12{$IFDEF VCL}, 0{$ENDIF}),
    (6, 4, 5{$IFDEF VCL}, 0{$ENDIF}),
    (9, 7, 8{$IFDEF VCL}, 0{$ENDIF})
  );
begin
...
begin
  ...
  if (TabVisibleIndex < 0) or (TabVisibleIndex >= VisibleTabCount) then
    raise Exception.Create('TcxTabs.GetVisibleTab: ' + Format(scxPCTabVisibleIndexOutsOfBounds,
...
begin
  raise Exception.Create('TcxTabs.' + SourceMethodName + ': ' + Format(scxPCTabIndexError, [Index]));
end;
...
{$ELSE}
procedure TcxTabSheet.AdjustClientRect(var Rect: TRect);
begin
...
procedure TcxTabSheet.InitWidget;
begin
...
{$IFDEF VCL}
procedure TcxTabSheet.DrawNativeBackground(DC: HDC);
var
  ATheme: TTheme;
  R: TRect;
begin
  R := PageControl.Painter.GetDisplayRect;
...
begin
  if PageControl <> nil then
    if not PageControl.Painter.IsNativePainting then
    begin
      OffsetWindowOrgEx(Message.DC, Left, Top, @P);
      try
        PageControl.Canvas.Canvas.Lock;
        try
          PageControl.Canvas.Canvas.Handle := Message.DC;
          try
            PageControl.Painter.FillDisplayRect;
          finally
        ...
        SetWindowOrgEx(Message.DC, P.X, P.Y, nil);
      end;
    end
    else
    begin
      DrawNativeBackground(Message.DC);
      Message.Result := 1;
    end;
end;
...
{$IFNDEF VCL}
procedure TcxTabSheet.SetBorderWidth(const Value: TBorderWidth);
begin
