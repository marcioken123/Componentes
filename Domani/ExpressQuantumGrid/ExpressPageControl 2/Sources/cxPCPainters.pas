...
type
  ...
  protected
    ...
    procedure PaintTabShape(TabVisibleIndex: Integer); virtual; abstract;
    procedure PrepareTabBackground(ATabVisibleIndex: Integer); virtual;
  ...
  protected
    ...
    procedure PaintFrame; override;
    procedure PaintTabShape(TabVisibleIndex: Integer); override;
    procedure PaintTabsRegion; override;
...
procedure TcxPCTabsPainter.PaintTabShape(TabVisibleIndex: Integer);
  ...
  begin
    ...
    InflateRect(ATabRect, -StandardPainterTabBorderWidth, -StandardPainterTabBorderWidth);
    InternalDrawFocusRect(TabVisibleIndex, ATabRect);
...
//  if MainTab or ParentInfo.IsTabsContainer then
  ...
  ParentInfo.Canvas.Brush.Color := GetTabBodyColor(TabVisibleIndex);
...
begin
  ...
  begin
    ...
    Tab.InitializePaintBitmap;
    ABitmap := Tab.PaintBitmap;
    ...
    if (ParentInfo.TabPosition = tpTop) and (not ParentInfo.MultiLine or
      not ParentInfo.ScrollOpposite) then
    begin
    ...
    begin
      ABitmap.Canvas.Brush.Color :=
        TWinControlAccess(ParentControl.Parent).Color;
...
begin
  ...
  if (ATab.PaintingPosition in [tpLeft, tpRight]) and
    (GetTextRotationAngle(ParentControl) <> raMinus90) then
      ExchangeValues(AColor1, AColor2);
...
procedure TcxPCFlatButtonsPainter.PaintTabShape(TabVisibleIndex: Integer);
var
  FullRect: TRect;
  Tab: TcxTab;
  TabBodyColor: TColor;
...
procedure TcxPCStandardPainter.PrepareTabBackground(ATabVisibleIndex: Integer);
begin
end;
...
begin
  ...
  try
    ...
    begin
      if IsNativePainting then
        PrepareTabBackground(TabVisibleIndex);
      DrawTabImageAndText(TabVisibleIndex);
      PaintTabShape(TabVisibleIndex);
    end;
    RestoreClipRgn;
...
procedure TcxPCButtonsPainter.PaintTabShape(TabVisibleIndex: Integer);
const
...
begin
  ATab := ParentInfo.VisibleTabs[TabVisibleIndex];
  ATabBodyColor := GetTabBodyColor(TabVisibleIndex);
...
constructor TcxPCButtonedPainter.Create(AParent: TcxCustomTabControl);
  ...
  begin
    ...
    begin
      ...
      {$IFNDEF VCL}Pen.Color := cxPCLightestEdgeColor;{$ENDIF}
      XStart := 0;
      X := XStart;
      Y := 0;
      repeat
...
function TcxPCExtraFlatPainter.GetClientColor: TColor;
begin
...
const
  ...
  procedure CalculateNormalParameters;
  var
    ButtonsWidth: Integer;
    ANormalTabsRectCorrection: TRect;
  begin
    ButtonsWidth := CalculateButtonsRegionWidth(NavigatorButtons);
    with ParentInfo, Result do
    begin
      ...
      NormalTabsRect := GetExtendedRect(GetControlRect(ParentControl),
        ANormalTabsRectCorrection, TabPosition);

      if TabPosition in [tpTop, tpBottom] then
      begin
        NormalRowWidth := Width - 2 * TabsRegionWOffset - ButtonsWidth;
        if NavigatorPosition in [npLeftTop, npLeftBottom] then
          Inc(NormalTabsRect.Left, ButtonsWidth)
        else
          Dec(NormalTabsRect.Right, ButtonsWidth);
      end else
      begin
        NormalRowWidth := Height - 2 * TabsRegionWOffset - ButtonsWidth;
        if NavigatorPosition in [npLeftTop, npRightTop] then
          Inc(NormalTabsRect.Top, ButtonsWidth)
        else
          Dec(NormalTabsRect.Bottom, ButtonsWidth);
      end;
    end;
  end;

begin
  CalculateNormalParameters;

  with ParentInfo, Result do
  begin
    ExtendedTabsRect := GetExtendedRect(NormalTabsRect, ExtendedTabsRectCorrection, TabPosition);
...
procedure TcxPCExtraFlatPainter.PaintTabShape(TabVisibleIndex: Integer);
var
  R: TRect;
  Tab: TcxTab;
begin
  Tab := ParentInfo.VisibleTabs[TabVisibleIndex];
  R := Tab.FullRect;
  if Tab.IsMainTab then
    PaintMainTabFrame(R);

  if not IsTabTransparent(TabVisibleIndex) then
    DrawBackground(ParentInfo.Canvas.Canvas, R, TabVisibleIndex);
  if Tab.IsMainTab then
    InternalDrawFocusRect(TabVisibleIndex, GetFocusRect);
end;
...
class function TcxPCOffice11Painter.HasLookAndFeel(
  ALookAndFeel: TcxLookAndFeel): Boolean;
begin
  Result := ALookAndFeel.GetAvailablePainter([totTab]).LookAndFeelStyle = lfsOffice11;
end;
...
procedure TcxPCSlantedPainter.PaintTabShape(TabVisibleIndex: Integer);
...
begin
  ...
  try
    ATab := ParentInfo.VisibleTabs[TabVisibleIndex];
    R := ATab.FullRect;
    PaintTabFrame(TabVisibleIndex, R);
    ParentInfo.Canvas.SetClipRegion(InternalGetTabClipRegion(TabVisibleIndex, True), roIntersect);
    PaintTabStateMark(TabVisibleIndex);
    DrawBackground(ParentInfo.Canvas.Canvas, R, TabVisibleIndex, ATab.PaintingPosition in [tpLeft, tpRight], False);
  finally
    RestoreClipRgn;
  end;
  if ParentInfo.VisibleTabs[TabVisibleIndex].IsMainTab then
    DrawTabUnderline(TabVisibleIndex);
  DrawFocusRect;
end;
