...
type
  ...
  published
    ...
    property Tag: TTag read FTag write SetTag stored IsTagStored;
  ...
  TcxGridBands = class(TOwnedCollection, {$IFNDEF DELPHI6}IUnknown,{$ENDIF}
...
uses
  SysUtils, Math, cxGridStrs, cxCustomData, cxCheckListBox;
  ...
  if CanScroll then
    ...
    begin
      ...
      begin
        ...
        Result := not HasPoint(R.Left, R.Top) or not HasPoint(R.Right - 1, R.Bottom - 1);
      end
      else
...
begin
  if AColumn.Position.Row <> nil then
    AColumn.Position.Row.RefreshVisibleItemsList;
end;
...
procedure TcxGridBand.SetTag(Value: TTag);
begin
  if FTag <> Value then
  begin
    FTag := Value;
...
function TcxGridBand.QueryInterface(const IID: TGUID; out Obj): HResult;
const
...
function TcxGridBand._AddRef: Integer;
begin
  Result := -1;
end;

function TcxGridBand._Release: Integer;
begin
  Result := -1;
end;
...
function TcxGridBands.QueryInterface(const IID: TGUID; out Obj): HResult;
const
  E_NOINTERFACE = HResult($80004002);
begin
  if GetInterface(IID, Obj) then
...
function TcxGridBands._AddRef: Integer;
begin
  Result := -1;
end;

function TcxGridBands._Release: Integer;
begin
  Result := -1;
end;
...
begin
  Result := not GridView.Bands.AreNested and
...
begin
  ...
  if AFixedKind1 = AFixedKind2 then
    ...
    if AFixedKind1 = fkNone then
      Result := 1
    else
      if AFixedKind2 = fkNone then
        Result := -1
      else
