...
//    property Owner: IcxGridFilterPopupOwner read GetOwnerValue write SetOwnerValue;
  ...
  public
    ...
    function FindItemByTag(ATag: TTag): TcxCustomGridTableItem;
...
begin
  ...
  begin
    Add(fviSpecial, foYesterday, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridYesterday), True);
    Add(fviSpecial, foToday, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridToday), True);
    Add(fviSpecial, foTomorrow, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridTomorrow), True);
  end;
  if dtfRelativeWeek in AFilterItems then
    Add(fviSpecial, foLastWeek, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridLastWeek), True);
  if dtfRelativeMonth in AFilterItems then
    Add(fviSpecial, foLastMonth, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridLastMonth), True);
  if dtfRelativeYear in AFilterItems then
    Add(fviSpecial, foLastYear, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridLastYear), True);
  if dtfRelativeWeek in AFilterItems then
    Add(fviSpecial, foThisWeek, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridThisWeek), True);
  if dtfRelativeMonth in AFilterItems then
    Add(fviSpecial, foThisMonth, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridThisMonth), True);
  if dtfRelativeYear in AFilterItems then
    Add(fviSpecial, foThisYear, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridThisYear), True);
  if dtfRelativeWeek in AFilterItems then
    Add(fviSpecial, foNextWeek, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridNextWeek), True);
  if dtfRelativeMonth in AFilterItems then
    Add(fviSpecial, foNextMonth, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridNextMonth), True);
  if dtfRelativeYear in AFilterItems then
    Add(fviSpecial, foNextYear, cxGetResourceString({$IFNDEF CLR}@{$ENDIF}scxGridNextYear), True);
            *)
...
function TcxCustomGridTableItem.GetTag: TTag;
begin
...
procedure TcxCustomGridTableItem.SetTag(Value: TTag);
begin
  if Tag <> Value then
  begin
    inherited Tag := Value;
...
function TcxCustomGridTableView.FindItemByTag(ATag: TTag): TcxCustomGridTableItem;
var
  I: Integer;
begin
  for I := 0 to ItemCount - 1 do
