...
uses
  SysUtils, Math, ImgList, cxGeometry, cxScrollBar, cxVariants, cxGrid, cxGridLevel, cxGridStrs;
...
begin
  ...
  Result := (AForm = nil) or AForm.Active or Application.Active and
...
procedure TcxCustomGridController.ShowHint(const AHintAreaBounds, ATextRect: TRect;
  ...
  function GetHintWindowRect: TRect;
    ...
    const
      TextOffset: TPoint = ({$IFDEF VCL}X: 2; Y: 2{$ELSE}X: 3; Y: 0{$ENDIF});
    begin
...
begin
  ...
  begin
    ...
    if {$IFDEF VCL}IsWindowVisible(FHintWindow.Handle){$ELSE}QWidget_isVisible(FHintWindow.Handle){$ENDIF} then
    begin
...
begin
  if Canvas.Canvas is TControlCanvas then
...
begin
  Result := not IsDesigning and GetIsControlFocused;
end;
