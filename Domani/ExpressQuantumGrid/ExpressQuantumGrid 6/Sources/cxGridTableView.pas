...
{$ENDIF}
  ...
  protected
    ...
    function SupportsBeginsWithFilterOperator: Boolean;
...
begin
  ...
  try
    ...
    if AValueIndex = -1 then
      Result := GridView.Columns[AIndex].GetProperties.GetDisplayText(AValue)
    else
...
begin
  ...
  begin
    ...
    if (GridView.FilterRow.ApplyChanges = fracImmediately) and
      SupportsBeginsWithFilterOperator then
...
function TcxGridColumn.SupportsBeginsWithFilterOperator: Boolean;
var
...
begin
  ...
  Result := (AFilterHelper <> nil) and
    (fcoLike in AFilterHelper.GetSupportedFilterOperators(GetProperties, DataBinding.ValueTypeClass));
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        APrevColumn.VisibleForCustomizationChanged;
      end;
      if FColumn <> nil then
      begin
        GridView.ItemVisibilityChanged(FColumn, False);
        FColumn.VisibleForCustomizationChanged;
      end;
    finally
      GridView.EndUpdate;
    end;
