...
uses
{$IFDEF CLR}
  System.IO, System.Text, Types, WinUtils,
{$ENDIF}
...
const
  ...
  SMetallic = 'METALLIC';
var
  PThemeFileName, PThemeColor, PThemeSize: {$IFNDEF CLR}PWideChar{$ELSE}StringBuilder{$ENDIF};
  S: string;
begin
  Result := schUnknown;
  ...
  {$ELSE}
    PThemeFileName := StringBuilder.Create(SZ_MAX_CHARS);
    PThemeColor := StringBuilder.Create(SZ_MAX_CHARS);
    PThemeSize := StringBuilder.Create(SZ_MAX_CHARS);
  {$ENDIF}
    try
      if FAILED(GetCurrentThemeName(PThemeFileName, SZ_MAX_CHARS, PThemeColor,
        SZ_MAX_CHARS, PThemeSize, SZ_MAX_CHARS)) then
        Exit;
      S := UpperCase(ExtractFileName(PThemeFileName{$IFDEF CLR}.ToString{$ENDIF}));
      // WindowsXP or above
      if (S = SStandardThemeFileName) or (Win32MajorVersion > 5) then
      begin
        S := UpperCase(PThemeColor{$IFDEF CLR}.ToString{$ENDIF});
