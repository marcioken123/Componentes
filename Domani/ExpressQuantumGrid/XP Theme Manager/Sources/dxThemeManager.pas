...
function OpenTheme(AThemedObjectType: TdxThemedObjectType;
  APClassNameList: {$IFNDEF CLR}PWideChar = nil{$ELSE}string = ''{$ENDIF}): TTheme;

implementation

uses
{$IFDEF CLR}
  System.IO, System.Text, WinUtils,
{$ENDIF}
...
var
  AThemeFileName: {$IFNDEF CLR}PWideChar{$ELSE}StringBuilder{$ENDIF};
  I: Integer;
  S: string;
begin
  Result := False;
  ...
  {$ELSE}
    AThemeFileName := StringBuilder.Create(SZ_MAX_CHARS);
  {$ENDIF}
    try
      if GetCurrentThemeName(AThemeFileName, SZ_MAX_CHARS, nil, 0, nil, 0) = S_OK then
      begin
        S := UpperCase(ExtractFileName(AThemeFileName{$IFDEF CLR}.ToString{$ENDIF}));
    ...
    if Theme <> 0 then
      ...
      if APClassNameList = nil then
        if Length(ClassNameList) = 0 then
          APClassNameList := {$IFNDEF CLR}PWideChar{$ENDIF}(dxThemedObjectNameA[AThemedObjectType])
        else
          APClassNameList := {$IFNDEF CLR}PWideChar{$ENDIF}(ClassNameList);
...
{$IFDEF DELPHI6}
  {$IFNDEF CLR}Classes.{$ENDIF}DeallocateHWnd(FWindowHandle);
{$ELSE}
