...
uses
   classes, SysUtils, gbCobranca, dialogs
   {$IFDEF VER150}
...
begin
   ...
   if ANossoNumero = '00000000000' then
      Result := '0'
   else
...
//      Remessa.Add('01REMESSA01'+Formatar('COBRANCA',15)+'000'+Formatar(Titulos[NumeroRegistro].Carteira,4,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,5,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.NumeroConta,7,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoConta,1)+Formatar(Titulos[NumeroRegistro].Cedente.Nome,30,true,' ')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Codigo,3,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Nome,15,true,' ')+FormatDateTime('ddmmyy',DataArquivo)+Formatar('',8)+'MX'+Formatar(IntToStr(NumeroArquivo),7,false,'0')+Formatar('',277)+'000001');
      Remessa.Add('01REMESSA01'+Formatar('COBRANCA',15)+Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,20,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.Nome,30,true,' ')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Codigo,3,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Nome,15,true,' ')+FormatDateTime('ddmmyy',DataArquivo)+Formatar('',8)+'MX'+Formatar(IntToStr(NumeroArquivo),7,false,'0')+Formatar('',277)+'000001');
      ...
      begin
         ...
         if Titulos[NumeroRegistro].EmissaoBoleto = ebBancoEmite then
            Registro := Registro + '1 '
         else
         ...
         Registro := Registro + Formatar('',3,false,'0');
...
//         if Titulos[NumeroRegistro].EspecieDocumento = edDuplicataMercantil then
         Registro := Registro + '01';
//         else
//         if Titulos[NumeroRegistro].EspecieDocumento = edRecibo then
//            Registro := Registro + '05';

         Registro := Registro + 'N';
         Registro := Registro + FormatDateTime('ddmmyy',Titulos[NumeroRegistro].DataDocumento);
         Registro := Registro + Formatar(' ',4,false,'0');
...
//               NumeroDocumento := Copy(Retorno.Strings[NumeroRegistro],38,25);
               ...
               try
                  DataProcessamento := StrToDateTime(Copy(Retorno.Strings[NumeroRegistro],147,2) + '/' + Copy(Retorno.Strings[NumeroRegistro],149,2) + '/' + Copy(Retorno.Strings[NumeroRegistro],151,2));
               except
                  DataProcessamento := 0;
               end;

               try
                  DataVencimento := StrToDateTime(Copy(Retorno.Strings[NumeroRegistro],147,2) + '/' + Copy(Retorno.Strings[NumeroRegistro],149,2) + '/' + Copy(Retorno.Strings[NumeroRegistro],151,2));
               except
                  DataVencimento := 0;
               end;
   ...
   EXCEPT
      ATitulo.Free;
      Result := FALSE;
      ShowMessage(Pchar('Erro na Leitura do Registro ' + IntToStr(NumeroRegistro)));
