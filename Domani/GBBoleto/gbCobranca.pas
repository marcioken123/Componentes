...
type
   ...
   published
      property LocalArquivoImagens: String read fArquivoImagem write fArquivoImagem;
...
{$IFNDEF VER120}
   ...
   private
      ...
      fTitulos : TgbTituloList;
      FBancoID: Integer;
    FNomeBanco: String;
    FAgencia: String;
    FContaCorrente: String;
    FNomeEmpresa: String;
    procedure SetBancoID(const Value: Integer);
    ...
    procedure SetNomeEmpresa(const Value: String); {Títulos incluídos no arquivo remessa ou retorno}
   public
      constructor Create(AOwner: TComponent); override;
      destructor Destroy; override;
   ...
   published
      property BancoID   : Integer read FBancoID write SetBancoID;
...
begin
   ...
   ArquivoIMG := fArquivoImagem + '\' + Cedente.ContaBancaria.Banco.Codigo + '.jpg';
   if FileExists(ArquivoIMG) then
      begin
         Aboleto.qrImgBanco1.Enabled   := True;
         Aboleto.qrImgBanco1.Picture.LoadFromFile(ArquivoIMG);
         Aboleto.qrImgBanco2.Enabled   := True;
         Aboleto.qrImgBanco2.Picture.LoadFromFile(ArquivoIMG);
         Aboleto.qrImgBanco3.Enabled   := True;
         Aboleto.qrImgBanco3.Picture.LoadFromFile(ArquivoIMG);

         Aboleto.txtNomeBanco.Enabled := False;
         Aboleto.txtNomeBanco2.Enabled := False;
         Aboleto.txtNomeBanco3.Enabled := False;
      end
   else
      begin
         Aboleto.qrImgBanco1.Enabled   := False;
         Aboleto.qrImgBanco2.Enabled   := False;
         Aboleto.qrImgBanco3.Enabled   := False;
         Aboleto.txtNomeBanco.Enabled := True;
         Aboleto.txtNomeBanco2.Enabled := True;
         Aboleto.txtNomeBanco3.Enabled := True;
      end;
   TRY
      PrepararBoleto(ABoleto);
      ABoleto.PreviewModal;
      FreeAndNil(ABoleto);
   EXCEPT
      FreeAndNil(ABoleto);
      Raise;
      ...
      case length(Retorno[0]) of
         ...
         400 :
            begin
               ...
               BancoID   := StrToInt(ACodigoBanco);
               Agencia   := Copy(Retorno.Strings[0],27,04);
               ContaCorrente := Copy(Retorno.Strings[0],33,05);
               NomeBanco := Copy(Retorno.Strings[0],80,15);
               NomeEmpresa := Copy(Retorno.Strings[0],47,30);
            end;
         else
         begin
...
procedure TgbCobranca.SetBancoID(const Value: Integer);
begin
  FBancoID := Value;
end;

procedure TgbCobranca.SetAgencia(const Value: String);
begin
  FAgencia := Value;
end;

procedure TgbCobranca.SetContaCorrente(const Value: String);
begin
  FContaCorrente := Value;
end;

procedure TgbCobranca.SetNomeBanco(const Value: String);
begin
  FNomeBanco := Value;
end;

procedure TgbCobranca.SetNomeEmpresa(const Value: String);
begin
  FNomeEmpresa := Value;
end;
