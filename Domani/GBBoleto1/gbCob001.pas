...
begin
   ...
   begin
      ...
      ADigitoNossoNumero := Modulo10(ANossoNumero); //Modulo11(ANossoNumero,9,true)
   end;
...
begin
   ...
   begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,18,false,'0'); //33 a 52 - Código do convênio no banco  // copy(Titulos[NumeroRegistro].Cedente.CodigoCedente+stringOfChar(20),1,20); //
      Registro := Registro + '  '; // alterado DP - 04/08/06
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,18,false,'0'); //34 a 53 - Código do convênio no banco  // Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,20,false,'0')
      Registro := Registro + '  '; // alterado DP - 04/08/06
      ...
      begin
         ...
         case Titulos[NumeroRegistro].EmissaoBoleto of //61 a 62 - Quem emite e quem distribui o boleto?
            ...
            ebBancoReemite    : Registro := Registro + '4' + '1'; // so para tipo de movimento = 31
            ebBancoNaoReemite : Registro := Registro + '5' + '2'; // so para tipo de movimento = 31
         else
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].SeuNumero,15,true,' '); //63 a 77 - Número que identifica o título na empresa // Formatar(Titulos[NumeroRegistro].SeuNumero,15,false,'0')
         ...
         begin
            ...
            if Titulos[NumeroRegistro].TipoDataProtesto = 'C' then
               Registro := Registro + '1' // dias corridos
            else
               Registro := Registro + '2'; // dias uteis
...
//            toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
        ...
        case Titulos[NumeroRegistro].AceiteDocumento of
         ...
         Registro := Registro + '    '; // BRANCOS   - ult.posicao 82 - descricao da observ. de 352 a 391
         ...
         Registro := Registro + Formatar(acobranca.Titulos[NumeroRegistro].instrucoes[0],40,true,' ');
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  if StrToInt(Copy(Retorno.Strings[NumeroRegistro],150,4)) = 0 then
                     DataCredito := EncodeDate(StrToInt(Copy(Retorno.Strings[NumeroRegistro],142,4)),StrToInt(Copy(Retorno.Strings[NumeroRegistro],140,2)),StrToInt(Copy(Retorno.Strings[NumeroRegistro],138,2)))
                  else
