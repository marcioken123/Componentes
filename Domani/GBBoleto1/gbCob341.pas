...
//            toRemessaBaixarPagoCedente: ATipoOcorrencia := '34';
         ...
         if (Titulos[NumeroRegistro].Instrucoes.Count > 0) and (length(Titulos[NumeroRegistro].Instrucoes.strings[0]) = 2) then  // Primeira Instrucao
            Registro := Registro + Formatar(Titulos[NumeroRegistro].Instrucoes.strings[0],2,false,'0')
         else
            Registro := Registro + '00';
         if (Titulos[NumeroRegistro].Instrucoes.Count > 1) and (length(Titulos[NumeroRegistro].Instrucoes.strings[1]) = 2) then  // Segunda Instrucao
         ...
         Registro := Registro + Formatar('',4,true,' '); // COMPLEMENTO DO REGISTRO
         Registro := Registro + FormatDateTime('ddmmyy',Titulos[NumeroRegistro].DataMultaJuros); // DATA DE MORA  //DataProtesto
         Registro := Registro + Titulos[NumeroRegistro].DiasMulta; //FormatCurr('00',Titulos[NumeroRegistro].DiasProtesto);// PRAZO // DiasProtesto
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               if Trim(Copy(Retorno.Strings[NumeroRegistro],300,2)) <> '' then
                 begin
