...
com o banco}
  ...
  with ATitulo, Cedente.ContaBancaria do
  begin
...
procedure TgbBanco356.FormatarBoleto(ATitulo: TgbTitulo; var AAgenciaCodigoCedente, ANossoNumero, ACarteira,
   AEspecieDocumento: String);
begin
  with ATitulo, Cedente.ContaBancaria do
  begin
     AAgenciaCodigoCedente   := Formatar(CodigoAgencia, 4, False,'0') + '/' + Formatar(NumeroConta, 7, False, '0') + '-' + DigitoNossoNumero;
...
begin
  ...
  with ACobranca do begin
    ...
    while NumeroRegistro <= (Titulos.Count - 1) do begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Carteira,2,true,'0');
      Registro := Registro + '5'; // tipo de cobrança
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].DiasProtesto,2,true,' ');
