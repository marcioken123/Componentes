...
{*********************************************}
// $Id: GR32_RangeBars.pas,v 1.11 2003/02/23 20:10:17 Alex Exp $
...
begin
  ...
  if C1 = C2 then Brush := CreateSolidBrush(ColorToRGB(C1))
  else
  begin
...
begin
  ...
  else Frame3D(Canvas, R, CHi, MixColors(clWindowFrame, Color, 96));
...
begin
  ...
  begin
    ...
    begin
      DrawRectEx(Canvas, R, Edges, clBtnShadow);
      FillRect(Canvas.Handle, R, GetSysColorBrush(COLOR_BTNFACE));
      InflateRect(R, -1, -1);
      OffsetRect(R, 1, 1);
      DrawArrow(Canvas, R, Direction, clBtnHighlight);
      OffsetRect(R, -1, -1);
      DrawArrow(Canvas, R, Direction, clBtnShadow);
    end
    else
    begin
      DrawRectEx(Canvas, R, Edges, clWindowFrame);
      if Pushed then
      begin
        FillRect(Canvas.Handle, R, GetSysColorBrush(COLOR_BTNFACE));
        OffsetRect(R, 1, 1);
      ...
      begin
        Frame3D(Canvas, R, clBtnHighlight, clBtnShadow, True);
        FillRect(Canvas.Handle, R, GetSysColorBrush(COLOR_BTNFACE));
      end;
...
begin
  ...
  case Style of
    ...
    rbsMac:
      DrawHandle(Canvas, R, HandleColor, Pushed, ShowHandleGrip, Horz);
  end;
end;
...
begin
  ...
  else with Canvas, R do
  begin
    if DrawEnabled then C := clWindowFrame
    else C := clBtnShadow;
    Edges := [drLeft, drUp, drRight, drDown];
    Exclude(Edges, OppositeDirection[Direction]);
    ...
    begin
      if DrawEnabled then
      begin
        Pen.Color := MixColors(clWindowFrame, MixColors(clBtnHighlight, Color, 127), 32);
        case Direction of
          drLeft, drUp:
            begin
        ...
        if Backgnd = bgPattern then DitherRect(Canvas, R, clBtnHighlight, Color)
