...
const
  ...
  {$IFDEF PORTUGUESE}
  Filtro     = 'Arquivo PDF (Alta Compressão)|*.PDF|Arquivo PDF (Baixa Compressão)|*.PDF|Arquivos JPG|*.JPG|Arquivos BMP|*.BMP|Arquivos EMF|*.EMF|Arquivos WMF|*.WMF';
  Error      = 'Erro ao exportar arquivo';
  Guardar    = 'Salvar arquivo como...';
  Exportando = 'Exportando...';
  Pagina1De  = 'Página... 1 de';
  Pagina     = 'Página... ';
  De         = ' de ';
...
type
  ...
  public
    ...
    procedure ExportQR (FileName : String = ''); {Only Export Report}
...
procedure TExportQR.ExportQR (FileName : String = '');
var
  SD    : TSaveDialog;
  Ruta  : String;
  Fich  : String;
  ...
  if (not FromPreview) then begin
    try
      ...
      if FReport.Page.Orientation = poLandscape then begin
        PPDF.PageWidth := 1123{792};
        PPDF.PageHeight:= 794{612};
      end;
      for i := 1 to FReport.QRPrinter.PageCount do begin
        Prg.Accion.Caption := Pagina + IntToStr(i) + De + IntToStr(FReport.QRPrinter.PageCount);
        Prg.Accion.Update;
    ...
    try
      ...
      if FPreview.QRPrinter.Orientation = poLandscape then begin
        PPDF.PageWidth := 1123{792};
        PPDF.PageHeight:= 794{612};
      end;
