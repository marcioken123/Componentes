{***********************************************************
       ...
       description : Parser for R&A Interpreter version 2
...
uses RAUtils, RAConst,
  RAI2 { ResBase and ieBadRemark constants };
...
{*************************** TRAI2Parser ****************************}
constructor TRAI2Parser.Create;
begin
  inherited Create;
end;

destructor TRAI2Parser.Destroy;
begin
  inherited Destroy;
end;

procedure TRAI2Parser.SetSource(Value: string);
begin
  FSource := Value;
  Init;
end;

procedure TRAI2Parser.Init;
begin
  FPCPos := PChar(FSource);
end;

function TRAI2Parser.Token : string;
var
  P, F : PChar;
  ...
  begin
    case P[0] of
      '{' :
        begin
          F := StrScan(P+1, '}');
          if F = nil then RAI2Error(ieBadRemark, P - PChar(FSource));
          P := F+1;
        end;
      '(' :
        if (P[1] = '*') then
      ...
      '}' : RAI2Error(ieBadRemark,  P - PChar(FSource));
      '*' : if (P[1] = ')') then RAI2Error(ieBadRemark,  P - PChar(FSource));
      '/' :
        if (P[1] = '/') then
        begin
          F := StrScan(P+1, #13);
...
function TRAI2Parser.GetPos: integer;
begin
  Result := FPCPos - PChar(FSource);
end;

procedure TRAI2Parser.SetPos(Value: integer);
begin
