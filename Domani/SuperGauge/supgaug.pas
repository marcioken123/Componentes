...
type
  ...
  TSuperGaugeKind = (gkText, gkHorizontalBar, gkVerticalBar, gkPie, gkNeedle, gkCilinder);
  ...
  private
    ...
    procedure PaintAsCilinder(AnImage : TBitmap; PaintRect: TRect);
    procedure SeTSuperGaugeKind(Value: TSuperGaugeKind);
  ...
  published
    ...
    property Font;
    property Kind: TSuperGaugeKind read FKind write SeTSuperGaugeKind default gkHorizontalBar;
...
function TSuperGauge.GetPercentDone: Longint;
begin
    ...
    try
      ...
      try
        ...
        case FKind of
          ...
          gkCilinder                     : PaintAsCilinder(OverlayImage, PaintRect);
        end;
...
procedure TSuperGauge.PaintBackground(AnImage: TBitmap);
var
  ARect: TRect;
begin
...
procedure TSuperGauge.PaintAsText(AnImage: TBitmap; PaintRect: TRect);
var
  S: string;
  X, Y: Integer;
  OverRect: TBltBitmap;
...
procedure TSuperGauge.PaintAsNothing(AnImage: TBitmap; PaintRect: TRect);
begin
  with AnImage do
  begin
...
procedure TSuperGauge.PaintAsBar(AnImage: TBitmap; PaintRect: TRect);
var
  FillSize: Longint;
  W, H: Integer;
begin
...
procedure TSuperGauge.PaintAsPie(AnImage: TBitmap; PaintRect: TRect);
var
...
procedure TSuperGauge.PaintAsNeedle(AnImage: TBitmap; PaintRect: TRect);
var
  MiddleX: Integer;
  Angle: Double;
  X, Y, W, H: Integer;
...
procedure TSuperGauge.SeTSuperGaugeKind(Value: TSuperGaugeKind);
begin
  if Value <> FKind then
  begin
    FKind := Value;
...
procedure TSuperGauge.SetShowText(Value: Boolean);
begin
...
procedure TSuperGauge.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TSuperGauge.SetForeColor(Value: TColor);
begin
...
procedure TSuperGauge.SetBackColor(Value: TColor);
begin
...
procedure TSuperGauge.SetMinValue(Value: Longint);
begin
...
procedure TSuperGauge.SetMaxValue(Value: Longint);
begin
  if Value <> FMaxValue then
  begin
    if Value < FMinValue then
...
procedure TSuperGauge.SetProgress(Value: Longint);
var
  TempPercent: Longint;
begin
...
procedure TSuperGauge.AddProgress(Value: Longint);
begin
...
procedure Register;
begin
  RegisterComponents('Domani', [TSuperGauge]);
end;

procedure TSuperGauge.PaintAsCilinder(AnImage: TBitmap; PaintRect: TRect);
var
//  MiddleX, MiddleY: Integer;
//  Angle: Double;
  W, H: Integer;
  FillSize  : Longint;
  ARect     : TRect;
  Diferenca,FatorX : Integer;
begin
  ARect.Left   := PaintRect.Left;
  ARect.Right  := PaintRect.Right;
  ARect.Top    := PaintRect.Top    + 25;
  ARect.Bottom := PaintRect.Bottom - 20;
  ...
  begin
    ...
    Ellipse(ARect.Left, ARect.Bottom - 1, W, ARect.Bottom - (H div 4) - 1);
    Ellipse(ARect.Left, ARect.Top, W, ARect.Top  - (H div 4));
    if PercentDone > 0 then
       begin
         FillSize := SolveForX(PercentDone, H);
         if FillSize >= H then FillSize := H - 1;
         ...
         Brush.Color := ForeColor;
         FatorX := 1;
         For Diferenca := 0  to Fillsize do
             begin
               if Diferenca = 0 then
                  Pen.Color := clBlack
               else
               ...
               Ellipse(ARect.Left, ARect.Bottom - Diferenca, W, ARect.Bottom - (H div 4) - Diferenca);
               FatorX := Diferenca;
             end;
         Pen.Color := clBlack;
         Ellipse(ARect.Left, ARect.Bottom - FatorX, W, ARect.Bottom - (H div 4) - FatorX);
       //  Ellipse(ARect.Left, ARect.Top, W, ARect.Top  - (H div 4));
       end;
       MoveTo(0,0);
       MoveTo(0,15);
       LineTo(0,ARect.Bottom-10);

       MoveTo(ARect.Right-1,0);
       MoveTo(ARect.Right-1,15);
       LineTo(ARect.Right-1,ARect.Bottom-10);
  end;
end;
