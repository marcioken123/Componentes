...
$Id: SynHighlighterPHP.pas,v 1.18 2004/04/23 13:45:21 harmeister Exp $
...
begin
  case FLine[Run + 1] of
    ...
    '<':
      begin
        fTokenID := tkSymbol;
        if (FLine[Run + 2] = '<') and (FLine[Run + 3] in cAlphaNumericNameChars) then
        begin
          fTokenID := tkString;
          inc(Run, 3);
        ...
        end else if FLine[Run + 2] = '=' then   {shift left assign}
        begin
          inc(Run, 3)
        end
        else                           {shift left}
...
procedure TSynPHPSyn.StringEndProc;
var i : Integer;
    DoEndToken : Boolean;
begin
  fTokenID := tkString;
  ...
  begin
    ...
    if DoEndToken and (FLine[Run + length(FTokenMatch)] = ';') then
    begin
      inc(Run, length(FTokenMatch));
      FTokenMatch := '';
    end else inc(Run);
  end else inc(Run);
