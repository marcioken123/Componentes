...
//    function  GetIndexCount Integer;
  ...
  public
    ...
    function  PrepareKey(Buffer: PChar; ResultType: TVarType): PChar;
...
begin
  ...
  begin
    ...
    begin
      ...
      dest := FEntry;
      size := (numEntries - EntryNo - 1) * PIndexHdr(FIndexFile.IndexHeader).KeyRecLen;
      if numEntries < PIndexHdr(FIndexFile.IndexHeader).NumKeys then
        size := size + FIndexFile.EntryHeaderSize;
      Move(source^, dest^, size);
    end;
    // one entry less now
    ...
    if FHighPage = FPageNo then
      dec(FHighBracket);
    // check if last entry index, numEntries already decreased
    if (EntryNo >= numEntries) then
    begin
      if UpperPage <> nil then
      begin
        // we just removed the last on this page, so update search key data
        EntryNo := numEntries - 1;
        UpperPage.SetEntry(0, GetKeyData, FPageNo);
      end;
      // zero it out to not get confused about internal or leaf pages
      SetRecLowerPageNoOfEntry(numEntries, 0, 0);
    end;
...
begin
  ...
  try
    ...
    case TempParser.ResultType of
      vtString: fieldType := 'C';
      {vtSmallInt, }vtInteger, vtLargeInt, vtFloat: fieldType := 'N';
    else
...
function TIndexFile.PrepareKey(Buffer: PChar; ResultType: TVarType): PChar;
var
  FloatRec: TFloatRec;
...
{$endif}
      ...
      IntSrc := 0;
      case ResultType of
//        vtSmallInt: IntSrc := PSmallInt(Result)^;
        vtInteger: IntSrc := PInteger(Result)^;
{$ifdef DELPHI_4}
        vtLargeInt:
          begin
...
{$endif}
      ...
      // if exponent negative then extra zeroes need to be inserted
      // DBF has only 20 places so there is always space
{
      if FloatRec.Exponent < 0 then
      begin
        Move(FloatRec.Digits[0], FloatRec.Digits[-FloatRec.Exponent], NumDecimals);
        Inc(NumDecimals, -FloatRec.Exponent);
        FloatRec.Exponent := 0;
      end;
}
