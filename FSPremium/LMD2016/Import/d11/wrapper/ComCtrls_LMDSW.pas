...
function TCustomListView_Items_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property Items: TListItems read <getter> write ...

  if IsGet then
    Result := TListItems_sw.ToVar(TCustomListView_sacc(TCustomListView(AObj)).
      Items)
  else
    TCustomListView_sacc(TCustomListView(AObj)).Items := TListItems_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('Items', TCustomListView_Items_si, TListItems_sw, True, True, 0, True);
  ...
  AData.AddProperty('OnCompare', TCustomListView_OnCompare_si, TLVCompareEvent_sw, True, True, 0, True);
...
function TCommonCalendar_CalExceptionClass_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property CalExceptionClass: ExceptClass read <g...

  if IsGet then
    Result := ExceptClass_sw.ToVar(TCommonCalendar_sacc(TCommonCalendar(AObj)).
      CalExceptionClass)
  else
    TCommonCalendar_sacc(TCommonCalendar(AObj)).CalExceptionClass := 
      ExceptClass_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('CalExceptionClass', TCommonCalendar_CalExceptionClass_si, ExceptClass_sw, True, True, 0, True);
