...
uses
  ...
  LMDSctSysWrappers, Types, SysUtils, Classes, Contnrs, SysUtils_LMDSW,
  Classes_LMDSW;
...
type
  ...
  TIntegerBucketList_sc = class of TIntegerBucketList;
  TIntegerBucketList_sw = class(TBucketList_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  public
    class function ToVar(const AValue: TIntegerBucketList): OleVariant;
    class function FromVar(const AValue: OleVariant): TIntegerBucketList;
    class function ClassToVar(AClass: TIntegerBucketList_sc): OleVariant;
    class function ClassFromVar(const AClass: OleVariant): TIntegerBucketList_sc;
  end;
...
begin
  ...
  AData.AddFunction('FindInstanceOf', TObjectList_FindInstanceOf_si, Integer_sw, 3, False);
  ...
  AData.AddProperty('OwnsObjects', TObjectList_OwnsObjects_si, Boolean_sw, True, True, 0, False);
  AData.AddProperty('Items', TObjectList_Items_si, TObject_sw, True, True, 1, False);

  { Class members }

  AData.AddConstructor('Create', TObjectList_Create_si, 1, False);
end;
...
begin
  ...
  AData.AddProperty('Items', TComponentList_Items_si, TComponent_sw, True, True, 1, False);
end;
...
begin
  ...
  AData.AddProperty('Items', TClassList_Items_si, TClass_sw, True, True, 1, False);
end;
...
begin
  ...
  AData.AddProperty('List', TOrderedList_List_si, TList_sw, True, False, 0, True);
...
begin
  ...
  AData.AddFunction('Copy', TBucketItem_Copy_si, TBucketItem_sw, 0);

  { Class members }

  AData.AddConstructor('Create', TBucketItem_Create_si, 0);
end;
...
begin
  ...
  AData.AddFunction('Copy', TBucket_Copy_si, TBucket_sw, 0);
  AData.AddField('Count', TBucket_Count_si, Integer_sw);

  { Class members }

  AData.AddConstructor('Create', TBucket_Create_si, 0);
end;
...
begin
  ...
  AData.AddProperty('BucketCount', TCustomBucketList_BucketCount_si, Integer_sw, True, True, 0, True);
...
class function TBucketListSizes_sw.GetEnumTypeInfo: TLMDTypeInfo;
begin
  Result := TypeInfo(TBucketListSizes);
end;
...
begin
  ...
  AData.AddConstructor('Create', TBucketList_Create_si, 1, False);
end;
...
begin
  ...
  AData.AddProperty('Data', TObjectBucketList_Data_si, TObject_sw, True, True, 1, False);
end;
...
{ TIntegerBucketList_sw }

function TIntegerBucketList_Add_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Add(AItem: Integer; AData: Integer): Integer;

  Result := TIntegerBucketList(AObj).Add(Integer(AArgs[0]), Integer(AArgs[1]));
end;

function TIntegerBucketList_Remove_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Remove(AItem: Integer): Integer;

  Result := TIntegerBucketList(AObj).Remove(Integer(AArgs[0]));
end;

function TIntegerBucketList_Data_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Data[AItem: Integer]: Integer read <getter> writ...

  if IsGet then
    Result := TIntegerBucketList(AObj).Data[Integer(AArgs[0])]
  else
    TIntegerBucketList(AObj).Data[Integer(AArgs[0])] := Integer(AArgs[1]);
end;

class function TIntegerBucketList_sw.GetTypeName: WideString;
begin
  Result := 'TIntegerBucketList';
end;

class function TIntegerBucketList_sw.GetWrappedClass: TClass;
begin
  Result := TIntegerBucketList;
end;

class procedure TIntegerBucketList_sw.DoInit(AData: TLMDClassWrapperData);
begin
  { Instance members }

  AData.AddFunction('Add', TIntegerBucketList_Add_si, Integer_sw, 2, False);
  AData.AddFunction('Remove', TIntegerBucketList_Remove_si, Integer_sw, 1, False);
  AData.AddProperty('Data', TIntegerBucketList_Data_si, Integer_sw, True, True, 1, False);
end;

class function TIntegerBucketList_sw.ToVar(
  const AValue: TIntegerBucketList): OleVariant;
begin
  Result := ConvToVar(AValue);
end;

class function TIntegerBucketList_sw.FromVar(
  const AValue: OleVariant): TIntegerBucketList;
begin
  Result := TIntegerBucketList(ConvFromVar(AValue, TIntegerBucketList));
end;

class function TIntegerBucketList_sw.ClassToVar(
  AClass: TIntegerBucketList_sc): OleVariant;
begin
  Result := ConvClsToVar(AClass);
end;

class function TIntegerBucketList_sw.ClassFromVar(
  const AClass: OleVariant): TIntegerBucketList_sc;
begin
  Result := TIntegerBucketList_sc(ConvClsFromVar(AClass, TIntegerBucketList));
end;
...
function Contnrs_TBucketListSizes_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TBucketListSizes = (bl4, bl8, bl16, bl32, bl64, bl12...
...
function Contnrs_TIntegerBucketList_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TIntegerBucketList = class(TBucketList);

  Result := TLMDUnitWrapper.TypeToVar(TIntegerBucketList_sw);
end;
...
begin
  ...
  AData.AddType(TIntegerBucketList_sw, Contnrs_TIntegerBucketList_si);
end;
...
initialization
  ...
  RegisterClassWrapper(TIntegerBucketList_sw);
