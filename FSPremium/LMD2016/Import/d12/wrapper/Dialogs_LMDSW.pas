...
uses
  ...
  Classes, Graphics, Controls, Forms, StdCtrls, CommCtrl, ComCtrls, Dialogs,
  Windows_LMDSW, Messages_LMDSW, SysUtils_LMDSW, CommDlg_LMDSW, Printers_LMDSW,
  Classes_LMDSW, Graphics_LMDSW, Controls_LMDSW, Forms_LMDSW, StdCtrls_LMDSW,
  CommCtrl_LMDSW, ComCtrls_LMDSW;
...
type
  ...
  TProgressBarState_sw = ComCtrls_LMDSW.TProgressBarState_sw;
...
function Dialogs_TProgressBarState_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TProgressBarState = ComCtrls.TProgressBarState;
...
begin
  ...
  case AArgsSize of
    7:
    begin
      Result := Dialogs.MessageDlgPosHelp(string(AArgs[0]), TMsgDlgType_sw.
...
begin
  ...
  case AArgsSize of
    5:
    begin
      Result := Dialogs.TaskMessageDlg(string(AArgs[0]), string(AArgs[1]), 
...
begin
  ...
  case AArgsSize of
    7:
    begin
      Result := Dialogs.TaskMessageDlgPos(string(AArgs[0]), string(AArgs[1]), 
        TMsgDlgType_sw.FromVar(AArgs[2]), TMsgDlgButtons_sw.FromVar(AArgs[3]),
        Longint(AArgs[4]), Integer(AArgs[5]), Integer(AArgs[6]));
    end;
    8:
    begin
      Result := Dialogs.TaskMessageDlgPos(string(AArgs[0]), string(AArgs[1]), 
        TMsgDlgType_sw.FromVar(AArgs[2]), TMsgDlgButtons_sw.FromVar(AArgs[3]),
        Longint(AArgs[4]), Integer(AArgs[5]), Integer(AArgs[6]), TMsgDlgBtn_sw.
        FromVar(AArgs[7]));
    end;
  else
...
begin
  ...
  case AArgsSize of
    8:
    begin
      Result := Dialogs.TaskMessageDlgPosHelp(string(AArgs[0]), string(
...
begin
  ...
  if IsGet then
    Result := Dialogs.UseLatestCommonDialogs
  else
    Dialogs.UseLatestCommonDialogs := Boolean(AArgs[0]);
end;
...
begin
  ...
  AData.AddConstant('tdiNone', Dialogs_tdiNone_si, nil, VarType(Dialogs.tdiNone));
  AData.AddConstant('tdiWarning', Dialogs_tdiWarning_si, nil, VarType(Dialogs.tdiWarning));
  AData.AddConstant('tdiError', Dialogs_tdiError_si, nil, VarType(Dialogs.tdiError));
  AData.AddConstant('tdiInformation', Dialogs_tdiInformation_si, nil, VarType(Dialogs.tdiInformation));
  AData.AddConstant('tdiShield', Dialogs_tdiShield_si, nil, VarType(Dialogs.tdiShield));
