...
begin
  ...
  case AArgsSize of
    2:
    begin
      Result := TSmallPoint_sw.ToVar(Types.SmallPoint(Integer(AArgs[0]), 
        Integer(AArgs[1])));
    end;
    1:
    begin
      Result := TSmallPoint_sw.ToVar(Types.SmallPoint(LongWord(AArgs[0])));
    end;
  else
...
begin
  ...
  Result := TRect_sw.ToVar(Types.SplitRect(TRect_sw.FromVar(AArgs[0]), 
...
function Types_CenteredRect_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function CenteredRect(SourceRect: TRect; CenteredRect: TR...
