...
uses
  Types, Classes, LMDTypes, LMDSctScripter, LMDSctWrappers, LMDSctSysWrappers,
...
function TThread_NameThreadForDebugging_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class procedure NameThreadForDebugging(AThreadName: AnsiS...

  case AArgsSize of
    1:
    begin
      TThread_sc(AObj).NameThreadForDebugging(AnsiString(AArgs[0]));
    end;
    2:
    begin
      TThread_sc(AObj).NameThreadForDebugging(AnsiString(AArgs[0]), LongWord(
        AArgs[1]));
    end;
  else
