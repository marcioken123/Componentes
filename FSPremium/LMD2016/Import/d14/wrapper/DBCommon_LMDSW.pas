...
uses
  Types, SysUtils, TypInfo, LMDTypes, LMDSctScripter, LMDSctWrappers,
...
type
  ...
  TFilterExpr2_sw = class;
  TExprParser_sw = class;
  TExprParser2_sw = class;
  ...
  TFilterExpr2_sc = class of TFilterExpr2;
  TFilterExpr2_sw = class(TFilterExpr_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  public
    class function ToVar(const AValue: TFilterExpr2): OleVariant;
    class function FromVar(const AValue: OleVariant): TFilterExpr2;
    class function ClassToVar(AClass: TFilterExpr2_sc): OleVariant;
    class function ClassFromVar(const AClass: OleVariant): TFilterExpr2_sc;
  end;
  ...
  TExprParser2_sc = class of TExprParser2;
  TExprParser2_sw = class(TExprParser_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  public
    class function ToVar(const AValue: TExprParser2): OleVariant;
    class function FromVar(const AValue: OleVariant): TExprParser2;
    class function ClassToVar(AClass: TExprParser2_sc): OleVariant;
    class function ClassFromVar(const AClass: OleVariant): TExprParser2_sc;
  end;
...
class procedure IDENTIFIEROption_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..3] of TLMDEnumerator = (
    (Name: 'idKeepQuotes'; Value: Integer({$IFDEF LMDSCT_12}IDENTIFIEROption.{$ENDIF}idKeepQuotes)),
    (Name: 'idMixCase'; Value: Integer({$IFDEF LMDSCT_12}IDENTIFIEROption.{$ENDIF}idMixCase)),
    (Name: 'idMakeLowerCase'; Value: Integer({$IFDEF LMDSCT_12}IDENTIFIEROption.{$ENDIF}idMakeLowerCase)),
    (Name: 'idMakeUpperCase'; Value: Integer({$IFDEF LMDSCT_12}IDENTIFIEROption.{$ENDIF}idMakeUpperCase))
  );
begin
  AEnums := @ENUMS;
  ACount := 4;
...
class function TFilterExpr2_sw.GetTypeName: WideString;
begin
  Result := 'TFilterExpr2';
end;

class function TFilterExpr2_sw.GetWrappedClass: TClass;
begin
  Result := TFilterExpr2;
end;

class procedure TFilterExpr2_sw.DoInit(AData: TLMDClassWrapperData);
begin
  // Do not call inherited.
end;

class function TFilterExpr2_sw.ToVar(const AValue: TFilterExpr2): OleVariant;
begin
  Result := ConvToVar(AValue);
end;

class function TFilterExpr2_sw.FromVar(const AValue: OleVariant): TFilterExpr2;
begin
  Result := TFilterExpr2(ConvFromVar(AValue, TFilterExpr2));
end;

class function TFilterExpr2_sw.ClassToVar(AClass: TFilterExpr2_sc): OleVariant;
begin
  Result := ConvClsToVar(AClass);
end;

class function TFilterExpr2_sw.ClassFromVar(
  const AClass: OleVariant): TFilterExpr2_sc;
begin
  Result := TFilterExpr2_sc(ConvClsFromVar(AClass, TFilterExpr2));
end;
...
function TExprParser_SetExprParams_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure SetExprParams(const Text: Widestring; Options: ...

  TExprParser(AObj).SetExprParams(Widestring(AArgs[0]), TFilterOptions_sw.
    FromVar(AArgs[1]), TParserOptions_sw.FromVar(AArgs[2]), Widestring(AArgs[3]));
end;
...
class function TExprParser2_sw.GetTypeName: WideString;
begin
  Result := 'TExprParser2';
end;

class function TExprParser2_sw.GetWrappedClass: TClass;
begin
  Result := TExprParser2;
end;

class procedure TExprParser2_sw.DoInit(AData: TLMDClassWrapperData);
begin
  // Do not call inherited.
end;

class function TExprParser2_sw.ToVar(const AValue: TExprParser2): OleVariant;
begin
  Result := ConvToVar(AValue);
end;

class function TExprParser2_sw.FromVar(const AValue: OleVariant): TExprParser2;
begin
  Result := TExprParser2(ConvFromVar(AValue, TExprParser2));
end;

class function TExprParser2_sw.ClassToVar(AClass: TExprParser2_sc): OleVariant;
begin
  Result := ConvClsToVar(AClass);
end;

class function TExprParser2_sw.ClassFromVar(
  const AClass: OleVariant): TExprParser2_sc;
begin
  Result := TExprParser2_sc(ConvClsFromVar(AClass, TExprParser2));
end;
...
function TFieldInfo_DatabaseName_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var DatabaseName: Widestring;

  if IsGet then
    Result := TFieldInfo_sw(AObj).FValue.DatabaseName
  else
    TFieldInfo_sw(AObj).FValue.DatabaseName := Widestring(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    ...
    TFieldInfo_sw(AObj).FValue.TableName := Widestring(AArgs[0]);
end;

function TFieldInfo_OriginalFieldName_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var OriginalFieldName: Widestring;

  if IsGet then
    Result := TFieldInfo_sw(AObj).FValue.OriginalFieldName
  else
    TFieldInfo_sw(AObj).FValue.OriginalFieldName := Widestring(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('DatabaseName', TFieldInfo_DatabaseName_si, Widestring_sw);
  AData.AddField('TableName', TFieldInfo_TableName_si, Widestring_sw);
  AData.AddField('OriginalFieldName', TFieldInfo_OriginalFieldName_si, Widestring_sw);
...
function DBCommon_IDENTIFIEROption_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type IDENTIFIEROption = (idKeepQuotes, idMixCase, idMakeL...
...
function DBCommon_TFilterExpr2_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TFilterExpr2 = class(TFilterExpr);

  Result := TLMDUnitWrapper.TypeToVar(TFilterExpr2_sw);
end;
...
function DBCommon_TExprParser2_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TExprParser2 = class(TExprParser);

  Result := TLMDUnitWrapper.TypeToVar(TExprParser2_sw);
end;
...
var
  v_1: TFieldInfo;
begin
  // function GetFieldInfo(const Origin: Widestring; var Field...

  v_1 := TFieldInfo_sw.FromVar(AArgs[1]);
  Result := DBCommon.GetFieldInfo(Widestring(AArgs[0]), v_1);
...
function DBCommon_GetIndexForOrderBy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function GetIndexForOrderBy(const SQL: WideString; DataSe...

  Result := TIndexDef_sw.ToVar(DBCommon.GetIndexForOrderBy(WideString(AArgs[0]),
...
function DBCommon_GetTableNameFromSQL_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function GetTableNameFromSQL(const SQL: WideString): Wide...

  Result := DBCommon.GetTableNameFromSQL(WideString(AArgs[0]));
end;

function DBCommon_GetTableNameFromQuery_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function GetTableNameFromQuery(const SQL: Widestring): Wi...

  Result := DBCommon.GetTableNameFromQuery(Widestring(AArgs[0]));
end;

function DBCommon_AddParamSQLForDetail_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function AddParamSQLForDetail(Params: TParams; SQL: WideS...

  case AArgsSize of
    3:
    begin
      Result := DBCommon.AddParamSQLForDetail(TParams_sw.FromVar(AArgs[0]), 
        WideString(AArgs[1]), Boolean(AArgs[2]));
    end;
    4:
    begin
      Result := DBCommon.AddParamSQLForDetail(TParams_sw.FromVar(AArgs[0]), 
        WideString(AArgs[1]), Boolean(AArgs[2]), WideString(AArgs[3]));
    end;
  else
    WrongArgCountError('AddParamSQLForDetail');
  end;
end;

function DBCommon_IsMultiTableQuery_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function IsMultiTableQuery(const SQL: WideString): Boolean;

  Result := DBCommon.IsMultiTableQuery(WideString(AArgs[0]));
end;
...
function DBCommon_MetaDataFromUnicode_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function MetaDataFromUnicode(const Value: string): RawByt...

  Result := RawByteString_sw.ToVar(DBCommon.MetaDataFromUnicode(string(
    AArgs[0])));
end;

function DBCommon_MetaDataToUnicode_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function MetaDataToUnicode(const Value: RawByteString): s...

  Result := DBCommon.MetaDataToUnicode(RawByteString_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  AData.AddType(TFilterExpr2_sw, DBCommon_TFilterExpr2_si);
  AData.AddType(TExprToken_sw, DBCommon_TExprToken_si);
  AData.AddType(TExprParser_sw, DBCommon_TExprParser_si);
  AData.AddType(TExprParser2_sw, DBCommon_TExprParser2_si);
  ...
  AData.AddFunction('GetTableNameFromSQL', DBCommon_GetTableNameFromSQL_si, WideString_sw, 1, False);
  AData.AddFunction('GetTableNameFromQuery', DBCommon_GetTableNameFromQuery_si, Widestring_sw, 1, False);
  AData.AddFunction('AddParamSQLForDetail', DBCommon_AddParamSQLForDetail_si, WideString_sw, 3, True);
  AData.AddFunction('IsMultiTableQuery', DBCommon_IsMultiTableQuery_si, Boolean_sw, 1, False);
  ...
  AData.AddFunction('MetaDataFromUnicode', DBCommon_MetaDataFromUnicode_si, RawByteString_sw, 1, False);
  AData.AddFunction('MetaDataToUnicode', DBCommon_MetaDataToUnicode_si, string_sw, 1, False);
end;
...
initialization
  ...
  RegisterClassWrapper(TFilterExpr2_sw);
  RegisterClassWrapper(TExprParser_sw);
  RegisterClassWrapper(TExprParser2_sw);
