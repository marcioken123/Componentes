...
type
  ...
  THintInfo_sw = Controls_LMDSW.THintInfo_sw;
...
begin
  ...
  v_1 := HintStr;
  v_2 := CanShow;
  v_3 := Controls_LMDSW.THintInfo_sw.ToVar(HintInfo);
  args[0] := MakeVarRef(v_1);
  args[1] := MakeVarRef(v_2);
  ...
  HintStr := string(v_1);
  CanShow := Boolean(v_2);
  HintInfo := Controls_LMDSW.THintInfo_sw.FromVar(v_3);
end;
...
function Forms_THintInfo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type THintInfo = Controls.THintInfo;
