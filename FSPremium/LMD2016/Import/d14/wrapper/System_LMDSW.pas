...
type
  ...
  TImageThunkData_sw = class(TLMDRecordWrapper)
  private
    FValue: TImageThunkData;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: TImageThunkData): OleVariant;
    class function FromVar(const AValue: OleVariant): TImageThunkData;
  end;

  ImgDelayDescr_sw = class(TLMDRecordWrapper)
  private
    FValue: ImgDelayDescr;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: ImgDelayDescr): OleVariant;
    class function FromVar(const AValue: OleVariant): ImgDelayDescr;
  end;

  TImgDelayDescr_sw = ImgDelayDescr_sw;

  dliNotification_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: dliNotification): OleVariant;
    class function FromVar(const AValue: OleVariant): dliNotification;
  end;

  DelayLoadProc_sw = class(TLMDRecordWrapper)
  private
    FValue: DelayLoadProc;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: DelayLoadProc): OleVariant;
    class function FromVar(const AValue: OleVariant): DelayLoadProc;
  end;

  TDelayLoadProc_sw = DelayLoadProc_sw;

  DelayLoadInfo_sw = class(TLMDRecordWrapper)
  private
    FValue: DelayLoadInfo;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: DelayLoadInfo): OleVariant;
    class function FromVar(const AValue: OleVariant): DelayLoadInfo;
  end;

  TDelayLoadInfo_sw = DelayLoadInfo_sw;
...
function TMonitor_SetSpinCount_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class procedure SetSpinCount(AObject: TObject; ASpinCount...
...
begin
  ...
  Result := TMonitor.Enter(TObject_sw.FromVar(AArgs[0]), Cardinal(AArgs[1]));
end;
...
begin
  ...
  if IsGet then
    ...
    TVarRec_sw(AObj).FValue.VChar := AnsiChar(Integer(AArgs[0]));
end;
...
begin
  ...
  AData.AddField('VChar', TVarRec_VChar_si, AnsiChar_sw);
...
function TSmallBlockTypeState_AllocatedBlockCount_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var AllocatedBlockCount: Cardinal;

  if IsGet then
    Result := TSmallBlockTypeState_sw(AObj).FValue.AllocatedBlockCount
  else
    TSmallBlockTypeState_sw(AObj).FValue.AllocatedBlockCount := Cardinal(
      AArgs[0]);
end;
...
function TSmallBlockTypeState_ReservedAddressSpace_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ReservedAddressSpace: Cardinal;

  if IsGet then
    Result := TSmallBlockTypeState_sw(AObj).FValue.ReservedAddressSpace
  else
    TSmallBlockTypeState_sw(AObj).FValue.ReservedAddressSpace := Cardinal(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddField('AllocatedBlockCount', TSmallBlockTypeState_AllocatedBlockCount_si, Cardinal_sw);
  AData.AddField('ReservedAddressSpace', TSmallBlockTypeState_ReservedAddressSpace_si, Cardinal_sw);
...
function TMemoryManagerState_TotalAllocatedMediumBlockSize_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var TotalAllocatedMediumBlockSize: Cardinal;

  if IsGet then
    Result := TMemoryManagerState_sw(AObj).FValue.TotalAllocatedMediumBlockSize
  else
...
function TMemoryManagerState_ReservedMediumBlockAddressSpace_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ReservedMediumBlockAddressSpace: Cardinal;

  if IsGet then
    Result := TMemoryManagerState_sw(AObj).FValue.
...
function TMemoryManagerState_TotalAllocatedLargeBlockSize_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var TotalAllocatedLargeBlockSize: Cardinal;

  if IsGet then
    Result := TMemoryManagerState_sw(AObj).FValue.TotalAllocatedLargeBlockSize
  else
...
function TMemoryManagerState_ReservedLargeBlockAddressSpace_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ReservedLargeBlockAddressSpace: Cardinal;

  if IsGet then
    Result := TMemoryManagerState_sw(AObj).FValue.ReservedLargeBlockAddressSpace
  else
...
begin
  ...
  AData.AddField('TotalAllocatedMediumBlockSize', TMemoryManagerState_TotalAllocatedMediumBlockSize_si, Cardinal_sw);
  AData.AddField('ReservedMediumBlockAddressSpace', TMemoryManagerState_ReservedMediumBlockAddressSpace_si, Cardinal_sw);
  AData.AddField('AllocatedLargeBlockCount', TMemoryManagerState_AllocatedLargeBlockCount_si, Cardinal_sw);
  AData.AddField('TotalAllocatedLargeBlockSize', TMemoryManagerState_TotalAllocatedLargeBlockSize_si, Cardinal_sw);
  AData.AddField('ReservedLargeBlockAddressSpace', TMemoryManagerState_ReservedLargeBlockAddressSpace_si, Cardinal_sw);
...
begin
  ...
  if IsGet then
    Result := TInitContext_sw(AObj).FValue.ThreadID
  else
    TInitContext_sw(AObj).FValue.ThreadID := LongWord(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('ThreadID', TInitContext_ThreadID_si, LongWord_sw);
...
class procedure TRuntimeError_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..26] of TLMDEnumerator = (
...
function TImageThunkData_ForwarderString_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ForwarderString: LongWord;

  if IsGet then
    Result := TImageThunkData_sw(AObj).FValue.ForwarderString
  else
    TImageThunkData_sw(AObj).FValue.ForwarderString := LongWord(AArgs[0]);
end;

function TImageThunkData__Function_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var _Function: LongWord;

  if IsGet then
    Result := TImageThunkData_sw(AObj).FValue._Function
  else
    TImageThunkData_sw(AObj).FValue._Function := LongWord(AArgs[0]);
end;

function TImageThunkData_Ordinal_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Ordinal: LongWord;

  if IsGet then
    Result := TImageThunkData_sw(AObj).FValue.Ordinal
  else
    TImageThunkData_sw(AObj).FValue.Ordinal := LongWord(AArgs[0]);
end;

function TImageThunkData_AddressOfData_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var AddressOfData: LongWord;

  if IsGet then
    Result := TImageThunkData_sw(AObj).FValue.AddressOfData
  else
    TImageThunkData_sw(AObj).FValue.AddressOfData := LongWord(AArgs[0]);
end;

function TImageThunkData_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := TImageThunkData_sw.Create as IDispatch;
end;

function TImageThunkData_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: TImageThunkData;

  // Implicit record copy method.
  Result := TImageThunkData_sw.ToVar(TImageThunkData(AObj));
end;

class function TImageThunkData_sw.GetTypeName: WideString;
begin
  Result := 'TImageThunkData';
end;

function TImageThunkData_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := TImageThunkData_sw.Create;
  TImageThunkData_sw(Result).FValue := FValue;
end;

class procedure TImageThunkData_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('ForwarderString', TImageThunkData_ForwarderString_si, LongWord_sw);
  AData.AddField('_Function', TImageThunkData__Function_si, LongWord_sw);
  AData.AddField('Ordinal', TImageThunkData_Ordinal_si, LongWord_sw);
  AData.AddField('AddressOfData', TImageThunkData_AddressOfData_si, LongWord_sw);
  AData.AddFunction('Copy', TImageThunkData_Copy_si, TImageThunkData_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TImageThunkData_Create_si, 0, False);
end;

class function TImageThunkData_sw.ToVar(
  const AValue: TImageThunkData): OleVariant;
var
  wrpr: TImageThunkData_sw;
begin
  wrpr        := TImageThunkData_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function TImageThunkData_sw.FromVar(
  const AValue: OleVariant): TImageThunkData;
begin
  Result := TImageThunkData_sw(ConvFromVar(AValue)).FValue;
end;

{ ImgDelayDescr_sw }

function ImgDelayDescr_grAttrs_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var grAttrs: LongWord;

  if IsGet then
    Result := ImgDelayDescr_sw(AObj).FValue.grAttrs
  else
    ImgDelayDescr_sw(AObj).FValue.grAttrs := LongWord(AArgs[0]);
end;

function ImgDelayDescr_hmod_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hmod: HMODULE;

  if IsGet then
    Result := HMODULE_sw.ToVar(ImgDelayDescr_sw(AObj).FValue.hmod)
  else
    ImgDelayDescr_sw(AObj).FValue.hmod := HMODULE_sw.FromVar(AArgs[0]);
end;

function ImgDelayDescr_dwTimeStamp_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwTimeStamp: LongWord;

  if IsGet then
    Result := ImgDelayDescr_sw(AObj).FValue.dwTimeStamp
  else
    ImgDelayDescr_sw(AObj).FValue.dwTimeStamp := LongWord(AArgs[0]);
end;

function ImgDelayDescr_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := ImgDelayDescr_sw.Create as IDispatch;
end;

function ImgDelayDescr_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: ImgDelayDescr;

  // Implicit record copy method.
  Result := ImgDelayDescr_sw.ToVar(ImgDelayDescr(AObj));
end;

class function ImgDelayDescr_sw.GetTypeName: WideString;
begin
  Result := 'ImgDelayDescr';
end;

function ImgDelayDescr_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := ImgDelayDescr_sw.Create;
  ImgDelayDescr_sw(Result).FValue := FValue;
end;

class procedure ImgDelayDescr_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('grAttrs', ImgDelayDescr_grAttrs_si, LongWord_sw);
  AData.AddField('hmod', ImgDelayDescr_hmod_si, HMODULE_sw);
  AData.AddField('dwTimeStamp', ImgDelayDescr_dwTimeStamp_si, LongWord_sw);
  AData.AddFunction('Copy', ImgDelayDescr_Copy_si, ImgDelayDescr_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', ImgDelayDescr_Create_si, 0, False);
end;

class function ImgDelayDescr_sw.ToVar(const AValue: ImgDelayDescr): OleVariant;
var
  wrpr: ImgDelayDescr_sw;
begin
  wrpr        := ImgDelayDescr_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function ImgDelayDescr_sw.FromVar(
  const AValue: OleVariant): ImgDelayDescr;
begin
  Result := ImgDelayDescr_sw(ConvFromVar(AValue)).FValue;
end;

{ dliNotification_sw }

class function dliNotification_sw.GetTypeName: WideString;
begin
  Result := 'dliNotification';
end;

class procedure dliNotification_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..5] of TLMDEnumerator = (
    (Name: 'dliNoteStartProcessing'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliNoteStartProcessing)),
    (Name: 'dliNotePreLoadLibrary'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliNotePreLoadLibrary)),
    (Name: 'dliNotePreGetProcAddress'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliNotePreGetProcAddress)),
    (Name: 'dliFailLoadLibrary'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliFailLoadLibrary)),
    (Name: 'dliFailGetProcAddress'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliFailGetProcAddress)),
    (Name: 'dliNoteEndProcessing'; Value: Integer({$IFDEF LMDSCT_12}dliNotification.{$ENDIF}dliNoteEndProcessing))
  );
begin
  AEnums := @ENUMS;
  ACount := 6;
...
class function dliNotification_sw.ToVar(
  const AValue: dliNotification): OleVariant;
begin
  Result := Integer(AValue);
end;

class function dliNotification_sw.FromVar(
  const AValue: OleVariant): dliNotification;
begin
  Result := dliNotification(Integer(AValue));
end;

{ DelayLoadProc_sw }

function DelayLoadProc_fImportByName_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var fImportByName: LongBool;

  if IsGet then
    Result := DelayLoadProc_sw(AObj).FValue.fImportByName
  else
    DelayLoadProc_sw(AObj).FValue.fImportByName := LongBool(AArgs[0]);
end;

function DelayLoadProc_dwOrdinal_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwOrdinal: LongWord;

  if IsGet then
    Result := DelayLoadProc_sw(AObj).FValue.dwOrdinal
  else
    DelayLoadProc_sw(AObj).FValue.dwOrdinal := LongWord(AArgs[0]);
end;

function DelayLoadProc_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := DelayLoadProc_sw.Create as IDispatch;
end;

function DelayLoadProc_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: DelayLoadProc;

  // Implicit record copy method.
  Result := DelayLoadProc_sw.ToVar(DelayLoadProc(AObj));
end;

class function DelayLoadProc_sw.GetTypeName: WideString;
begin
  Result := 'DelayLoadProc';
end;

function DelayLoadProc_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := DelayLoadProc_sw.Create;
  DelayLoadProc_sw(Result).FValue := FValue;
end;

class procedure DelayLoadProc_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('fImportByName', DelayLoadProc_fImportByName_si, LongBool_sw);
  AData.AddField('dwOrdinal', DelayLoadProc_dwOrdinal_si, LongWord_sw);
  AData.AddFunction('Copy', DelayLoadProc_Copy_si, DelayLoadProc_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', DelayLoadProc_Create_si, 0, False);
end;

class function DelayLoadProc_sw.ToVar(const AValue: DelayLoadProc): OleVariant;
var
  wrpr: DelayLoadProc_sw;
begin
  wrpr        := DelayLoadProc_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function DelayLoadProc_sw.FromVar(
  const AValue: OleVariant): DelayLoadProc;
begin
  Result := DelayLoadProc_sw(ConvFromVar(AValue)).FValue;
end;

{ DelayLoadInfo_sw }

function DelayLoadInfo_cb_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var cb: LongWord;

  if IsGet then
    Result := DelayLoadInfo_sw(AObj).FValue.cb
  else
    DelayLoadInfo_sw(AObj).FValue.cb := LongWord(AArgs[0]);
end;

function DelayLoadInfo_dlp_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dlp: TDelayLoadProc;

  if IsGet then
    Result := TDelayLoadProc_sw.ToVar(DelayLoadInfo_sw(AObj).FValue.dlp)
  else
    DelayLoadInfo_sw(AObj).FValue.dlp := TDelayLoadProc_sw.FromVar(AArgs[0]);
end;

function DelayLoadInfo_hmodCur_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hmodCur: HMODULE;

  if IsGet then
    Result := HMODULE_sw.ToVar(DelayLoadInfo_sw(AObj).FValue.hmodCur)
  else
    DelayLoadInfo_sw(AObj).FValue.hmodCur := HMODULE_sw.FromVar(AArgs[0]);
end;

function DelayLoadInfo_dwLastError_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwLastError: LongWord;

  if IsGet then
    Result := DelayLoadInfo_sw(AObj).FValue.dwLastError
  else
    DelayLoadInfo_sw(AObj).FValue.dwLastError := LongWord(AArgs[0]);
end;

function DelayLoadInfo_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := DelayLoadInfo_sw.Create as IDispatch;
end;

function DelayLoadInfo_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: DelayLoadInfo;

  // Implicit record copy method.
  Result := DelayLoadInfo_sw.ToVar(DelayLoadInfo(AObj));
end;

class function DelayLoadInfo_sw.GetTypeName: WideString;
begin
  Result := 'DelayLoadInfo';
end;

function DelayLoadInfo_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := DelayLoadInfo_sw.Create;
  DelayLoadInfo_sw(Result).FValue := FValue;
end;

class procedure DelayLoadInfo_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('cb', DelayLoadInfo_cb_si, LongWord_sw);
  AData.AddField('dlp', DelayLoadInfo_dlp_si, TDelayLoadProc_sw);
  AData.AddField('hmodCur', DelayLoadInfo_hmodCur_si, HMODULE_sw);
  AData.AddField('dwLastError', DelayLoadInfo_dwLastError_si, LongWord_sw);
  AData.AddFunction('Copy', DelayLoadInfo_Copy_si, DelayLoadInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', DelayLoadInfo_Create_si, 0, False);
end;

class function DelayLoadInfo_sw.ToVar(const AValue: DelayLoadInfo): OleVariant;
var
  wrpr: DelayLoadInfo_sw;
begin
  wrpr        := DelayLoadInfo_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function DelayLoadInfo_sw.FromVar(
  const AValue: OleVariant): DelayLoadInfo;
begin
  Result := DelayLoadInfo_sw(ConvFromVar(AValue)).FValue;
end;
...
function System_MonitorEnter_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function MonitorEnter(AObject: TObject; Timeout: Cardinal...
...
function System_MonitorWait_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function MonitorWait(AObject: TObject; Timeout: Cardinal)...
...
function System_MemoryBarrier_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
begin
  ...
  Result := System.UpCase(WideChar(Integer(AArgs[0])));
end;
...
begin
  ...
  Result := RawByteString_sw.ToVar(System.UTF8Encode(WideString(AArgs[0])));
end;
...
begin
  ...
  Result := System.UTF8EncodeToShortString(WideString(AArgs[0]));
end;
...
begin
  ...
  Result := System.UTF8Decode(RawByteString_sw.FromVar(AArgs[0]));
end;

function System_UTF8ToWideString_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function UTF8ToWideString(const S: RawByteString): WideSt...

  Result := System.UTF8ToWideString(RawByteString_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := System.UTF8ToUnicodeString(RawByteString_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := System.UTF8ToString(RawByteString_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := RawByteString_sw.ToVar(System.AnsiToUtf8(string(AArgs[0])));
end;
...
begin
  ...
  Result := System.Utf8ToAnsi(RawByteString_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  System._LStrFromChar(v_1, AnsiChar(Integer(AArgs[1])), Word(AArgs[2]));
...
begin
  ...
  v_1 := AnsiString(AArgs[0]);
  System._LStrFromString(v_1, ShortString(AArgs[1]), Word(AArgs[2]));
  AssignRefParam(AArgs[0], v_1);
end;
...
begin
  ...
  v_1 := AnsiString(AArgs[0]);
  System._LStrFromWStr(v_1, WideString(AArgs[1]), Word(AArgs[2]));
  AssignRefParam(AArgs[0], v_1);
end;
...
var
  v_1: AnsiString;
begin
  // procedure _LStrFromUStr(var Dest: AnsiString; const Sourc...
...
var
  v_1: RawByteString;
begin
  // procedure SetCodePage(var S: RawByteString; CodePage: Wor...

  case AArgsSize of
    2:
    begin
      v_1 := RawByteString_sw.FromVar(AArgs[0]);
      System.SetCodePage(v_1, Word(AArgs[1]));
      AssignRefParam(AArgs[0], RawByteString_sw.ToVar(v_1));
    end;
    3:
    begin
      v_1 := RawByteString_sw.FromVar(AArgs[0]);
...
var
  v_1: WideString;
begin
  // procedure _WStrFromUStr(var Dest: WideString; const Sourc...
...
begin
  ...
  v_1 := UnicodeString(AArgs[0]);
  System._UStrFromChar(v_1, AnsiChar(Integer(AArgs[1])));
  AssignRefParam(AArgs[0], v_1);
end;
...
begin
  ...
  v_1 := UnicodeString(AArgs[0]);
  System._UStrFromLStr(v_1, AnsiString(AArgs[1]));
  AssignRefParam(AArgs[0], v_1);
end;
...
begin
  ...
  v_1 := UnicodeString(AArgs[0]);
  System._UStrFromWStr(v_1, WideString(AArgs[1]));
  AssignRefParam(AArgs[0], v_1);
end;
...
begin
  ...
  v_1 := UnicodeString(AArgs[0]);
  System._UStrFromString(v_1, ShortString(AArgs[1]));
  AssignRefParam(AArgs[0], v_1);
end;
...
var
  v_1: UnicodeString;
begin
  // procedure _UStrSetLength(var S: UnicodeString; NewLength:...
...
function System__InitUnicodeStrings_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure _InitUnicodeStrings;

  System._InitUnicodeStrings();
end;

function System__UniqueStringU_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
var
  v_1: UnicodeString;
begin
...
begin
  ...
  Result := System.Pos(WideString(AArgs[0]), WideString(AArgs[1]));
end;
...
function System__WriteUInt64_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
function System__Write0UInt64_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure _Write0UInt64;

  System._Write0UInt64();
end;
...
function System__StrUInt64_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function _StrUInt64(val: UInt64; width: Integer): ShortSt...

  Result := System._StrUInt64(UInt64_sw.FromVar(AArgs[0]), Integer(AArgs[1]));
end;
...
begin
  ...
  Result := System._Str0UInt64(Int64_sw.FromVar(AArgs[0]));
end;
...
function System_TImageThunkData_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TImageThunkData = record;

  Result := TLMDUnitWrapper.TypeToVar(TImageThunkData_sw);
end;

function System_ImgDelayDescr_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type ImgDelayDescr = record;

  Result := TLMDUnitWrapper.TypeToVar(ImgDelayDescr_sw);
end;

function System_TImgDelayDescr_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TImgDelayDescr = ImgDelayDescr;

  Result := TLMDUnitWrapper.TypeToVar(TImgDelayDescr_sw);
end;

function System_dliNotification_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type dliNotification = (dliNoteStartProcessing, dliNotePr...

  Result := TLMDUnitWrapper.TypeToVar(dliNotification_sw);
end;

function System_DelayLoadProc_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type DelayLoadProc = record;

  Result := TLMDUnitWrapper.TypeToVar(DelayLoadProc_sw);
end;

function System_TDelayLoadProc_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDelayLoadProc = DelayLoadProc;

  Result := TLMDUnitWrapper.TypeToVar(TDelayLoadProc_sw);
end;

function System_DelayLoadInfo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type DelayLoadInfo = record;

  Result := TLMDUnitWrapper.TypeToVar(DelayLoadInfo_sw);
end;

function System_TDelayLoadInfo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDelayLoadInfo = DelayLoadInfo;

  Result := TLMDUnitWrapper.TypeToVar(TDelayLoadInfo_sw);
end;

function System____pfnDliNotifyHook_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure ___pfnDliNotifyHook;

  System.___pfnDliNotifyHook();
end;

function System____pfnDliFailureHook_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure ___pfnDliFailureHook;

  System.___pfnDliFailureHook();
end;

function System___delayLoadHelper_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure __delayLoadHelper;

  System.__delayLoadHelper();
end;

function System___FUnloadDelayLoadedDLL_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure __FUnloadDelayLoadedDLL;

  System.__FUnloadDelayLoadedDLL();
end;

function System__delayLoadHelper_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure _delayLoadHelper;

  System._delayLoadHelper();
end;
...
begin
  ...
  AData.AddConstant('hfFieldSize', System_hfFieldSize_si, nil, VarType(System.hfFieldSize));
  ...
  AData.AddFunction('MonitorWait', System_MonitorWait_si, Boolean_sw, 2, False);
  ...
  AData.AddProcedure('MemoryBarrier', System_MemoryBarrier_si, 0, False);
  ...
  AData.AddFunction('UTF8ToWideString', System_UTF8ToWideString_si, WideString_sw, 1, False);
  ...
  AData.AddFunction('AnsiToUtf8', System_AnsiToUtf8_si, RawByteString_sw, 1, False);
  ...
  AData.AddFunction('_ReadChar', System__ReadChar_si, AnsiChar_sw, 1, False);
  ...
  AData.AddProcedure('_InitUnicodeStrings', System__InitUnicodeStrings_si, 0, False);
  AData.AddProcedure('_UniqueStringU', System__UniqueStringU_si, 1, False);
  AData.AddFunction('Pos', System_Pos_si, nil, 2, False);
  ...
  AData.AddProcedure('_WriteUInt64', System__WriteUInt64_si, 0, False);
  AData.AddProcedure('_Write0UInt64', System__Write0UInt64_si, 0, False);
  ...
  AData.AddFunction('_StrUInt64', System__StrUInt64_si, ShortString_sw, 2, False);
  AData.AddFunction('_Str0UInt64', System__Str0UInt64_si, ShortString_sw, 1, False);
  ...
  AData.AddType(TImageThunkData_sw, System_TImageThunkData_si);
  AData.AddType(ImgDelayDescr_sw, System_ImgDelayDescr_si);
  AData.AddType(TImgDelayDescr_sw, System_TImgDelayDescr_si);
  AData.AddType(dliNotification_sw, System_dliNotification_si);
  AData.AddType(DelayLoadProc_sw, System_DelayLoadProc_si);
  AData.AddType(TDelayLoadProc_sw, System_TDelayLoadProc_si);
  AData.AddType(DelayLoadInfo_sw, System_DelayLoadInfo_si);
  AData.AddType(TDelayLoadInfo_sw, System_TDelayLoadInfo_si);
  AData.AddProcedure('___pfnDliNotifyHook', System____pfnDliNotifyHook_si, 0, False);
  AData.AddProcedure('___pfnDliFailureHook', System____pfnDliFailureHook_si, 0, False);
  AData.AddProcedure('__delayLoadHelper', System___delayLoadHelper_si, 0, False);
  AData.AddProcedure('__FUnloadDelayLoadedDLL', System___FUnloadDelayLoadedDLL_si, 0, False);
  AData.AddProcedure('_delayLoadHelper', System__delayLoadHelper_si, 0, False);
end;
