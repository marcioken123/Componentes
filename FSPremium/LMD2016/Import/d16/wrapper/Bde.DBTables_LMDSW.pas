...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Bde.DBTables' unit.
  #
  # WARNING:
...
uses
  ...
  LMDSctSysWrappers, System.Variants, Winapi.Windows, System.SysUtils,
  System.Classes, Data.DB, Data.DBCommon, Data.DBCommonTypes, Bde.DBTables,
  System.Variants_LMDSW, Winapi.Windows_LMDSW, System.SysUtils_LMDSW,
  System.Classes_LMDSW, Data.DB_LMDSW, Data.DBCommon_LMDSW,
...
type
  ...
  TIndexName_sw = class(string_sw)
  protected
    class function GetTypeName: WideString; override;
  public
    class function ToVar(const AValue: TIndexName): OleVariant;
    class function FromVar(const AValue: OleVariant): TIndexName;
  end;
...
begin
  ...
  AData.AddProperty('ErrorCount', EDBEngineError_ErrorCount_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('Errors', EDBEngineError_Errors_si, TDBError_sw, True, False, 1, False, False);
end;
...
begin
  ...
  AData.AddProperty('Category', TDBError_Category_si, Byte_sw, True, False, 0, False, False);
  AData.AddProperty('SubCode', TDBError_SubCode_si, Byte_sw, True, False, 0, False, False);
  AData.AddProperty('Message', TDBError_Message_si, string_sw, True, False, 0, False, False);
  AData.AddProperty('NativeError', TDBError_NativeError_si, Longint_sw, True, False, 0, False, False);
end;
...
begin
  ...
  AData.AddProperty('CurrentSession', TSessionList_CurrentSession_si, TSession_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('Count', TSessionList_Count_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('Sessions', TSessionList_Sessions_si, TSession_sw, True, False, 1, False, True);
  AData.AddProperty('List', TSessionList_List_si, TSession_sw, True, False, 1, False, False);
...
class procedure TConfigModes_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'cfmVirtual'; Value: Integer({$IFDEF LMDSCT_12}TConfigModes.{$ENDIF}cfmVirtual)),
    (Name: 'cfmPersistent'; Value: Integer({$IFDEF LMDSCT_12}TConfigModes.{$ENDIF}cfmPersistent)),
    (Name: 'cfmSession'; Value: Integer({$IFDEF LMDSCT_12}TConfigModes.{$ENDIF}cfmSession))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
class procedure TDatabaseEvent_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..7] of TLMDEnumerator = (
    (Name: 'dbOpen'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbOpen)),
    (Name: 'dbClose'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbClose)),
    (Name: 'dbAdd'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbAdd)),
    (Name: 'dbRemove'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbRemove)),
    (Name: 'dbAddAlias'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbAddAlias)),
    (Name: 'dbDeleteAlias'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbDeleteAlias)),
    (Name: 'dbAddDriver'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbAddDriver)),
    (Name: 'dbDeleteDriver'; Value: Integer({$IFDEF LMDSCT_12}TDatabaseEvent.{$ENDIF}dbDeleteDriver))
  );
begin
  AEnums := @ENUMS;
  ACount := 8;
...
class procedure TTraceFlag_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..10] of TLMDEnumerator = (
    (Name: 'tfQPrepare'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfQPrepare)),
    (Name: 'tfQExecute'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfQExecute)),
    (Name: 'tfError'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfError)),
    (Name: 'tfStmt'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfStmt)),
    (Name: 'tfConnect'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfConnect)),
    (Name: 'tfTransact'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfTransact)),
    (Name: 'tfBlob'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfBlob)),
    (Name: 'tfMisc'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfMisc)),
    (Name: 'tfVendor'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfVendor)),
    (Name: 'tfDataIn'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfDataIn)),
    (Name: 'tfDataOut'; Value: Integer({$IFDEF LMDSCT_12}TTraceFlag.{$ENDIF}tfDataOut))
  );
begin
  AEnums := @ENUMS;
  ACount := 11;
...
begin
  ...
  AData.AddProperty('BDEOwnsLoginCbDb', TSession_BDEOwnsLoginCbDb_si, Boolean_sw, True, True, 0, True, False);
  ...
  AData.AddProperty('ConfigMode', TSession_ConfigMode_si, TConfigMode_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('DatabaseCount', TSession_DatabaseCount_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('Databases', TSession_Databases_si, TDatabase_sw, True, False, 1, False, False);
  AData.AddProperty('TraceFlags', TSession_TraceFlags_si, TTraceFlags_sw, True, True, 0, False, False);
  AData.AddProperty('Active', TSession_Active_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('AutoSessionName', TSession_AutoSessionName_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('KeepConnections', TSession_KeepConnections_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('NetFileDir', TSession_NetFileDir_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('PrivateDir', TSession_PrivateDir_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('SessionName', TSession_SessionName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('SQLHourGlass', TSession_SQLHourGlass_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('OnPassword', TSession_OnPassword_si, TPasswordEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnStartup', TSession_OnStartup_si, TNotifyEvent_sw, True, True, 0, False, False);
end;
...
begin
  ...
  AData.AddProperty('FieldCount', TParamList_FieldCount_si, Integer_sw, True, False, 0, False, False);
...
class procedure TTransIsolation_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'tiDirtyRead'; Value: Integer({$IFDEF LMDSCT_12}TTransIsolation.{$ENDIF}tiDirtyRead)),
    (Name: 'tiReadCommitted'; Value: Integer({$IFDEF LMDSCT_12}TTransIsolation.{$ENDIF}tiReadCommitted)),
    (Name: 'tiRepeatableRead'; Value: Integer({$IFDEF LMDSCT_12}TTransIsolation.{$ENDIF}tiRepeatableRead))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
begin
  ...
  case AArgsSize of
    1:
    begin
      TDatabase(AObj).GetTableNames(TStrings_sw.FromVar(AArgs[0]));
    end;
    2:
    begin
...
begin
  ...
  AData.AddProcedure('GetTableNames', TDatabase_GetTableNames_si, 1, True);
  ...
  AData.AddProperty('DataSets', TDatabase_DataSets_si, TDBDataSet_sw, True, False, 1, False, False);
  AData.AddProperty('Directory', TDatabase_Directory_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('IsSQLBased', TDatabase_IsSQLBased_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('InTransaction', TDatabase_InTransaction_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('Session', TDatabase_Session_si, TSession_sw, True, False, 0, False, False);
  AData.AddProperty('Temporary', TDatabase_Temporary_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('SessionAlias', TDatabase_SessionAlias_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('TraceFlags', TDatabase_TraceFlags_si, TTraceFlags_sw, True, True, 0, False, False);
  AData.AddProperty('AliasName', TDatabase_AliasName_si, string_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Connected', False);
  AData.AddProperty('DatabaseName', TDatabase_DatabaseName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('DriverName', TDatabase_DriverName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('Exclusive', TDatabase_Exclusive_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('HandleShared', TDatabase_HandleShared_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('KeepConnection', TDatabase_KeepConnection_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('LoginPrompt', False);
  AData.AddProperty('Params', TDatabase_Params_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('ReadOnly', TDatabase_ReadOnly_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('SessionName', TDatabase_SessionName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('TransIsolation', TDatabase_TransIsolation_si, TTransIsolation_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('OnLogin', TDatabase_OnLogin_si, TDatabaseLoginEvent_sw, True, True, 0, False, False);
end;
...
class procedure TRecNoStatus_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'rnDbase'; Value: Integer({$IFDEF LMDSCT_12}TRecNoStatus.{$ENDIF}rnDbase)),
    (Name: 'rnParadox'; Value: Integer({$IFDEF LMDSCT_12}TRecNoStatus.{$ENDIF}rnParadox)),
    (Name: 'rnNotSupported'; Value: Integer({$IFDEF LMDSCT_12}TRecNoStatus.{$ENDIF}rnNotSupported))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
begin
  ...
  AData.AddProperty('DataSet', TDataSetUpdateObject_DataSet_si, TDataSet_sw, True, True, 0, True, False);
...
begin
  ...
  AData.AddFunction('Copy', TBDEKeyBuffer_Copy_si, TBDEKeyBuffer_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TBDEKeyBuffer_Create_si, 0, False);
end;
...
begin
  ...
  AData.AddFunction('Copy', TBDERecInfo_Copy_si, TBDERecInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TBDERecInfo_Create_si, 0, False);
end;
...
begin
  ...
  AData.AddProperty('CacheBlobs', TBDEDataSet_CacheBlobs_si, Boolean_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('ExpIndex', TBDEDataSet_ExpIndex_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('KeySize', TBDEDataSet_KeySize_si, Word_sw, True, False, 0, False, False);
  AData.AddProperty('UpdateObject', TBDEDataSet_UpdateObject_si, TDataSetUpdateObject_sw, True, True, 0, False, False);
  AData.AddProperty('UpdatesPending', TBDEDataSet_UpdatesPending_si, Boolean_sw, True, False, 0, False, False);
  AData.AddPropertyRedecl('Active', False);
  AData.AddPropertyRedecl('AutoCalcFields', False);
  AData.AddProperty('CachedUpdates', TBDEDataSet_CachedUpdates_si, Boolean_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('OnUpdateError', TBDEDataSet_OnUpdateError_si, TUpdateErrorEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnUpdateRecord', TBDEDataSet_OnUpdateRecord_si, TUpdateRecordEvent_sw, True, True, 0, False, False);
end;
...
begin
  ...
  AData.AddProperty('UpdateMode', TDBDataSet_UpdateMode_si, TUpdateMode_sw, True, True, 0, True, False);
  ...
  AData.AddProperty('Database', TDBDataSet_Database_si, TDatabase_sw, True, False, 0, False, False);
  AData.AddProperty('DBSession', TDBDataSet_DBSession_si, TSession_sw, True, False, 0, False, False);
  AData.AddProperty('AutoRefresh', TDBDataSet_AutoRefresh_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('DatabaseName', TDBDataSet_DatabaseName_si, string_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Filter', False);
  AData.AddPropertyRedecl('Filtered', False);
  AData.AddPropertyRedecl('FilterOptions', False);
  AData.AddProperty('SessionName', TDBDataSet_SessionName_si, string_sw, True, True, 0, False, False);
...
class procedure TBatchMode_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..4] of TLMDEnumerator = (
    (Name: 'batAppend'; Value: Integer({$IFDEF LMDSCT_12}TBatchMode.{$ENDIF}batAppend)),
    (Name: 'batUpdate'; Value: Integer({$IFDEF LMDSCT_12}TBatchMode.{$ENDIF}batUpdate)),
    (Name: 'batAppendUpdate'; Value: Integer({$IFDEF LMDSCT_12}TBatchMode.{$ENDIF}batAppendUpdate)),
    (Name: 'batDelete'; Value: Integer({$IFDEF LMDSCT_12}TBatchMode.{$ENDIF}batDelete)),
    (Name: 'batCopy'; Value: Integer({$IFDEF LMDSCT_12}TBatchMode.{$ENDIF}batCopy))
  );
begin
  AEnums := @ENUMS;
  ACount := 5;
...
class procedure TTableType_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..4] of TLMDEnumerator = (
    (Name: 'ttDefault'; Value: Integer({$IFDEF LMDSCT_12}TTableType.{$ENDIF}ttDefault)),
    (Name: 'ttParadox'; Value: Integer({$IFDEF LMDSCT_12}TTableType.{$ENDIF}ttParadox)),
    (Name: 'ttDBase'; Value: Integer({$IFDEF LMDSCT_12}TTableType.{$ENDIF}ttDBase)),
    (Name: 'ttFoxPro'; Value: Integer({$IFDEF LMDSCT_12}TTableType.{$ENDIF}ttFoxPro)),
    (Name: 'ttASCII'; Value: Integer({$IFDEF LMDSCT_12}TTableType.{$ENDIF}ttASCII))
  );
begin
  AEnums := @ENUMS;
  ACount := 5;
...
class procedure TLockType_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..1] of TLMDEnumerator = (
    (Name: 'ltReadLock'; Value: Integer({$IFDEF LMDSCT_12}TLockType.{$ENDIF}ltReadLock)),
    (Name: 'ltWriteLock'; Value: Integer({$IFDEF LMDSCT_12}TLockType.{$ENDIF}ltWriteLock))
  );
begin
  AEnums := @ENUMS;
  ACount := 2;
...
class function TIndexName_sw.GetTypeName: WideString;
begin
  Result := 'TIndexName';
end;

class function TIndexName_sw.ToVar(const AValue: TIndexName): OleVariant;
begin
  Result := string_sw.ToVar(string(AValue));
end;

class function TIndexName_sw.FromVar(const AValue: OleVariant): TIndexName;
begin
  Result := TIndexName(string_sw.FromVar(AValue));
end;
...
begin
  ...
  case AArgsSize of
    3:
    begin
      TTable(AObj).AddIndex(string(AArgs[0]), string(AArgs[1]), 
...
begin
  ...
  AData.AddProperty('MasterLink', TTable_MasterLink_si, TMasterDataLink_sw, True, False, 0, True, False);
  AData.AddFunction('BatchMove', TTable_BatchMove_si, Longint_sw, 2, False);
  AData.AddProcedure('AddIndex', TTable_AddIndex_si, 3, True);
  ...
  AData.AddProperty('Exists', TTable_Exists_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('IndexFieldCount', TTable_IndexFieldCount_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('IndexFields', TTable_IndexFields_si, TField_sw, True, True, 1, False, False);
  AData.AddProperty('KeyExclusive', TTable_KeyExclusive_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('KeyFieldCount', TTable_KeyFieldCount_si, Integer_sw, True, True, 0, False, False);
  AData.AddProperty('TableLevel', TTable_TableLevel_si, Integer_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Constraints', False);
  AData.AddProperty('DefaultIndex', TTable_DefaultIndex_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('Exclusive', TTable_Exclusive_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('FieldDefs', False);
  AData.AddProperty('IndexDefs', TTable_IndexDefs_si, TIndexDefs_sw, True, True, 0, False, False);
  AData.AddProperty('IndexFieldNames', TTable_IndexFieldNames_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('IndexFiles', TTable_IndexFiles_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('IndexName', TTable_IndexName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('MasterFields', TTable_MasterFields_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('MasterSource', TTable_MasterSource_si, TDataSource_sw, True, True, 0, False, False);
  AData.AddProperty('ReadOnly', TTable_ReadOnly_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('StoreDefs', TTable_StoreDefs_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('TableName', TTable_TableName_si, TFileName_sw, True, True, 0, False, False);
  AData.AddProperty('TableType', TTable_TableType_si, TTableType_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('UpdateMode', False);
  AData.AddPropertyRedecl('UpdateObject', False);
  AData.AddProperty('Ranged', TTable_Ranged_si, Boolean_sw, True, False, 0, False, False);
end;
...
begin
  ...
  AData.AddProperty('ChangedCount', TBatchMove_ChangedCount_si, Longint_sw, True, False, 0, False, False);
  AData.AddProperty('KeyViolCount', TBatchMove_KeyViolCount_si, Longint_sw, True, False, 0, False, False);
  AData.AddProperty('MovedCount', TBatchMove_MovedCount_si, Longint_sw, True, False, 0, False, False);
  AData.AddProperty('ProblemCount', TBatchMove_ProblemCount_si, Longint_sw, True, False, 0, False, False);
  AData.AddProperty('AbortOnKeyViol', TBatchMove_AbortOnKeyViol_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('AbortOnProblem', TBatchMove_AbortOnProblem_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('CommitCount', TBatchMove_CommitCount_si, Integer_sw, True, True, 0, False, False);
  AData.AddProperty('ChangedTableName', TBatchMove_ChangedTableName_si, TFileName_sw, True, True, 0, False, False);
  AData.AddProperty('Destination', TBatchMove_Destination_si, TTable_sw, True, True, 0, False, False);
  AData.AddProperty('KeyViolTableName', TBatchMove_KeyViolTableName_si, TFileName_sw, True, True, 0, False, False);
  AData.AddProperty('Mappings', TBatchMove_Mappings_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('Mode', TBatchMove_Mode_si, TBatchMode_sw, True, True, 0, False, False);
  AData.AddProperty('ProblemTableName', TBatchMove_ProblemTableName_si, TFileName_sw, True, True, 0, False, False);
  AData.AddProperty('RecordCount', TBatchMove_RecordCount_si, Longint_sw, True, True, 0, False, False);
  AData.AddProperty('Source', TBatchMove_Source_si, TBDEDataSet_sw, True, True, 0, False, False);
  AData.AddProperty('Transliterate', TBatchMove_Transliterate_si, Boolean_sw, True, True, 0, False, False);
end;
...
class procedure TParamBindMode_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..1] of TLMDEnumerator = (
    (Name: 'pbByName'; Value: Integer({$IFDEF LMDSCT_12}TParamBindMode.{$ENDIF}pbByName)),
    (Name: 'pbByNumber'; Value: Integer({$IFDEF LMDSCT_12}TParamBindMode.{$ENDIF}pbByNumber))
  );
begin
  AEnums := @ENUMS;
  ACount := 2;
...
begin
  ...
  AData.AddFunction('Copy', TServerDesc_Copy_si, TServerDesc_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TServerDesc_Create_si, 0, False);
end;
...
begin
  ...
  AData.AddProperty('ParamCount', TStoredProc_ParamCount_si, Word_sw, True, False, 0, False, False);
  AData.AddPropertyRedecl('StmtHandle', False);
  AData.AddProperty('Prepared', TStoredProc_Prepared_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('StoredProcName', TStoredProc_StoredProcName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('Overload', TStoredProc_Overload_si, Word_sw, True, True, 0, False, False);
  AData.AddProperty('Params', TStoredProc_Params_si, TParams_sw, True, True, 0, False, False);
  AData.AddProperty('ParamBindMode', TStoredProc_ParamBindMode_si, TParamBindMode_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('UpdateObject', False);
end;
...
begin
  ...
  AData.AddProperty('DataLink', TQuery_DataLink_si, TDataLink_sw, True, False, 0, True, False);
  ...
  AData.AddProperty('Prepared', TQuery_Prepared_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('ParamCount', TQuery_ParamCount_si, Word_sw, True, False, 0, False, False);
  AData.AddProperty('Local', TQuery_Local_si, Boolean_sw, True, False, 0, False, False);
  AData.AddPropertyRedecl('StmtHandle', False);
  AData.AddProperty('Text', TQuery_Text_si, string_sw, True, False, 0, False, False);
  AData.AddProperty('RowsAffected', TQuery_RowsAffected_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('Constrained', TQuery_Constrained_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Constraints', False);
  AData.AddProperty('DataSource', TQuery_DataSource_si, TDataSource_sw, True, True, 0, False, False);
  AData.AddProperty('ParamCheck', TQuery_ParamCheck_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('RequestLive', TQuery_RequestLive_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('SQL', TQuery_SQL_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('Params', TQuery_Params_si, TParams_sw, True, True, 0, False, False);
  AData.AddProperty('UniDirectional', TQuery_UniDirectional_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('UpdateMode', False);
  AData.AddPropertyRedecl('UpdateObject', False);
end;
...
function TUpdateSQL_Apply_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function Apply(...); overload;

  TUpdateSQL(AObj).Apply(TDataset_sw.FromVar(AArgs[0]), TUpdateKind_sw.FromVar(
    AArgs[1]));
end;
...
function TUpdateSQL_SetParams_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function SetParams(...); overload;

  case AArgsSize of
    2:
    begin
      TUpdateSQL(AObj).SetParams(TDataset_sw.FromVar(AArgs[0]), TUpdateKind_sw.
        FromVar(AArgs[1]));
    end;
    1:
    begin
      TUpdateSQL(AObj).SetParams(TUpdateKind_sw.FromVar(AArgs[0]));
    end;
  else
    WrongArgCountError('SetParams');
  end;
end;
...
begin
  ...
  AData.AddProcedure('Apply', TUpdateSQL_Apply_si, 2, False);
  AData.AddProcedure('ExecSQL', TUpdateSQL_ExecSQL_si, 1, False);
  AData.AddProcedure('SetParams', TUpdateSQL_SetParams_si, 1, True);
  AData.AddProperty('DatabaseName', TUpdateSQL_DatabaseName_si, string_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('DataSet', False);
  AData.AddProperty('Query', TUpdateSQL_Query_si, TQuery_sw, True, False, 1, False, False);
  AData.AddProperty('SQL', TUpdateSQL_SQL_si, TStrings_sw, True, True, 1, False, False);
  AData.AddProperty('SessionName', TUpdateSQL_SessionName_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('ModifySQL', TUpdateSQL_ModifySQL_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('InsertSQL', TUpdateSQL_InsertSQL_si, TStrings_sw, True, True, 0, False, False);
  AData.AddProperty('DeleteSQL', TUpdateSQL_DeleteSQL_si, TStrings_sw, True, True, 0, False, False);
end;
...
function DBTables_smTraceBufSize_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const smTraceBufSize = <value>;

  Result := smTraceBufSize;
end;

function DBTables_dbfOpened_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfOpened = <value>;

  Result := dbfOpened;
end;

function DBTables_dbfPrepared_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfPrepared = <value>;

  Result := dbfPrepared;
end;

function DBTables_dbfExecSQL_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfExecSQL = <value>;

  Result := dbfExecSQL;
end;

function DBTables_dbfTable_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfTable = <value>;

  Result := dbfTable;
end;

function DBTables_dbfFieldList_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfFieldList = <value>;

  Result := dbfFieldList;
end;

function DBTables_dbfIndexList_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfIndexList = <value>;

  Result := dbfIndexList;
end;

function DBTables_dbfStoredProc_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfStoredProc = <value>;

  Result := dbfStoredProc;
end;

function DBTables_dbfExecProc_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfExecProc = <value>;

  Result := dbfExecProc;
end;

function DBTables_dbfProcDesc_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfProcDesc = <value>;

  Result := dbfProcDesc;
end;

function DBTables_dbfDatabase_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfDatabase = <value>;

  Result := dbfDatabase;
end;

function DBTables_dbfProvider_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dbfProvider = <value>;
...
function DBTables_TConfigModes_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TConfigModes = (cfmVirtual, cfmPersistent, cfmSession);
...
function DBTables_TDatabaseEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDatabaseEvent = (dbOpen, dbClose, dbAdd, dbRemove, ...
...
function DBTables_TTraceFlag_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TTraceFlag = (tfQPrepare, tfQExecute, tfError, tfStm...
...
function DBTables_TTransIsolation_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TTransIsolation = (tiDirtyRead, tiReadCommitted, tiR...
...
function DBTables_TRecNoStatus_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TRecNoStatus = (rnDbase, rnParadox, rnNotSupported);
...
function DBTables_TTableType_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TTableType = (ttDefault, ttParadox, ttDBase, ttFoxPr...
...
function DBTables_cmVirtual_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const cmVirtual = <value>;

  Result := LMDSetToVar(SizeOf(cmVirtual), cmVirtual);
end;

function DBTables_cmPersistent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const cmPersistent = <value>;

  Result := LMDSetToVar(SizeOf(cmPersistent), cmPersistent);
end;

function DBTables_cmSession_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const cmSession = <value>;

  Result := LMDSetToVar(SizeOf(cmSession), cmSession);
end;

function DBTables_cmAll_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const cmAll = <value>;

  Result := LMDSetToVar(SizeOf(cmAll), cmAll);
end;
...
begin
  ...
  if IsGet then
    Result := TSession_sw.ToVar(Bde.DBTables.Session)
  else
    Bde.DBTables.Session := TSession_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := TSessionList_sw.ToVar(Bde.DBTables.Sessions)
  else
    Bde.DBTables.Sessions := TSessionList_sw.FromVar(AArgs[0]);
end;
...
begin
  AData.AddConstant('smTraceBufSize', DBTables_smTraceBufSize_si, nil, VarType(Bde.DBTables.smTraceBufSize));
  AData.AddConstant('dbfOpened', DBTables_dbfOpened_si, nil, VarType(Bde.DBTables.dbfOpened));
  AData.AddConstant('dbfPrepared', DBTables_dbfPrepared_si, nil, VarType(Bde.DBTables.dbfPrepared));
  AData.AddConstant('dbfExecSQL', DBTables_dbfExecSQL_si, nil, VarType(Bde.DBTables.dbfExecSQL));
  AData.AddConstant('dbfTable', DBTables_dbfTable_si, nil, VarType(Bde.DBTables.dbfTable));
  AData.AddConstant('dbfFieldList', DBTables_dbfFieldList_si, nil, VarType(Bde.DBTables.dbfFieldList));
  AData.AddConstant('dbfIndexList', DBTables_dbfIndexList_si, nil, VarType(Bde.DBTables.dbfIndexList));
  AData.AddConstant('dbfStoredProc', DBTables_dbfStoredProc_si, nil, VarType(Bde.DBTables.dbfStoredProc));
  AData.AddConstant('dbfExecProc', DBTables_dbfExecProc_si, nil, VarType(Bde.DBTables.dbfExecProc));
  AData.AddConstant('dbfProcDesc', DBTables_dbfProcDesc_si, nil, VarType(Bde.DBTables.dbfProcDesc));
  AData.AddConstant('dbfDatabase', DBTables_dbfDatabase_si, nil, VarType(Bde.DBTables.dbfDatabase));
  AData.AddConstant('dbfProvider', DBTables_dbfProvider_si, nil, VarType(Bde.DBTables.dbfProvider));
  ...
  AData.AddConstant('cmVirtual', DBTables_cmVirtual_si, nil, varInteger);
  AData.AddConstant('cmPersistent', DBTables_cmPersistent_si, nil, varInteger);
  AData.AddConstant('cmSession', DBTables_cmSession_si, nil, varInteger);
  AData.AddConstant('cmAll', DBTables_cmAll_si, nil, varInteger);
