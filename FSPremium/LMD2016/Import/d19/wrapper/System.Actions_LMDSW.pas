...
type
  ...
  TEnumActionListEvent_sh = class(TLMDScriptEventHandler)
  public
    function  GetHandler: TMethod; override;
    procedure Handler(const Action: TContainedAction; var Done: boolean);
  end;

  TEnumActionListEvent_sw = class(TLMDEventWrapper)
  protected
    class function GetTypeName: WideString; override;
  public
    class function ToVar(const AValue: TEnumActionListEvent): OleVariant;
    class function FromVar(const AValue: OleVariant): TEnumActionListEvent;
    class function GetScriptHandlerClass: TClass; override;
    class function GetHandler(AScriptControl: TLMDScriptControl;
                              const AProcName: string): TEnumActionListEvent;
  end;
...
begin
  ...
  Result := TCustomShortCutList(AObj).IndexOfShortCut(System.Classes_LMDSW.
    TShortCut_sw.FromVar(AArgs[0]));
end;
...
{ TEnumActionListEvent_sh }

function TEnumActionListEvent_sh.GetHandler: TMethod;
var
  hdr: TEnumActionListEvent;
begin
  hdr    := Handler;
  Result := TMethod(hdr);
end;

procedure TEnumActionListEvent_sh.Handler(const Action: TContainedAction;
  var Done: boolean);
var
  args: array[0..1] of OleVariant;
  v_1: OleVariant;
begin
  // procedure (const Action: TContainedAction; var Done: bool...

  v_1 := Done;
  args[0] := TContainedAction_sw.ToVar(Action);
  args[1] := MakeVarRef(v_1);
  Self.Owner.RunProc(Self.ProcName, args);
  Done := boolean(v_1);
end;

{ TEnumActionListEvent_sw }

class function TEnumActionListEvent_sw.GetTypeName: WideString;
begin
  Result := 'TEnumActionListEvent';
end;

class function TEnumActionListEvent_sw.GetScriptHandlerClass: TClass;
begin
  Result := TEnumActionListEvent_sh;
end;

class function TEnumActionListEvent_sw.ToVar(
  const AValue: TEnumActionListEvent): OleVariant;
begin
  Result := ConvToVar(TMethod(AValue));
end;

class function TEnumActionListEvent_sw.FromVar(
  const AValue: OleVariant): TEnumActionListEvent;
begin
  Result := TEnumActionListEvent(ConvFromVar(AValue));
end;

class function TEnumActionListEvent_sw.GetHandler(
  AScriptControl: TLMDScriptControl;
  const AProcName: string): TEnumActionListEvent;
var
  hdlr: TEnumActionListEvent_sh;
begin
  hdlr   := TEnumActionListEvent_sh(AScriptControl.GetEventHandler(AProcName, 
    TEnumActionListEvent_sw));
  Result := hdlr.Handler;
end;
...
function TContainedActionList_EnumByCategory_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function EnumByCategory(...); overload;

  case AArgsSize of
    2:
    begin
      Result := TContainedActionList(AObj).EnumByCategory(
        TEnumActionListEvent_sw.FromVar(AArgs[0]), string(AArgs[1]));
    end;
    3:
    begin
      Result := TContainedActionList(AObj).EnumByCategory(
        TEnumActionListEvent_sw.FromVar(AArgs[0]), string(AArgs[1]), Boolean(
        AArgs[2]));
    end;
  else
    WrongArgCountError('EnumByCategory');
  end;
end;
...
begin
  ...
  AData.AddFunction('EnumByCategory', TContainedActionList_EnumByCategory_si, nil, 2, True);
...
function Actions_TEnumActionListEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TEnumActionListEvent = procedure (const Action: TCon...

  Result := TLMDUnitWrapper.TypeToVar(TEnumActionListEvent_sw);
end;
...
begin
  ...
  AData.AddType(TEnumActionListEvent_sw, Actions_TEnumActionListEvent_si);
...
initialization
  ...
  RegisterEventWrapper(TypeInfo(TEnumActionListEvent), TEnumActionListEvent_sw);
