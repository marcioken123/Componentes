...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Vcl.Controls' unit.
  #
  # WARNING:
...
uses
  System.Variants, System.TypInfo, LMDTypes, LMDSctScripter, LMDSctWrappers,
  LMDSctSysWrappers, Winapi.Messages, Winapi.Windows, Winapi.MultiMon,
  System.Classes, System.SysUtils, Vcl.Graphics, Vcl.Menus, Winapi.CommCtrl,
  Winapi.Imm, Vcl.ImgList, Vcl.ActnList, System.Types, System.UITypes,
  Vcl.Controls, Winapi.Messages_LMDSW, Winapi.Windows_LMDSW,
  Winapi.MultiMon_LMDSW, System.Classes_LMDSW, System.SysUtils_LMDSW,
  Vcl.Graphics_LMDSW, Vcl.Menus_LMDSW, Winapi.CommCtrl_LMDSW, Winapi.Imm_LMDSW,
  Vcl.ImgList_LMDSW, Vcl.ActnList_LMDSW, System.Types_LMDSW,
...
type
  ...
  TModalResult_sw = System.UITypes_LMDSW.TModalResult_sw;

  TCursor_sw = System.UITypes_LMDSW.TCursor_sw;
  ...
  TCustomLongData_sw = NativeInt_sw;
  ...
  TMouseButton_sw = System.UITypes_LMDSW.TMouseButton_sw;

  TMouseActivate_sw = System.UITypes_LMDSW.TMouseActivate_sw;

  TDragMode_sw = System.UITypes_LMDSW.TDragMode_sw;

  TDragState_sw = System.UITypes_LMDSW.TDragState_sw;

  TDragKind_sw = System.UITypes_LMDSW.TDragKind_sw;

  TTabOrder_sw = System.UITypes_LMDSW.TTabOrder_sw;
  ...
  TAnchorKind_sw = System.UITypes_LMDSW.TAnchorKind_sw;

  TAnchors_sw = System.UITypes_LMDSW.TAnchors_sw;
  ...
  TWndMethod_sw = System.Classes_LMDSW.TWndMethod_sw;
  ...
  TTipMode_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TTipMode): OleVariant;
    class function FromVar(const AValue: OleVariant): TTipMode;
  end;
...
function TCMMouseWheel_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMMouseWheel_sw(AObj).FValue.MsgFiller)
  else
    TCMMouseWheel_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMMouseWheel_ShiftStateWheel_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ShiftStateWheel: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMMouseWheel_sw(AObj).FValue.
      ShiftStateWheel)
  else
    TCMMouseWheel_sw(AObj).FValue.ShiftStateWheel := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMMouseWheel_XYPos_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var XYPos: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMMouseWheel_sw(AObj).FValue.XYPos)
  else
    TCMMouseWheel_sw(AObj).FValue.XYPos := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
function TCMMouseWheel_PosFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var PosFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMMouseWheel_sw(AObj).FValue.PosFiller)
  else
    TCMMouseWheel_sw(AObj).FValue.PosFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMMouseWheel_sw(AObj).FValue.Result)
  else
    TCMMouseWheel_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMMouseWheel_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('WheelDelta', TCMMouseWheel_WheelDelta_si, SmallInt_sw);
  AData.AddField('ShiftStateWheel', TCMMouseWheel_ShiftStateWheel_si, TDWordFiller_sw);
  AData.AddField('XPos', TCMMouseWheel_XPos_si, Smallint_sw);
  AData.AddField('YPos', TCMMouseWheel_YPos_si, Smallint_sw);
  AData.AddField('XYPos', TCMMouseWheel_XYPos_si, TDWordFiller_sw);
  AData.AddField('Pos', TCMMouseWheel_Pos_si, TSmallPoint_sw);
  AData.AddField('PosFiller', TCMMouseWheel_PosFiller_si, TDWordFiller_sw);
  AData.AddField('Result', TCMMouseWheel_Result_si, LRESULT_sw);
...
function TCMCancelMode_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMCancelMode_sw(AObj).FValue.MsgFiller)
  else
    TCMCancelMode_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := WPARAM_sw.ToVar(TCMCancelMode_sw(AObj).FValue.Unused)
  else
    TCMCancelMode_sw(AObj).FValue.Unused := WPARAM_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMCancelMode_sw(AObj).FValue.Result)
  else
    TCMCancelMode_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMCancelMode_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Unused', TCMCancelMode_Unused_si, WPARAM_sw);
  AData.AddField('Sender', TCMCancelMode_Sender_si, TControl_sw);
  AData.AddField('Result', TCMCancelMode_Result_si, LRESULT_sw);
...
function TCMFocusChanged_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMFocusChanged_sw(AObj).FValue.MsgFiller)
  else
    TCMFocusChanged_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := WPARAM_sw.ToVar(TCMFocusChanged_sw(AObj).FValue.Unused)
  else
    TCMFocusChanged_sw(AObj).FValue.Unused := WPARAM_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMFocusChanged_sw(AObj).FValue.Result)
  else
    TCMFocusChanged_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMFocusChanged_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Unused', TCMFocusChanged_Unused_si, WPARAM_sw);
  AData.AddField('Sender', TCMFocusChanged_Sender_si, TWinControl_sw);
  AData.AddField('Result', TCMFocusChanged_Result_si, LRESULT_sw);
...
function TCMControlListChange_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMControlListChange_sw(AObj).FValue.
      MsgFiller)
  else
    TCMControlListChange_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMControlListChange_InsertingFiller_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var InsertingFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMControlListChange_sw(AObj).FValue.
      InsertingFiller)
  else
    TCMControlListChange_sw(AObj).FValue.InsertingFiller := TDWordFiller_sw.
      FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMControlListChange_sw(AObj).FValue.Result)
  else
    TCMControlListChange_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMControlListChange_MsgFiller_si, TDWordFiller_sw);
  ...
  AData.AddField('InsertingFiller', TCMControlListChange_InsertingFiller_si, TDWordFiller_sw);
  AData.AddField('Result', TCMControlListChange_Result_si, LRESULT_sw);
...
function TCMControlListChanging_MsgFiller_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMControlListChanging_sw(AObj).FValue.
      MsgFiller)
  else
    TCMControlListChanging_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMControlListChanging_InsertingFiller_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var InsertingFiller: TDWordFiller;
  ...
  if IsGet then
    ...
    TCMControlListChanging_sw(AObj).FValue.InsertingFiller := TDWordFiller_sw.
      FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMControlListChanging_sw(AObj).FValue.Result)
  else
    TCMControlListChanging_sw(AObj).FValue.Result := LRESULT_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMControlListChanging_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Inserting', TCMControlListChanging_Inserting_si, LongBool_sw);
  AData.AddField('InsertingFiller', TCMControlListChanging_InsertingFiller_si, TDWordFiller_sw);
  AData.AddField('Result', TCMControlListChanging_Result_si, LRESULT_sw);
...
function TCMChildKey_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMChildKey_sw(AObj).FValue.MsgFiller)
  else
    TCMChildKey_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
function TCMChildKey_CharCodeFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var CharCodeFiller: TWordFiller;

  if IsGet then
    Result := TWordFiller_sw.ToVar(TCMChildKey_sw(AObj).FValue.CharCodeFiller)
  else
    TCMChildKey_sw(AObj).FValue.CharCodeFiller := TWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMChildKey_sw(AObj).FValue.Result)
  else
    TCMChildKey_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMChildKey_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('CharCode', TCMChildKey_CharCode_si, Word_sw);
  AData.AddField('CharCodeFiller', TCMChildKey_CharCodeFiller_si, TWordFiller_sw);
  AData.AddField('Sender', TCMChildKey_Sender_si, TWinControl_sw);
  AData.AddField('Result', TCMChildKey_Result_si, LRESULT_sw);
...
function TCMControlChange_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMControlChange_sw(AObj).FValue.MsgFiller)
  else
    TCMControlChange_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMControlChange_InsertingFiller_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var InsertingFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMControlChange_sw(AObj).FValue.
      InsertingFiller)
  else
    TCMControlChange_sw(AObj).FValue.InsertingFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMControlChange_sw(AObj).FValue.Result)
  else
    TCMControlChange_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMControlChange_MsgFiller_si, TDWordFiller_sw);
  ...
  AData.AddField('InsertingFiller', TCMControlChange_InsertingFiller_si, TDWordFiller_sw);
  AData.AddField('Result', TCMControlChange_Result_si, LRESULT_sw);
...
function TCMChanged_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMChanged_sw(AObj).FValue.MsgFiller)
  else
    TCMChanged_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
function TCMChanged_UnusedFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var UnusedFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMChanged_sw(AObj).FValue.UnusedFiller)
  else
    TCMChanged_sw(AObj).FValue.UnusedFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMChanged_sw(AObj).FValue.Result)
  else
    TCMChanged_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMChanged_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Unused', TCMChanged_Unused_si, Longint_sw);
  AData.AddField('UnusedFiller', TCMChanged_UnusedFiller_si, TDWordFiller_sw);
  AData.AddField('Child', TCMChanged_Child_si, TControl_sw);
  AData.AddField('Result', TCMChanged_Result_si, LRESULT_sw);
...
function TCMHintShow_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMHintShow_sw(AObj).FValue.MsgFiller)
  else
    TCMHintShow_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := NativeInt_sw.ToVar(TCMHintShow_sw(AObj).FValue.Reserved)
  else
    TCMHintShow_sw(AObj).FValue.Reserved := NativeInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMHintShow_sw(AObj).FValue.Result)
  else
    TCMHintShow_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMHintShow_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Reserved', TCMHintShow_Reserved_si, NativeInt_sw);
  AData.AddField('Result', TCMHintShow_Result_si, LRESULT_sw);
...
function TCMParentFontChanged_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMParentFontChanged_sw(AObj).FValue.
      MsgFiller)
  else
    TCMParentFontChanged_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMParentFontChanged_MsgFiller_si, TDWordFiller_sw);
...
function TCMDrag_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMDrag_sw(AObj).FValue.MsgFiller)
  else
    TCMDrag_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
function TCMDrag_Unused1_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Unused1: Byte;

  if IsGet then
    Result := TCMDrag_sw(AObj).FValue.Unused1
  else
    TCMDrag_sw(AObj).FValue.Unused1 := Byte(AArgs[0]);
end;

function TCMDrag_Unused2_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Unused2: TWordFiller;

  if IsGet then
    Result := TWordFiller_sw.ToVar(TCMDrag_sw(AObj).FValue.Unused2)
  else
    TCMDrag_sw(AObj).FValue.Unused2 := TWordFiller_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMDrag_sw(AObj).FValue.Result)
  else
    TCMDrag_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMDrag_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('DragMessage', TCMDrag_DragMessage_si, TDragMessage_sw);
  AData.AddField('Unused1', TCMDrag_Unused1_si, Byte_sw);
  AData.AddField('Unused2', TCMDrag_Unused2_si, TWordFiller_sw);
  AData.AddField('Result', TCMDrag_Result_si, LRESULT_sw);
...
function TDockNotifyRec_ClientMsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ClientMsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TDockNotifyRec_sw(AObj).FValue.
      ClientMsgFiller)
  else
    TDockNotifyRec_sw(AObj).FValue.ClientMsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddField('ClientMsgFiller', TDockNotifyRec_ClientMsgFiller_si, TDWordFiller_sw);
...
function TCMDockClient_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMDockClient_sw(AObj).FValue.MsgFiller)
  else
    TCMDockClient_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMDockClient_MousePosFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MousePosFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMDockClient_sw(AObj).FValue.
      MousePosFiller)
  else
    TCMDockClient_sw(AObj).FValue.MousePosFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMDockClient_sw(AObj).FValue.Result)
  else
    TCMDockClient_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMDockClient_MsgFiller_si, TDWordFiller_sw);
  ...
  AData.AddField('MousePosFiller', TCMDockClient_MousePosFiller_si, TDWordFiller_sw);
  AData.AddField('Result', TCMDockClient_Result_si, LRESULT_sw);
...
function TCMUnDockClient_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMUnDockClient_sw(AObj).FValue.MsgFiller)
  else
    TCMUnDockClient_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMUnDockClient_sw(AObj).FValue.Result)
  else
    TCMUnDockClient_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMUnDockClient_MsgFiller_si, TDWordFiller_sw);
  ...
  AData.AddField('Result', TCMUnDockClient_Result_si, LRESULT_sw);
...
function TCMFloat_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMFloat_sw(AObj).FValue.MsgFiller)
  else
    TCMFloat_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := WPARAM_sw.ToVar(TCMFloat_sw(AObj).FValue.Reserved)
  else
    TCMFloat_sw(AObj).FValue.Reserved := WPARAM_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMFloat_sw(AObj).FValue.Result)
  else
    TCMFloat_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMFloat_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Reserved', TCMFloat_Reserved_si, WPARAM_sw);
  AData.AddField('DockSource', TCMFloat_DockSource_si, TDragDockObject_sw);
  AData.AddField('Result', TCMFloat_Result_si, LRESULT_sw);
...
function TCMDockNotification_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMDockNotification_sw(AObj).FValue.
      MsgFiller)
  else
    TCMDockNotification_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMDockNotification_sw(AObj).FValue.Result)
  else
    TCMDockNotification_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMDockNotification_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Client', TCMDockNotification_Client_si, TControl_sw);
  AData.AddField('Result', TCMDockNotification_Result_si, LRESULT_sw);
...
function TCMPopupHWndDestroy_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMPopupHWndDestroy_sw(AObj).FValue.
      MsgFiller)
  else
    TCMPopupHWndDestroy_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMPopupHWndDestroy_sw(AObj).FValue.Result)
  else
    TCMPopupHWndDestroy_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMPopupHWndDestroy_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('PopupControlWnd', TCMPopupHWndDestroy_PopupControlWnd_si, HWND_sw);
  AData.AddField('Result', TCMPopupHWndDestroy_Result_si, LRESULT_sw);
...
function TCMCreatePopup_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMCreatePopup_sw(AObj).FValue.MsgFiller)
  else
    TCMCreatePopup_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
function TCMCreatePopup_PopupUDFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var PopupUDFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMCreatePopup_sw(AObj).FValue.
      PopupUDFiller)
  else
    TCMCreatePopup_sw(AObj).FValue.PopupUDFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMCreatePopup_sw(AObj).FValue.Result)
  else
    TCMCreatePopup_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMCreatePopup_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('PopupID', TCMCreatePopup_PopupID_si, Integer_sw);
  AData.AddField('PopupUDFiller', TCMCreatePopup_PopupUDFiller_si, TDWordFiller_sw);
  AData.AddField('OwnerWnd', TCMCreatePopup_OwnerWnd_si, HWND_sw);
  AData.AddField('Result', TCMCreatePopup_Result_si, LRESULT_sw);
...
function TCMMouseActivate_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMMouseActivate_sw(AObj).FValue.MsgFiller)
  else
    TCMMouseActivate_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := WPARAM_sw.ToVar(TCMMouseActivate_sw(AObj).FValue.Reserved)
  else
    TCMMouseActivate_sw(AObj).FValue.Reserved := WPARAM_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMMouseActivate_sw(AObj).FValue.Result)
  else
    TCMMouseActivate_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMMouseActivate_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Reserved', TCMMouseActivate_Reserved_si, WPARAM_sw);
  AData.AddField('Result', TCMMouseActivate_Result_si, LRESULT_sw);
...
function TCMGesture_MsgFiller_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var MsgFiller: TDWordFiller;

  if IsGet then
    Result := TDWordFiller_sw.ToVar(TCMGesture_sw(AObj).FValue.MsgFiller)
  else
    TCMGesture_sw(AObj).FValue.MsgFiller := TDWordFiller_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := WPARAM_sw.ToVar(TCMGesture_sw(AObj).FValue.Reserved)
  else
    TCMGesture_sw(AObj).FValue.Reserved := WPARAM_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := LRESULT_sw.ToVar(TCMGesture_sw(AObj).FValue.Result)
  else
    TCMGesture_sw(AObj).FValue.Result := LRESULT_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('MsgFiller', TCMGesture_MsgFiller_si, TDWordFiller_sw);
  AData.AddField('Reserved', TCMGesture_Reserved_si, WPARAM_sw);
  AData.AddField('Result', TCMGesture_Result_si, LRESULT_sw);
...
class function TTipMode_sw.GetTypeName: WideString;
begin
  Result := 'TTipMode';
end;

class procedure TTipMode_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'tipClose'; Value: Integer({$IFDEF LMDSCT_12}TTipMode.{$ENDIF}tipClose)),
    (Name: 'tipOpen'; Value: Integer({$IFDEF LMDSCT_12}TTipMode.{$ENDIF}tipOpen)),
    (Name: 'tipDontCare'; Value: Integer({$IFDEF LMDSCT_12}TTipMode.{$ENDIF}tipDontCare))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
class function TTipMode_sw.ToVar(const AValue: TTipMode): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TTipMode_sw.FromVar(const AValue: OleVariant): TTipMode;
begin
  Result := TTipMode(Integer(AValue));
end;
...
function TWinControl_TipMode_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property TipMode: TTipMode read <getter> write ...

  if IsGet then
    Result := TTipMode_sw.ToVar(TWinControl_sacc(TWinControl(AObj)).TipMode)
  else
    TWinControl_sacc(TWinControl(AObj)).TipMode := TTipMode_sw.FromVar(
      AArgs[0]);
end;
...
function TWinControl_GetTabControlList_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure GetTabControlList(List: TList);

  TWinControl(AObj).GetTabControlList(TList_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  AData.AddProperty('TipMode', TWinControl_TipMode_si, TTipMode_sw, True, True, 0, True, False);
  ...
  AData.AddProcedure('GetTabControlList', TWinControl_GetTabControlList_si, 1, False);
...
begin
  ...
  AData.AddPropertyRedecl('GrayscaleFactor', False);
...
function Controls_CM_CONTROLLISTCHANGING_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const CM_CONTROLLISTCHANGING = <value>;
...
function Controls_CM_STYLECHANGED_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const CM_STYLECHANGED = <value>;

  Result := CM_STYLECHANGED;
end;
...
function Controls_CM_CUSTOMSTYLECHANGED_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const CM_CUSTOMSTYLECHANGED = <value>;

  Result := CM_CUSTOMSTYLECHANGED;
end;
...
function Controls_akLeft_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const akLeft = <value>;

  Result := TAnchorKind_sw.ToVar(akLeft);
end;

function Controls_akTop_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const akTop = <value>;

  Result := TAnchorKind_sw.ToVar(akTop);
end;

function Controls_akRight_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const akRight = <value>;

  Result := TAnchorKind_sw.ToVar(akRight);
end;

function Controls_akBottom_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const akBottom = <value>;

  Result := TAnchorKind_sw.ToVar(akBottom);
end;
...
begin
  ...
  Result := Vcl.Controls.IsPositiveResult(TModalResult_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.IsNegativeResult(TModalResult_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.IsAbortResult(TModalResult_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.IsAnAllResult(TModalResult_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := TModalResult_sw.ToVar(Vcl.Controls.StripAllFromResult(
...
function Controls_TCursor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TCursor = System.UITypes.TCursor;
...
function Controls_dmAutomatic_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dmAutomatic = <value>;

  Result := TDragMode_sw.ToVar(dmAutomatic);
end;

function Controls_dmManual_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dmManual = <value>;

  Result := TDragMode_sw.ToVar(dmManual);
end;

function Controls_dsDragEnter_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dsDragEnter = <value>;

  Result := TDragState_sw.ToVar(dsDragEnter);
end;

function Controls_dsDragLeave_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dsDragLeave = <value>;

  Result := TDragState_sw.ToVar(dsDragLeave);
end;

function Controls_dsDragMove_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dsDragMove = <value>;

  Result := TDragState_sw.ToVar(dsDragMove);
end;

function Controls_dkDrag_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dkDrag = <value>;

  Result := TDragKind_sw.ToVar(dkDrag);
end;

function Controls_dkDock_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const dkDock = <value>;

  Result := TDragKind_sw.ToVar(dkDock);
end;

function Controls_maDefault_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const maDefault = <value>;

  Result := TMouseActivate_sw.ToVar(maDefault);
end;

function Controls_maActivate_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const maActivate = <value>;

  Result := TMouseActivate_sw.ToVar(maActivate);
end;

function Controls_maActivateAndEat_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const maActivateAndEat = <value>;

  Result := TMouseActivate_sw.ToVar(maActivateAndEat);
end;

function Controls_maNoActivate_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const maNoActivate = <value>;

  Result := TMouseActivate_sw.ToVar(maNoActivate);
end;

function Controls_maNoActivateAndEat_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const maNoActivateAndEat = <value>;

  Result := TMouseActivate_sw.ToVar(maNoActivateAndEat);
end;

function Controls_mbLeft_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const mbLeft = <value>;

  Result := TMouseButton_sw.ToVar(mbLeft);
end;

function Controls_mbRight_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const mbRight = <value>;

  Result := TMouseButton_sw.ToVar(mbRight);
end;

function Controls_mbMiddle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const mbMiddle = <value>;

  Result := TMouseButton_sw.ToVar(mbMiddle);
end;
...
function Controls_TCustomLongData_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TCustomLongData = NativeInt;
...
function Controls_TMouseButton_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TMouseButton = System.UITypes.TMouseButton;
...
function Controls_TMouseActivate_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TMouseActivate = System.UITypes.TMouseActivate;
...
function Controls_TDragMode_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDragMode = System.UITypes.TDragMode;
...
function Controls_TDragState_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDragState = System.UITypes.TDragState;
...
function Controls_TDragKind_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDragKind = System.UITypes.TDragKind;
...
function Controls_TTabOrder_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TTabOrder = System.UITypes.TTabOrder;
...
function Controls_TAnchorKind_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TAnchorKind = System.UITypes.TAnchorKind;
...
function Controls_TAnchors_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TAnchors = System.UITypes.TAnchors;
...
function Controls_TWndMethod_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWndMethod = System.Classes.TWndMethod;
...
function Controls_TTipMode_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TTipMode = (tipClose, tipOpen, tipDontCare);

  Result := TLMDUnitWrapper.TypeToVar(TTipMode_sw);
end;
...
begin
  ...
  if IsGet then
    Result := TMouse_sw.ToVar(Vcl.Controls.Mouse)
  else
    Vcl.Controls.Mouse := TMouse_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  Result := Vcl.Controls.IsDragObject(TObject_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.IsVCLControl(HWND_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := TWinControl_sw.ToVar(Vcl.Controls.FindControl(HWND_sw.FromVar(
    AArgs[0])));
end;
...
begin
  ...
  Result := TWinControl_sw.ToVar(Vcl.Controls.FindVCLWindow(TPoint_sw.FromVar(
    AArgs[0])));
end;
...
begin
  ...
  Result := TControl_sw.ToVar(Vcl.Controls.FindDragTarget(TPoint_sw.FromVar(
...
begin
  ...
  Result := TControl_sw.ToVar(Vcl.Controls.GetCaptureControl());
end;
...
begin
  ...
  Vcl.Controls.SetCaptureControl(TControl_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.CursorToString(TCursor_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := TCursor_sw.ToVar(Vcl.Controls.StringToCursor(string(AArgs[0])));
end;
...
begin
  ...
  Vcl.Controls.GetCursorValues(TGetStrProc_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  v_1 := string(AArgs[1]);
  Result := Vcl.Controls.CursorToIdent(Longint(AArgs[0]), v_1);
...
begin
  ...
  v_1 := Longint(AArgs[1]);
  Result := Vcl.Controls.IdentToCursor(string(AArgs[0]), v_1);
  AssignRefParam(AArgs[1], v_1);
end;
...
begin
  ...
  Result := Vcl.Controls.GetShortHint(string(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.GetLongHint(string(AArgs[0]));
end;
...
begin
  ...
  Vcl.Controls.PerformEraseBackground(TControl_sw.FromVar(AArgs[0]), HDC_sw.
    FromVar(AArgs[1]));
end;
...
begin
  ...
  Vcl.Controls.PerformBufferedPrintClient(HWND_sw.FromVar(AArgs[0]), TRect_sw.
    FromVar(AArgs[1]));
end;
...
begin
  ...
  Result := LRESULT_sw.ToVar(Vcl.Controls.InitWndProc(HWND_sw.FromVar(AArgs[0]),
...
begin
  ...
  if IsGet then
    Result := TDockTreeClass_sw.ToVar(Vcl.Controls.DefaultDockTreeClass)
  else
    Vcl.Controls.DefaultDockTreeClass := TDockTreeClass_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := TWinControl_sw.ToVar(Vcl.Controls.CreationControl)
  else
    Vcl.Controls.CreationControl := TWinControl_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := Vcl.Controls.NewStyleControls
  else
    Vcl.Controls.NewStyleControls := Boolean(AArgs[0]);
end;
...
begin
  ...
  Vcl.Controls.ChangeBiDiModeAlignment(v_1);
...
function Controls_SendAppMessage_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function SendAppMessage(Msg: Cardinal; WParam: WPARAM; LP...

  Result := LRESULT_sw.ToVar(Vcl.Controls.SendAppMessage(Cardinal(AArgs[0]), 
    WPARAM_sw.FromVar(AArgs[1]), LPARAM_sw.FromVar(AArgs[2])));
end;
...
begin
  ...
  Vcl.Controls.MoveWindowOrg(HDC_sw.FromVar(AArgs[0]), Integer(AArgs[1]), 
    Integer(AArgs[2]));
end;

function Controls_SetTextInputPanelStatus_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure SetTextInputPanelStatus(Control: TWinControl; O...

  Vcl.Controls.SetTextInputPanelStatus(TWinControl_sw.FromVar(AArgs[0]), 
    Boolean(AArgs[1]));
end;
...
begin
  ...
  Vcl.Controls.SetImeMode(HWND_sw.FromVar(AArgs[0]), TImeMode_sw.FromVar(
    AArgs[1]));
end;
...
begin
  ...
  Vcl.Controls.SetImeName(TImeName_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.Win32NLSEnableIME(HWND_sw.FromVar(AArgs[0]), Boolean(
    AArgs[1]));
end;
...
begin
  ...
  Result := HIMC_sw.ToVar(Vcl.Controls.Imm32GetContext(HWND_sw.FromVar(
    AArgs[0])));
end;
...
begin
  ...
  Result := Vcl.Controls.Imm32ReleaseContext(HWND_sw.FromVar(AArgs[0]), HIMC_sw.
    FromVar(AArgs[1]));
end;
...
begin
  ...
  Result := Vcl.Controls.Imm32GetConversionStatus(HIMC_sw.FromVar(AArgs[0]), 
    v_1, v_2);
...
begin
  ...
  Result := Vcl.Controls.Imm32SetConversionStatus(HIMC_sw.FromVar(AArgs[0]), 
...
begin
  ...
  Result := Vcl.Controls.Imm32SetOpenStatus(HIMC_sw.FromVar(AArgs[0]), Boolean(
    AArgs[1]));
end;
...
begin
  ...
  Result := Vcl.Controls.Imm32SetCompositionWindow(HIMC_sw.FromVar(AArgs[0]), 
...
begin
  ...
  Result := Vcl.Controls.Imm32SetCompositionFont(HIMC_sw.FromVar(AArgs[0]), 
    v_1);
...
begin
  ...
  Result := Vcl.Controls.Imm32IsIME(HKL_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Controls.Imm32NotifyIME(HIMC_sw.FromVar(AArgs[0]), DWORD_sw.
    FromVar(AArgs[1]), DWORD_sw.FromVar(AArgs[2]), DWORD_sw.FromVar(AArgs[3]));
end;
...
begin
  ...
  Vcl.Controls.DragDone(Boolean(AArgs[0]));
end;
...
begin
  AData.AddConstant('CM_BASE', Controls_CM_BASE_si, nil, VarType(Vcl.Controls.CM_BASE));
  AData.AddConstant('CM_CLROFFSET', Controls_CM_CLROFFSET_si, nil, VarType(Vcl.Controls.CM_CLROFFSET));
  AData.AddConstant('CM_ACTIVATE', Controls_CM_ACTIVATE_si, nil, VarType(Vcl.Controls.CM_ACTIVATE));
  AData.AddConstant('CM_DEACTIVATE', Controls_CM_DEACTIVATE_si, nil, VarType(Vcl.Controls.CM_DEACTIVATE));
  AData.AddConstant('CM_GOTFOCUS', Controls_CM_GOTFOCUS_si, nil, VarType(Vcl.Controls.CM_GOTFOCUS));
  AData.AddConstant('CM_LOSTFOCUS', Controls_CM_LOSTFOCUS_si, nil, VarType(Vcl.Controls.CM_LOSTFOCUS));
  AData.AddConstant('CM_CANCELMODE', Controls_CM_CANCELMODE_si, nil, VarType(Vcl.Controls.CM_CANCELMODE));
  AData.AddConstant('CM_DIALOGKEY', Controls_CM_DIALOGKEY_si, nil, VarType(Vcl.Controls.CM_DIALOGKEY));
  AData.AddConstant('CM_DIALOGCHAR', Controls_CM_DIALOGCHAR_si, nil, VarType(Vcl.Controls.CM_DIALOGCHAR));
  AData.AddConstant('CM_FOCUSCHANGED', Controls_CM_FOCUSCHANGED_si, nil, VarType(Vcl.Controls.CM_FOCUSCHANGED));
  AData.AddConstant('CM_PARENTFONTCHANGED', Controls_CM_PARENTFONTCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTFONTCHANGED));
  AData.AddConstant('CM_PARENTCOLORCHANGED', Controls_CM_PARENTCOLORCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTCOLORCHANGED));
  AData.AddConstant('CM_HITTEST', Controls_CM_HITTEST_si, nil, VarType(Vcl.Controls.CM_HITTEST));
  AData.AddConstant('CM_VISIBLECHANGED', Controls_CM_VISIBLECHANGED_si, nil, VarType(Vcl.Controls.CM_VISIBLECHANGED));
  AData.AddConstant('CM_ENABLEDCHANGED', Controls_CM_ENABLEDCHANGED_si, nil, VarType(Vcl.Controls.CM_ENABLEDCHANGED));
  AData.AddConstant('CM_COLORCHANGED', Controls_CM_COLORCHANGED_si, nil, VarType(Vcl.Controls.CM_COLORCHANGED));
  AData.AddConstant('CM_FONTCHANGED', Controls_CM_FONTCHANGED_si, nil, VarType(Vcl.Controls.CM_FONTCHANGED));
  AData.AddConstant('CM_CURSORCHANGED', Controls_CM_CURSORCHANGED_si, nil, VarType(Vcl.Controls.CM_CURSORCHANGED));
  AData.AddConstant('CM_CTL3DCHANGED', Controls_CM_CTL3DCHANGED_si, nil, VarType(Vcl.Controls.CM_CTL3DCHANGED));
  AData.AddConstant('CM_PARENTCTL3DCHANGED', Controls_CM_PARENTCTL3DCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTCTL3DCHANGED));
  AData.AddConstant('CM_TEXTCHANGED', Controls_CM_TEXTCHANGED_si, nil, VarType(Vcl.Controls.CM_TEXTCHANGED));
  AData.AddConstant('CM_MOUSEENTER', Controls_CM_MOUSEENTER_si, nil, VarType(Vcl.Controls.CM_MOUSEENTER));
  AData.AddConstant('CM_MOUSELEAVE', Controls_CM_MOUSELEAVE_si, nil, VarType(Vcl.Controls.CM_MOUSELEAVE));
  AData.AddConstant('CM_MENUCHANGED', Controls_CM_MENUCHANGED_si, nil, VarType(Vcl.Controls.CM_MENUCHANGED));
  AData.AddConstant('CM_APPKEYDOWN', Controls_CM_APPKEYDOWN_si, nil, VarType(Vcl.Controls.CM_APPKEYDOWN));
  AData.AddConstant('CM_APPSYSCOMMAND', Controls_CM_APPSYSCOMMAND_si, nil, VarType(Vcl.Controls.CM_APPSYSCOMMAND));
  AData.AddConstant('CM_BUTTONPRESSED', Controls_CM_BUTTONPRESSED_si, nil, VarType(Vcl.Controls.CM_BUTTONPRESSED));
  AData.AddConstant('CM_SHOWINGCHANGED', Controls_CM_SHOWINGCHANGED_si, nil, VarType(Vcl.Controls.CM_SHOWINGCHANGED));
  AData.AddConstant('CM_ENTER', Controls_CM_ENTER_si, nil, VarType(Vcl.Controls.CM_ENTER));
  AData.AddConstant('CM_EXIT', Controls_CM_EXIT_si, nil, VarType(Vcl.Controls.CM_EXIT));
  AData.AddConstant('CM_DESIGNHITTEST', Controls_CM_DESIGNHITTEST_si, nil, VarType(Vcl.Controls.CM_DESIGNHITTEST));
  AData.AddConstant('CM_ICONCHANGED', Controls_CM_ICONCHANGED_si, nil, VarType(Vcl.Controls.CM_ICONCHANGED));
  AData.AddConstant('CM_WANTSPECIALKEY', Controls_CM_WANTSPECIALKEY_si, nil, VarType(Vcl.Controls.CM_WANTSPECIALKEY));
  AData.AddConstant('CM_INVOKEHELP', Controls_CM_INVOKEHELP_si, nil, VarType(Vcl.Controls.CM_INVOKEHELP));
  AData.AddConstant('CM_WINDOWHOOK', Controls_CM_WINDOWHOOK_si, nil, VarType(Vcl.Controls.CM_WINDOWHOOK));
  AData.AddConstant('CM_RELEASE', Controls_CM_RELEASE_si, nil, VarType(Vcl.Controls.CM_RELEASE));
  AData.AddConstant('CM_SHOWHINTCHANGED', Controls_CM_SHOWHINTCHANGED_si, nil, VarType(Vcl.Controls.CM_SHOWHINTCHANGED));
  AData.AddConstant('CM_PARENTSHOWHINTCHANGED', Controls_CM_PARENTSHOWHINTCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTSHOWHINTCHANGED));
  AData.AddConstant('CM_SYSCOLORCHANGE', Controls_CM_SYSCOLORCHANGE_si, nil, VarType(Vcl.Controls.CM_SYSCOLORCHANGE));
  AData.AddConstant('CM_WININICHANGE', Controls_CM_WININICHANGE_si, nil, VarType(Vcl.Controls.CM_WININICHANGE));
  AData.AddConstant('CM_FONTCHANGE', Controls_CM_FONTCHANGE_si, nil, VarType(Vcl.Controls.CM_FONTCHANGE));
  AData.AddConstant('CM_TIMECHANGE', Controls_CM_TIMECHANGE_si, nil, VarType(Vcl.Controls.CM_TIMECHANGE));
  AData.AddConstant('CM_TABSTOPCHANGED', Controls_CM_TABSTOPCHANGED_si, nil, VarType(Vcl.Controls.CM_TABSTOPCHANGED));
  AData.AddConstant('CM_UIACTIVATE', Controls_CM_UIACTIVATE_si, nil, VarType(Vcl.Controls.CM_UIACTIVATE));
  AData.AddConstant('CM_UIDEACTIVATE', Controls_CM_UIDEACTIVATE_si, nil, VarType(Vcl.Controls.CM_UIDEACTIVATE));
  AData.AddConstant('CM_DOCWINDOWACTIVATE', Controls_CM_DOCWINDOWACTIVATE_si, nil, VarType(Vcl.Controls.CM_DOCWINDOWACTIVATE));
  AData.AddConstant('CM_CONTROLLISTCHANGE', Controls_CM_CONTROLLISTCHANGE_si, nil, VarType(Vcl.Controls.CM_CONTROLLISTCHANGE));
  AData.AddConstant('CM_GETDATALINK', Controls_CM_GETDATALINK_si, nil, VarType(Vcl.Controls.CM_GETDATALINK));
  AData.AddConstant('CM_CHILDKEY', Controls_CM_CHILDKEY_si, nil, VarType(Vcl.Controls.CM_CHILDKEY));
  AData.AddConstant('CM_DRAG', Controls_CM_DRAG_si, nil, VarType(Vcl.Controls.CM_DRAG));
  AData.AddConstant('CM_HINTSHOW', Controls_CM_HINTSHOW_si, nil, VarType(Vcl.Controls.CM_HINTSHOW));
  AData.AddConstant('CM_DIALOGHANDLE', Controls_CM_DIALOGHANDLE_si, nil, VarType(Vcl.Controls.CM_DIALOGHANDLE));
  AData.AddConstant('CM_ISTOOLCONTROL', Controls_CM_ISTOOLCONTROL_si, nil, VarType(Vcl.Controls.CM_ISTOOLCONTROL));
  AData.AddConstant('CM_RECREATEWND', Controls_CM_RECREATEWND_si, nil, VarType(Vcl.Controls.CM_RECREATEWND));
  AData.AddConstant('CM_INVALIDATE', Controls_CM_INVALIDATE_si, nil, VarType(Vcl.Controls.CM_INVALIDATE));
  AData.AddConstant('CM_SYSFONTCHANGED', Controls_CM_SYSFONTCHANGED_si, nil, VarType(Vcl.Controls.CM_SYSFONTCHANGED));
  AData.AddConstant('CM_CONTROLCHANGE', Controls_CM_CONTROLCHANGE_si, nil, VarType(Vcl.Controls.CM_CONTROLCHANGE));
  AData.AddConstant('CM_CHANGED', Controls_CM_CHANGED_si, nil, VarType(Vcl.Controls.CM_CHANGED));
  AData.AddConstant('CM_DOCKCLIENT', Controls_CM_DOCKCLIENT_si, nil, VarType(Vcl.Controls.CM_DOCKCLIENT));
  AData.AddConstant('CM_UNDOCKCLIENT', Controls_CM_UNDOCKCLIENT_si, nil, VarType(Vcl.Controls.CM_UNDOCKCLIENT));
  AData.AddConstant('CM_FLOAT', Controls_CM_FLOAT_si, nil, VarType(Vcl.Controls.CM_FLOAT));
  AData.AddConstant('CM_BORDERCHANGED', Controls_CM_BORDERCHANGED_si, nil, VarType(Vcl.Controls.CM_BORDERCHANGED));
  AData.AddConstant('CM_BIDIMODECHANGED', Controls_CM_BIDIMODECHANGED_si, nil, VarType(Vcl.Controls.CM_BIDIMODECHANGED));
  AData.AddConstant('CM_PARENTBIDIMODECHANGED', Controls_CM_PARENTBIDIMODECHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTBIDIMODECHANGED));
  AData.AddConstant('CM_ALLCHILDRENFLIPPED', Controls_CM_ALLCHILDRENFLIPPED_si, nil, VarType(Vcl.Controls.CM_ALLCHILDRENFLIPPED));
  AData.AddConstant('CM_ACTIONUPDATE', Controls_CM_ACTIONUPDATE_si, nil, VarType(Vcl.Controls.CM_ACTIONUPDATE));
  AData.AddConstant('CM_ACTIONEXECUTE', Controls_CM_ACTIONEXECUTE_si, nil, VarType(Vcl.Controls.CM_ACTIONEXECUTE));
  AData.AddConstant('CM_HINTSHOWPAUSE', Controls_CM_HINTSHOWPAUSE_si, nil, VarType(Vcl.Controls.CM_HINTSHOWPAUSE));
  AData.AddConstant('CM_DOCKNOTIFICATION', Controls_CM_DOCKNOTIFICATION_si, nil, VarType(Vcl.Controls.CM_DOCKNOTIFICATION));
  AData.AddConstant('CM_MOUSEWHEEL', Controls_CM_MOUSEWHEEL_si, nil, VarType(Vcl.Controls.CM_MOUSEWHEEL));
  AData.AddConstant('CM_ISSHORTCUT', Controls_CM_ISSHORTCUT_si, nil, VarType(Vcl.Controls.CM_ISSHORTCUT));
  AData.AddConstant('CM_UPDATEACTIONS', Controls_CM_UPDATEACTIONS_si, nil, VarType(Vcl.Controls.CM_UPDATEACTIONS));
  AData.AddConstant('CM_INVALIDATEDOCKHOST', Controls_CM_INVALIDATEDOCKHOST_si, nil, VarType(Vcl.Controls.CM_INVALIDATEDOCKHOST));
  AData.AddConstant('CM_SETACTIVECONTROL', Controls_CM_SETACTIVECONTROL_si, nil, VarType(Vcl.Controls.CM_SETACTIVECONTROL));
  AData.AddConstant('CM_POPUPHWNDDESTROY', Controls_CM_POPUPHWNDDESTROY_si, nil, VarType(Vcl.Controls.CM_POPUPHWNDDESTROY));
  AData.AddConstant('CM_CREATEPOPUP', Controls_CM_CREATEPOPUP_si, nil, VarType(Vcl.Controls.CM_CREATEPOPUP));
  AData.AddConstant('CM_DESTROYHANDLE', Controls_CM_DESTROYHANDLE_si, nil, VarType(Vcl.Controls.CM_DESTROYHANDLE));
  AData.AddConstant('CM_MOUSEACTIVATE', Controls_CM_MOUSEACTIVATE_si, nil, VarType(Vcl.Controls.CM_MOUSEACTIVATE));
  AData.AddConstant('CM_CONTROLLISTCHANGING', Controls_CM_CONTROLLISTCHANGING_si, nil, VarType(Vcl.Controls.CM_CONTROLLISTCHANGING));
  AData.AddConstant('CM_BUFFEREDPRINTCLIENT', Controls_CM_BUFFEREDPRINTCLIENT_si, nil, VarType(Vcl.Controls.CM_BUFFEREDPRINTCLIENT));
  AData.AddConstant('CM_UNTHEMECONTROL', Controls_CM_UNTHEMECONTROL_si, nil, VarType(Vcl.Controls.CM_UNTHEMECONTROL));
  AData.AddConstant('CM_DOUBLEBUFFEREDCHANGED', Controls_CM_DOUBLEBUFFEREDCHANGED_si, nil, VarType(Vcl.Controls.CM_DOUBLEBUFFEREDCHANGED));
  AData.AddConstant('CM_PARENTDOUBLEBUFFEREDCHANGED', Controls_CM_PARENTDOUBLEBUFFEREDCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTDOUBLEBUFFEREDCHANGED));
  AData.AddConstant('CM_STYLECHANGED', Controls_CM_STYLECHANGED_si, nil, VarType(Vcl.Controls.CM_STYLECHANGED));
  AData.AddConstant('CM_THEMECHANGED', Controls_CM_THEMECHANGED_si, Integer_sw);
  AData.AddConstant('CM_GESTURE', Controls_CM_GESTURE_si, nil, VarType(Vcl.Controls.CM_GESTURE));
  AData.AddConstant('CM_CUSTOMGESTURESCHANGED', Controls_CM_CUSTOMGESTURESCHANGED_si, nil, VarType(Vcl.Controls.CM_CUSTOMGESTURESCHANGED));
  AData.AddConstant('CM_GESTUREMANAGERCHANGED', Controls_CM_GESTUREMANAGERCHANGED_si, nil, VarType(Vcl.Controls.CM_GESTUREMANAGERCHANGED));
  AData.AddConstant('CM_STANDARDGESTURESCHANGED', Controls_CM_STANDARDGESTURESCHANGED_si, nil, VarType(Vcl.Controls.CM_STANDARDGESTURESCHANGED));
  AData.AddConstant('CM_INPUTLANGCHANGE', Controls_CM_INPUTLANGCHANGE_si, nil, VarType(Vcl.Controls.CM_INPUTLANGCHANGE));
  AData.AddConstant('CM_TABLETOPTIONSCHANGED', Controls_CM_TABLETOPTIONSCHANGED_si, nil, VarType(Vcl.Controls.CM_TABLETOPTIONSCHANGED));
  AData.AddConstant('CM_PARENTTABLETOPTIONSCHANGED', Controls_CM_PARENTTABLETOPTIONSCHANGED_si, nil, VarType(Vcl.Controls.CM_PARENTTABLETOPTIONSCHANGED));
  AData.AddConstant('CM_CUSTOMSTYLECHANGED', Controls_CM_CUSTOMSTYLECHANGED_si, nil, VarType(Vcl.Controls.CM_CUSTOMSTYLECHANGED));
  AData.AddConstant('CN_BASE', Controls_CN_BASE_si, nil, VarType(Vcl.Controls.CN_BASE));
  AData.AddConstant('CN_CHARTOITEM', Controls_CN_CHARTOITEM_si, nil, VarType(Vcl.Controls.CN_CHARTOITEM));
  AData.AddConstant('CN_COMMAND', Controls_CN_COMMAND_si, nil, VarType(Vcl.Controls.CN_COMMAND));
  AData.AddConstant('CN_COMPAREITEM', Controls_CN_COMPAREITEM_si, nil, VarType(Vcl.Controls.CN_COMPAREITEM));
  AData.AddConstant('CN_CTLCOLORBTN', Controls_CN_CTLCOLORBTN_si, nil, VarType(Vcl.Controls.CN_CTLCOLORBTN));
  AData.AddConstant('CN_CTLCOLORDLG', Controls_CN_CTLCOLORDLG_si, nil, VarType(Vcl.Controls.CN_CTLCOLORDLG));
  AData.AddConstant('CN_CTLCOLOREDIT', Controls_CN_CTLCOLOREDIT_si, nil, VarType(Vcl.Controls.CN_CTLCOLOREDIT));
  AData.AddConstant('CN_CTLCOLORLISTBOX', Controls_CN_CTLCOLORLISTBOX_si, nil, VarType(Vcl.Controls.CN_CTLCOLORLISTBOX));
  AData.AddConstant('CN_CTLCOLORMSGBOX', Controls_CN_CTLCOLORMSGBOX_si, nil, VarType(Vcl.Controls.CN_CTLCOLORMSGBOX));
  AData.AddConstant('CN_CTLCOLORSCROLLBAR', Controls_CN_CTLCOLORSCROLLBAR_si, nil, VarType(Vcl.Controls.CN_CTLCOLORSCROLLBAR));
  AData.AddConstant('CN_CTLCOLORSTATIC', Controls_CN_CTLCOLORSTATIC_si, nil, VarType(Vcl.Controls.CN_CTLCOLORSTATIC));
  AData.AddConstant('CN_DELETEITEM', Controls_CN_DELETEITEM_si, nil, VarType(Vcl.Controls.CN_DELETEITEM));
  AData.AddConstant('CN_DRAWITEM', Controls_CN_DRAWITEM_si, nil, VarType(Vcl.Controls.CN_DRAWITEM));
  AData.AddConstant('CN_HSCROLL', Controls_CN_HSCROLL_si, nil, VarType(Vcl.Controls.CN_HSCROLL));
  AData.AddConstant('CN_MEASUREITEM', Controls_CN_MEASUREITEM_si, nil, VarType(Vcl.Controls.CN_MEASUREITEM));
  AData.AddConstant('CN_PARENTNOTIFY', Controls_CN_PARENTNOTIFY_si, nil, VarType(Vcl.Controls.CN_PARENTNOTIFY));
  AData.AddConstant('CN_VKEYTOITEM', Controls_CN_VKEYTOITEM_si, nil, VarType(Vcl.Controls.CN_VKEYTOITEM));
  AData.AddConstant('CN_VSCROLL', Controls_CN_VSCROLL_si, nil, VarType(Vcl.Controls.CN_VSCROLL));
  AData.AddConstant('CN_KEYDOWN', Controls_CN_KEYDOWN_si, nil, VarType(Vcl.Controls.CN_KEYDOWN));
  AData.AddConstant('CN_KEYUP', Controls_CN_KEYUP_si, nil, VarType(Vcl.Controls.CN_KEYUP));
  AData.AddConstant('CN_CHAR', Controls_CN_CHAR_si, nil, VarType(Vcl.Controls.CN_CHAR));
  AData.AddConstant('CN_SYSKEYDOWN', Controls_CN_SYSKEYDOWN_si, nil, VarType(Vcl.Controls.CN_SYSKEYDOWN));
  AData.AddConstant('CN_SYSCHAR', Controls_CN_SYSCHAR_si, nil, VarType(Vcl.Controls.CN_SYSCHAR));
  AData.AddConstant('CN_NOTIFY', Controls_CN_NOTIFY_si, nil, VarType(Vcl.Controls.CN_NOTIFY));
  AData.AddConstant('akLeft', Controls_akLeft_si, TAnchorKind_sw);
  AData.AddConstant('akTop', Controls_akTop_si, TAnchorKind_sw);
  AData.AddConstant('akRight', Controls_akRight_si, TAnchorKind_sw);
  AData.AddConstant('akBottom', Controls_akBottom_si, TAnchorKind_sw);
  AData.AddConstant('mrNone', Controls_mrNone_si, Integer_sw);
  ...
  AData.AddConstant('mrAll', Controls_mrAll_si, Integer_sw);
  AData.AddConstant('mrNoToAll', Controls_mrNoToAll_si, Integer_sw);
  AData.AddConstant('mrYesToAll', Controls_mrYesToAll_si, Integer_sw);
  AData.AddConstant('mrClose', Controls_mrClose_si, Integer_sw);
  ...
  AData.AddConstant('sgiNoGesture', Controls_sgiNoGesture_si, nil, VarType(Vcl.Controls.sgiNoGesture));
  AData.AddConstant('sgiLeft', Controls_sgiLeft_si, nil, VarType(Vcl.Controls.sgiLeft));
  AData.AddConstant('sgiRight', Controls_sgiRight_si, nil, VarType(Vcl.Controls.sgiRight));
  AData.AddConstant('sgiUp', Controls_sgiUp_si, nil, VarType(Vcl.Controls.sgiUp));
  AData.AddConstant('sgiDown', Controls_sgiDown_si, nil, VarType(Vcl.Controls.sgiDown));
  AData.AddConstant('sgiUpLeft', Controls_sgiUpLeft_si, nil, VarType(Vcl.Controls.sgiUpLeft));
  AData.AddConstant('sgiUpRight', Controls_sgiUpRight_si, nil, VarType(Vcl.Controls.sgiUpRight));
  AData.AddConstant('sgiDownLeft', Controls_sgiDownLeft_si, nil, VarType(Vcl.Controls.sgiDownLeft));
  AData.AddConstant('sgiDownRight', Controls_sgiDownRight_si, nil, VarType(Vcl.Controls.sgiDownRight));
  AData.AddConstant('sgiLeftUp', Controls_sgiLeftUp_si, nil, VarType(Vcl.Controls.sgiLeftUp));
  AData.AddConstant('sgiLeftDown', Controls_sgiLeftDown_si, nil, VarType(Vcl.Controls.sgiLeftDown));
  AData.AddConstant('sgiRightUp', Controls_sgiRightUp_si, nil, VarType(Vcl.Controls.sgiRightUp));
  AData.AddConstant('sgiRightDown', Controls_sgiRightDown_si, nil, VarType(Vcl.Controls.sgiRightDown));
  AData.AddConstant('sgiUpDown', Controls_sgiUpDown_si, nil, VarType(Vcl.Controls.sgiUpDown));
  AData.AddConstant('sgiDownUp', Controls_sgiDownUp_si, nil, VarType(Vcl.Controls.sgiDownUp));
  AData.AddConstant('sgiLeftRight', Controls_sgiLeftRight_si, nil, VarType(Vcl.Controls.sgiLeftRight));
  AData.AddConstant('sgiRightLeft', Controls_sgiRightLeft_si, nil, VarType(Vcl.Controls.sgiRightLeft));
  AData.AddConstant('sgiUpLeftLong', Controls_sgiUpLeftLong_si, nil, VarType(Vcl.Controls.sgiUpLeftLong));
  AData.AddConstant('sgiUpRightLong', Controls_sgiUpRightLong_si, nil, VarType(Vcl.Controls.sgiUpRightLong));
  AData.AddConstant('sgiDownLeftLong', Controls_sgiDownLeftLong_si, nil, VarType(Vcl.Controls.sgiDownLeftLong));
  AData.AddConstant('sgiDownRightLong', Controls_sgiDownRightLong_si, nil, VarType(Vcl.Controls.sgiDownRightLong));
  AData.AddConstant('sgiScratchout', Controls_sgiScratchout_si, nil, VarType(Vcl.Controls.sgiScratchout));
  AData.AddConstant('sgiTriangle', Controls_sgiTriangle_si, nil, VarType(Vcl.Controls.sgiTriangle));
  AData.AddConstant('sgiSquare', Controls_sgiSquare_si, nil, VarType(Vcl.Controls.sgiSquare));
  AData.AddConstant('sgiCheck', Controls_sgiCheck_si, nil, VarType(Vcl.Controls.sgiCheck));
  AData.AddConstant('sgiCurlicue', Controls_sgiCurlicue_si, nil, VarType(Vcl.Controls.sgiCurlicue));
  AData.AddConstant('sgiDoubleCurlicue', Controls_sgiDoubleCurlicue_si, nil, VarType(Vcl.Controls.sgiDoubleCurlicue));
  AData.AddConstant('sgiCircle', Controls_sgiCircle_si, nil, VarType(Vcl.Controls.sgiCircle));
  AData.AddConstant('sgiDoubleCircle', Controls_sgiDoubleCircle_si, nil, VarType(Vcl.Controls.sgiDoubleCircle));
  AData.AddConstant('sgiSemiCircleLeft', Controls_sgiSemiCircleLeft_si, nil, VarType(Vcl.Controls.sgiSemiCircleLeft));
  AData.AddConstant('sgiSemiCircleRight', Controls_sgiSemiCircleRight_si, nil, VarType(Vcl.Controls.sgiSemiCircleRight));
  AData.AddConstant('sgiChevronUp', Controls_sgiChevronUp_si, nil, VarType(Vcl.Controls.sgiChevronUp));
  AData.AddConstant('sgiChevronDown', Controls_sgiChevronDown_si, nil, VarType(Vcl.Controls.sgiChevronDown));
  AData.AddConstant('sgiChevronLeft', Controls_sgiChevronLeft_si, nil, VarType(Vcl.Controls.sgiChevronLeft));
  AData.AddConstant('sgiChevronRight', Controls_sgiChevronRight_si, nil, VarType(Vcl.Controls.sgiChevronRight));
  AData.AddConstant('sgiFirst', Controls_sgiFirst_si, Integer_sw);
  AData.AddConstant('sgiLast', Controls_sgiLast_si, Integer_sw);
  AData.AddConstant('cgiFirst', Controls_cgiFirst_si, nil, VarType(Vcl.Controls.cgiFirst));
  AData.AddConstant('cgiLast', Controls_cgiLast_si, nil, VarType(Vcl.Controls.cgiLast));
  AData.AddConstant('rgiFirst', Controls_rgiFirst_si, nil, VarType(Vcl.Controls.rgiFirst));
  AData.AddConstant('rgiLast', Controls_rgiLast_si, nil, VarType(Vcl.Controls.rgiLast));
  AData.AddConstant('igiFirst', Controls_igiFirst_si, nil, VarType(Vcl.Controls.igiFirst));
  AData.AddConstant('igiLast', Controls_igiLast_si, nil, VarType(Vcl.Controls.igiLast));
  AData.AddConstant('igiBegin', Controls_igiBegin_si, nil, VarType(Vcl.Controls.igiBegin));
  AData.AddConstant('igiEnd', Controls_igiEnd_si, nil, VarType(Vcl.Controls.igiEnd));
  AData.AddConstant('igiZoom', Controls_igiZoom_si, nil, VarType(Vcl.Controls.igiZoom));
  AData.AddConstant('igiPan', Controls_igiPan_si, nil, VarType(Vcl.Controls.igiPan));
  AData.AddConstant('igiRotate', Controls_igiRotate_si, nil, VarType(Vcl.Controls.igiRotate));
  AData.AddConstant('igiTwoFingerTap', Controls_igiTwoFingerTap_si, nil, VarType(Vcl.Controls.igiTwoFingerTap));
  AData.AddConstant('igiPressAndTap', Controls_igiPressAndTap_si, nil, VarType(Vcl.Controls.igiPressAndTap));
  AData.AddConstant('gcnRefreshAll', Controls_gcnRefreshAll_si, nil, VarType(Vcl.Controls.gcnRefreshAll));
  AData.AddConstant('gcnAdded', Controls_gcnAdded_si, nil, VarType(Vcl.Controls.gcnAdded));
  AData.AddConstant('gcnRemoved', Controls_gcnRemoved_si, nil, VarType(Vcl.Controls.gcnRemoved));
  AData.AddConstant('gcnModified', Controls_gcnModified_si, nil, VarType(Vcl.Controls.gcnModified));
  AData.AddConstant('dmAutomatic', Controls_dmAutomatic_si, TDragMode_sw);
  AData.AddConstant('dmManual', Controls_dmManual_si, TDragMode_sw);
  AData.AddConstant('dsDragEnter', Controls_dsDragEnter_si, TDragState_sw);
  AData.AddConstant('dsDragLeave', Controls_dsDragLeave_si, TDragState_sw);
  AData.AddConstant('dsDragMove', Controls_dsDragMove_si, TDragState_sw);
  AData.AddConstant('dkDrag', Controls_dkDrag_si, TDragKind_sw);
  AData.AddConstant('dkDock', Controls_dkDock_si, TDragKind_sw);
  AData.AddConstant('maDefault', Controls_maDefault_si, TMouseActivate_sw);
  AData.AddConstant('maActivate', Controls_maActivate_si, TMouseActivate_sw);
  AData.AddConstant('maActivateAndEat', Controls_maActivateAndEat_si, TMouseActivate_sw);
  AData.AddConstant('maNoActivate', Controls_maNoActivate_si, TMouseActivate_sw);
  AData.AddConstant('maNoActivateAndEat', Controls_maNoActivateAndEat_si, TMouseActivate_sw);
  AData.AddConstant('mbLeft', Controls_mbLeft_si, TMouseButton_sw);
  AData.AddConstant('mbRight', Controls_mbRight_si, TMouseButton_sw);
  AData.AddConstant('mbMiddle', Controls_mbMiddle_si, TMouseButton_sw);
  ...
  AData.AddType(TTipMode_sw, Controls_TTipMode_si);
  ...
  AData.AddConstant('cImageMargin', Controls_cImageMargin_si, nil, VarType(Vcl.Controls.cImageMargin));
  AData.AddConstant('cTextVerticalMargin', Controls_cTextVerticalMargin_si, nil, VarType(Vcl.Controls.cTextVerticalMargin));
  AData.AddConstant('cTextHorizontalMargin', Controls_cTextHorizontalMargin_si, nil, VarType(Vcl.Controls.cTextHorizontalMargin));
  AData.AddConstant('cBalloonStemHeight', Controls_cBalloonStemHeight_si, nil, VarType(Vcl.Controls.cBalloonStemHeight));
  AData.AddConstant('cEdgeRadius', Controls_cEdgeRadius_si, nil, VarType(Vcl.Controls.cEdgeRadius));
  ...
  AData.AddConstant('CTL3D_ALL', Controls_CTL3D_ALL_si, nil, VarType(Vcl.Controls.CTL3D_ALL));
  ...
  AData.AddFunction('SendAppMessage', Controls_SendAppMessage_si, LRESULT_sw, 3, False);
  AData.AddProcedure('MoveWindowOrg', Controls_MoveWindowOrg_si, 3, False);
  AData.AddProcedure('SetTextInputPanelStatus', Controls_SetTextInputPanelStatus_si, 2, False);
