...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'System.Contnrs' unit.
  #
  # WARNING:
...
uses
  ...
  System.Contnrs, System.SysUtils_LMDSW, System.Classes_LMDSW;
...
function TObjectList_Remove_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function Remove(...); overload;
...
begin
  ...
  AData.AddFunction('Remove', TObjectList_Remove_si, nil, 1, False);
...
function TComponentList_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function Create(...); overload;
  ...
  case AArgsSize of
    0:
    begin
      Result := TComponentList_sw.ToVar(TComponentList_sc(AObj).Create());
    end;
    1:
    begin
      Result := TComponentList_sw.ToVar(TComponentList_sc(AObj).Create(Boolean(
        AArgs[0])));
    end;
  else
    WrongArgCountError('Create');
...
begin
  ...
  AData.AddConstructor('Create', TComponentList_Create_si, 0, True);
end;
...
function TIntegerBucketList_Add_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Add(AItem: NativeInt; AData: NativeInt): NativeInt;

  Result := NativeInt_sw.ToVar(TIntegerBucketList(AObj).Add(NativeInt_sw.
...
begin
  ...
  Result := NativeInt_sw.ToVar(TIntegerBucketList(AObj).Remove(NativeInt_sw.
    FromVar(AArgs[0])));
end;
...
function TIntegerBucketList_Data_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Data[AItem: NativeInt]: NativeInt read <getter> ...

  if IsGet then
    Result := NativeInt_sw.ToVar(TIntegerBucketList(AObj).Data[NativeInt_sw.
      FromVar(AArgs[0])])
  else
    TIntegerBucketList(AObj).Data[NativeInt_sw.FromVar(AArgs[0])] := 
...
begin
  ...
  AData.AddFunction('Add', TIntegerBucketList_Add_si, NativeInt_sw, 2, False);
  AData.AddFunction('Remove', TIntegerBucketList_Remove_si, NativeInt_sw, 1, False);
  AData.AddProperty('Data', TIntegerBucketList_Data_si, NativeInt_sw, True, True, 1, False, True);
end;
