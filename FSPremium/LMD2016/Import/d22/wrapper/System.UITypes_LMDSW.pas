...
type
  ...
  TDialogType_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TDialogType): OleVariant;
    class function FromVar(const AValue: OleVariant): TDialogType;
  end;
  ...
  TAlphaColorF_sw = class(TLMDRecordWrapper)
  private
    FValue: TAlphaColorF;
  protected
    class function  GetTypeName: WideString; override;
    ...
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: TAlphaColorF): OleVariant;
    class function FromVar(const AValue: OleVariant): TAlphaColorF;
  end;
...
class function TDialogType_sw.GetTypeName: WideString;
begin
  Result := 'TDialogType';
end;

class procedure TDialogType_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..1] of TLMDEnumerator = (
    (Name: 'Standard'; Value: Integer({$IFDEF LMDSCT_12}TDialogType.{$ENDIF}Standard)),
    (Name: 'Directory'; Value: Integer({$IFDEF LMDSCT_12}TDialogType.{$ENDIF}Directory))
  );
begin
  AEnums := @ENUMS;
  ACount := 2;
...
class function TDialogType_sw.ToVar(const AValue: TDialogType): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TDialogType_sw.FromVar(const AValue: OleVariant): TDialogType;
begin
  Result := TDialogType(Integer(AValue));
end;
...
function TAlphaColorF_Epsilon_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const Epsilon = <value>;

  Result := TAlphaColorF.Epsilon;
end;

function TAlphaColorF_R_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var R: Single;

  if IsGet then
    Result := TAlphaColorF_sw(AObj).FValue.R
  else
    TAlphaColorF_sw(AObj).FValue.R := Single(AArgs[0]);
end;

function TAlphaColorF_G_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var G: Single;

  if IsGet then
    Result := TAlphaColorF_sw(AObj).FValue.G
  else
    TAlphaColorF_sw(AObj).FValue.G := Single(AArgs[0]);
end;

function TAlphaColorF_B_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var B: Single;

  if IsGet then
    Result := TAlphaColorF_sw(AObj).FValue.B
  else
    TAlphaColorF_sw(AObj).FValue.B := Single(AArgs[0]);
end;

function TAlphaColorF_A_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var A: Single;

  if IsGet then
    Result := TAlphaColorF_sw(AObj).FValue.A
  else
    TAlphaColorF_sw(AObj).FValue.A := Single(AArgs[0]);
end;

function TAlphaColorF_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  case AArgsSize of
    3:
    begin
      Result := TAlphaColorF_sw.ToVar(TAlphaColorF.Create(Single(AArgs[0]), 
    ...
    1:
    begin
      Result := TAlphaColorF_sw.ToVar(TAlphaColorF.Create(TAlphaColor_sw.
        FromVar(AArgs[0])));
    end;
  else
...
function TAlphaColorF_PremultipliedAlpha_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function PremultipliedAlpha: TAlphaColorF;

  Result := TAlphaColorF_sw.ToVar(TAlphaColorF_sw(AObj).FValue.
    PremultipliedAlpha());
end;

function TAlphaColorF_UnpremultipliedAlpha_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
function TAlphaColorF_Clamp_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Clamp: TAlphaColorF;

  Result := TAlphaColorF_sw.ToVar(TAlphaColorF_sw(AObj).FValue.Clamp());
end;

function TAlphaColorF_ToAlphaColor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function ToAlphaColor: TAlphaColor;

  Result := TAlphaColor_sw.ToVar(TAlphaColorF_sw(AObj).FValue.ToAlphaColor());
end;

function TAlphaColorF_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: TAlphaColorF;

  // Implicit record copy method.
  Result := TAlphaColorF_sw.ToVar(TAlphaColorF(AObj));
end;

class function TAlphaColorF_sw.GetTypeName: WideString;
begin
  Result := 'TAlphaColorF';
end;

function TAlphaColorF_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := TAlphaColorF_sw.Create;
  TAlphaColorF_sw(Result).FValue := FValue;
end;

class procedure TAlphaColorF_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('R', TAlphaColorF_R_si, Single_sw);
  AData.AddField('G', TAlphaColorF_G_si, Single_sw);
  AData.AddField('B', TAlphaColorF_B_si, Single_sw);
  AData.AddField('A', TAlphaColorF_A_si, Single_sw);
  AData.AddFunction('PremultipliedAlpha', TAlphaColorF_PremultipliedAlpha_si, TAlphaColorF_sw, 0, False);
  AData.AddFunction('UnpremultipliedAlpha', TAlphaColorF_UnpremultipliedAlpha_si, TAlphaColorF_sw, 0, False);
  AData.AddFunction('Clamp', TAlphaColorF_Clamp_si, TAlphaColorF_sw, 0, False);
  AData.AddFunction('ToAlphaColor', TAlphaColorF_ToAlphaColor_si, TAlphaColor_sw, 0, False);
  AData.AddFunction('Copy', TAlphaColorF_Copy_si, TAlphaColorF_sw, 0, False);

  { Class members }

  AData.AddConstant('Epsilon', TAlphaColorF_Epsilon_si, nil, VarType(TAlphaColorF.Epsilon));
  AData.AddClassFunction('Create', TAlphaColorF_Create_si, nil, 1, True);
end;

class function TAlphaColorF_sw.ToVar(const AValue: TAlphaColorF): OleVariant;
var
  wrpr: TAlphaColorF_sw;
begin
  wrpr        := TAlphaColorF_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function TAlphaColorF_sw.FromVar(const AValue: OleVariant): TAlphaColorF;
begin
  Result := TAlphaColorF_sw(ConvFromVar(AValue)).FValue;
end;
...
function UITypes_TDialogType_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TDialogType = (Standard, Directory);

  Result := TLMDUnitWrapper.TypeToVar(TDialogType_sw);
end;
...
function UITypes_TAlphaColorF_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TAlphaColorF = record;

  Result := TLMDUnitWrapper.TypeToVar(TAlphaColorF_sw);
end;
...
begin
  ...
  AData.AddType(TDialogType_sw, UITypes_TDialogType_si);
  ...
  AData.AddType(TAlphaColorF_sw, UITypes_TAlphaColorF_si);
