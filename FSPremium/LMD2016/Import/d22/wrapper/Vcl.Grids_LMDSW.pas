...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Vcl.Grids' unit.
  #
  # WARNING:
...
uses
  ...
  System.Variants, System.Types, Vcl.Graphics, Vcl.Menus, Vcl.Controls,
  Vcl.Forms, Vcl.StdCtrls, Vcl.Mask, System.UITypes, Vcl.Grids,
  Winapi.Messages_LMDSW, Winapi.Windows_LMDSW, System.SysUtils_LMDSW,
  System.Classes_LMDSW, System.Variants_LMDSW, System.Types_LMDSW,
  Vcl.Graphics_LMDSW, Vcl.Menus_LMDSW, Vcl.Controls_LMDSW, Vcl.Forms_LMDSW,
  Vcl.StdCtrls_LMDSW, Vcl.Mask_LMDSW, System.UITypes_LMDSW;
...
type
  ...
  THotTrackCellInfo_sw = class(TLMDRecordWrapper)
  private
    FValue: THotTrackCellInfo;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: THotTrackCellInfo): OleVariant;
    class function FromVar(const AValue: OleVariant): THotTrackCellInfo;
  end;
  ...
  TFixedCellClickEvent_sh = class(TLMDScriptEventHandler)
  public
    function  GetHandler: TMethod; override;
    procedure Handler(Sender: TObject; ACol: Longint; ARow: Longint);
  end;

  TFixedCellClickEvent_sw = class(TLMDEventWrapper)
  protected
    class function GetTypeName: WideString; override;
  public
    class function ToVar(const AValue: TFixedCellClickEvent): OleVariant;
    class function FromVar(const AValue: OleVariant): TFixedCellClickEvent;
    class function GetScriptHandlerClass: TClass; override;
    class function GetHandler(AScriptControl: TLMDScriptControl;
                              const AProcName: string): TFixedCellClickEvent;
  end;

  TGridDrawingStyle_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TGridDrawingStyle): OleVariant;
    class function FromVar(const AValue: OleVariant): TGridDrawingStyle;
  end;
...
function TGridAxisDrawInfo_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: TGridAxisDrawInfo;
...
begin
  ...
  AData.AddFunction('Copy', TGridAxisDrawInfo_Copy_si, TGridAxisDrawInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TGridAxisDrawInfo_Create_si, 0, False);
end;
...
begin
  ...
  AData.AddFunction('Copy', TGridDrawInfo_Copy_si, TGridDrawInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TGridDrawInfo_Create_si, 0, False);
end;
...
class procedure TGridState_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..5] of TLMDEnumerator = (
    (Name: 'gsNormal'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsNormal)),
    (Name: 'gsSelecting'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsSelecting)),
    (Name: 'gsRowSizing'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsRowSizing)),
    (Name: 'gsColSizing'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsColSizing)),
    (Name: 'gsRowMoving'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsRowMoving)),
    (Name: 'gsColMoving'; Value: Integer({$IFDEF LMDSCT_12}TGridState.{$ENDIF}gsColMoving))
  );
begin
  AEnums := @ENUMS;
  ACount := 6;
...
begin
  ...
  AData.AddProperty('Grid', TInplaceEdit_Grid_si, TCustomGrid_sw, True, False, 0, True, False);
...
class procedure TGridOption_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..17] of TLMDEnumerator = (
    (Name: 'goFixedVertLine'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goFixedVertLine)),
    (Name: 'goFixedHorzLine'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goFixedHorzLine)),
    (Name: 'goVertLine'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goVertLine)),
    (Name: 'goHorzLine'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goHorzLine)),
    (Name: 'goRangeSelect'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goRangeSelect)),
    (Name: 'goDrawFocusSelected'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goDrawFocusSelected)),
    (Name: 'goRowSizing'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goRowSizing)),
    (Name: 'goColSizing'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goColSizing)),
    (Name: 'goRowMoving'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goRowMoving)),
    (Name: 'goColMoving'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goColMoving)),
    (Name: 'goEditing'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goEditing)),
    (Name: 'goTabs'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goTabs)),
    (Name: 'goRowSelect'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goRowSelect)),
    (Name: 'goAlwaysShowEditor'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goAlwaysShowEditor)),
    (Name: 'goThumbTracking'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goThumbTracking)),
    (Name: 'goFixedColClick'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goFixedColClick)),
    (Name: 'goFixedRowClick'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goFixedRowClick)),
    (Name: 'goFixedHotTrack'; Value: Integer({$IFDEF LMDSCT_12}TGridOption.{$ENDIF}goFixedHotTrack))
  );
begin
  AEnums := @ENUMS;
  ACount := 18;
...
begin
  ...
  AData.AddFunction('Copy', TGridCoord_Copy_si, TGridCoord_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TGridCoord_Create_si, 0, False);
end;
...
{ THotTrackCellInfo_sw }

function THotTrackCellInfo_Coord_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Coord: TGridCoord;

  if IsGet then
    Result := TGridCoord_sw.ToVar(THotTrackCellInfo_sw(AObj).FValue.Coord)
  else
    THotTrackCellInfo_sw(AObj).FValue.Coord := TGridCoord_sw.FromVar(AArgs[0]);
end;

function THotTrackCellInfo_Pressed_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Pressed: Boolean;

  if IsGet then
    Result := THotTrackCellInfo_sw(AObj).FValue.Pressed
  else
    THotTrackCellInfo_sw(AObj).FValue.Pressed := Boolean(AArgs[0]);
end;

function THotTrackCellInfo_Button_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var Button: TMouseButton;

  if IsGet then
    Result := TMouseButton_sw.ToVar(THotTrackCellInfo_sw(AObj).FValue.Button)
  else
    THotTrackCellInfo_sw(AObj).FValue.Button := TMouseButton_sw.FromVar(
      AArgs[0]);
end;

function THotTrackCellInfo_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := THotTrackCellInfo_sw.Create as IDispatch;
end;

function THotTrackCellInfo_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: THotTrackCellInfo;

  // Implicit record copy method.
  Result := THotTrackCellInfo_sw.ToVar(THotTrackCellInfo(AObj));
end;

class function THotTrackCellInfo_sw.GetTypeName: WideString;
begin
  Result := 'THotTrackCellInfo';
end;

function THotTrackCellInfo_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := THotTrackCellInfo_sw.Create;
  THotTrackCellInfo_sw(Result).FValue := FValue;
end;

class procedure THotTrackCellInfo_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('Coord', THotTrackCellInfo_Coord_si, TGridCoord_sw);
  AData.AddField('Pressed', THotTrackCellInfo_Pressed_si, Boolean_sw);
  AData.AddField('Button', THotTrackCellInfo_Button_si, TMouseButton_sw);
  AData.AddFunction('Copy', THotTrackCellInfo_Copy_si, THotTrackCellInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', THotTrackCellInfo_Create_si, 0, False);
end;

class function THotTrackCellInfo_sw.ToVar(
  const AValue: THotTrackCellInfo): OleVariant;
var
  wrpr: THotTrackCellInfo_sw;
begin
  wrpr        := THotTrackCellInfo_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function THotTrackCellInfo_sw.FromVar(
  const AValue: OleVariant): THotTrackCellInfo;
begin
  Result := THotTrackCellInfo_sw(ConvFromVar(AValue)).FValue;
end;
...
begin
  ...
  AData.AddFunction('Copy', TGridRect_Copy_si, TGridRect_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TGridRect_Create_si, 0, False);
end;
...
class procedure TEditStyle_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'esSimple'; Value: Integer({$IFDEF LMDSCT_12}TEditStyle.{$ENDIF}esSimple)),
    (Name: 'esEllipsis'; Value: Integer({$IFDEF LMDSCT_12}TEditStyle.{$ENDIF}esEllipsis)),
    (Name: 'esPickList'; Value: Integer({$IFDEF LMDSCT_12}TEditStyle.{$ENDIF}esPickList))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
{ TFixedCellClickEvent_sh }

function TFixedCellClickEvent_sh.GetHandler: TMethod;
var
  hdr: TFixedCellClickEvent;
begin
  hdr    := Handler;
  Result := TMethod(hdr);
end;

procedure TFixedCellClickEvent_sh.Handler(Sender: TObject; ACol: Longint;
  ARow: Longint);
var
...
{ TFixedCellClickEvent_sw }

class function TFixedCellClickEvent_sw.GetTypeName: WideString;
begin
  Result := 'TFixedCellClickEvent';
end;

class function TFixedCellClickEvent_sw.GetScriptHandlerClass: TClass;
begin
  Result := TFixedCellClickEvent_sh;
end;

class function TFixedCellClickEvent_sw.ToVar(
  const AValue: TFixedCellClickEvent): OleVariant;
begin
  Result := ConvToVar(TMethod(AValue));
end;

class function TFixedCellClickEvent_sw.FromVar(
  const AValue: OleVariant): TFixedCellClickEvent;
begin
  Result := TFixedCellClickEvent(ConvFromVar(AValue));
end;

class function TFixedCellClickEvent_sw.GetHandler(
  AScriptControl: TLMDScriptControl;
  const AProcName: string): TFixedCellClickEvent;
var
  hdlr: TFixedCellClickEvent_sh;
begin
  hdlr   := TFixedCellClickEvent_sh(AScriptControl.GetEventHandler(AProcName, 
    TFixedCellClickEvent_sw));
  Result := hdlr.Handler;
end;

{ TGridDrawingStyle_sw }

class function TGridDrawingStyle_sw.GetTypeName: WideString;
begin
  Result := 'TGridDrawingStyle';
end;

class procedure TGridDrawingStyle_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..2] of TLMDEnumerator = (
    (Name: 'gdsClassic'; Value: Integer({$IFDEF LMDSCT_12}TGridDrawingStyle.{$ENDIF}gdsClassic)),
    (Name: 'gdsThemed'; Value: Integer({$IFDEF LMDSCT_12}TGridDrawingStyle.{$ENDIF}gdsThemed)),
    (Name: 'gdsGradient'; Value: Integer({$IFDEF LMDSCT_12}TGridDrawingStyle.{$ENDIF}gdsGradient))
  );
begin
  AEnums := @ENUMS;
  ACount := 3;
...
class function TGridDrawingStyle_sw.ToVar(
  const AValue: TGridDrawingStyle): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TGridDrawingStyle_sw.FromVar(
  const AValue: OleVariant): TGridDrawingStyle;
begin
  Result := TGridDrawingStyle(Integer(AValue));
end;
...
function TCustomGrid_DrawingStyle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property DrawingStyle: TGridDrawingStyle read <...

  if IsGet then
    Result := TGridDrawingStyle_sw.ToVar(TCustomGrid_sacc(TCustomGrid(AObj)).
      DrawingStyle)
  else
    TCustomGrid_sacc(TCustomGrid(AObj)).DrawingStyle := TGridDrawingStyle_sw.
      FromVar(AArgs[0]);
end;
...
function TCustomGrid_GradientEndColor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property GradientEndColor: TColor read <getter>...

  if IsGet then
    Result := TColor_sw.ToVar(TCustomGrid_sacc(TCustomGrid(AObj)).
      GradientEndColor)
  else
    TCustomGrid_sacc(TCustomGrid(AObj)).GradientEndColor := TColor_sw.FromVar(
      AArgs[0]);
end;

function TCustomGrid_GradientStartColor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  if IsGet then
    ...
    TCustomGrid_sacc(TCustomGrid(AObj)).GradientStartColor := TColor_sw.FromVar(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := System.UITypes_LMDSW.TScrollStyle_sw.ToVar(TCustomGrid_sacc(
      TCustomGrid(AObj)).ScrollBars)
  else
    TCustomGrid_sacc(TCustomGrid(AObj)).ScrollBars := System.UITypes_LMDSW.
...
function TCustomGrid_OnFixedCellClick_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property OnFixedCellClick: TFixedCellClickEvent...

  if IsGet then
    Result := TFixedCellClickEvent_sw.ToVar(TCustomGrid_sacc(TCustomGrid(AObj)).
      OnFixedCellClick)
  else
    TCustomGrid_sacc(TCustomGrid(AObj)).OnFixedCellClick := 
      TFixedCellClickEvent_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('BorderStyle', TCustomGrid_BorderStyle_si, TBorderStyle_sw, True, True, 0, True, False);
  AData.AddProperty('Col', TCustomGrid_Col_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('ColCount', TCustomGrid_ColCount_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('ColWidths', TCustomGrid_ColWidths_si, Integer_sw, True, True, 1, True, False);
  AData.AddProperty('DefaultColWidth', TCustomGrid_DefaultColWidth_si, Integer_sw, True, True, 0, True, False);
  AData.AddProperty('DefaultDrawing', TCustomGrid_DefaultDrawing_si, Boolean_sw, True, True, 0, True, False);
  AData.AddProperty('DefaultRowHeight', TCustomGrid_DefaultRowHeight_si, Integer_sw, True, True, 0, True, False);
  AData.AddProperty('DrawingStyle', TCustomGrid_DrawingStyle_si, TGridDrawingStyle_sw, True, True, 0, True, False);
  AData.AddProperty('EditorMode', TCustomGrid_EditorMode_si, Boolean_sw, True, True, 0, True, False);
  AData.AddProperty('FixedColor', TCustomGrid_FixedColor_si, TColor_sw, True, True, 0, True, False);
  AData.AddProperty('FixedCols', TCustomGrid_FixedCols_si, Integer_sw, True, True, 0, True, False);
  AData.AddProperty('FixedRows', TCustomGrid_FixedRows_si, Integer_sw, True, True, 0, True, False);
  AData.AddProperty('GradientEndColor', TCustomGrid_GradientEndColor_si, TColor_sw, True, True, 0, True, False);
  AData.AddProperty('GradientStartColor', TCustomGrid_GradientStartColor_si, TColor_sw, True, True, 0, True, False);
  AData.AddProperty('GridHeight', TCustomGrid_GridHeight_si, Integer_sw, True, False, 0, True, False);
  AData.AddProperty('GridLineWidth', TCustomGrid_GridLineWidth_si, Integer_sw, True, True, 0, True, False);
  AData.AddProperty('GridWidth', TCustomGrid_GridWidth_si, Integer_sw, True, False, 0, True, False);
  AData.AddProperty('HitTest', TCustomGrid_HitTest_si, TPoint_sw, True, False, 0, True, False);
  AData.AddProperty('InplaceEditor', TCustomGrid_InplaceEditor_si, TInplaceEdit_sw, True, False, 0, True, False);
  AData.AddProperty('LeftCol', TCustomGrid_LeftCol_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('Options', TCustomGrid_Options_si, TGridOptions_sw, True, True, 0, True, False);
  AData.AddProperty('Row', TCustomGrid_Row_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('RowCount', TCustomGrid_RowCount_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('RowHeights', TCustomGrid_RowHeights_si, Integer_sw, True, True, 1, True, False);
  AData.AddProperty('ScrollBars', TCustomGrid_ScrollBars_si, System.UITypes_LMDSW.TScrollStyle_sw, True, True, 0, True, False);
  AData.AddProperty('Selection', TCustomGrid_Selection_si, TGridRect_sw, True, True, 0, True, False);
  AData.AddProperty('TabStops', TCustomGrid_TabStops_si, Boolean_sw, True, True, 1, True, False);
  AData.AddProperty('TopRow', TCustomGrid_TopRow_si, Longint_sw, True, True, 0, True, False);
  AData.AddProperty('VisibleColCount', TCustomGrid_VisibleColCount_si, Integer_sw, True, False, 0, True, False);
  AData.AddProperty('VisibleRowCount', TCustomGrid_VisibleRowCount_si, Integer_sw, True, False, 0, True, False);
  AData.AddProperty('OnFixedCellClick', TCustomGrid_OnFixedCellClick_si, TFixedCellClickEvent_sw, True, True, 0, True, False);
...
begin
  ...
  AData.AddProperty('OnColumnMoved', TCustomDrawGrid_OnColumnMoved_si, TMovedEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnGetEditMask', TCustomDrawGrid_OnGetEditMask_si, TGetEditEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnGetEditText', TCustomDrawGrid_OnGetEditText_si, TGetEditEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnRowMoved', TCustomDrawGrid_OnRowMoved_si, TMovedEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnSelectCell', TCustomDrawGrid_OnSelectCell_si, TSelectCellEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnSetEditText', TCustomDrawGrid_OnSetEditText_si, TSetEditEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnTopLeftChanged', TCustomDrawGrid_OnTopLeftChanged_si, TNotifyEvent_sw, True, True, 0, True, False);
  ...
  AData.AddPropertyRedecl('Col', False);
  AData.AddPropertyRedecl('ColWidths', False);
  AData.AddPropertyRedecl('DrawingStyle', False);
  AData.AddPropertyRedecl('EditorMode', False);
  AData.AddPropertyRedecl('GridHeight', False);
  AData.AddPropertyRedecl('GridWidth', False);
  AData.AddPropertyRedecl('LeftCol', False);
  AData.AddPropertyRedecl('Selection', False);
  AData.AddPropertyRedecl('Row', False);
  AData.AddPropertyRedecl('RowHeights', False);
  AData.AddPropertyRedecl('TabStops', False);
  AData.AddPropertyRedecl('TopRow', False);
end;
...
begin
  ...
  AData.AddPropertyRedecl('RowCount', False);
  AData.AddPropertyRedecl('FixedRows', False);
  ...
  AData.AddPropertyRedecl('OnFixedCellClick', False);
...
begin
  ...
  AData.AddProperty('Cells', TStringGrid_Cells_si, string_sw, True, True, 2, False, False);
  AData.AddProperty('Cols', TStringGrid_Cols_si, TStrings_sw, True, True, 1, False, False);
  AData.AddProperty('Objects', TStringGrid_Objects_si, TObject_sw, True, True, 2, False, False);
  AData.AddProperty('Rows', TStringGrid_Rows_si, TStrings_sw, True, True, 1, False, False);
end;
...
begin
  ...
  AData.AddProperty('ActiveList', TInplaceEditList_ActiveList_si, TWinControl_sw, True, True, 0, False, False);
  AData.AddProperty('ButtonWidth', TInplaceEditList_ButtonWidth_si, Integer_sw, True, True, 0, False, False);
  AData.AddProperty('DropDownRows', TInplaceEditList_DropDownRows_si, Integer_sw, True, True, 0, False, False);
  AData.AddProperty('EditStyle', TInplaceEditList_EditStyle_si, TEditStyle_sw, True, False, 0, False, False);
  AData.AddProperty('ListVisible', TInplaceEditList_ListVisible_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('PickList', TInplaceEditList_PickList_si, TCustomListbox_sw, True, False, 0, False, False);
  AData.AddProperty('PickListLoaded', TInplaceEditList_PickListLoaded_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('Pressed', TInplaceEditList_Pressed_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('OnEditButtonClick', TInplaceEditList_OnEditButtonClick_si, TNotifyEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnGetPickListitems', TInplaceEditList_OnGetPickListitems_si, TOnGetPickListItems_sw, True, True, 0, False, False);
end;
...
function Grids_MaxGridSize_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const MaxGridSize = <value>;

  Result := MaxGridSize;
end;

function Grids_MaxShortInt_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const MaxShortInt = <value>;

  Result := MaxShortInt;
end;

function Grids_MaxCustomExtents_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const MaxCustomExtents = <value>;

  Result := MaxCustomExtents;
end;
...
function Grids_TGridState_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TGridState = (gsNormal, gsSelecting, gsRowSizing, gs...
...
function Grids_TGridOption_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TGridOption = (goFixedVertLine, goFixedHorzLine, goV...
...
function Grids_THotTrackCellInfo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type THotTrackCellInfo = record;

  Result := TLMDUnitWrapper.TypeToVar(THotTrackCellInfo_sw);
end;
...
function Grids_TFixedCellClickEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TFixedCellClickEvent = procedure (Sender: TObject; A...

  Result := TLMDUnitWrapper.TypeToVar(TFixedCellClickEvent_sw);
end;

function Grids_TGridDrawingStyle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TGridDrawingStyle = (gdsClassic, gdsThemed, gdsGradi...

  Result := TLMDUnitWrapper.TypeToVar(TGridDrawingStyle_sw);
end;
...
begin
  AData.AddConstant('MaxGridSize', Grids_MaxGridSize_si, nil, VarType(Vcl.Grids.MaxGridSize));
  AData.AddConstant('MaxShortInt', Grids_MaxShortInt_si, ShortInt_sw);
  AData.AddConstant('MaxCustomExtents', Grids_MaxCustomExtents_si, Integer_sw);
  ...
  AData.AddType(THotTrackCellInfo_sw, Grids_THotTrackCellInfo_si);
  ...
  AData.AddType(TFixedCellClickEvent_sw, Grids_TFixedCellClickEvent_si);
  AData.AddType(TGridDrawingStyle_sw, Grids_TGridDrawingStyle_si);
...
initialization
  ...
  RegisterEventWrapper(TypeInfo(TFixedCellClickEvent), TFixedCellClickEvent_sw);
