...
begin
  ...
  Result := Data.DBCommon.GetFieldInfo(string(AArgs[0]), v_1);
...
begin
  ...
  Result := TIndexDef_sw.ToVar(Data.DBCommon.GetIndexForOrderBy(string(
...
begin
  ...
  Result := Data.DBCommon.GetTableNameFromSQL(string(AArgs[0]));
end;
...
begin
  ...
  Result := Data.DBCommon.GetTableNameFromQuery(string(AArgs[0]));
end;
  ...
  case AArgsSize of
    ...
    begin
      Result := Data.DBCommon.AddParamSQLForDetail(TParams_sw.FromVar(AArgs[0]),
        string(AArgs[1]), Boolean(AArgs[2]));
    end;
    4:
    begin
      Result := Data.DBCommon.AddParamSQLForDetail(TParams_sw.FromVar(AArgs[0]),
        string(AArgs[1]), Boolean(AArgs[2]), string(AArgs[3]));
    end;
  else
...
begin
  ...
  Result := Data.DBCommon.IsMultiTableQuery(string(AArgs[0]));
end;
...
begin
  ...
  Result := Data.DBCommon.SQLRequiresParams(string(AArgs[0]));
end;

function DBCommon_GetTableNameFromSQLEx_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function GetTableNameFromSQLEx(const SQL: string; IdOptio...

  Result := Data.DBCommon.GetTableNameFromSQLEx(string(AArgs[0]), 
...
begin
  ...
  AData.AddFunction('AddParamSQLForDetail', DBCommon_AddParamSQLForDetail_si, string_sw, 3, True);
  ...
  AData.AddFunction('GetTableNameFromSQLEx', DBCommon_GetTableNameFromSQLEx_si, string_sw, 2, False);
