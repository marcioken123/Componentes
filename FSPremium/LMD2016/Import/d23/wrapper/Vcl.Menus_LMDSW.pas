...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Vcl.Menus' unit.
  #
  # WARNING:
...
uses
  ...
  LMDSctWrappers, LMDSctSysWrappers, Winapi.Windows, System.SysUtils,
  System.Classes, System.Contnrs, Winapi.Messages, Vcl.Graphics, Vcl.ImgList,
  Vcl.ActnList, Vcl.Menus, Winapi.Windows_LMDSW, System.SysUtils_LMDSW,
  System.Classes_LMDSW, System.Contnrs_LMDSW, Winapi.Messages_LMDSW,
  Vcl.Graphics_LMDSW, Vcl.ImgList_LMDSW, Vcl.ActnList_LMDSW;
...
function TMenuItem_OriginalPPI_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const OriginalPPI = <value>;

  Result := TMenuItem_sc(AObj).OriginalPPI;
end;
...
function TMenuItem_Merged_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property Merged: TMenuItem read <getter>;

  Result := TMenuItem_sw.ToVar(TMenuItem_sacc(TMenuItem(AObj)).Merged);
end;
...
begin
  ...
  AData.AddProperty('Merged', TMenuItem_Merged_si, TMenuItem_sw, True, False, 0, True, False);
  ...
  AData.AddConstant('OriginalPPI', TMenuItem_OriginalPPI_si, nil, VarType(TMenuItem.OriginalPPI));
end;
...
function TMenu_FindItem_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function FindItem(Value: NativeUInt; Kind: TFindItemKind)...

  Result := TMenuItem_sw.ToVar(TMenu(AObj).FindItem(NativeUInt_sw.FromVar(
    AArgs[0]), TFindItemKind_sw.FromVar(AArgs[1])));
end;

function TMenu_GetHelpContext_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function GetHelpContext(Value: NativeUInt; ByCommand: Boo...

  Result := THelpContext_sw.ToVar(TMenu(AObj).GetHelpContext(NativeUInt_sw.
...
function TMenu_IsShortCut_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
var
  v_1: TWMKey;
begin
  ...
  Result := TMenu(AObj).IsShortCut(v_1);
...
function TMenu_ParentBiDiModeChanged_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function ParentBiDiModeChanged(...); overload;
  ...
  case AArgsSize of
    ...
    1:
    begin
      TMenu(AObj).ParentBiDiModeChanged(TObject_sw.FromVar(AArgs[0]));
    end;
  else
    WrongArgCountError('ParentBiDiModeChanged');
  end;
end;

function TMenu_ProcessMenuChar_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
var
  v_1: TWMMenuChar;
begin
  ...
  v_1 := TWMMenuChar_sw.FromVar(AArgs[0]);
  TMenu(AObj).ProcessMenuChar(v_1);
  AssignRefParam(AArgs[0], TWMMenuChar_sw.ToVar(v_1));
end;
...
begin
  ...
  AData.AddFunction('IsShortCut', TMenu_IsShortCut_si, Boolean_sw, 1, False);
  AData.AddProcedure('ParentBiDiModeChanged', TMenu_ParentBiDiModeChanged_si, 0, True);
  AData.AddProcedure('ProcessMenuChar', TMenu_ProcessMenuChar_si, 1, False);
...
function TPopupMenu_CloseMenu_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure CloseMenu;

  TPopupMenu(AObj).CloseMenu();
end;
...
begin
  ...
  AData.AddProcedure('CloseMenu', TPopupMenu_CloseMenu_si, 0, False);
...
function Menus_TMenuHandle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TMenuHandle = HMENU;

  Result := TLMDUnitWrapper.TypeToVar(TMenuHandle_sw);
end;
...
begin
  ...
  if IsGet then
    Result := TPopupList_sw.ToVar(Vcl.Menus.PopupList)
  else
    Vcl.Menus.PopupList := TPopupList_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := TMenuItemStack_sw.ToVar(Vcl.Menus.ShortCutItems)
  else
    Vcl.Menus.ShortCutItems := TMenuItemStack_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  Result := Vcl.Menus.ShortCutToText(TShortCut_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  Result := TShortCut_sw.ToVar(Vcl.Menus.TextToShortCut(string(AArgs[0])));
end;

function Menus_ShortCutFromMessage_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function ShortCutFromMessage(Message: TWMKey): TShortCut;

  Result := TShortCut_sw.ToVar(Vcl.Menus.ShortCutFromMessage(TWMKey_sw.FromVar(
    AArgs[0])));
end;
...
begin
  ...
  Result := TMenuItem_sw.ToVar(Vcl.Menus.NewItem(string(AArgs[0]), TShortCut_sw.
...
begin
  ...
  Result := TMenuItem_sw.ToVar(Vcl.Menus.NewLine());
end;
...
begin
  ...
  if IsGet then
    Result := Vcl.Menus.ValidMenuHotkeys
  else
    Vcl.Menus.ValidMenuHotkeys := string(AArgs[0]);
end;
...
begin
  ...
  Result := Vcl.Menus.StripHotkey(string(AArgs[0]));
end;
...
begin
  ...
  Result := Vcl.Menus.GetHotkey(string(AArgs[0]));
end;

function Menus_SameCaption_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function SameCaption(const Text1: string; const Text2: st...

  Result := Vcl.Menus.SameCaption(string(AArgs[0]), string(AArgs[1]));
end;
...
begin
  ...
  Result := Vcl.Menus.AnsiSameCaption(string(AArgs[0]), string(AArgs[1]));
end;
...
begin
  ...
  Result := Vcl.Menus.IsAltGRPressed();
end;
...
begin
  ...
  AData.AddType(TMenuHandle_sw, Menus_TMenuHandle_si);
  ...
  AData.AddFunction('ShortCutFromMessage', Menus_ShortCutFromMessage_si, TShortCut_sw, 1, False);
  ...
  AData.AddConstant('cHotkeyPrefix', Menus_cHotkeyPrefix_si, nil, VarType(Vcl.Menus.cHotkeyPrefix));
  AData.AddConstant('cLineCaption', Menus_cLineCaption_si, nil, VarType(Vcl.Menus.cLineCaption));
  AData.AddConstant('cDialogSuffix', Menus_cDialogSuffix_si, nil, VarType(Vcl.Menus.cDialogSuffix));
  ...
  AData.AddFunction('SameCaption', Menus_SameCaption_si, Boolean_sw, 2, False);
