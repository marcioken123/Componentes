...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Datasnap.DBClient' unit.
  #
  # WARNING:
...
uses
  ...
  LMDSctSysWrappers, System.SysUtils, System.Variants, System.Classes, Data.DB,
  Data.DBCommon, Data.DBCommonTypes, Datasnap.DBClient, System.SysUtils_LMDSW,
  System.Variants_LMDSW, System.Classes_LMDSW, Data.DB_LMDSW,
  Data.DBCommon_LMDSW, Data.DBCommonTypes_LMDSW;
...
begin
  ...
  if IsGet then
    ...
    TRecInfo_sw(AObj).FValue.RecordNumber := Integer(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('RecordNumber', TRecInfo_RecordNumber_si, Integer_sw);
...
begin
  ...
  if IsGet then
    ...
    TCustomClientDataSet(AObj).IndexFieldNames := string(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    ...
    TCustomClientDataSet(AObj).MasterFields := string(AArgs[0]);
end;
...
function TCustomClientDataSet_ParamByName_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function ParamByName(const Value: string): TParam;

  Result := TParam_sw.ToVar(TCustomClientDataSet(AObj).ParamByName(string(
    AArgs[0])));
end;
...
function TCustomClientDataSet_ChangeCount_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property ChangeCount: Int64 read <getter>;

  Result := Int64_sw.ToVar(TCustomClientDataSet(AObj).ChangeCount);
end;
...
function TCustomClientDataSet_SavePoint_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property SavePoint: Int64 read <getter> write <setter>;

  if IsGet then
    Result := Int64_sw.ToVar(TCustomClientDataSet(AObj).SavePoint)
  else
    TCustomClientDataSet(AObj).SavePoint := Int64_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('IndexFieldNames', TCustomClientDataSet_IndexFieldNames_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('MasterFields', TCustomClientDataSet_MasterFields_si, string_sw, True, True, 0, False, False);
  ...
  AData.AddFunction('ParamByName', TCustomClientDataSet_ParamByName_si, TParam_sw, 1, False);
  ...
  AData.AddProperty('ChangeCount', TCustomClientDataSet_ChangeCount_si, Int64_sw, True, False, 0, False, False);
  ...
  AData.AddProperty('SavePoint', TCustomClientDataSet_SavePoint_si, Int64_sw, True, True, 0, False, False);
...
begin
  ...
  AData.AddPropertyRedecl('PersistDataPacket', False);
  AData.AddPropertyRedecl('Active', False);
...
begin
  ...
  case AArgsSize of
    1:
    begin
      Result := Datasnap.DBClient.PackageParams(TParams_sw.FromVar(AArgs[0]));
    end;
    2:
    begin
      Result := Datasnap.DBClient.PackageParams(TParams_sw.FromVar(AArgs[0]), 
...
begin
  ...
  Datasnap.DBClient.UnpackParams(OleVariant(FindVarData(AArgs[0])^), TParams_sw.
    FromVar(AArgs[1]));
end;
...
begin
  ...
  AData.AddConstant('AllRecords', DBClient_AllRecords_si, nil, VarType(Datasnap.DBClient.AllRecords));
end;
