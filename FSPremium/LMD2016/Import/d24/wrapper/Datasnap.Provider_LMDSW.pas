...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Datasnap.Provider' unit.
  #
  # WARNING:
...
uses
  ...
  System.Types, Datasnap.Provider, System.SysUtils_LMDSW,
  System.Variants_LMDSW, System.Classes_LMDSW, Datasnap.DBClient_LMDSW,
  Data.DB_LMDSW, System.Types_LMDSW;
...
type
  ...
  public
    function  GetHandler: TMethod; override;
    procedure Handler(DataSet: TDataSet; Params: TPacketAttributeList);
  end;
...
begin
  // constructor Create(ErrMsg: string; Status: Integer; Dummy...
...
begin
  ...
  AData.AddProperty('ErrorCode', EDSWriter_ErrorCode_si, Integer_sw, True, False, 0, False, False);

  { Class members }

  AData.AddConstructor('Create', EDSWriter_Create_si, 2, True);
end;
...
class procedure TGetRecordOption_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..3] of TLMDEnumerator = (
    (Name: 'grMetaData'; Value: Integer({$IFDEF LMDSCT_12}TGetRecordOption.{$ENDIF}grMetaData)),
    (Name: 'grReset'; Value: Integer({$IFDEF LMDSCT_12}TGetRecordOption.{$ENDIF}grReset)),
    (Name: 'grXML'; Value: Integer({$IFDEF LMDSCT_12}TGetRecordOption.{$ENDIF}grXML)),
    (Name: 'grXMLUTF8'; Value: Integer({$IFDEF LMDSCT_12}TGetRecordOption.{$ENDIF}grXMLUTF8))
  );
begin
  AEnums := @ENUMS;
  ACount := 4;
...
begin
  ...
  args[1] := Input;
  Result := OleVariant(FindVarData(Self.Owner.RunProc(Self.ProcName, args))^);
end;
...
class procedure TProviderOption_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..15] of TLMDEnumerator = (
    (Name: 'poFetchBlobsOnDemand'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poFetchBlobsOnDemand)),
    (Name: 'poFetchDetailsOnDemand'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poFetchDetailsOnDemand)),
    (Name: 'poIncFieldProps'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poIncFieldProps)),
    (Name: 'poCascadeDeletes'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poCascadeDeletes)),
    (Name: 'poCascadeUpdates'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poCascadeUpdates)),
    (Name: 'poReadOnly'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poReadOnly)),
    (Name: 'poAllowMultiRecordUpdates'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poAllowMultiRecordUpdates)),
    (Name: 'poDisableInserts'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poDisableInserts)),
    (Name: 'poDisableEdits'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poDisableEdits)),
    (Name: 'poDisableDeletes'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poDisableDeletes)),
    (Name: 'poNoReset'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poNoReset)),
    (Name: 'poAutoRefresh'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poAutoRefresh)),
    (Name: 'poPropogateChanges'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poPropogateChanges)),
    (Name: 'poAllowCommandText'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poAllowCommandText)),
    (Name: 'poRetainServerOrder'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poRetainServerOrder)),
    (Name: 'poUseQuoteChar'; Value: Integer({$IFDEF LMDSCT_12}TProviderOption.{$ENDIF}poUseQuoteChar))
  );
begin
  AEnums := @ENUMS;
  ACount := 16;
...
begin
  ...
  AData.AddFunction('Copy', TPutFieldInfo_Copy_si, TPutFieldInfo_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TPutFieldInfo_Create_si, 0, False);
end;
...
procedure TGetParamsEvent_sh.Handler(DataSet: TDataSet;
  Params: TPacketAttributeList);
var
  args: array[0..1] of OleVariant;
begin
  // procedure (DataSet: TDataSet; Params: TPacketAttributeLis...

  args[0] := TDataSet_sw.ToVar(DataSet);
  args[1] := TPacketAttributeList_sw.ToVar(Params);
...
begin
  ...
  AData.AddProperty('Constraints', TDataPacketWriter_Constraints_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('PacketOptions', TDataPacketWriter_PacketOptions_si, TGetRecordOptions_sw, True, True, 0, False, False);
  AData.AddProperty('Options', TDataPacketWriter_Options_si, TProviderOptions_sw, True, True, 0, False, False);
  AData.AddProperty('OnGetParams', TDataPacketWriter_OnGetParams_si, TGetParamsEvent_sw, True, True, 0, False, False);
end;
...
function TPacketDataSet_AssignCurValues_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function AssignCurValues(...); overload;

  TPacketDataSet(AObj).AssignCurValues(Variant(FindVarData(AArgs[0])^));
end;
...
begin
  ...
  case AArgsSize of
    0:
    begin
      TPacketDataSet(AObj).InitAltRecBuffers();
    end;
    1:
    begin
    ...
    WrongArgCountError('InitAltRecBuffers');
  end;
end;
...
begin
  ...
  AData.AddProperty('WritingCurValues', TPacketDataSet_WritingCurValues_si, Boolean_sw, True, True, 0, True, False);
  ...
  AData.AddProcedure('InitAltRecBuffers', TPacketDataSet_InitAltRecBuffers_si, 0, True);
  AData.AddFunction('UpdateKind', TPacketDataSet_UpdateKind_si, TUpdateKind_sw, 0, False);
  AData.AddProperty('NewValuesModified', TPacketDataSet_NewValuesModified_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('StreamMetaData', TPacketDataSet_StreamMetaData_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('UseCurValues', TPacketDataSet_UseCurValues_si, Boolean_sw, True, True, 0, False, False);
end;
...
var
  ...
  v_3: OleVariant;
begin
  // procedure/function ApplyUpdates(...); overload;

  case AArgsSize of
    3:
    begin
      Result := TCustomProvider(AObj).ApplyUpdates(OleVariant(FindVarData(
        AArgs[0])^), Integer(AArgs[1]), v_1);
      AssignRefParam(AArgs[2], v_1);
    end;
    4:
    begin
      v_3 := OleVariant(FindVarData(AArgs[3])^);
      Result := TCustomProvider(AObj).ApplyUpdates(OleVariant(FindVarData(
        AArgs[0])^), Integer(AArgs[1]), v_2, v_3);
      AssignRefParam(AArgs[2], v_2);
      AssignRefParam(AArgs[3], v_3);
    end;
  else
    WrongArgCountError('ApplyUpdates');
  end;
end;
...
var
  ...
  v_4: OleVariant;
begin
  // procedure/function GetRecords(...); overload;

  case AArgsSize of
    3:
    begin
      Result := TCustomProvider(AObj).GetRecords(Integer(AArgs[0]), v_1, 
        Integer(AArgs[2]));
      AssignRefParam(AArgs[1], v_1);
    end;
    6:
    begin
      v_3 := OleVariant(FindVarData(AArgs[4])^);
      v_4 := OleVariant(FindVarData(AArgs[5])^);
      Result := TCustomProvider(AObj).GetRecords(Integer(AArgs[0]), v_2, 
        Integer(AArgs[2]), string(AArgs[3]), v_3, v_4);
      AssignRefParam(AArgs[1], v_2);
      AssignRefParam(AArgs[4], v_3);
      AssignRefParam(AArgs[5], v_4);
    end;
  else
    WrongArgCountError('GetRecords');
  end;
end;
...
begin
  ...
  v_1 := OleVariant(FindVarData(AArgs[2])^);
  Result := TCustomProvider(AObj).RowRequest(OleVariant(FindVarData(AArgs[0])^),
    Integer(AArgs[1]), v_1);
  AssignRefParam(AArgs[2], v_1);
end;
...
begin
  ...
  v_1 := OleVariant(FindVarData(AArgs[1])^);
  v_2 := OleVariant(FindVarData(AArgs[2])^);
  TCustomProvider(AObj).Execute(OleStr_sw.FromVar(AArgs[0]), v_1, v_2);
  AssignRefParam(AArgs[1], v_1);
  AssignRefParam(AArgs[2], v_2);
end;
...
begin
  ...
  v_1 := OleVariant(FindVarData(AArgs[0])^);
...
begin
  ...
  Result := TCustomProvider(AObj).DataRequest(OleVariant(FindVarData(
    AArgs[0])^));
end;
...
begin
  ...
  AData.AddProperty('OnDataRequest', TCustomProvider_OnDataRequest_si, TDataRequestEvent_sw, True, True, 0, True, False);
  AData.AddProperty('OnValidate', TCustomProvider_OnValidate_si, TOnValidate_sw, True, True, 0, True, False);
  AData.AddProperty('BeforeApplyUpdates', TCustomProvider_BeforeApplyUpdates_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('AfterApplyUpdates', TCustomProvider_AfterApplyUpdates_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('BeforeGetRecords', TCustomProvider_BeforeGetRecords_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('AfterGetRecords', TCustomProvider_AfterGetRecords_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('BeforeRowRequest', TCustomProvider_BeforeRowRequest_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('AfterRowRequest', TCustomProvider_AfterRowRequest_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('BeforeExecute', TCustomProvider_BeforeExecute_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('AfterExecute', TCustomProvider_AfterExecute_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('BeforeGetParams', TCustomProvider_BeforeGetParams_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('AfterGetParams', TCustomProvider_AfterGetParams_si, TRemoteEvent_sw, True, True, 0, True, False);
  AData.AddProperty('ActiveUpdateException', TCustomProvider_ActiveUpdateException_si, Exception_sw, True, False, 0, True, False);
  AData.AddProperty('OwnerData', TCustomProvider_OwnerData_si, OleVariant_sw, True, True, 0, True, False);
  AData.AddFunction('ApplyUpdates', TCustomProvider_ApplyUpdates_si, nil, 3, True);
  AData.AddFunction('GetRecords', TCustomProvider_GetRecords_si, nil, 3, True);
  ...
  AData.AddProperty('Data', TCustomProvider_Data_si, OleVariant_sw, True, False, 0, False, False);
  AData.AddProperty('Exported', TCustomProvider_Exported_si, Boolean_sw, True, True, 0, False, False);
end;
...
class procedure TResolverResponse_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..4] of TLMDEnumerator = (
    (Name: 'rrSkip'; Value: Integer({$IFDEF LMDSCT_12}TResolverResponse.{$ENDIF}rrSkip)),
    (Name: 'rrAbort'; Value: Integer({$IFDEF LMDSCT_12}TResolverResponse.{$ENDIF}rrAbort)),
    (Name: 'rrMerge'; Value: Integer({$IFDEF LMDSCT_12}TResolverResponse.{$ENDIF}rrMerge)),
    (Name: 'rrApply'; Value: Integer({$IFDEF LMDSCT_12}TResolverResponse.{$ENDIF}rrApply)),
    (Name: 'rrIgnore'; Value: Integer({$IFDEF LMDSCT_12}TResolverResponse.{$ENDIF}rrIgnore))
  );
begin
  AEnums := @ENUMS;
  ACount := 5;
...
begin
  ...
  AData.AddProperty('PacketDataSet', TBaseProvider_PacketDataSet_si, TPacketDataSet_sw, True, True, 0, True, False);
  AData.AddProperty('Resolver', TBaseProvider_Resolver_si, TCustomResolver_sw, True, False, 0, False, False);
  AData.AddProperty('Options', TBaseProvider_Options_si, TProviderOptions_sw, True, True, 0, False, False);
  AData.AddProperty('UpdateMode', TBaseProvider_UpdateMode_si, TUpdateMode_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('OnDataRequest', False);
  AData.AddProperty('OnGetData', TBaseProvider_OnGetData_si, TProviderDataEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnUpdateData', TBaseProvider_OnUpdateData_si, TProviderDataEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnUpdateError', TBaseProvider_OnUpdateError_si, TResolverErrorEvent_sw, True, True, 0, False, False);
  AData.AddProperty('BeforeUpdateRecord', TBaseProvider_BeforeUpdateRecord_si, TBeforeUpdateRecordEvent_sw, True, True, 0, False, False);
  AData.AddProperty('AfterUpdateRecord', TBaseProvider_AfterUpdateRecord_si, TAfterUpdateRecordEvent_sw, True, True, 0, False, False);
end;
...
begin
  ...
  args[2] := MakeVarRef(v_1);
  Self.Owner.RunProc(Self.ProcName, args);
  Properties := OleVariant(FindVarData(v_1)^);
end;
...
begin
  ...
  AData.AddProperty('TransactionStarted', TDataSetProvider_TransactionStarted_si, Boolean_sw, True, True, 0, True, False);
  AData.AddPropertyRedecl('ActiveUpdateException', False);
  AData.AddPropertyRedecl('OwnerData', False);
  AData.AddProperty('Params', TDataSetProvider_Params_si, TParams_sw, True, False, 0, False, False);
  AData.AddProperty('DataSet', TDataSetProvider_DataSet_si, TDataSet_sw, True, True, 0, False, False);
  AData.AddProperty('Constraints', TDataSetProvider_Constraints_si, Boolean_sw, True, True, 0, False, False);
  AData.AddProperty('ResolveToDataSet', TDataSetProvider_ResolveToDataSet_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Exported', False);
  AData.AddPropertyRedecl('Options', False);
  AData.AddPropertyRedecl('UpdateMode', False);
  AData.AddPropertyRedecl('OnDataRequest', False);
  AData.AddPropertyRedecl('OnGetData', False);
  AData.AddPropertyRedecl('OnUpdateData', False);
  AData.AddPropertyRedecl('OnUpdateError', False);
  AData.AddPropertyRedecl('AfterUpdateRecord', False);
  AData.AddPropertyRedecl('BeforeUpdateRecord', False);
  AData.AddPropertyRedecl('OnValidate', False);
  ...
  AData.AddProperty('OnGetTableName', TDataSetProvider_OnGetTableName_si, TGetTableNameEvent_sw, True, True, 0, False, False);
  AData.AddProperty('OnGetDataSetProperties', TDataSetProvider_OnGetDataSetProperties_si, TGetDSProps_sw, True, True, 0, False, False);
end;
...
function TUpdateTree_InitDelta_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function InitDelta(...); overload;

  TUpdateTree(AObj).InitDelta(OleVariant(FindVarData(AArgs[0])^));
end;
...
begin
  ...
  if IsGet then
    ...
    TUpdateTree(AObj).Name := string(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('Opened', TUpdateTree_Opened_si, Boolean_sw, True, True, 0, True, False);
  ...
  AData.AddProperty('Delta', TUpdateTree_Delta_si, TPacketDataSet_sw, True, True, 0, False, False);
  AData.AddProperty('DetailCount', TUpdateTree_DetailCount_si, Integer_sw, True, False, 0, False, False);
  AData.AddProperty('Details', TUpdateTree_Details_si, TUpdateTree_sw, True, False, 1, False, False);
  AData.AddProperty('ErrorDS', TUpdateTree_ErrorDS_si, TPacketDataSet_sw, True, True, 0, False, False);
  AData.AddProperty('HasErrors', TUpdateTree_HasErrors_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('IsNested', TUpdateTree_IsNested_si, Boolean_sw, True, False, 0, False, False);
  AData.AddProperty('Name', TUpdateTree_Name_si, string_sw, True, True, 0, False, False);
  AData.AddProperty('Parent', TUpdateTree_Parent_si, TUpdateTree_sw, True, False, 0, False, False);
  AData.AddProperty('Resolver', TUpdateTree_Resolver_si, TCustomResolver_sw, True, False, 0, False, False);
  AData.AddProperty('Source', TUpdateTree_Source_si, TDataSet_sw, True, True, 0, False, False);
...
begin
  ...
  Result := TCustomResolver(AObj).ApplyUpdates(OleVariant(FindVarData(
    AArgs[0])^), Integer(AArgs[1]), v_1);
  AssignRefParam(AArgs[2], v_1);
end;
...
begin
  ...
  Result := TCustomResolver(AObj).RowRequest(OleVariant(FindVarData(AArgs[0])^),
...
begin
  ...
  AData.AddProperty('Provider', TCustomResolver_Provider_si, TBaseProvider_sw, True, False, 0, False, False);
  AData.AddProperty('UpdateTree', TCustomResolver_UpdateTree_si, TUpdateTree_sw, True, False, 0, False, False);
...
begin
  ...
  AData.AddProperty('Provider', TDataSetResolver_Provider_si, TDataSetProvider_sw, True, False, 0, True, False);
...
begin
  ...
  AData.AddProperty('Provider', TSQLResolver_Provider_si, TDataSetProvider_sw, True, False, 0, True, False);
...
begin
  ...
  if IsGet then
    Result := Datasnap.Provider.InformixLob
  else
    Datasnap.Provider.InformixLob := Boolean(AArgs[0]);
end;
...
function Provider_TGetRecordOption_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TGetRecordOption = (grMetaData, grReset, grXML, grXM...
...
function Provider_TProviderOption_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TProviderOption = (poFetchBlobsOnDemand, poFetchDeta...
...
function Provider_TResolverResponse_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TResolverResponse = (rrSkip, rrAbort, rrMerge, rrApp...
...
function Provider_TLocalAppServer_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TLocalAppServer = class(TInterfacedObject);
...
begin
  ...
  Result := TObject_sw.ToVar(Datasnap.Provider.GetObjectProperty(TPersistent_sw.
...
begin
  ...
  Result := Datasnap.Provider.GetStringProperty(TPersistent_sw.FromVar(
...
begin
  ...
  Result := Datasnap.Provider.VarArrayFromStrings(TStrings_sw.FromVar(
    AArgs[0]));
end;
...
begin
  ...
  AData.AddFunction('GetStringProperty', Provider_GetStringProperty_si, string_sw, 2, False);
