...
type
  ...
  TMultiWaitEvent_sw = class;

  OleStr_sw = WideString_sw;

  TEndian_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TEndian): OleVariant;
    class function FromVar(const AValue: OleVariant): TEndian;
  end;
  ...
  TWaitResult_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TWaitResult): OleVariant;
    class function FromVar(const AValue: OleVariant): TWaitResult;
  end;

  TMultiWaitEvent_sc = class of TMultiWaitEvent;
  TMultiWaitEvent_sw = class(TObject_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  public
    class function ToVar(const AValue: TMultiWaitEvent): OleVariant;
    class function FromVar(const AValue: OleVariant): TMultiWaitEvent;
    class function ClassToVar(AClass: TMultiWaitEvent_sc): OleVariant;
    class function ClassFromVar(const AClass: OleVariant): TMultiWaitEvent_sc;
  end;
...
type
  TMultiWaitEvent_sacc = class(TMultiWaitEvent);

{ TEndian_sw }

class function TEndian_sw.GetTypeName: WideString;
begin
  Result := 'TEndian';
end;

class procedure TEndian_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..1] of TLMDEnumerator = (
    (Name: 'Big'; Value: Integer({$IFDEF LMDSCT_12}TEndian.{$ENDIF}Big)),
    (Name: 'Little'; Value: Integer({$IFDEF LMDSCT_12}TEndian.{$ENDIF}Little))
  );
begin
  AEnums := @ENUMS;
  ACount := 2;
...
class function TEndian_sw.ToVar(const AValue: TEndian): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TEndian_sw.FromVar(const AValue: OleVariant): TEndian;
begin
  Result := TEndian(Integer(AValue));
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TSize_sw(AObj).FValue.cx)
  else
    TSize_sw(AObj).FValue.cx := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TSize_sw(AObj).FValue.cy)
  else
    TSize_sw(AObj).FValue.cy := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('cx', TSize_cx_si, FixedInt_sw);
  AData.AddField('cy', TSize_cy_si, FixedInt_sw);
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TPoint_sw(AObj).FValue.X)
  else
    TPoint_sw(AObj).FValue.X := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TPoint_sw(AObj).FValue.Y)
  else
    TPoint_sw(AObj).FValue.Y := FixedInt_sw.FromVar(AArgs[0]);
end;
...
function TPoint_PointInCircle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class function PointInCircle(const Point: TPoint; const C...

  Result := TPoint.PointInCircle(TPoint_sw.FromVar(AArgs[0]), TPoint_sw.FromVar(
    AArgs[1]), Integer(AArgs[2]));
end;

function TPoint_Zero_si(var AObj; const AArgs: TLMDDispArgs; AArgsSize: Integer;
  IsGet: Boolean): OleVariant;
begin
  // class function Zero: TPoint;

  Result := TPoint_sw.ToVar(TPoint.Zero());
end;
...
function TPoint_Angle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Angle(const APoint: TPoint): Single;

  Result := TPoint_sw(AObj).FValue.Angle(TPoint_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  AData.AddField('X', TPoint_X_si, FixedInt_sw);
  AData.AddField('Y', TPoint_Y_si, FixedInt_sw);
  ...
  AData.AddFunction('Angle', TPoint_Angle_si, Single_sw, 1, False);
  AData.AddFunction('Copy', TPoint_Copy_si, TPoint_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', TPoint_Create_si, 0, True);
  AData.AddClassFunction('PointInCircle', TPoint_PointInCircle_si, Boolean_sw, 3, False);
  AData.AddClassFunction('Zero', TPoint_Zero_si, TPoint_sw, 0, False);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TRect_sw(AObj).FValue.Left)
  else
    TRect_sw(AObj).FValue.Left := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TRect_sw(AObj).FValue.Top)
  else
    TRect_sw(AObj).FValue.Top := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TRect_sw(AObj).FValue.Right)
  else
    TRect_sw(AObj).FValue.Right := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := FixedInt_sw.ToVar(TRect_sw(AObj).FValue.Bottom)
  else
    TRect_sw(AObj).FValue.Bottom := FixedInt_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddField('Left', TRect_Left_si, FixedInt_sw);
  AData.AddField('Top', TRect_Top_si, FixedInt_sw);
  AData.AddField('Right', TRect_Right_si, FixedInt_sw);
  AData.AddField('Bottom', TRect_Bottom_si, FixedInt_sw);
...
function TPointF_PointInCircle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class function PointInCircle(const Point: TPointF; const ...

  Result := TPointF.PointInCircle(TPointF_sw.FromVar(AArgs[0]), TPointF_sw.
    FromVar(AArgs[1]), Integer(AArgs[2]));
end;

function TPointF_Zero_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class function Zero: TPointF;

  Result := TPointF_sw.ToVar(TPointF.Zero());
end;
...
function TPointF_EqualsTo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function EqualsTo(const Point: TPointF; const Epsilon: Si...

  case AArgsSize of
    1:
    begin
      Result := TPointF_sw(AObj).FValue.EqualsTo(TPointF_sw.FromVar(AArgs[0]));
    end;
    2:
    begin
      Result := TPointF_sw(AObj).FValue.EqualsTo(TPointF_sw.FromVar(AArgs[0]), 
...
function TPointF_Angle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Angle(const APoint: TPointF): Single;

  Result := TPointF_sw(AObj).FValue.Angle(TPointF_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  AData.AddFunction('EqualsTo', TPointF_EqualsTo_si, Boolean_sw, 1, True);
  ...
  AData.AddFunction('Angle', TPointF_Angle_si, Single_sw, 1, False);
  ...
  AData.AddClassFunction('PointInCircle', TPointF_PointInCircle_si, Boolean_sw, 3, False);
  AData.AddClassFunction('Zero', TPointF_Zero_si, TPointF_sw, 0, False);
end;
...
function TRectF_Fit_si(var AObj; const AArgs: TLMDDispArgs; AArgsSize: Integer;
  IsGet: Boolean): OleVariant;
begin
  // function Fit(const BoundsRect: TRectF): Single;

  Result := TRectF_sw(AObj).FValue.Fit(TRectF_sw.FromVar(AArgs[0]));
end;

function TRectF_FitInto_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
var
  v_1: Single;
begin
  // procedure/function FitInto(...); overload;

  case AArgsSize of
    2:
    begin
      Result := TRectF_sw.ToVar(TRectF_sw(AObj).FValue.FitInto(TRectF_sw.
        FromVar(AArgs[0]), v_1));
...
function TRectF_EqualsTo_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function EqualsTo(const R: TRectF; const Epsilon: Single)...

  case AArgsSize of
    1:
    begin
      Result := TRectF_sw(AObj).FValue.EqualsTo(TRectF_sw.FromVar(AArgs[0]));
    end;
    2:
    begin
      Result := TRectF_sw(AObj).FValue.EqualsTo(TRectF_sw.FromVar(AArgs[0]), 
...
begin
  ...
  AData.AddFunction('FitInto', TRectF_FitInto_si, nil, 1, True);
  ...
  AData.AddFunction('EqualsTo', TRectF_EqualsTo_si, Boolean_sw, 1, True);
...
class function TWaitResult_sw.GetTypeName: WideString;
begin
  Result := 'TWaitResult';
end;

class procedure TWaitResult_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..4] of TLMDEnumerator = (
    (Name: 'wrSignaled'; Value: Integer({$IFDEF LMDSCT_12}TWaitResult.{$ENDIF}wrSignaled)),
    (Name: 'wrTimeout'; Value: Integer({$IFDEF LMDSCT_12}TWaitResult.{$ENDIF}wrTimeout)),
    (Name: 'wrAbandoned'; Value: Integer({$IFDEF LMDSCT_12}TWaitResult.{$ENDIF}wrAbandoned)),
    (Name: 'wrError'; Value: Integer({$IFDEF LMDSCT_12}TWaitResult.{$ENDIF}wrError)),
    (Name: 'wrIOCompletion'; Value: Integer({$IFDEF LMDSCT_12}TWaitResult.{$ENDIF}wrIOCompletion))
  );
begin
  AEnums := @ENUMS;
  ACount := 5;
...
class function TWaitResult_sw.ToVar(const AValue: TWaitResult): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TWaitResult_sw.FromVar(const AValue: OleVariant): TWaitResult;
begin
  Result := TWaitResult(Integer(AValue));
end;

{ TMultiWaitEvent_sw }

function TMultiWaitEvent_WaiterCount_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property WaiterCount: Integer read <getter>;

  Result := TMultiWaitEvent_sacc(TMultiWaitEvent(AObj)).WaiterCount;
end;

function TMultiWaitEvent_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // class function Create: TMultiWaitEvent;

  Result := TMultiWaitEvent_sw.ToVar(TMultiWaitEvent_sc(AObj).Create());
end;

function TMultiWaitEvent_WaitFor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  case AArgsSize of
    0:
    begin
      Result := TWaitResult_sw.ToVar(TMultiWaitEvent(AObj).WaitFor());
    end;
    1:
    begin
      Result := TWaitResult_sw.ToVar(TMultiWaitEvent(AObj).WaitFor(Cardinal(
        AArgs[0])));
    end;
  else
    WrongArgCountError('WaitFor');
  end;
end;

function TMultiWaitEvent_SetEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure SetEvent;

  TMultiWaitEvent(AObj).SetEvent();
end;

function TMultiWaitEvent_ResetEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure ResetEvent;

  TMultiWaitEvent(AObj).ResetEvent();
end;

class function TMultiWaitEvent_sw.GetTypeName: WideString;
begin
  Result := 'TMultiWaitEvent';
end;

class function TMultiWaitEvent_sw.GetWrappedClass: TClass;
begin
  Result := TMultiWaitEvent;
end;

class procedure TMultiWaitEvent_sw.DoInit(AData: TLMDClassWrapperData);
begin
  { Instance members }

  AData.AddProperty('WaiterCount', TMultiWaitEvent_WaiterCount_si, Integer_sw, True, False, 0, True, False);
  AData.AddFunction('WaitFor', TMultiWaitEvent_WaitFor_si, TWaitResult_sw, 0, True);
  AData.AddProcedure('SetEvent', TMultiWaitEvent_SetEvent_si, 0, False);
  AData.AddProcedure('ResetEvent', TMultiWaitEvent_ResetEvent_si, 0, False);

  { Class members }

  AData.AddClassFunction('Create', TMultiWaitEvent_Create_si, TMultiWaitEvent_sw, 0, False);
end;

class function TMultiWaitEvent_sw.ToVar(
  const AValue: TMultiWaitEvent): OleVariant;
begin
  Result := ConvToVar(AValue);
end;

class function TMultiWaitEvent_sw.FromVar(
  const AValue: OleVariant): TMultiWaitEvent;
begin
  Result := TMultiWaitEvent(ConvFromVar(AValue, TMultiWaitEvent));
end;

class function TMultiWaitEvent_sw.ClassToVar(
  AClass: TMultiWaitEvent_sc): OleVariant;
begin
  Result := ConvClsToVar(AClass);
end;

class function TMultiWaitEvent_sw.ClassFromVar(
  const AClass: OleVariant): TMultiWaitEvent_sc;
begin
  Result := TMultiWaitEvent_sc(ConvClsFromVar(AClass, TMultiWaitEvent));
end;
...
function Types_TEndian_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TEndian = (Big, Little);

  Result := TLMDUnitWrapper.TypeToVar(TEndian_sw);
end;

function Types_PlatformEndian_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // const PlatformEndian = <value>;

  Result := TEndian_sw.ToVar(PlatformEndian);
end;
...
function Types_TBitConverter_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
function Types_PtInCircle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function PtInCircle(...); overload;
...
function Types_TWaitResult_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWaitResult = (wrSignaled, wrTimeout, wrAbandoned, w...

  Result := TLMDUnitWrapper.TypeToVar(TWaitResult_sw);
end;

function Types_TMultiWaitEvent_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TMultiWaitEvent = class(TObject);

  Result := TLMDUnitWrapper.TypeToVar(TMultiWaitEvent_sw);
end;
...
begin
  ...
  AData.AddType(TEndian_sw, Types_TEndian_si);
  AData.AddConstant('PlatformEndian', Types_PlatformEndian_si, TEndian_sw);
  ...
  AData.AddType(TBitConverter_sw, Types_TBitConverter_si);
  ...
  AData.AddFunction('PointF', Types_PointF_si, nil, 2, False);
  ...
  AData.AddFunction('PtInCircle', Types_PtInCircle_si, nil, 3, False);
  ...
  AData.AddType(TWaitResult_sw, Types_TWaitResult_si);
  AData.AddType(TMultiWaitEvent_sw, Types_TMultiWaitEvent_si);
end;
...
initialization
  ...
  RegisterClassWrapper(TMultiWaitEvent_sw);
