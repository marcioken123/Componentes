...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'System.WideStrings' unit.
  #
  # WARNING:
...
uses
  ...
  LMDSctSysWrappers, System.Types, System.Classes, System.SysUtils,
  System.WideStrings, System.Types_LMDSW, System.Classes_LMDSW,
  System.SysUtils_LMDSW;
...
function TWideStrings_AddStrings_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function AddStrings(...); overload;

  TWideStrings(AObj).AddStrings(TWideStrings_sw.FromVar(AArgs[0]));
end;
...
begin
  ...
  case AArgsSize of
    ...
    2:
    begin
      TWideStrings(AObj).LoadFromFile(WideString(AArgs[0]), TEncoding_sw.
        FromVar(AArgs[1]));
    end;
  else
...
begin
  ...
  case AArgsSize of
    ...
    2:
    begin
      TWideStrings(AObj).SaveToFile(WideString(AArgs[0]), TEncoding_sw.FromVar(
        AArgs[1]));
    end;
  else
...
begin
  ...
  AData.AddProcedure('AddStrings', TWideStrings_AddStrings_si, 1, False);
  ...
  AData.AddProcedure('LoadFromFile', TWideStrings_LoadFromFile_si, 1, True);
  AData.AddProcedure('LoadFromStream', TWideStrings_LoadFromStream_si, 1, True);
  AData.AddProcedure('Move', TWideStrings_Move_si, 2, False);
  AData.AddProcedure('SaveToFile', TWideStrings_SaveToFile_si, 1, True);
  AData.AddProcedure('SaveToStream', TWideStrings_SaveToStream_si, 1, True);
...
function TWideStringList_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  case AArgsSize of
    0:
    begin
      Result := TWideStringList_sw.ToVar(TWideStringList_sc(AObj).Create());
    end;
    1:
    begin
      Result := TWideStringList_sw.ToVar(TWideStringList_sc(AObj).Create(
...
begin
  ...
  AData.AddConstructor('Create', TWideStringList_Create_si, 0, True);
end;
