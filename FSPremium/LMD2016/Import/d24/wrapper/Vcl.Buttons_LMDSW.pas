...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Vcl.Buttons' unit.
  #
  # WARNING:
...
uses
  ...
  Vcl.StdCtrls, Vcl.ExtCtrls, Winapi.CommCtrl, Vcl.ImgList, Vcl.Buttons,
  Winapi.Windows_LMDSW, Winapi.Messages_LMDSW, System.Classes_LMDSW,
  Vcl.Controls_LMDSW, Vcl.Forms_LMDSW, Vcl.Graphics_LMDSW, Vcl.StdCtrls_LMDSW,
  Vcl.ExtCtrls_LMDSW, Winapi.CommCtrl_LMDSW, Vcl.ImgList_LMDSW;
...
type
  ...
  TBitBtn_sw = class(TCustomButton_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  ...
  TNavBtnID_sw = Integer_sw;

  TBaseNavigator_sc = class of TBaseNavigator;
  TBaseNavigator_sw = class(TCustomPanel_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  public
    class function ToVar(const AValue: TBaseNavigator): OleVariant;
    class function FromVar(const AValue: OleVariant): TBaseNavigator;
    class function ClassToVar(AClass: TBaseNavigator_sc): OleVariant;
    class function ClassFromVar(const AClass: OleVariant): TBaseNavigator_sc;
  end;
...
function TBaseNavigator_Flat_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Flat: Boolean read <getter> write <setter>;

  if IsGet then
    Result := TBaseNavigator(AObj).Flat
  else
    TBaseNavigator(AObj).Flat := Boolean(AArgs[0]);
end;

function TBaseNavigator_ConfirmDelete_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property ConfirmDelete: Boolean read <getter> write <sett...

  if IsGet then
    Result := TBaseNavigator(AObj).ConfirmDelete
  else
    TBaseNavigator(AObj).ConfirmDelete := Boolean(AArgs[0]);
end;

class function TBaseNavigator_sw.GetTypeName: WideString;
begin
  Result := 'TBaseNavigator';
end;

class function TBaseNavigator_sw.GetWrappedClass: TClass;
begin
  Result := TBaseNavigator;
end;

class procedure TBaseNavigator_sw.DoInit(AData: TLMDClassWrapperData);
begin
  { Instance members }
  ...
  AData.AddPropertyRedecl('DragMode', False);
  AData.AddPropertyRedecl('Enabled', False);
  AData.AddProperty('Flat', TBaseNavigator_Flat_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Ctl3D', False);
  AData.AddPropertyRedecl('ParentCtl3D', False);
  AData.AddPropertyRedecl('ParentShowHint', False);
  AData.AddPropertyRedecl('PopupMenu', False);
  AData.AddProperty('ConfirmDelete', TBaseNavigator_ConfirmDelete_si, Boolean_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('ShowHint', False);
  AData.AddPropertyRedecl('TabOrder', False);
  AData.AddPropertyRedecl('TabStop', False);
  AData.AddPropertyRedecl('Visible', False);
...
class function TBaseNavigator_sw.ToVar(
  const AValue: TBaseNavigator): OleVariant;
begin
  Result := ConvToVar(AValue);
end;

class function TBaseNavigator_sw.FromVar(
  const AValue: OleVariant): TBaseNavigator;
begin
  Result := TBaseNavigator(ConvFromVar(AValue, TBaseNavigator));
end;

class function TBaseNavigator_sw.ClassToVar(
  AClass: TBaseNavigator_sc): OleVariant;
begin
  Result := ConvClsToVar(AClass);
end;

class function TBaseNavigator_sw.ClassFromVar(
  const AClass: OleVariant): TBaseNavigator_sc;
begin
  Result := TBaseNavigator_sc(ConvClsFromVar(AClass, TBaseNavigator));
end;
...
function TNavButton_Index_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Index: TNavBtnID read <getter> write <setter>;

  if IsGet then
    Result := TNavBtnID_sw.ToVar(TNavButton(AObj).Index)
  else
    TNavButton(AObj).Index := TNavBtnID_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('Index', TNavButton_Index_si, TNavBtnID_sw, True, True, 0, False, False);
end;
...
function Buttons_TBitBtn_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TBitBtn = class(TCustomButton);

  Result := TLMDUnitWrapper.TypeToVar(TBitBtn_sw);
end;

function Buttons_TNavBtnID_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TNavBtnID = Integer;

  Result := TLMDUnitWrapper.TypeToVar(TNavBtnID_sw);
end;

function Buttons_TBaseNavigator_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TBaseNavigator = class(TCustomPanel);

  Result := TLMDUnitWrapper.TypeToVar(TBaseNavigator_sw);
end;

function Buttons_TNavButton_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
begin
  ...
  Result := TRect_sw.ToVar(Vcl.Buttons.DrawButtonFace(TCanvas_sw.FromVar(
    AArgs[0]), TRect_sw.FromVar(AArgs[1]), Integer(AArgs[2]), TButtonStyle_sw.
    FromVar(AArgs[3]), Boolean(AArgs[4]), Boolean(AArgs[5]), Boolean(
    AArgs[6])));
end;
...
begin
  ...
  AData.AddType(TNavBtnID_sw, Buttons_TNavBtnID_si);
  AData.AddType(TBaseNavigator_sw, Buttons_TBaseNavigator_si);
  AData.AddType(TNavButton_sw, Buttons_TNavButton_si);
...
initialization
  ...
  RegisterClassWrapper(TBaseNavigator_sw);
