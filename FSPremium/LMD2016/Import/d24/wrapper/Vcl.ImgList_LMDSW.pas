...

{
  ##############################################################################
  #
  # LMD Script Pack wrappers for 'Vcl.ImgList' unit.
  #
  # WARNING:
...
uses
  ...
  System.Classes, Vcl.Graphics, Winapi.CommCtrl, System.ImageList,
  System.UITypes, Vcl.ImgList, Winapi.Windows_LMDSW, System.Classes_LMDSW,
  Vcl.Graphics_LMDSW, Winapi.CommCtrl_LMDSW, System.ImageList_LMDSW,
...
type
  ...
  TChangeLink_sw = class(TImageLink_sw)
  protected
  ...
  TImageListHandle_sw = HImageList_sw;
  ...
  TImageIndex_sw = System.UITypes_LMDSW.TImageIndex_sw;

  TColorDepth_sw = class(TLMDEnumWrapper)
  protected
    class function  GetTypeName: WideString; override;
    class procedure GetEnumerators(out AEnums: PLMDEnumerator;
                                   out ACount: Integer); override;
  public
    class function ToVar(const AValue: TColorDepth): OleVariant;
    class function FromVar(const AValue: OleVariant): TColorDepth;
  end;

  TCustomImageList_sc = class of TCustomImageList;
  TCustomImageList_sw = class(TBaseImageList_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
...
class function TColorDepth_sw.GetTypeName: WideString;
begin
  Result := 'TColorDepth';
end;

class procedure TColorDepth_sw.GetEnumerators(out AEnums: PLMDEnumerator;
  out ACount: Integer);
const
  ENUMS: array[0..6] of TLMDEnumerator = (
    (Name: 'cdDefault'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cdDefault)),
    (Name: 'cdDeviceDependent'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cdDeviceDependent)),
    (Name: 'cd4Bit'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cd4Bit)),
    (Name: 'cd8Bit'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cd8Bit)),
    (Name: 'cd16Bit'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cd16Bit)),
    (Name: 'cd24Bit'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cd24Bit)),
    (Name: 'cd32Bit'; Value: Integer({$IFDEF LMDSCT_12}TColorDepth.{$ENDIF}cd32Bit))
  );
begin
  AEnums := @ENUMS;
  ACount := 7;
...
class function TColorDepth_sw.ToVar(const AValue: TColorDepth): OleVariant;
begin
  Result := Integer(AValue);
end;

class function TColorDepth_sw.FromVar(const AValue: OleVariant): TColorDepth;
begin
  Result := TColorDepth(Integer(AValue));
end;
...
function TCustomImageList_BeginUpdate_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure BeginUpdate;

  TCustomImageList(AObj).BeginUpdate();
end;
...
begin
  ...
  case AArgsSize of
    ...
    begin
      TCustomImageList(AObj).Draw(TCanvas_sw.FromVar(AArgs[0]), Integer(
        AArgs[1]), Integer(AArgs[2]), Integer(AArgs[3]), TDrawingStyle_sw.FromVar(
        AArgs[4]), TImageType_sw.FromVar(AArgs[5]));
    end;
    7:
    begin
      TCustomImageList(AObj).Draw(TCanvas_sw.FromVar(AArgs[0]), Integer(
        AArgs[1]), Integer(AArgs[2]), Integer(AArgs[3]), TDrawingStyle_sw.FromVar(
        AArgs[4]), TImageType_sw.FromVar(AArgs[5]), Boolean(AArgs[6]));
    end;
  else
...
function TCustomImageList_DrawOverlay_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function DrawOverlay(...); overload;
  ...
  case AArgsSize of
    ...
    begin
      TCustomImageList(AObj).DrawOverlay(TCanvas_sw.FromVar(AArgs[0]), Integer(
        AArgs[1]), Integer(AArgs[2]), Integer(AArgs[3]), TOverlay_sw.FromVar(
        AArgs[4]), TDrawingStyle_sw.FromVar(AArgs[5]), TImageType_sw.FromVar(
        AArgs[6]));
    end;
    8:
    begin
...
function TCustomImageList_EndUpdate_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure EndUpdate;

  TCustomImageList(AObj).EndUpdate();
end;
...
function TCustomImageList_GetIcon_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function GetIcon(...); overload;

  case AArgsSize of
    2:
    begin
      TCustomImageList(AObj).GetIcon(Integer(AArgs[0]), TIcon_sw.FromVar(
        AArgs[1]));
    end;
    4:
    begin
      TCustomImageList(AObj).GetIcon(Integer(AArgs[0]), TIcon_sw.FromVar(
        AArgs[1]), TDrawingStyle_sw.FromVar(AArgs[2]), TImageType_sw.FromVar(
        AArgs[3]));
    end;
  else
    WrongArgCountError('GetIcon');
  end;
end;
...
function TCustomImageList_GetInstRes_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure/function GetInstRes(...); overload;
...
function TCustomImageList_SetSize_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // procedure SetSize(AWidth: Integer; AHeight: Integer);

  TCustomImageList(AObj).SetSize(Integer(AArgs[0]), Integer(AArgs[1]));
end;
...
function TCustomImageList_Handle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Handle: HImageList read <getter> write <setter>;

  if IsGet then
    Result := HImageList_sw.ToVar(TCustomImageList(AObj).Handle)
  else
    TCustomImageList(AObj).Handle := HImageList_sw.FromVar(AArgs[0]);
end;
...
function TCustomImageList_GrayscaleFactor_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property GrayscaleFactor: Byte read <getter> write <setter>;

  if IsGet then
    Result := TCustomImageList(AObj).GrayscaleFactor
  else
    TCustomImageList(AObj).GrayscaleFactor := Byte(AArgs[0]);
end;
...
function TCustomImageList_ColorDepth_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property ColorDepth: TColorDepth read <getter> write <set...

  if IsGet then
    Result := TColorDepth_sw.ToVar(TCustomImageList(AObj).ColorDepth)
  else
    TCustomImageList(AObj).ColorDepth := TColorDepth_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProcedure('BeginUpdate', TCustomImageList_BeginUpdate_si, 0, False);
  ...
  AData.AddProcedure('EndUpdate', TCustomImageList_EndUpdate_si, 0, False);
  ...
  AData.AddProcedure('GetIcon', TCustomImageList_GetIcon_si, 2, True);
  ...
  AData.AddFunction('GetInstRes', TCustomImageList_GetInstRes_si, nil, 6, False);
  ...
  AData.AddProcedure('SetSize', TCustomImageList_SetSize_si, 2, False);
  AData.AddProcedure('UnRegisterChanges', TCustomImageList_UnRegisterChanges_si, 1, False);
  AData.AddProperty('Handle', TCustomImageList_Handle_si, HImageList_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('GrayscaleFactor', TCustomImageList_GrayscaleFactor_si, Byte_sw, True, True, 0, False, False);
  ...
  AData.AddProperty('ColorDepth', TCustomImageList_ColorDepth_si, TColorDepth_sw, True, True, 0, False, False);
...
function ImgList_TChangeLink_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TChangeLink = class(TImageLink);

  Result := TLMDUnitWrapper.TypeToVar(TChangeLink_sw);
end;

function ImgList_TImageListHandle_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TImageListHandle = HImageList;

  Result := TLMDUnitWrapper.TypeToVar(TImageListHandle_sw);
end;
...
function ImgList_TImageIndex_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TImageIndex = System.UITypes.TImageIndex;

  Result := TLMDUnitWrapper.TypeToVar(TImageIndex_sw);
end;

function ImgList_TColorDepth_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TColorDepth = (cdDefault, cdDeviceDependent, cd4Bit,...

  Result := TLMDUnitWrapper.TypeToVar(TColorDepth_sw);
end;

function ImgList_TCustomImageList_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TCustomImageList = class(TBaseImageList);
...
begin
  ...
  AData.AddType(TImageListHandle_sw, ImgList_TImageListHandle_si);
  ...
  AData.AddType(TColorDepth_sw, ImgList_TColorDepth_si);
