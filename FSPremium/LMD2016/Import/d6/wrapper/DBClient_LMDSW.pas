...
uses
  ...
  LMDSctSysWrappers, Windows, SysUtils, Variants, Classes, DB, DBCommon,
  DBClient, Windows_LMDSW, SysUtils_LMDSW, Variants_LMDSW, Classes_LMDSW,
...
function TCustomClientDataSet_CommandText_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property CommandText: string read <getter> writ...

  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).CommandText
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).CommandText := string(
      AArgs[0]);
end;
...
function TCustomClientDataSet_ConnectionBroker_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property ConnectionBroker: TConnectionBroker re...

  if IsGet then
    Result := TConnectionBroker_sw.ToVar(TCustomClientDataSet_sacc(
      TCustomClientDataSet(AObj)).ConnectionBroker)
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).ConnectionBroker := 
      TConnectionBroker_sw.FromVar(AArgs[0]);
end;
...
function TCustomClientDataSet_FileName_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property FileName: string read <getter> write <...

  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).FileName
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).FileName := string(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    ...
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).IndexFieldNames := 
      string(AArgs[0]);
end;
...
function TCustomClientDataSet_IndexName_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property IndexName: string read <getter> write ...

  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).IndexName
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).IndexName := string(
      AArgs[0]);
end;
...
begin
  ...
  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).MasterFields
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).MasterFields := 
      string(AArgs[0]);
end;
...
function TCustomClientDataSet_PacketRecords_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property PacketRecords: Integer read <getter> w...

  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).
      PacketRecords
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).PacketRecords := 
      Integer(AArgs[0]);
end;
...
function TCustomClientDataSet_ProviderName_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property ProviderName: string read <getter> wri...

  if IsGet then
    Result := TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).ProviderName
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).ProviderName := 
      string(AArgs[0]);
end;
...
function TCustomClientDataSet_RemoteServer_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // protected property RemoteServer: TCustomRemoteServer read...

  if IsGet then
    Result := TCustomRemoteServer_sw.ToVar(TCustomClientDataSet_sacc(
      TCustomClientDataSet(AObj)).RemoteServer)
  else
    TCustomClientDataSet_sacc(TCustomClientDataSet(AObj)).RemoteServer := 
      TCustomRemoteServer_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('CommandText', TCustomClientDataSet_CommandText_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('ConnectionBroker', TCustomClientDataSet_ConnectionBroker_si, TConnectionBroker_sw, True, True, 0, True, False);
  AData.AddProperty('DisableStringTrim', TCustomClientDataSet_DisableStringTrim_si, Boolean_sw, True, True, 0, True, False);
  AData.AddProperty('FileName', TCustomClientDataSet_FileName_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('IndexDefs', TCustomClientDataSet_IndexDefs_si, TIndexDefs_sw, True, True, 0, True, False);
  AData.AddProperty('IndexFieldNames', TCustomClientDataSet_IndexFieldNames_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('IndexName', TCustomClientDataSet_IndexName_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('FetchOnDemand', TCustomClientDataSet_FetchOnDemand_si, Boolean_sw, True, True, 0, True, False);
  AData.AddProperty('MasterFields', TCustomClientDataSet_MasterFields_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('MasterSource', TCustomClientDataSet_MasterSource_si, TDataSource_sw, True, True, 0, True, False);
  AData.AddProperty('PacketRecords', TCustomClientDataSet_PacketRecords_si, Integer_sw, True, True, 0, True, False);
  ...
  AData.AddProperty('ProviderName', TCustomClientDataSet_ProviderName_si, string_sw, True, True, 0, True, False);
  AData.AddProperty('ReadOnly', TCustomClientDataSet_ReadOnly_si, Boolean_sw, True, True, 0, True, False);
  AData.AddProperty('RemoteServer', TCustomClientDataSet_RemoteServer_si, TCustomRemoteServer_sw, True, True, 0, True, False);
...
function DBClient_TKeyIndex_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
...
begin
  ...
  AData.AddType(TKeyIndex_sw, DBClient_TKeyIndex_si);
