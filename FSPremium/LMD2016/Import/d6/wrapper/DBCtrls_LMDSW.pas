...
uses
  ...
  LMDSctSysWrappers, Variants, Windows, SysUtils, Messages, Controls, Forms,
  Classes, Graphics, Menus, StdCtrls, ExtCtrls, Mask, Buttons, ComCtrls, DB,
  DBCtrls, Variants_LMDSW, Windows_LMDSW, SysUtils_LMDSW, Messages_LMDSW,
...
type
  ...
  TButtonSet_sw = class(TLMDSetWrapper)
  protected
    class function GetTypeName: WideString; override;
    class function GetSetSizeOf: Integer; override;
  public
    class function ToVar(const AValue: TButtonSet): OleVariant;
    class function FromVar(const AValue: OleVariant): TButtonSet;
  end;
...
class function TButtonSet_sw.GetTypeName: WideString;
begin
  Result := 'TButtonSet';
end;

class function TButtonSet_sw.GetSetSizeOf: Integer;
begin
  Result := SizeOf(TButtonSet);
end;

class function TButtonSet_sw.ToVar(const AValue: TButtonSet): OleVariant;
begin
  Result := ConvToVar(@AValue);
end;

class function TButtonSet_sw.FromVar(const AValue: OleVariant): TButtonSet;
begin
...
function TDBNavigator_VisibleButtons_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property VisibleButtons: TButtonSet read <getter> write <...

  if IsGet then
    Result := TButtonSet_sw.ToVar(TDBNavigator(AObj).VisibleButtons)
  else
    TDBNavigator(AObj).VisibleButtons := TButtonSet_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('VisibleButtons', TDBNavigator_VisibleButtons_si, TButtonSet_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('Align', False);
  AData.AddPropertyRedecl('Anchors', False);
  AData.AddPropertyRedecl('Constraints', False);
  AData.AddPropertyRedecl('DragCursor', False);
...
function DBCtrls_TButtonSet_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TButtonSet = set of TNavigateBtn;

  Result := TLMDUnitWrapper.TypeToVar(TButtonSet_sw);
end;
...
begin
  ...
  AData.AddType(TButtonSet_sw, DBCtrls_TButtonSet_si);
