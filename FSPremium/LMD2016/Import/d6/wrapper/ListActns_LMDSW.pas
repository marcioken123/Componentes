...
uses
  ...
  LMDSctWrappers, LMDSctSysWrappers, Classes, Controls, ActnList, ImgList,
  ListActns, Classes_LMDSW, Controls_LMDSW, ActnList_LMDSW, ImgList_LMDSW;
...
type
  ...
  TVirtualListAction_sw = class(TCustomListAction_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
  ...
  TStaticListAction_sw = class(TCustomListAction_sw)
  protected
    class function  GetTypeName: WideString; override;
    class function  GetWrappedClass: TClass; override;
    class procedure DoInit(AData: TLMDClassWrapperData); override;
...
begin
  ...
  if IsGet then
    Result := TCustomListAction_sacc(TCustomListAction(AObj)).ItemIndex
  else
    TCustomListAction_sacc(TCustomListAction(AObj)).ItemIndex := Integer(
      AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('ItemIndex', TCustomListAction_ItemIndex_si, Integer_sw, True, True, 0, True, False);
...
function TStaticListAction_Items_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Items: TStaticListItems read <getter> write <set...

  if IsGet then
    Result := TStaticListItems_sw.ToVar(TStaticListAction(AObj).Items)
  else
    TStaticListAction(AObj).Items := TStaticListItems_sw.FromVar(AArgs[0]);
end;

function TStaticListAction_OnGetItem_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property OnGetItem: TGetItemEvent read <getter> write <se...

  if IsGet then
    Result := TGetItemEvent_sw.ToVar(TStaticListAction(AObj).OnGetItem)
  else
    TStaticListAction(AObj).OnGetItem := TGetItemEvent_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddPropertyRedecl('Images', False);
  AData.AddPropertyRedecl('ItemIndex', False);
  AData.AddProperty('Items', TStaticListAction_Items_si, TStaticListItems_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('ShortCut', False);
  AData.AddPropertyRedecl('SecondaryShortCuts', False);
  AData.AddPropertyRedecl('Visible', False);
  AData.AddProperty('OnGetItem', TStaticListAction_OnGetItem_si, TGetItemEvent_sw, True, True, 0, False, False);
  AData.AddPropertyRedecl('OnItemSelected', False);
  AData.AddPropertyRedecl('OnHint', False);
...
function ListActns_TVirtualListAction_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TVirtualListAction = class(TCustomListAction);
...
function ListActns_TStaticListAction_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TStaticListAction = class(TCustomListAction);
