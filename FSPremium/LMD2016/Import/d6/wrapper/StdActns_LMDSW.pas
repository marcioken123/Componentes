...
function TWindowAction_Form_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // property Form: TForm read <getter> write <setter>;

  if IsGet then
    Result := TForm_sw.ToVar(TWindowAction(AObj).Form)
  else
    TWindowAction(AObj).Form := TForm_sw.FromVar(AArgs[0]);
end;
...
begin
  ...
  AData.AddProperty('Form', TWindowAction_Form_si, TForm_sw, True, True, 0, False, False);
end;
