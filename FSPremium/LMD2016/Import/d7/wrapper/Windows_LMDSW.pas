...
type
  ...
  _WIN32_FIND_DATAA_sw = class(TLMDRecordWrapper)
  private
    FValue: _WIN32_FIND_DATAA;
  protected
    class function  GetTypeName: WideString; override;
    function        CloneVal: TLMDRecordWrapper; override;
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: _WIN32_FIND_DATAA): OleVariant;
    class function FromVar(const AValue: OleVariant): _WIN32_FIND_DATAA;
  end;

  TWin32FindDataA_sw = _WIN32_FIND_DATAA_sw;

  TWin32FindData_sw = TWin32FindDataA_sw;

  tagWNDCLASSA_sw = class(TLMDRecordWrapper)
  private
    FValue: tagWNDCLASSA;
  protected
    class function  GetTypeName: WideString; override;
    ...
    class procedure DoInit(AData: TLMDRecordWrapperData); override;
  public
    class function ToVar(const AValue: tagWNDCLASSA): OleVariant;
    class function FromVar(const AValue: OleVariant): tagWNDCLASSA;
  end;

  TWndClassA_sw = tagWNDCLASSA_sw;

  TWndClass_sw = TWndClassA_sw;
...
{ _WIN32_FIND_DATAA_sw }

function _WIN32_FIND_DATAA_dwFileAttributes_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwFileAttributes: DWORD;

  if IsGet then
    Result := DWORD_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.dwFileAttributes)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.dwFileAttributes := DWORD_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_ftCreationTime_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var ftCreationTime: TFileTime;

  if IsGet then
    Result := TFileTime_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.
      ftCreationTime)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.ftCreationTime := TFileTime_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_ftLastAccessTime_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  if IsGet then
    ...
    _WIN32_FIND_DATAA_sw(AObj).FValue.ftLastAccessTime := TFileTime_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_ftLastWriteTime_si(var AObj;
  const AArgs: TLMDDispArgs; AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  ...
  if IsGet then
    ...
    _WIN32_FIND_DATAA_sw(AObj).FValue.ftLastWriteTime := TFileTime_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_nFileSizeHigh_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var nFileSizeHigh: DWORD;

  if IsGet then
    Result := DWORD_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.nFileSizeHigh)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.nFileSizeHigh := DWORD_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_nFileSizeLow_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var nFileSizeLow: DWORD;

  if IsGet then
    Result := DWORD_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.nFileSizeLow)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.nFileSizeLow := DWORD_sw.FromVar(
      AArgs[0]);
end;

function _WIN32_FIND_DATAA_dwReserved0_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwReserved0: DWORD;

  if IsGet then
    Result := DWORD_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.dwReserved0)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.dwReserved0 := DWORD_sw.FromVar(AArgs[0]);
end;

function _WIN32_FIND_DATAA_dwReserved1_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var dwReserved1: DWORD;

  if IsGet then
    Result := DWORD_sw.ToVar(_WIN32_FIND_DATAA_sw(AObj).FValue.dwReserved1)
  else
    _WIN32_FIND_DATAA_sw(AObj).FValue.dwReserved1 := DWORD_sw.FromVar(AArgs[0]);
end;

function _WIN32_FIND_DATAA_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := _WIN32_FIND_DATAA_sw.Create as IDispatch;
end;

function _WIN32_FIND_DATAA_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: _WIN32_FIND_DATAA;

  // Implicit record copy method.
  Result := _WIN32_FIND_DATAA_sw.ToVar(_WIN32_FIND_DATAA(AObj));
end;

class function _WIN32_FIND_DATAA_sw.GetTypeName: WideString;
begin
  Result := '_WIN32_FIND_DATAA';
end;

function _WIN32_FIND_DATAA_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := _WIN32_FIND_DATAA_sw.Create;
  _WIN32_FIND_DATAA_sw(Result).FValue := FValue;
end;

class procedure _WIN32_FIND_DATAA_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('dwFileAttributes', _WIN32_FIND_DATAA_dwFileAttributes_si, DWORD_sw);
  AData.AddField('ftCreationTime', _WIN32_FIND_DATAA_ftCreationTime_si, TFileTime_sw);
  AData.AddField('ftLastAccessTime', _WIN32_FIND_DATAA_ftLastAccessTime_si, TFileTime_sw);
  AData.AddField('ftLastWriteTime', _WIN32_FIND_DATAA_ftLastWriteTime_si, TFileTime_sw);
  AData.AddField('nFileSizeHigh', _WIN32_FIND_DATAA_nFileSizeHigh_si, DWORD_sw);
  AData.AddField('nFileSizeLow', _WIN32_FIND_DATAA_nFileSizeLow_si, DWORD_sw);
  AData.AddField('dwReserved0', _WIN32_FIND_DATAA_dwReserved0_si, DWORD_sw);
  AData.AddField('dwReserved1', _WIN32_FIND_DATAA_dwReserved1_si, DWORD_sw);
  AData.AddFunction('Copy', _WIN32_FIND_DATAA_Copy_si, _WIN32_FIND_DATAA_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', _WIN32_FIND_DATAA_Create_si, 0, False);
end;

class function _WIN32_FIND_DATAA_sw.ToVar(
  const AValue: _WIN32_FIND_DATAA): OleVariant;
var
  wrpr: _WIN32_FIND_DATAA_sw;
begin
  wrpr        := _WIN32_FIND_DATAA_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function _WIN32_FIND_DATAA_sw.FromVar(
  const AValue: OleVariant): _WIN32_FIND_DATAA;
begin
  Result := _WIN32_FIND_DATAA_sw(ConvFromVar(AValue)).FValue;
end;

{ tagWNDCLASSA_sw }

function tagWNDCLASSA_style_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var style: UINT;

  if IsGet then
    Result := UINT_sw.ToVar(tagWNDCLASSA_sw(AObj).FValue.style)
  else
    tagWNDCLASSA_sw(AObj).FValue.style := UINT_sw.FromVar(AArgs[0]);
end;

function tagWNDCLASSA_cbClsExtra_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var cbClsExtra: Integer;

  if IsGet then
    Result := tagWNDCLASSA_sw(AObj).FValue.cbClsExtra
  else
    tagWNDCLASSA_sw(AObj).FValue.cbClsExtra := Integer(AArgs[0]);
end;

function tagWNDCLASSA_cbWndExtra_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var cbWndExtra: Integer;

  if IsGet then
    Result := tagWNDCLASSA_sw(AObj).FValue.cbWndExtra
  else
    tagWNDCLASSA_sw(AObj).FValue.cbWndExtra := Integer(AArgs[0]);
end;

function tagWNDCLASSA_hInstance_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hInstance: HINST;

  if IsGet then
    Result := HINST_sw.ToVar(tagWNDCLASSA_sw(AObj).FValue.hInstance)
  else
    tagWNDCLASSA_sw(AObj).FValue.hInstance := HINST_sw.FromVar(AArgs[0]);
end;

function tagWNDCLASSA_hIcon_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hIcon: HICON;

  if IsGet then
    Result := HICON_sw.ToVar(tagWNDCLASSA_sw(AObj).FValue.hIcon)
  else
    tagWNDCLASSA_sw(AObj).FValue.hIcon := HICON_sw.FromVar(AArgs[0]);
end;

function tagWNDCLASSA_hCursor_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hCursor: HCURSOR;

  if IsGet then
    Result := HCURSOR_sw.ToVar(tagWNDCLASSA_sw(AObj).FValue.hCursor)
  else
    tagWNDCLASSA_sw(AObj).FValue.hCursor := HCURSOR_sw.FromVar(AArgs[0]);
end;

function tagWNDCLASSA_hbrBackground_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // var hbrBackground: HBRUSH;

  if IsGet then
    Result := HBRUSH_sw.ToVar(tagWNDCLASSA_sw(AObj).FValue.hbrBackground)
  else
    tagWNDCLASSA_sw(AObj).FValue.hbrBackground := HBRUSH_sw.FromVar(AArgs[0]);
end;

function tagWNDCLASSA_Create_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // constructor Create;

  // Implicit parameterless constructor.
  Result := tagWNDCLASSA_sw.Create as IDispatch;
end;

function tagWNDCLASSA_Copy_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // function Copy: tagWNDCLASSA;

  // Implicit record copy method.
  Result := tagWNDCLASSA_sw.ToVar(tagWNDCLASSA(AObj));
end;

class function tagWNDCLASSA_sw.GetTypeName: WideString;
begin
  Result := 'tagWNDCLASSA';
end;

function tagWNDCLASSA_sw.CloneVal: TLMDRecordWrapper;
begin
  Result := tagWNDCLASSA_sw.Create;
  tagWNDCLASSA_sw(Result).FValue := FValue;
end;

class procedure tagWNDCLASSA_sw.DoInit(AData: TLMDRecordWrapperData);
begin
  { Instance members }

  AData.AddField('style', tagWNDCLASSA_style_si, UINT_sw);
  AData.AddField('cbClsExtra', tagWNDCLASSA_cbClsExtra_si, Integer_sw);
  AData.AddField('cbWndExtra', tagWNDCLASSA_cbWndExtra_si, Integer_sw);
  AData.AddField('hInstance', tagWNDCLASSA_hInstance_si, HINST_sw);
  AData.AddField('hIcon', tagWNDCLASSA_hIcon_si, HICON_sw);
  AData.AddField('hCursor', tagWNDCLASSA_hCursor_si, HCURSOR_sw);
  AData.AddField('hbrBackground', tagWNDCLASSA_hbrBackground_si, HBRUSH_sw);
  AData.AddFunction('Copy', tagWNDCLASSA_Copy_si, tagWNDCLASSA_sw, 0, False);

  { Class members }

  AData.AddConstructor('Create', tagWNDCLASSA_Create_si, 0, False);
end;

class function tagWNDCLASSA_sw.ToVar(const AValue: tagWNDCLASSA): OleVariant;
var
  wrpr: tagWNDCLASSA_sw;
begin
  wrpr        := tagWNDCLASSA_sw.Create;
  wrpr.FValue := AValue;
  
  Result := ConvToVar(wrpr);
end;

class function tagWNDCLASSA_sw.FromVar(const AValue: OleVariant): tagWNDCLASSA;
begin
  Result := tagWNDCLASSA_sw(ConvFromVar(AValue)).FValue;
end;
...
function Windows__WIN32_FIND_DATAA_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type _WIN32_FIND_DATAA = record;

  Result := TLMDUnitWrapper.TypeToVar(_WIN32_FIND_DATAA_sw);
end;

function Windows_TWin32FindDataA_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWin32FindDataA = _WIN32_FIND_DATAA;

  Result := TLMDUnitWrapper.TypeToVar(TWin32FindDataA_sw);
end;

function Windows_TWin32FindData_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWin32FindData = TWin32FindDataA;

  Result := TLMDUnitWrapper.TypeToVar(TWin32FindData_sw);
end;

function Windows_tagWNDCLASSA_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type tagWNDCLASSA = record;

  Result := TLMDUnitWrapper.TypeToVar(tagWNDCLASSA_sw);
end;

function Windows_TWndClassA_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWndClassA = tagWNDCLASSA;

  Result := TLMDUnitWrapper.TypeToVar(TWndClassA_sw);
end;

function Windows_TWndClass_si(var AObj; const AArgs: TLMDDispArgs;
  AArgsSize: Integer; IsGet: Boolean): OleVariant;
begin
  // type TWndClass = TWndClassA;
...
begin
  ...
  AData.AddType(_WIN32_FIND_DATAA_sw, Windows__WIN32_FIND_DATAA_si);
  AData.AddType(TWin32FindDataA_sw, Windows_TWin32FindDataA_si);
  AData.AddType(TWin32FindData_sw, Windows_TWin32FindData_si);
  AData.AddType(tagWNDCLASSA_sw, Windows_tagWNDCLASSA_si);
  AData.AddType(TWndClassA_sw, Windows_TWndClassA_si);
