...
// The unit is provided as is except two minor changes, marked as "LMD adaption":
// [+] ApplyThemeChange method added in TThemeServices class;
// [*] TThemeServices.UpdateThemes changed so that LMD native controls support themes without using XP manifest
//----------------------------------------------------------------------------------------------------------------------
...
uses
  Windows, Classes, Messages, Graphics, Controls, UxTheme;
...
type
  ...
  public
    ...
    procedure ApplyThemeChange; //LMD Adaption (VB Apr 2009)
...
begin
  if FUseThemes then
    UnloadThemeData;
  FUseThemes := UseThemes {and (GetComCtlVersion >= ComCtlVersionIE6)}; //LMD Adaption (VB Apr 2009)

  //if FUseThemes then
  if (GetComCtlVersion >= ComCtlVersionIE6) then                        //LMD Adaption (VB Apr 2009)
  begin
