...
type
  ...
  TAppBarSettings = record
    ...
    nCurMonNum: Integer; // number of current monitor
    end;

  // TAppBar class ////////////////////////////////////////////////////////////
  TElAppBar = class(TElComponent)
  private
...
begin
  ...
  begin
    ...
    with rcProposed do
    begin
    rcProposed := Screen.Monitors[FABS.nCurMonNum].BoundsRect;
    if KeepSize then
      begin
        GetCursorPos(P);
...
begin
  ...
  begin
    ...
    x := Screen.Monitors[FABS.nCurMonNum].Left;
    y := Screen.Monitors[FABS.nCurMonNum].Top; 
    if bShow then
    // If we are on the right or bottom, calculate our visible position
      case GetEdge of
        abeRight :
         x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right - (rc.Right - rc.Left);
         abeBottom :
         y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Bottom - (rc.Bottom - rc.Top);
         end
    else
      // Calculate our x or y coordinate so that only the border is visible
      case GetEdge of
        abeLeft :
         x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Left -((rc.Right - rc.Left) - cxVisibleBorder);
         abeRight :
         x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right - cxVisibleBorder;
         abeTop :
         y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Top -((rc.Bottom - rc.Top) - cyVisibleBorder);
         abeBottom :
         y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Bottom - cyVisibleBorder;
         end;
  end
  else
  begin
    ...
    if bShow then
      case GetEdge of
      abeTop : y := Screen.Monitors[FABS.nCurMonNum].Top;
        abeBottom : y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Bottom - (rc.Bottom - rc.Top);
        abeLeft : x := Screen.Monitors[FABS.nCurMonNum].Left;
        abeRight : x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right - (rc.Right - rc.Left);
      end
    else
      case GetEdge of
      abeTop : y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Top -((rc.Bottom - rc.Top) - cyVisibleBorder);
        abeBottom : y := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Bottom - cyVisibleBorder;
        abeLeft : x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Left -((rc.Right - rc.Left) - cxVisibleBorder);
        abeRight : x := Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right - cxVisibleBorder;
      end;
  end;
...
begin
  ...
  begin
    FInPosChanged := true;
    SetWindowPos(LMDGetOwnerForm(Self).Handle, hwnd_TopMost, 0, 0, 0, 0, swp_NoMove + swp_NoSize + swp_NoActivate);
    FInPosChanged := false;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Right > Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right then
        OffsetRect(Rc, Screen.Monitors[FABS.nCurMonNum].BoundsRect.Right - Right, 0);
      AForm.BoundsRect := Rc;
    end;
  end;
end;
...
begin
  ...
  pt := GetMessagePosition;
  FABS.nCurMonNum := Screen.MonitorFromPoint(SmallPointToPoint(pt)).MonitorNum;
...
procedure TElAppBar.WMGetMinMaxInfo(var Message : TMessage);
  ...
  if GetEdge = abeFloat then
    ...
    with  TWMGetMinMaxInfo(Message).MinMaxInfo^  do
      begin
        ptMinTrackSize.X := 0;
        ptMinTrackSize.Y := 0;
        ptMaxTrackSize.X := LMDGetWorkSpaceRect.Right; 
        ptMaxTrackSize.Y := LMDGetWorkSpaceRect.Bottom;
...
begin
  ...
  begin
    ...
    rc := Screen.Monitors[FABS.nCurMonNum].BoundsRect;
  ...
  cxScreen := Screen.Monitors[FABS.nCurMonNum].Width;
  cyScreen := Screen.Monitors[FABS.nCurMonNum].Height;
  ptCenter.X := cxScreen div 2;
  ptCenter.Y := cyScreen div 2;
  ptOffset.X := pt.X - ptCenter.X - Screen.Monitors[FABS.nCurMonNum].Left;
  ptOffset.Y := pt.Y - ptCenter.Y - Screen.Monitors[FABS.nCurMonNum].Top;
...
begin
  ...
  FHook.Control := LMDGetOwnerForm(Owner);
