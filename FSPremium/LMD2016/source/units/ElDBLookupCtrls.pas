...
23/10/2004
  Released support fkLookup fields like native Delphi DBLookup
  controllers such TDBLookupComboBox or TDBLookupListBox
...
begin
  ...
  begin
    ...
    begin
      ListLink.DataSet.Locate(DataSource.DataSet.fieldbyname(DataField).LookupKeyFields,FElDBLookupListBox.KeyValue,[]);
      SelectCurrent;
    end;
  end;
end;
      ...
      if FElDBLookupListBox.Items.Count<>RecCount then
        begin
           ...
           try
             if FElDBLookupListBox.Items.Count>RecCount then 
               begin
                 i := (FElDBLookupListBox.Items.Count-RecCount); 
                 if i=FElDBLookupListBox.Items.Count then 
                   FElDBLookupListBox.Items.Clear
                 else
                 if (i/100*(RecCount)) > 25 then 
                   begin
                       FElDBLookupListBox.Items.Clear;
               ...
               for i:= 1 to RecCount-FElDBLookupListBox.Items.Count do
                 FElDBLookupListBox.Items.Add('');
           finally
...
begin
  if Assigned(FElDBLookupListBox) then
  begin
    try
...
begin
  ...
  then
    begin
      ...
      begin
        if Assigned(ListLink.DataSet.FindField(DataSource.DataSet.fieldbyname(DataField).LookupKeyFields)) then
...
procedure TElDBLookupListBox.DrawItem( Index: Integer; Rect: TRect; State: TOwnerDrawState );
var
    ...
    begin 
        ...
        begin
          if Assigned(DataSource) and Assigned(DataSource.DataSet) and
             Assigned(DataSource.DataSet.FindField(DataField)) and
             (not VarIsNull(KeyValue)) and
             Assigned(FElDBLookupControl.ListLink.DataSet) and
             Assigned(FElDBLookupControl.ListLink.DataSet.FindField(DataSource.DataSet.fieldbyname(DataField).LookupResultField)) and
             VarEquals(FElDBLookupControl.ListLink.DataSet.FindField(DataSource.DataSet.fieldbyname(DataField).LookupKeyFields).Value, KeyValue)
          then
          // is selected
          begin
             Canvas.Font.Color  := SelectedFont.Color;
...
procedure TElDBLookupComboBox.DrawItem( Index: Integer; Rect: TRect; State: TOwnerDrawState );
var
  i: integer;
  R: TRect;
  S: String;
