...
type
  ...
  TElDBTreeStringGrid = class;

  THackElStylesDefsItem = class(TElStylesDefsItem);

  TElDBTreeGridColumns = class(TPersistent)
  private
  ...
  published
    property NoRebuildOnDataChange default False;
    property DataSource;
  ...
  protected
    ...
    procedure WMGetDlgCode(var Message: TWMGetDlgCode); message WM_GETDLGCODE;
  public
    function FindNewFocused(Key : word; PVal1 : PInteger; var Sel : boolean): TElTreeItem; override;
  end;
...
procedure TCustomElDBTreeGrid.WMChar(var Message: TWMChar);
begin
...
type
  TCustomElHackTree = class (TCustomElTree);

function TElDBTreeGridView.FindNewFocused(Key: word; PVal1 : PInteger; var Sel: boolean): TElTreeItem;
begin
  result:=inherited FindNewFocused(Key, PVal1, Sel);
end;
...
{  FPressCoord := Point(X, Y);
  ...
  begin
    ...
    begin
      if Assigned(Item) and (ItemPart = ipCheckBox) then
      begin
        FClickedItem := nil;
        if not (Item.Enabled or (TCustomElHackTree(FOwner).IgnoreEnabled)) then
          exit;
...
function TElDBTreeStringGrid.GetCols(Index: Integer): TLMDStrings;
var i: integer;
  Item: TElTreeItem;
  hs: integer;
begin
...
procedure TElDBTreeStringGrid.SetCols(Index: Integer; Value: TLMDStrings);
var i: integer;
begin
  IsUpdating := true;
  try
...
function TElDBTreeStringGrid.GetRows(Index: Integer): TLMDStrings;
var i: integer;
  Item: TElTreeItem;
begin
  if FRows = nil then
...
procedure TElDBTreeStringGrid.SetRows(Index: Integer; Value: TLMDStrings);
var i: integer;
begin
  IsUpdating := true;
  try
...
begin
  ...
  TElDBTreeTrickyInplaceEdit(FEditor).FOwner := Self;
...
function TElDBTreeStringGrid.GetCells(ACol, ARow: Integer): TLMDString;
begin
...
procedure TElDBTreeStringGrid.SetCells(ACol, ARow: Integer; Value: TLMDString);
var Item: TElTreeItem;
  hs: integer;
begin
  IsUpdating := true;
...
begin
  ...
  for j := 0 to Tree.StylesDefs.Count - 1 do
  begin
    if (Tree.StylesDefs[j].HasColumn) or (THackElStylesDefsItem(Tree.StylesDefs[j]).DataFields.Count > 0) then
...
procedure TElDBTreeGridColumns.Insert(FieldName: string;HeaderTitle: TLMDString;
  ColumnIndex: Integer);
var
  s, s1, h, h1: string;
  j, i : Integer;
...
procedure TElDBTreeGridColumns.Add(FieldName: string; HeaderTitle: TLMDString);
begin
...
{ TElDBTreeTrickyInplaceEdit }

procedure TElDBTreeTrickyInplaceEdit.TriggerBeforeOperation(
  var DefaultConversion: boolean);
begin
  Font.Assign(TElDBTreeTrickyInplaceEdit(self).FOwner.Font);
  Font.Charset := TElDBTreeTrickyInplaceEdit(self).FOwner.Font.Charset;
{$IFDEF LMD_UNICODE}
  UnicodeMode := TElDBTreeTrickyInplaceEdit(self).FOwner.UnicodeMode;
{$endif}
  inherited;
end;
