...
type
  ...
  private
    ...
    FList: TStringlist;
    {$IFDEF LMD_UNICODE}
    FWideList: TLMDWideStringList;
    FWideString: string;
    {$endif}
  protected
    {$IFDEF LMD_UNICODE}
    function GetWideFileList : TLMDWideStringList;
...
{$IFDEF LMD_UNICODE}
    property WideFileList : TLMDWideStringList read GetWideFileList;
    property WideStringData: WideString read GetWideString;
{$endif}
  ...
  TElDropTarget = class(TElComponent, IUnknown, IDropTarget)
  private
    FRefCount : Integer;
    FTarget : TWinControl;
    FDragObj : ToleDragObject;
  ...
  published
    ...
    property Visible;
    property Enabled;
    property DragTypes : TDragTypes read FDragTypes write FDragTypes default [dtCopy, dtLink];
    ...
    property IsDropSource : Boolean read FIsDropSource write FIsDropSource default False; { Published }
    property IsDropTarget : Boolean read FIsDropTarget write SetIsDropTarget default False; { Published }
    property ThemeMode;
...
begin
  ...
  begin
    ...
    if FTarget <> nil then
      if not (csDestroying in FTarget.ComponentState) then
...
var
  ...
  if dataObj.EnumFormatEtc(DATADIR_GET, FormatEtc) = S_OK then
    while FormatEtc.Next(1, Fmt, nil) = S_OK do
      FDataFormats.Add(LMDGetClipboardFormatName(Fmt.cfFormat));
...
var
  ...
  i : integer;
begin
  s := LMDGetClipboardFormatName(Format);
  result := false;
...
procedure TElDropTarget.AfterMessage(Sender: TObject; var Msg: TMessage; var Handled: boolean);
var Rslt : HRESULT;
    obj  : IDropTarget;
begin
...
begin
  ...
  begin
    ...
    Result := SendMessage(Handle, CM_DRAG, WPARAM(Msg), TLMDPtrInt(@DragRec));
  end;
end;}
...
{$IFDEF LMD_UNICODE}
  FWideList := TLMDWideStringList.Create;
{$endif}
end;
...
begin
  ...
  repeat
    fmtCount := 0;
    if efe.Next(1, fmt, @fmtCount) <> S_OK then
      break;
...
begin
  result := 0;
  while aPWideChar[result] <> #0 do
    Inc(result);
end;

{$IFDEF LMD_UNICODE}
function TOleDragObject.GetWideString: WideString;
var
  mdm : TStgMedium;
  pz : PWideChar;
  fmt : TFormatEtc;
...
begin
  Result := '';
  if (FDown) or (not assigned(DataObj)) or (not HasDataFormat(CF_UNICODETEXT)) then
    exit;
  fillchar(fmt, sizeof(fmt), 0);
  DataObj.EnumFormatEtc(datadir_get, efe);
  EFE.Reset;
  repeat
    fmtCount := 0;
    efe.Next(1, fmt, @fmtCount);
  until (fmt.cfFormat = CF_UNICODETEXT) or (fmtCount = 0);
  if fmt.cfFormat <> CF_UNICODETEXT then
  begin
    result := '';
    exit;
  end;
  ...
  try
    if (fmt.cfFormat = CF_UNICODETEXT) and (mdm.tymed = TYMED_HGLOBAL) then
    begin
      pz := GlobalLock(mdm.HGlobal);
      FWideString := pz;
      Result := FWideString;
...
function TOleDragObject.GetWideFileList;
var
  mdm : TStgMedium;
  pz : PAnsiChar;
  fmt : TFormatEtc;
  ...
  try
    ...
    begin
      ...
      if not (pdf.fWide) then
        ...
        begin
          s := PWideChar(pz);
          SetLength(s, PWideCharLen(PWideChar(pz)));
          FWideList.Add(s);
...
var
  ...
  if dataObj.EnumFormatEtc(DATADIR_GET, FormatEtc) = S_OK then
    while FormatEtc.Next(1, Fmt, nil) = S_OK do
    FOwner.FDataFormats.Add(LMDGetClipboardFormatName(Fmt.cfFormat));
...
begin
  ...
  if IsDropTarget then
    IsDropTarget := False;
//  FElDropTarget := nil;
  if assigned(FDataFormats) then
    FDataFormats.Free;
  inherited;
end; { Destroy }

procedure TElDragDrop.TriggerOleStartDragEvent(var DragData : Pointer; var DragDataType : integer; var DragDataSize : integer);
begin
  if (assigned(FOnOleStartDrag)) then
    FOnOleStartDrag(Self, DragData, DragDataType, DragDataSize);
...
begin
  if IsThemed then
  begin
    LMDThemeServices.DrawElement(UseThemeMode, Canvas.Handle, teEditDontCare, ClientRect);
  end
  else
    inherited;
...
procedure TElDragDrop.WMLButtonDown(var Msg : TWMLButtonDown);
begin
...
procedure TElDragDrop.WMCreate(var Message: TWMCreate);
begin
  inherited;
  if FIsDropTarget then
  begin
...
function TElDragDrop.GetThemedElement: TThemedElement;
begin
  result := teEdit;
end;
