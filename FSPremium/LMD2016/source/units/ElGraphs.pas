...
type
  ...
  private
    ...
    FMinGrid,
      FMaxGrid : TLMDPtrInt; // for legend and fraph
    FOwner : TElGraph;
    FVisible : boolean;
    FAutoGrid : boolean;
    FFaults : integer;

    function GetLimit : TLMDPtrInt;
    procedure SetLimit(newValue : TLMDPtrInt);
    procedure SetColor(value : TColor);
    procedure SetName(value : string);
    procedure SetMinGrid(newValue : TLMDPtrInt);
    procedure SetMaxGrid(newValue : TLMDPtrInt);
  ...
  public
    ...
    procedure CalcMinMax(var Min, Max, Avg : TLMDPtrInt);
    constructor Create;
    destructor Destroy; override;
    procedure AddValue(value : TLMDPtrInt);
    function GetValue(index : integer) : TLMDPtrInt;

    procedure Reset;

    property Name : string read FName write SetName; { Public }
    property Owner : TElGraph read FOwner;
    property Values : TLMDObjectList read FValues;
    property Value[index : integer] : TLMDPtrInt read GetValue; default;
    property ValueCount : integer read GetValueCount;
    property Color : TColor read FColor write SetColor;
    property MinGrid : TLMDPtrInt read FMinGrid write SetMinGrid default 0; { Public }
    property MaxGrid : TLMDPtrInt read FMaxGrid write SetMaxGrid default 100; { Public }
    property Visible : boolean read FVisible write SetVisible default true;
    property Limit : TLMDPtrInt read GetLimit write SetLimit default 1000; { Public }
...
//    procedure SetGraphType( newValue : TGraphType ) ;
  ...
  published
    ...
    property ShowLegend : Boolean read FShowLegend write SetShowLegend default False; { Published }
    { Show min, max and average values of the selected entry? }
    property ShowMinMax : Boolean read FShowMinMax write SetShowMinMax default False; { Published }
    { Show grid lines? }
    property ShowGrid : Boolean read FShowGrid write SetShowGrid default False; { Published }
//    property GraphType : TGraphType read FGraphType write SetGraphType ;  { Published }
    ...
    property LegendBkColor : TColor read FLegendBkColor write SetLegendBkColor default clBlack; { Published }
...
procedure TDataEntry.AddValue(value : TLMDPtrInt);
begin
...
function TDataEntry.GetValue(index : integer) : TLMDPtrInt;
begin
  result := TLMDPtrInt(FValues[index]);
end;

procedure TDataEntry.SetMinGrid(newValue : TLMDPtrInt);
...
procedure TDataEntry.SetMaxGrid(newValue : TLMDPtrInt);
...
procedure TDataEntry.CalcMinMax(var Min, Max, Avg : TLMDPtrInt); { public }
var
  I, C, J : TLMDPtrInt;
begin
  Min := $7FFFFFFF;
...
function TDataEntry.GetLimit : TLMDPtrInt;
begin
  result := FValues.Capacity;
end;

procedure TDataEntry.SetLimit(newValue : TLMDPtrInt);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      inc(X, Math.Max(20, Surface.TextWidth('Min:')) + 3);
      Surface.Brush.Color := clBtnText;
      Surface.FrameRect(Rect(MinMaxRect.Left + X, MinMaxRect.Top + 1,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Min))), MinMaxRect.Bottom - 1));
      Surface.Brush.Color := clWindow;
      Surface.TextRect(Rect(MinMaxRect.Left + X + 1, MinMaxRect.Top + 2,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Min))) - 1, MinMaxRect.Bottom - 2),
        MinMaxRect.Left + X + 1, MinMaxRect.Top + 1, IntToStr(Min));
      Inc(X, Math.Max(20, Surface.TextWidth(IntToStr(Min))) + 3);

      // avg value
      Surface.Brush.Color := FLegendBkColor;
      Surface.TextOut(MinMaxRect.Left + X, MinMaxRect.Top + 1, 'Avg:');
      inc(X, Math.Max(20, Surface.TextWidth('Avg:')) + 3);
      Surface.Brush.Color := clBtnText;
      Surface.FrameRect(Rect(MinMaxRect.Left + X, MinMaxRect.Top + 1,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Avg))), MinMaxRect.Bottom - 1));
      Surface.Brush.Color := clWindow;
      Surface.TextRect(Rect(MinMaxRect.Left + X + 1, MinMaxRect.Top + 2,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Avg))) - 1, MinMaxRect.Bottom - 2),
        MinMaxRect.Left + X + 1, MinMaxRect.Top + 1, IntToStr(Avg));
      Inc(X, Math.Max(20, Surface.TextWidth(IntToStr(Avg))) + 3);

      // max value
      Surface.Brush.Color := FLegendBkColor;
      Surface.TextOut(MinMaxRect.Left + X, MinMaxRect.Top + 1, 'Max:');
      inc(X, Math.Max(20, Surface.TextWidth('Max:')) + 3);
      Surface.Brush.Color := clBtnText;
      Surface.FrameRect(Rect(MinMaxRect.Left + X, MinMaxRect.Top + 1,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Max))), MinMaxRect.Bottom - 1));
      Surface.Brush.Color := clWindow;
      Surface.TextRect(Rect(MinMaxRect.Left + X + 1, MinMaxRect.Top + 2,
        MinMaxRect.Left + X + Math.Max(20, Surface.TextWidth(IntToStr(Max))) - 1, MinMaxRect.Bottom - 2),
        MinMaxRect.Left + X + 1, MinMaxRect.Top + 1, IntToStr(Max));
      Inc(X, Math.Max(20, Surface.TextWidth(IntToStr(Max))) + 3);
      ...
      begin
        ...
        inc(X, Math.Max(20, Surface.TextWidth('T/O:')) + 3);
        Surface.Brush.Color := clBtnText;
        Surface.FrameRect(Rect(MinMaxRect.Left + X, MinMaxRect.Top + 1,
          MinMaxRect.Left + X + Math.Max(30, Surface.TextWidth(IntToStr(MinMaxEntry.Faults))), MinMaxRect.Bottom - 1));
        Surface.Brush.Color := clWindow;
        Surface.TextRect(Rect(MinMaxRect.Left + X + 1, MinMaxRect.Top + 2,
          MinMaxRect.Left + X + Math.Max(30, Surface.TextWidth(IntToStr(MinMaxEntry.Faults))) - 1, MinMaxRect.Bottom - 2),
  ...
  begin
    ...
    begin
      ...
      Surface.Brush.Color := FLegendBkColor;
      Surface.Font.Color := clCaptionText;
...
begin
  ...
  R := ClientRect;
//  Bitmap.Canvas.Handle := CreateCompatibleBitmap(Canvas.Handle, R.Right - R.Left, R.Bottom - R.Top);
  Bitmap.Width := R.Right - R.Left;
  Bitmap.Height := R.Bottom - R.Top;
  DoDrawGraph(Bitmap.Canvas);
...
var
  LDataEntry: TDataEntry;
begin
  while FEntryList.Count > 0 do
  begin
    LDataEntry := GetDataList(0);
    DeleteEntry(0);
    LDataEntry.Free;
  end;
  FEntryList.Free;
