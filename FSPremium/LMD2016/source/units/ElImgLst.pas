...
uses
  Classes, SysUtils, Controls, Forms, Windows, Types,
  Consts, CommCtrl, ImgList, Graphics,
  {$IFDEF USE_JPEG}JPEG,{$ENDIF}
  ElVCLUtils, LMDTypes, intfLMDBase;

type
  TElImageList = class(TImageList, ILMDComponent)
  private
    FAbout: TLMDAboutVar;
    {$IFDEF USE_JPEG}
...
function GetPixel(P: PByte; BPP: Byte = 0): Cardinal;
  ...
  begin
    ...
    begin
      ...
      Tmp.Transparent := false;
        ...
        begin
          if GetDeviceCaps(Tmp.Canvas.Handle, BITSPIXEL) >= 8 then
...
begin
  ...
  try
    ...
    on E: Exception do
      MessageBox(0, PChar('Failed to write ElImageList: ' + E.Message), nil, 0);
      end;
end; { WriteData }

{$HINTS ON}
function TElImageList.Equal(IL : TElImageList) : Boolean;
// ToDo Alex  --> code below makes no sense?
var
...
{------------------------------------------------------------------------------}
function TElImageList.getLMDPackage: TLMDPackageID;
begin
