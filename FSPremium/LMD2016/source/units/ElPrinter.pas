...
type
  ...
  TElPrinter = class(TElComponent)
  private
    FActive : boolean;
    FCanvas   : TLMDObjectList;
    FDC     : HDC;
  ...
  TElControlPrinter = class(TElComponent)
  private
    FPrinter: TElPrinter;
    procedure SetElPrinter(Value: TElPrinter);
  protected
    procedure Notification(AComponent : TComponent; Operation : TOperation);
        override;
  public
  published
    property Printer: TElPrinter read FPrinter write SetElPrinter;
  end;
...
var
  ...
  FDC := CreateDC(Int_Ref(Device.FDriver), Int_Ref(Device.FDevice), Int_Ref(Device.FPort), nil);
  FActive := true;
  FPageIndex := AddPage;
...
procedure TElControlPrinter.SetElPrinter(Value: TElPrinter);
begin
  if FPrinter <> Value then
  begin
    if FPrinter <> nil then
      if not (csDestroying in FPrinter.ComponentState) then
