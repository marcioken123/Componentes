...
uses
  ...
  Types,
  ExtCtrls, ElPanel, ElToolBar, ElStatBar, ElPopBtn, StdCtrls,
  Printers, ElPrinter, ElSpin, LMDObjectList, ElHook, ElCombos, LMDProcs,
  ElXPThemedControl, ElEdits, ElScrollBox, ElBtnEdit, ElComponent;

type

  TElBeforeSavePageEvent = procedure(Sender: TObject; AFileName: string; var CanSave: boolean);
  ...
  private
    ...
    FScaleIdx : integer;
    FRealIdx  : integer;
    FOnBeforeSavePage: TElBeforeSavePageEvent;
  ...
  public
    ...
    property OnBeforeSavePage: TElBeforeSavePageEvent read FOnBeforeSavePage write FOnBeforeSavePage;
...
begin
  ...
  if FPrinter.PageCount >= APanel.Tag then
  begin
    AMetafile := FPrinter.Page[APanel.Tag - 1];
    R := Classes.Rect(0, 0, APanel.ClientWidth, APanel.ClientHeight);
...
begin
  ...
  begin
    ...
    Items.Add('25%');
    Items.Add('10%');
    Items.Add('Page width');
    Items.Add('Text width');
    Items.Add('Whole Page');
    Items.Add('Multiple Pages');
  end;
...
begin
  ...
  begin
    ...
    FScaleIdx := -1;
    // PostMessage(ScaleCombo.Handle, WM_SETTEXT, 0, TLMDPtrInt(PChar(IntToStr(FScale) + '%')));
...
begin
  ...
  begin
    ...
    LMDCenterRects(RealSize.cx + Margin * 2, ScrollBox.ClientWidth,
...
begin
  ...
  begin
    if Assigned(FOnBeforeSavePage) then
      FOnBeforeSavePage(Self, SaveDialog.FileName, LCanSave);
    if LCanSave then
