...
uses
  Windows, SysUtils, Messages, TlHelp32, LMDTypes, LMDObjectList, LMDSysIn, ElVCLUtils;
...
procedure FillProcessList(L : TLMDObjectList);
procedure FillModuleList(PID: DWORD; L : TLMDObjectList);
...
{$R-}
    ...
    EnumProcessesProc = function (lpidProcess:PProcessIDArray; {pointer to process ID array}
                                  cb:LongInt;                  {size of process ID array (in bytes)}
                                  var cbNeedee:DWORD           {size of filled data  (in bytes)}
                                 ):BOOL; stdcall;
...
function IsUsePSAPI: Boolean;
begin
  Result := LMDSIWindowsNT and (not LMDSIWindows2000Up);
  //Result := True; // - Debug internal PSAPI
end;
      ...
      begin
        ...
        begin
          if GetModuleFileNameEx(ph, mhArray[0], Buf, MAX_PATH)>0 then
            Info.ModuleName := StrPas({$ifdef lmdcomp12}PChar{$endif}(@Buf[0]))
          else
            Info.ModuleName := '';
  ...
  begin
    ...
    if Process32First(hSnapshot, pe32) then
      repeat
        ...
        Info.ModuleName := String(pe32.szExeFile);
...
function GetModuleFileNameExANT4(hProcess: THandle; hModule: HMODULE; lpFilename: PAnsiChar; nSize: DWORD): DWORD stdcall;
var
  Buff: String;
begin
  Result := 0;
...
function EnumProcessModulesNT4(hProcess: THandle; lphModule: LPDWORD; cb: DWORD; var lpcbNeeded: DWORD): BOOL stdcall;
{$ifdef LMDX64}
begin
  Result := false;
  LMDPlatformNotImplemented;
