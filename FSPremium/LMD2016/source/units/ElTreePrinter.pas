...
type
  ...
  published
    ...
    property showHeaderExpandMarks: Boolean read FshowHeaderExpandMarks write SetshowHeaderExpandMarks default false;
    property ShowHeaderImages: Boolean read FShowHeaderImages write SetShowHeaderImages default true;
    property ShowHeaderSortMarks: Boolean read FShowHeaderSortMarks write SetShowHeaderSortMarks default false;
    property ShowHiddenItems: Boolean read FShowHiddenItems write SetShowHiddenItems default true;
    property ShowHiddenSections: Boolean read FShowHiddenSections write SetShowHiddenSections default true;
    property ShowImages: Boolean read FShowImages write SetShowImages default true;
    property ShowInvisibleItems: Boolean read FShowInvisibleItems write SetShowInvisibleItems default true;
    property ShowInvisibleSections: Boolean read FShowInvisibleSections write SetShowInvisibleSections default false;
    ...
    property HeaderOnEveryPage: Boolean read FHeaderOnEveryPage write SetHeaderOnEveryPage default false;
    property OnItemPrinting: TPrintTreeItemEvent read FOnItemPrinting write FOnItemPrinting;
    property OnSectionPrinting : TPrintHeaderSectionEvent read FOnSectionPrinting write FOnSectionPrinting;
    property Scale: Integer read FScale write SetScale default 100;
    property StripedEvenColor: TColor read FStripedEvenColor write FStripedEvenColor;
    property StripedItems: Boolean read FStripedItems write FStripedItems default false;
...
type
    ...
    public
      procedure TriggerVirtualStyleNeeded(Item: TElTreeItem; SectionIndex: Integer; Style: TElCellStyle);override;
      procedure DoItemDraw(Item: TElTreeItem; Surface: TCanvas; R: TRect; SectionIndex: integer); override;
      procedure TriggerVirtualTextNeeded(Item : TElTreeItem; SectionIndex : Integer; var Text : TLMDString); override;
      function  DoGetPicture(Item: TElTreeItem): integer; override;
      function  DoGetPicture2(Item: TElTreeItem): integer; override;
    ...
    function THackElTree.DoGetPicture(Item: TElTreeItem): integer;
    begin
      result:=inherited DoGetPicture(Item);
    end;
    function THackElTree.DoGetPicture2(Item: TElTreeItem): integer;
    begin
      result:=inherited DoGetPicture2(Item);
    end;
    procedure THackElTree.DoItemDraw(Item: TElTreeItem; Surface: TCanvas; R: TRect; SectionIndex: integer);
    begin
      inherited;
    end;
    procedure THackElTree.TriggerVirtualStyleNeeded(Item : TElTreeItem; SectionIndex : Integer; Style : TElCellStyle);
    begin
      inherited;
    end;
    procedure THackElTree.TriggerVirtualTextNeeded(Item: TElTreeItem;
...
type
    ...
    function THackElHeader.DoGetPicture(Section: TElHeaderSection): integer;
    begin
      result:=inherited DoGetPicture(Section);
    end;
    procedure THackElHeader.DoSectionDraw(Canvas: TCanvas;
...
type
    THackElHeaderSection = class(TElHeaderSection)
      function GetFVisible:Boolean;
    end;
    function THackElHeaderSection.GetFVisible:Boolean;
    begin
      result:=FVisible;
    end;
...
type
    ...
    THackElTreeItem = class(TElTreeItem)
      function GetHTMLData: TLMDHTMLData;
      function GetParent: TElTreeItem;
    end;
    function THackElTreeItem.GetHTMLData: TLMDHTMLData;
    begin
       result:=FHTMLData;
    end;
    function THackElTreeItem.GetParent: TElTreeItem;
    begin
      result:=FParent;
    end;
...
begin
  ...
  begin
    ...
    if ((not ShowHiddenSections) and (not THackElHeaderSection(Section).GetFVisible)) or
    ((not ShowInvisibleSections) and (not Section.Visible) and (THackElHeaderSection(Section).GetFVisible)) then
      Continue;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if LMDInRange(StImIndex, 0, AHeader.Images.Count - 1) then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.MoveTo(ARect.Left - 1, SR.Top);
      Canvas.LineTo(Arect.Left - 1, SR.Bottom);
    end;
    for i := 0 to THackElTree(Tree).HeaderSections.Count - 1 do
    begin
      Section := THackElTree(Tree).HeaderSections.ItemByPos[i];
        ...
        begin
          ...
          begin
            if Section.Index = THackElTree(Tree).MainTreeColumn then
              ...
              DoDrawItemCellContents(Canvas, Item, Section, R, Self.TextColor, THackElTree(Tree).BkColor, THackElTree(Tree).BkColor, THackElTree(Tree).Font.Style);
          end;
        end;
        if VerticalLines then
        begin
...
begin
  lMultiLineEllipseFlags[false] := DT_END_ELLIPSIS*ord(TElTree(Item.Owner).SinglelineEllipsis);
  lMultiLineEllipseFlags[true] := DT_END_ELLIPSIS*ord(TElTree(Item.Owner).MultilineEllipsis);
  ...
  begin
    if THackElTree(Tree).RightAlignedView then
    ...
    begin
      ...
      begin
        ...
        if CurStyle.Picture.Transparent then
          LMDDrawTransparentBitmapEx(Canvas.Handle, CurStyle.Picture, R2.Left, R2.Top, Rect(0, 0, R.Right - min(w, Max(R.Left, R.Right - w + 1) + 1), h), CurStyle.Picture.TransparentColor)
        else
      ...
      begin
        ...
        if CurStyle.Picture.Transparent then
          LMDDrawTransparentBitmapEx(Canvas.Handle, CurStyle.Picture, R2.Left, R2.Top, Rect(0, 0, min(w, Min(R.Right, R.Left + w - 1) - R.Left + 1), h), CurStyle.Picture.TransparentColor)
        else
  ...
  begin
    ...
    begin
      CurStyle.Control.Paint(Tree, Item, Canvas, R);
    end
    else
    ...
    begin
      ...
      TextFlags := TextFlags or DT_NOPREFIX or MultiLineFlags[Item.Multiline] or
                   WordWrapFlags[Item.Multiline, Item.WordWrap] or
                   lMultiLineEllipseFlags[Item.Multiline];

      if THackElTree(Tree).RightAlignedText then
        TextFlags := TextFlags or DT_RTLREADING;

      if THackElTree(Tree).HeaderSections[SectionIndex] <> nil then
      begin
      ...
      begin
        if (Section <> nil) and Section.Password then
          SText := '******'
        else
      ...
      begin
        ...
        if THackElTree(Tree).VirtualityLevel = vlNone then
        begin
          FData := THackElTreeItem(Item).GetHTMLData;
      ...
      begin
        ...
        LMDDrawText(Canvas.Handle, SText, Length(SText), ARect, TextFlags);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    FTree := Value;
    if FTree<>nil then
      FRender.OnImageNeeded := THackElTree(FTree).OnHTMLImageNeeded
    else
...
begin
  ...
  try
    Printer.Canvas[Printer.PageIndex].Font.Assign(FFont);
    ...
    begin
      ...
      begin
        ...
        if ((not ShowHiddenItems) and Item.Hidden and THackElTree(Tree).FilteredVisibility) or
           ((not ShowInvisibleItems) and not Item.FullyExpanded and (Item.Parent <> nil) and (not Item.Parent.Expanded)) then
        begin
          inc(i);
          Continue;
        end;
...
begin
  ...
  begin
    ...
    LMDCenterRects(w, THackElTree(Tree).ItemIndent, h, R.Bottom - R.Top + 1, R1);
  ...
  begin
    w := 10;
    LMDCenterRects(w, THackElTree(Tree).ItemIndent, w, R.Bottom - R.Top + 1, R1);
    if not IsNode then
    begin
    ...
    try
      ...
      begin
        ...
        begin
          ...
          LMDDrawTransparentBitmapEx(ACanvas.Handle, THackElTree(Tree).RadioButtonGlyph, 0, 0, R3, THackElTree(Tree).RadioButtonGlyph.TransparentColor);
        end
        else
        begin
          THackElTree(Tree).CheckBoxGlyph.Pixelformat := pfDevice;
          LMDDrawTransparentBitmapEx(ACanvas.Handle, THackElTree(Tree).CheckBoxGlyph, 0, 0, R3, THackElTree(Tree).CheckBoxGlyph.TransparentColor);
        end;
...
begin
  ...
  try
    ...
    begin
      ...
      if ShowRoot and (GetNextVisChild(THackElTreeItem(TSI).GetParent, TSI, false) <> nil) then
      begin
        if THackElTree(Tree).RightAlignedView then
        begin
          Inc(R.Right, ItemExt);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if GetPrevVisChild(THackElTreeItem(Item).GetParent, Item, false) <> nil then
              DrawLine(ACanvas, R.Right - (ItemExt div 2 {+ 4}), R.Top, R.Right - (ItemExt div 2 {+ 4}), R.Top + ((R.Bottom - R.Top + 1) div 2));
            if not Item.SuppressLines then
              if GetNextVisChild(THackElTreeItem(Item).GetParent, Item, false) <> nil then
                 DrawLine(ACanvas, R.Right - (ItemExt div 2 {+ 4}), R.Top + ((R.Bottom - R.Top + 1) div 2), R.Right - (ItemExt div 2 {+ 4}), R.Top + (R.Bottom - R.Top + 1));
          end;
        end else
        begin
...
procedure TElTreePrinter.ReadPageNumbersLayout(Reader: TReader);
var
  LIdent: String;
begin
...
procedure TElTreePrinter.DefineProperties(Filer: TFiler);
begin
  inherited;
...
procedure TElTreePrinter.SetFont(const Value: TFont);
begin
  FFont.Assign(Value);
end;
