...
type
  { ******************* TLMDChartCTAreaLine ********************************** }
  /// <summary>
  /// An area line chart displays series as a set of points connected by a line,
  ...
  /// over time.
  /// </summary>
  TLMDChartCTAreaLine = class(TLMDChartCTAbstract)
  private
...
procedure TLMDChartCTAreaLine.SetYLine(Value: TLMDChartLine);
begin
...
procedure TLMDChartCTAreaLine.SetXLine(Value: TLMDChartLine);
begin
...
procedure TLMDChartCTAreaLine.SetAreaLabels(Value: TLMDChartLabel);
begin
...
procedure TLMDChartCTAreaLine.SetAreaTransparency(Value: byte);
begin
...
procedure TLMDChartCTAreaLine.SetLabelsMargin(Value: integer);
begin
  FLabelsMargin := Value;
  FChanged := true;
  HighestOwnerRepaint;
...
procedure TLMDChartCTAreaLine.SetLabelsXDeviation(Value: integer);
begin
...
procedure TLMDChartCTAreaLine.SetLabelsYDeviation(Value: integer);
begin
...
procedure TLMDChartCTAreaLine.SetShowAreaLabels(Value: boolean);
begin
...
procedure TLMDChartCTAreaLine.SetAreaStacked(const Value: boolean);
begin
  FAreaStacked := Value;
  FChanged := true;
  HighestOwnerRepaint;
...
procedure TLMDChartCTAreaLine.DrawAreaLines(ATarget: TCanvas;
  AValues: TLMDChartData; AStartX, AEndX, AStartY, AEndY: integer; AYAxis: TLMDChartYAxis;
  AShowMarkers: boolean);
var
  a, i, j, c, l: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      LAreaPoly[AValues.ColCount].Y := AEndY - AYAxis.CalcPosFromBottom(AEndY - AStartY, 0, l);
      LAreaPoly[AValues.ColCount].X := AEndX;
      LAreaPoly[AValues.ColCount + 1].Y := AEndY - AYAxis.CalcPosFromBottom(AEndY - AStartY, 0, l);
      LAreaPoly[AValues.ColCount + 1].X := AStartX;
...
procedure TLMDChartCTAreaLine.DrawAreaLinesStacked(ATarget: TCanvas;
  AValues: TLMDChartData; AStartX, AEndX, AStartY, AEndY: integer; AYAxis: TLMDChartYAxis;
  AShowMarkers: boolean);
var
  a, i, j, c, l: integer;
...
function TLMDChartCTAreaLine.IsChanged: boolean;
begin
...
procedure TLMDChartCTAreaLine.NoChanged;
begin
  FChanged := false;
...
procedure TLMDChartCTAreaLine.ResetMainOwner(AmainOwner: TPersistent);
begin
...
procedure TLMDChartCTAreaLine.LoadStateFromXML(BaseNode: ILMDXmlNode; Comment: string);
var
...
procedure TLMDChartCTAreaLine.SaveStateToXML(BaseNode: ILMDXmlNode; Comment: string);
var
  WorkNode, IterNode: ILMDXmlNode;
begin
...
procedure TLMDChartCTAreaLine.Assign(ASource: TPersistent);
begin
  if ASource is TLMDChartCTAreaLine then
  begin
    try
      FAreaLabels.Assign(TLMDChartCTAreaLine(ASource).FAreaLabels);
      FShowAreaLabels := TLMDChartCTAreaLine(ASource).FShowAreaLabels;
      AreaTransparency := TLMDChartCTAreaLine(ASource).FAreaTransparency;
      FLabelsYDeviation := TLMDChartCTAreaLine(ASource).FLabelsYDeviation;
      FLabelsXDeviation := TLMDChartCTAreaLine(ASource).FLabelsXDeviation;
      FLabelsMargin := TLMDChartCTAreaLine(ASource).FLabelsMargin;
      FAreaStacked := TLMDChartCTAreaLine(ASource).FAreaStacked;
      FYLine := TLMDChartCTAreaLine(ASource).FYLine;
      FXLine := TLMDChartCTAreaLine(ASource).FXLine;
    finally
    end;
  end;
...
destructor TLMDChartCTAreaLine.Destroy;
begin
  FAreaLabels.Free;
  FYLine.Free;
  FXLine.Free;
...
procedure TLMDChartCTAreaLine.Draw(ABuffer: TBitmap; AArea: TRect; AValues: TLMDChartData;
...
constructor TLMDChartCTAreaLine.Create(AMainOwner: TComponent);
begin
