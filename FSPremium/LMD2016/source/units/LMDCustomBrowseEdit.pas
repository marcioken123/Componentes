unit LMDCustomBrowseEdit;
{$I lmdcmps.inc}

{###############################################################################

LMD VCL Series 2016
© by LMD Innovative
-------------------

For support or information contact us at:

email              : mail@lmdsupport.com
WWW                : http://www.lmdinnovative.com
SupportSite        : http://www.lmdsupport.com
Wiki               : http://wiki.lmd.de
Fax                : ++49 6131 4984372

This code is for reference purposes only and may not be copied
or distributed in any format electronic or otherwise except one
copy for backup purposes.

No Component Kit or Component individually or in a collection,
subclassed or otherwise from the code in this unit, or associated
.pas, .dfm, .dcu, .ocx, .dll or ActiveX files may be sold or
distributed without express permission from LMD Innovative.

For further license information please refer to the associated
license html file in \info folder.

################################################################################

LMDCustomBrowseEdit unit ()
---------------------------

Changes
-------
Release 8.03 (Dec 2007)
 -  Triggering FOnAfterBrowse removed from DoOnBrowseClick method
    (caused the event being triggered twice) - VB

-------
Release 8.0 (October 2006)
 - Initial Release

###############################################################################}

interface

uses
  Classes,
  LMDClass, LMDCustomEdit;

type
  { ----------------------- TLMDCustomBrowseEdit ----------------------------- }
  TLMDCustomBrowseEdit = class(TLMDCustomEdit)
  private
    FOnDlgChange,
    FOnBeforeBrowse,
    FOnAfterBrowse   : TNotifyEvent;
  protected
    procedure DoOnBrowseClick (Sender : TObject; index: Integer);
    procedure DoBrowse; virtual;
    procedure Loaded; override;
  public
    constructor Create (aOwner : TComponent); override;
  published
    property CustomButtons;
    property CustomButtonsStyle;
    property CustomButtonWidth;
    property CustomButtonParentCtlXP;
    property OnChange;
    property OnDlgChange:TNotifyEvent read FOnDlgChange write FOnDlgChange;
    property OnBeforeBrowse: TNotifyEvent read FOnBeforeBrowse write FOnBeforeBrowse;
    property OnBrowse: TNotifyEvent read FOnAfterBrowse write FOnAfterBrowse;
  end;

implementation

{ ---------------------------------- protected ------------------------------- }
procedure TLMDCustomBrowseEdit.DoOnBrowseClick (Sender : TObject; index: Integer);
begin
  DoBrowse;
end;

{ ---------------------------------------------------------------------------- }
procedure TLMDCustomBrowseEdit.DoBrowse;
begin
end;

{ ---------------------------------- public ---------------------------------- }
constructor TLMDCustomBrowseEdit.Create (aOwner : TComponent);
begin
  inherited Create (aOwner);
  CustomButtons.Add;
  CustomButtonWidth := 18;
  CustomButtons.Items[0].ThemeMode := ttmNone;
end;

procedure TLMDCustomBrowseEdit.Loaded;
begin
  inherited;
  CustomButtons.Items[0].OnClick := DoOnBrowseClick;
end;

end.
