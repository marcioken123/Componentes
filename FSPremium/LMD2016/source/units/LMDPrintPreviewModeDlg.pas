...
type
  TLMDPrintPreviewModeDialog = class(TForm)
    Panel: TPanel;
    imSelected1: TImage;
...
procedure TLMDPrintPreviewModeDialog.MsgClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TLMDPrintPreviewModeDialog.FormCreate(Sender: TObject);
begin
  Width := 90;
  FRowCount := 0;
  FColCount := 0;

  Msg.Caption := Format(SLMDPrintPmPages, [FRowCount, FColCount]);
end;

procedure TLMDPrintPreviewModeDialog.MsgMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  Msg.Caption := SLMDPrintPmCancel;
  Msg.Alignment := taLeftJustify;
end;

procedure TLMDPrintPreviewModeDialog.imSelected1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var
  P : TPoint;
begin
...
procedure TLMDPrintPreviewModeDialog.PanelMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TLMDPrintPreviewModeDialog.imSelected4Click(Sender: TObject);
begin
  ModalResult := mrOk;
end;
