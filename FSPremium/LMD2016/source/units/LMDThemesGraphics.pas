...
LMDThemesGraphics unit (VB)
---------------------------
Theme specific graphic routines
...
- TileRect fixed to work faster for themes with tiled bitmap
  in controls like pagecontrol
  (For Win2000 and higher)
...
begin
  ...
  begin
    ...
    hThemesLib := SafeLoadLibrary(ThemesLib);
  ...
  if (W - RM - LM > 0) and (H - TM - BM > 0) then
    ...
    begin
      ...
      begin
        ...
        FillRect(DC, Rect(LM, TM, W - RM, H - BM), 0);
        Brush := SelectObject(DC, Brush);
        if Brush <> 0 then
          DeleteObject(Brush);
      end;
            ...
            begin
              ...
              if LMDSIWindows2000Up then
                begin
                  ...
                  LBrush.Bitmap := TBitmap.Create;
                  LBrush.Bitmap.PixelFormat := pf32bit;
                  LBrush.Bitmap.Width := SW;
                  LBrush.Bitmap.Height := SH;
                  BitBlt(LBrush.Bitmap.Canvas.Handle, 0, 0, SW, SH, SrcDC, SrcX + L, SrcY + T, SRCCopy);
                  FillRect(DC, Rect(LM, TM, W - RM, H - BM), LBrush.Handle);
                  LBrush.Bitmap.Free;
                  LBrush.Free;
                end
              else
                //FillRect with bitmapped brush doesn't work for win98
                for I := 0 to CH - 1 do
                  begin
                    L := LM;
                    if T + SH > TH then
...
begin
  if not LMDSIWindows2000Up or LMDIsAlphaEmpty(Image) then
    exit;
...
initialization
  ...
  UseMMX := False; //UseMMX is used only in asm functions here.
  {$IFDEF LMDX86}
  try
    InitThemesGraphics;
    if UseMMX then
...
finalization
  ...
  if {(not SimpleAppFinallize) and} (hThemesLib <> 0) then
  begin
    FreeLibrary(hThemesLib);
    hThemesLib := 0;
  end;
