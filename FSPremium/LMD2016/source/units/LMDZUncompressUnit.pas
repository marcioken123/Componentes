...
LMDZUnCompressUnit unit - ZLib interface (AH)
---------------------------------------------
Decompress routines.
...
//  code_s = packed record
  code = packed  record
    ...
    val: word;  //      /* offset in table or code value */

  end;

  TArrShr = array[0..320] of Smallint;
  PArrShr = ^TArrShr;
  TArrShr288 = array[0..288] of Smallint;
  PArrShr288 = ^TArrShr;
 ...
 inflate_mode =  (
    ...
    TYPEB,       // (вместо TYPE) i: waiting for type bits, including last-flag bit */
    TYPEDO,     // i: same, but skip check to exit inflate on new block */
    STORED,     // i: waiting for stored size (length and complement) */
    SCOPY,     // i/o: waiting for input or output to copy stored block */ COPY
...
function inflateReset(var z :TZStreamRec):integer; function inflateInit_(var z :TZStreamRec;
  const version: AnsiString; stream_size: integer): integer; function inflateInit2_(var z :TZStreamRec;w :integer;
  const version: AnsiString; stream_size: integer): integer; function inflateEnd(var z :TZStreamRec):Integer; {   inflate decompresses as much data as possible, and stops when the input
  ...
  compression block.}
function inflate(var z :TZStreamRec;f :integer):integer; function uncompress(var dest :ArrayPtr;var destLen :Cardinal; source :Bytef;sourceLen :uLong):integer; //function inflateSetDictionary(var strm: TZStreamRec; dictionary: PChar; dictLength :uInt):integer;
...
type
 ...
 const lbase : array[0..30] of short =
   (
    3, 4, 5, 6, 7, 8, 9, 10, 11, 13, 15, 17, 19, 23, 27, 31,
    35, 43, 51, 59, 67, 83, 99, 115, 131, 163, 195, 227, 258, 0, 0
   ); //* Length codes 257..285 base */

 const lext : array[0..30]  of short =
   (
    16, 16, 16, 16, 16, 16, 16, 16, 17, 17, 17, 17, 18, 18, 18, 18,
    19, 19, 19, 19, 20, 20, 20, 20, 21, 21, 21, 21, 16, 201, 196 // , 199, 198.
   ); //* Length codes 257..285 extra */

 const dbase : array[0..31]  of short =
   (
 ...
 const dext : array[0..31] of short  =
   (
...
function inflateInit2_(var z :TZStreamRec;w :integer;
  const version: AnsiString; stream_size: integer): integer;
var
  s         :inflate_state;
begin
  if ((@version=nil) or (version[1] <> {$IFDEF LMDCOMP12}AnsiString(ZLIB_VERSION)[1]{$ELSE}ZLIB_VERSION[1]{$ENDIF})
      or (stream_size<>sizeof(TZStreamRec)))
  then
    begin
...
begin
 result:=inflateInit_(strm, ZLIB_VERSION
    , SizeOf(TZStreamRec));
end;
...
//    / * copy inflate_state(strm.internal).wsize or less output bytes into the circular window */
  ...
  begin
    LMDMemCpy(inflate_state(strm.internal).window, strm.next_out - inflate_state(strm.internal).wsize, inflate_state(strm.internal).wsize);
    inflate_state(strm.internal).write := 0;
  ...
  begin
    ...
    if (dist > copy1) then
      dist := copy1;
    LMDMemCpy(inflate_state(strm.internal).window + inflate_state(strm.internal).write, strm.next_out - copy1, dist);
    copy1 := copy1 - dist;
    if (Copy1 <> 0) then
    begin
      LMDMemCpy(inflate_state(strm.internal).window, strm.next_out - copy1, copy1);
...
{  if inflate_state(z.internal).flags <> 0 then
  ...
  // т.к. это zlib то flag=0 и проверку можно не делать
...
procedure fixedtables(var state : inflate_state);
...
function inflate_table(types : codetype; llens : PArrShr ; c : uInt; var Next : PCode; var  bits : uInt;var work: TArrShr288) : integer;
  ...
  case types of
    ...
    LENS:
      begin
        base := @lbase;
//        base := base - 257; (257*2)
        Dec(PSmallInt(base), 257) ;
        extra := @lext;
//        extra := extra - 257;
        Dec(PSmallInt(extra), 257) ;
        endd := 256;
      end;
    else    //         /* DISTS */
      begin
...
//             Tracev((stderr, "inflate:     stored block%s\n",state->last ? " (last)" : ""));
       ...
       1: //                            /* fixed block */
         begin
           fixedtables(inflate_state(z.internal));
...
//            Tracev((stderr, "inflate:       stored length %u\n",state->length));
      INITBITS(bits,hold);
      inflate_state(z.internal).mode := SCOPY;
    end;
//    COPY:
    if (inflate_state(z.internal).mode = SCOPY) then
    begin
...
//    LENLENS:
    ...
    begin
      ...
      ret := inflate_table(CODES, @inflate_state(z.internal).lens, 19, inflate_state(z.internal).Next, inflate_state(z.internal).lenbits, inflate_state(z.internal).work);
      if (ret <> 0) then
      begin
...
//    CODELENS:
    ...
    begin
      ...
      ret := inflate_table(LENS, @inflate_state(z.internal).lens, inflate_state(z.internal).nlen, inflate_state(z.internal).Next,inflate_state(z.internal).lenbits, inflate_state(z.internal).work);
      if (ret<>0) then
      begin
      ...
      ret := inflate_table(DISTS, @inflate_state(z.internal).lens[inflate_state(z.internal).nlen],inflate_state(z.internal).ndist, inflate_state(z.internal).next, inflate_state(z.internal).distbits, inflate_state(z.internal).work);
      if (ret<>0) then
      begin
