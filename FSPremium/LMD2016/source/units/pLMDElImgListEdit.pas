...
{
 todo:
   - Loading a list (*.eli) of pictures of another size... stretch, crop ...
   - Fill Color: cursor for choosing a colour (crHandPoint)
   - Fill Color: (if the size of a bitmap is less than current size (crop))
   - ??? Scrolling with drag drop with a timer, not with dragging a mouse like now
}

{.$DEFINE DEBUG_THIS} // - for debug для отладки
...
uses
  pLMDElStyleImageSelect, pLMDElImgListEditDlg, ElVCLUtils;
    ...
    begin
      ...
      begin
        MessageBox(
          Handle,
          PChar(
          'The chosen file does not coincide with the current sizes of pictures ! '#10#13+
...
begin
  ...
  begin
    ...
    WorkImageList.GetIcon(Item.ImageIndex, IconImage.Picture.Icon);
  end
  else
  ...
  if Selected and (WorkListView.SelCount < 2) then
    Caption := SLMDCaption + Format(' [%d]', [Item.ImageIndex])
  else
    Caption := SLMDCaption;
end;
  ...
  case Index of
    0: begin
         ...
         //try
           ...
           if Modified then
           begin
             if TElImageList(Component).Owner is TCustomForm then
             begin
               ...
               if TElImageList(Component).Owner is TDataModule then
               begin
                 FDesigner := Self.GetDesigner;
