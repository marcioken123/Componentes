...
uses
  ...
  ExtCtrls, ElTreeStdEditors, ElComponent, ElTreeInplaceEditors
  ;
...
type
  TElDesignMenu = class(TForm)
    ...
    LevelUp: TButton;
    LevelDown: TButton;
    MenuInplaceEditor: TElTreeInplaceEdit;
    Bevel3: TBevel;
    Font: TButton;
    ...
    procedure MenuInplaceEditorAfterOperation(Sender: TObject;
      var Accepted, DefaultConversion: Boolean);
    procedure FontClick(Sender: TObject);
    procedure MenuEditItemSelectedChange(Sender: TObject;
      Item: TElTreeItem);
  private
    FMenu: TMenu;
...
function NewItem(Owner: TComponent; const ACaption: TLMDString; AShortCut: TShortCut;
                 AChecked, AEnabled: Boolean; AOnClick: TNotifyEvent; hCtx: Word;
                 const AName: TLMDString): TElMenuItem;
begin
  ...
  begin
    ...
    begin
      if (TElPopupMenu(FMenu).Images <> nil) and (MenuEdit.Images = nil) then
        MenuEdit.Images := TCustomImageList(TElPopupMenu(FMenu).Images);
    end
    else
    begin
      if (TElMainMenu(FMenu).Images <> nil) then
        MenuEdit.Images := TCustomImageList(TElMainMenu(FMenu).Images);
    end;
    ...
    MenuEdit.ItemFocused.ParentFontStyle := false;
...
var
  ...
  begin
    ...
    result := TreeNode;
    TreeNode.ParentFontStyle := false;
...
begin
  ...
  if csDesigning in FMenu.ComponentState then
    begin
      if (Form <> nil) and (Form.Designer <> nil) then
        FElName := Form.Designer.UniqueName('ElMenuItem')
                               else
        FElName := 'ElMenuItem';
    end
  else
  ...
  if Assigned(ADesigner) then
   ...
   FElMenuItem := TElMenuItem.Create(FMenu);

  FElMenuItem.FreeNotification(Self);
  ...
  begin
    Index:= MenuEdit.ItemFocused.Index;// + 1;
    if (MenuEdit.ItemFocused.Level <> 0) then
    begin
      TreeNode:= MenuEdit.ItemFocused.Parent;
      {
      if TreeNode <> nil then
        Index := MenuEdit.ItemFocused.Parent.Index
      else
        Index := -1;
      }
    end;

    MenuEdit.EnsureVisible(MenuEdit.ItemFocused);
    MenuEdit.ItemFocused.Expand(False);
...
begin
  ...
  begin
    if Assigned(ADesigner) then
      ...
      FElMenuItem:= NewItem(FMenu, 'ElMenuItem', 0, false, true, nil, 0, '');
...
var Form: TCustomForm;
    ItemToDelete : TElMenuItem;
begin
  if ADesigner <> nil then
    ADesigner.SelectComponent(nil);
  MenuEdit.OnItemFocused := nil;

  //MessageBox(0, '1', nil, 0);
  if (MenuEdit.ItemFocused <> nil) and (MenuEdit.ItemFocused.Data <> nil) then
  begin
    ItemToDelete := TElMenuItem(MenuEdit.ItemFocused.Data);
    ItemToDelete.OnChange:= nil;
    (*
    if (ItemToDelete.Parent <> nil) and (ItemToDelete.Parent is TElMenuItem) then
    begin
      TElMenuItem(ItemToDelete.Parent).Remove(ItemToDelete);
      //MessageBox(0, '2', nil, 0);
    end
    else
    begin
      TElMenuItem(FMenu.Items).Remove(ItemToDelete);
      //MessageBox(0, '2', nil, 0);
    end;
    *)
    ItemToDelete.Free;
  ...
  MenuEdit.OnItemFocused := MenuEditItemFocused;
  MenuEditItemFocused(Self);
  if (Form <> nil) and not (csDestroying in Form.ComponentState) then
    DrawMenuBar(Form.Handle);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        //MessageBox(0, 'Deleting tree item', nil, 0);
        (*
        if FMenu is TElPopupMenu then
        ...
        *)
        Temp := TElMenuItem(AComponent);
        //MessageBox(0, 'Search for item done', nil, 0);

        if (Temp <> nil) then
        begin
          //MessageBox(0, 'Menu item found - deleting', nil, 0);
          if MenuEdit.Items.Count > 0 then
            Temp1 := MenuEdit.Items.LookForItem(MenuEdit.Items[0], '', Temp, -1, true, true, false, true, true)
          else
            Temp1 := nil;
          if Temp1 <> nil then
          begin
            //MessageBox(0, 'Tree item found - deleting', nil, 0);
            MenuEdit.Items.DeleteItem(Temp1);
          end
          // else
           // MessageBox(0, 'Tree item not found', nil, 0);

        end
        // else
          // MessageBox(0, 'Menu item not found', nil, 0);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    //vAd:
    { // debug:
    if Classes.GetClass(TElMenuItem.ClassName)=nil then
      begin
         ShowMessage(TElMenuItem.ClassName+' - not registered');
         Classes.RegisterClass(TElMenuItem);
         if Classes.GetClass(TElMenuItem.ClassName)=nil then begin
           ShowMessage('error registering '+TElMenuItem.ClassName); //<- в IDE не может зарегистрировать класс. Почему ???
         end;
       end;
    {}
    ...
    if (FMenu is TElMainMenu) then
      TElMainMenu(FMenu).BeginUpdate;
    try
      ...
      begin
        ...
        try
          Temp := TElMenuItem(S.ReadComponent(nil)); // <- error in design Mode
          if (FMenu is TElPopupMenu) then
...
procedure TElDesignMenu.MenuInplaceEditorAfterOperation(Sender: TObject;
  var Accepted, DefaultConversion: Boolean);
var
  Temp: TElMenuItem;
begin
  Accepted := True;
  Temp := TElMenuItem(MenuEdit.ItemFocused.Data);
  Temp.Caption := MenuInplaceEditor.Editor.Text;
end;

procedure TElDesignMenu.FontClick(Sender: TObject);
begin
  if FontDialog.execute then
  begin
    if FMenu is TElPopupMenu then
      (FMenu as TElPopupMenu).Font := FontDialog.Font
    else
      (FMenu as TElMainMenu).Font := FontDialog.Font;
    MenuEdit.Font := FontDialog.Font;
    MenuInplaceEditor.Editor.Font := FontDialog.Font;
  end;
end;

procedure TElDesignMenu.MenuEditItemSelectedChange(Sender: TObject;
  Item: TElTreeItem);
begin
