...
pLMDWideStringsEditDlg unit (YB)
------------------------
TWideStrings editor dialog unit.
...
uses
  ...
  LMDTypes, LMDRTLConst, LMDUnicodeStrings;

type
  {************************ TLMDFrmWideStringsEditor **************************}
  TLMDFrmWideStringsEditor = class(TForm)
    lbLineCount: TLabel;
    bvlMain: TBevel;
    btnOk: TButton;
    btnCancel: TButton;
...
type
  ...
  TEditorMemoStrings = class(TLMDWideStrings)
  private
    FMemo:       TEditorMemo;
...
function TLMDFrmWideStringsEditor.Execute: Boolean;
begin
  Caption             := SLMDWideStringsEditorCaption;
  btnOk.Caption       := SLMDWideStringsEditorOkBtnCaption;
  btnCancel.Caption   := SLMDWideStringsEditorCancelBtnCaption;
  lbLineCount.Caption := Format(SLMDWideStringsEditorLineCount,
...
procedure TLMDFrmWideStringsEditor.MemoChange(Sender: TObject);
begin
  lbLineCount.Caption := Format(SLMDWideStringsEditorLineCount,
                                [TEditorMemo(FMemo).Lines.Count]);
end;
...
procedure TLMDFrmWideStringsEditor.SetLines(const Value: TLMDWideStrings);
begin
  FLines.Assign(Value);
end;
...
procedure TLMDFrmWideStringsEditor.CopyToMemo;
{$IFNDEF LMD_UNICODE}
var
  i: Integer;
{$ENDIF}
begin
  {$IFDEF LMD_UNICODE}
  TEditorMemo(FMemo).Lines.Assign(FLines);
  {$ELSE}
...
procedure TLMDFrmWideStringsEditor.CopyFromMemo;
{$IFNDEF LMD_UNICODE}
var
  i: Integer;
{$ENDIF}
...
procedure TLMDFrmWideStringsEditor.FormCreate(Sender: TObject);
begin
  FMemo := TEditorMemo.Create(Self);
  TEditorMemo(FMemo).OnChange   := MemoChange;
  TEditorMemo(FMemo).ScrollBars := ssBoth;
...
procedure TLMDFrmWideStringsEditor.FormDestroy(Sender: TObject);
begin
  FMemo.Free;
  FLines.Free;
end;
...
{------------------------------------------------------------------------------}
procedure TEditorMemo.SetLines(const Value: TLMDWideStrings);
begin
  FLines.Assign(Value);
end;
