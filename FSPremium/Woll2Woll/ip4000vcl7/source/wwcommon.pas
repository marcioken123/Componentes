...
begin
   ...
   if (key='') and
   {$ifdef win32}
   (fld.datatype in [ftCurrency, ftFloat, ftBCD, ftFMTBCD, ftInteger, ftSmallInt, ftWord,
                    ftAutoInc, ftTime, ftDate, ftDateTime, ftTimeStamp]) then result:= False;
   {$else}
...
begin
   result:= False;
   case FldType of
     ftCurrency, ftFloat, ftBCD, ftFMTBCD  : if not wwStrToFloat(key) then exit;
...
begin
   ...
   {$ifdef ver130}
   HelpFile := 'IP3000D5.HLP';
   {$endif}

   {$ifdef ver135}
   HelpFile := 'IP3000C5.HLP';
   {$endif}

   {$ifdef ver140}
   HelpFile := 'IP3000D6.HLP';
   {$endif}
...
begin
   ...
   {$endif}
   {$ifdef ver130}
   WinHelp(Handle, 'IP3000D5.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver135}
   WinHelp(Handle, 'IP3000C5.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver140}
   WinHelp(Handle, 'IP3000D6.HLP', HELP_KEY, longint(@context));
   {$endif}
end;
...
begin
  ...
  begin
     ...
     for i:= 0 to controlType.count-1 do begin
        ...
        if parts.count<2 then continue;
        if isWWEditControl(parts[1]) or (parts[1]='RichEdit') then begin
           OwnerForm:= wwGetOwnerForm(Component);
