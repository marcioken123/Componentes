...
uses Windows, Classes, StdCtrls, DB, Controls, Messages, SysUtils,
  ...
  wwtypes, wwframe, wwdbedit, wwcombobutton;
...
begin
   ...
   if result then // 9/13/00 - Workaround problem where locate ignores trailing spaces
   begin
   ...
   if (text <> FLastSearchKey) and (LookupTable<>Nil)
     ...
     try
      ...
      if UseSeqSearch then begin
         if (ssoEnabled in SeqSearchOptions) and (FGrid.DisplayFld<>nil) then begin
...
//    (not FCombo.InDropDownEvent) and (not FCombo.InCustomSearch) then
    if not FCombo.InCustomSearch then
    begin
...
begin
  if not (csLoading in ComponentState) and FCombo.Navigator then
  begin
