...
uses
  ...
  Winapi.Windows,
  {$ENDIF MSWINDOWS}
  System.Classes, System.SysUtils, System.Contnrs, System.SyncObjs,
  {$ELSE ~HAS_UNITSCOPE}
  {$IFDEF MSWINDOWS}
  Windows,
  {$ENDIF MSWINDOWS}
  ...
  {$ENDIF ~HAS_UNITSCOPE}
  IWJclBase, IWJclPeImage,
  {$IFDEF BORLAND}
  IWJclTD32,
  {$ENDIF BORLAND}
  IWJclSynch, IW.Common.Threads;
...
// Optimized functionality of IWJclSysInfo functions ModuleFromAddr and IsSystemModule
type
...
function JclGetThreadStackListToStrings(ThreadID: DWORD; Strings: TStrings;
  IncludeModuleName: Boolean = False; IncludeAddressOffset: Boolean = False;
  IncludeStartProcLineOffset: Boolean = False; IncludeVAddress: Boolean = False): Boolean;
...
uses
  ...
  {$ENDIF SUPPORTS_GENERICS}
  {$ENDIF ~HAS_UNITSCOPE}
  IWJclHookExcept, IWJclStrings, IWJclSysInfo, IWJclSysUtils, IWJclWin32,
  IWJclStringConversions, IWJclResources, IWJclFileUtils, IW.Common.Lists;
...
begin
  ...
  while (PExtension >= MapString) and (PExtension^ <> '.') and (PExtension^ <> '|') do
    Dec(PExtension);
  if (_StrLIComp(PExtension, '.pas ', 5) = 0) or
     (_StrLIComp(PExtension, '.obj ', 5) = 0) then
    PEnd := PExtension;
  PExtension := PEnd;
  while (PExtension >= MapString) and (PExtension^ <> '|') and (PExtension^ <> '\') do
    Dec(PExtension);
...
begin
  ...
  if _StrLIComp(GroupName, 'TLS', 3) = 0 then
...
begin
  ...
  begin
    if _StrLIComp(FSegmentClasses[SegIndex].GroupName.RawValue, 'TLS', 3) = 0 then
      Va := Address.Offset
    else
  ...
  try
    try
      ...
      begin
        ...
        // IWJclDebug Section name
        _StrPLCopy(PAnsiChar(@JclDebugSection.Name), JclDbgDataResName, IMAGE_SIZEOF_SHORT_NAME);
        // IWJclDebug Characteristics flags
        ...
        ImageStream.Position := {0 +} JclDebugSection.PointerToRawData;
...
type
  ...
  TSymGetSymFromAddrAFunc = function (hProcess: THandle; dwAddr: DWORD;
    pdwDisplacement: PDWORD; var Symbol: IWJclWin32.TImagehlpSymbolA): Bool; stdcall;
  TSymGetSymFromAddrWFunc = function (hProcess: THandle; dwAddr: DWORD;
    pdwDisplacement: PDWORD; var Symbol: IWJclWin32.TImagehlpSymbolW): Bool; stdcall;
  TSymGetModuleInfoAFunc = function (hProcess: THandle; dwAddr: DWORD;
    var ModuleInfo: IWJclWin32.TImagehlpModuleA): Bool; stdcall;
  TSymGetModuleInfoWFunc = function (hProcess: THandle; dwAddr: DWORD;
    var ModuleInfo: IWJclWin32.TImagehlpModuleW): Bool; stdcall;
  ...
  TSymGetLineFromAddrAFunc = function (hProcess: THandle; dwAddr: DWORD;
    pdwDisplacement: PDWORD; var Line: IWJclWin32.TImageHlpLineA): Bool; stdcall;
  TSymGetLineFromAddrWFunc = function (hProcess: THandle; dwAddr: DWORD;
    pdwDisplacement: PDWORD; var Line: IWJclWin32.TImageHlpLineW): Bool; stdcall;
  {$ENDIF CPU32}
  {$IFDEF CPU64}
  TSymGetSymFromAddrAFunc = function (hProcess: THandle; dwAddr: DWORD64;
    pdwDisplacement: PDWORD64; var Symbol: IWJclWin32.TImagehlpSymbolA64): Bool; stdcall;
  TSymGetSymFromAddrWFunc = function (hProcess: THandle; dwAddr: DWORD64;
    pdwDisplacement: PDWORD64; var Symbol: IWJclWin32.TImagehlpSymbolW64): Bool; stdcall;
  TSymGetModuleInfoAFunc = function (hProcess: THandle; dwAddr: DWORD64;
    var ModuleInfo: IWJclWin32.TImagehlpModuleA64): Bool; stdcall;
  TSymGetModuleInfoWFunc = function (hProcess: THandle; dwAddr: DWORD64;
    var ModuleInfo: IWJclWin32.TImagehlpModuleW64): Bool; stdcall;
  TSymLoadModuleFunc = function (hProcess: THandle; hFile: THandle; ImageName,
  ...
  TSymGetLineFromAddrAFunc = function (hProcess: THandle; dwAddr: DWORD64;
    pdwDisplacement: PDWORD; var Line: IWJclWin32.TImageHlpLineA64): Bool; stdcall;
  TSymGetLineFromAddrWFunc = function (hProcess: THandle; dwAddr: DWORD64;
    pdwDisplacement: PDWORD; var Line: IWJclWin32.TImageHlpLineW64): Bool; stdcall;
  {$ENDIF CPU64}
  ...
  begin
    ...
    try
      ...
      begin
        ...
        IWJclPeImage.UnDecorateSymbolName(string(WideString(SymbolW^.Name)), Info.ProcedureName, UNDNAME_NAME_ONLY or UNDNAME_NO_ARGUMENTS);
      end;
    finally
      FreeMem(SymbolW);
    end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Info.BinaryFileName := FileName;
        Info.OffsetFromProcName := Displacement;
        IWJclPeImage.UnDecorateSymbolName(string(AnsiString(SymbolA^.Name)), Info.ProcedureName, UNDNAME_NAME_ONLY or UNDNAME_NO_ARGUMENTS);
      end;
    finally
      FreeMem(SymbolA);
    end;
...
type
  ...
  TJclGlobalStackList = class(TIWThreadList)
  private
    FLockedTID: DWORD;
    FTIDLocked: Boolean;
...
procedure TJclGlobalStackList.ClearList;
begin
  with LockList do
  try
    while Count > 0 do
...
var
  List: TJclStackInfoList;
begin
  List := JclCreateThreadStackTraceFromID(False, ThreadID); // JclGetExceptStackList(ThreadID);
  Result := Assigned(List);
  if Result then
    List.AddToStrings(Strings, IncludeModuleName, IncludeAddressOffset, IncludeStartProcLineOffset,
      IncludeVAddress);
...
var
  ...
  IgnoredExceptions: TIWListBase = nil;
...
begin
  ...
  begin
    if not Assigned(IgnoredExceptions) then
      IgnoredExceptions := TIWListBase.Create;
...
begin
  ...
  begin
    IgnoredExceptions.Remove(ExceptionClass);
  end;
end;
...
begin
  ...
  begin
    for Index := 0 to IgnoredExceptions.Count - 1 do
      if ExceptionClass.InheritsFrom(TClass(IgnoredExceptions.Items[Index])) then
    begin
      Result := True;
      Break;
    end;
...
begin
  ...
  begin
    if JclHookExceptions and JclAddExceptNotifier(DoExceptNotify) then
    begin
...
finalization
  { TODO -oPV -cInvestigate : Calling JclStopExceptionTracking causes linking of various classes to
    ...
    safely because we need to be covered by IWJclHookExcept.Notifiers critical section }
