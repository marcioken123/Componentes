...
uses
  ...
  {$ENDIF ~HAS_UNITSCOPE}
  IWJclBase, IWJclFileUtils, IWJclSysInfo, IWJclWin32;
...
uses
  ...
  {$ENDIF HAS_UNIT_CHARACTER}
  {$ENDIF ~HAS_UNITSCOPE}
  IWJclResources, IWJclSysUtils, IWJclStrings, IWJclStringConversions;
...
begin
  if not Val then
    raise Exception.CreateRes(@RsDateConversion);
end;
...
var
  ...
  begin
    ...
    try
      ...
      begin
        F_FileName := IncludeTrailingPathDelimiter(ExtractFilePath(DirName)) + Se.Name;
        F_Process := True;
...
begin
  ...
  try
    ...
    for I := 0 to PathList.Count - 1 do
      ProcessDirectorySearch(IncludeTrailingPathDelimiter(Trim(PathList[I])) + '*.*');
  finally
    PathList.Free;
    FPeImage.Free;
  end;
...
{ TODO -cHelp : Author: Uwe Schuster(just a generic version of IWJclDebug.InsertDebugDataIntoExecutableFile) }
function PeInsertSection(const FileName: TFileName; SectionStream: TStream; SectionName: string): Boolean;
  ...
  begin
    ...
    try
      ...
      _StrPLCopy(PAnsiChar(@NewSection^.Name), UTF8Name, IMAGE_SIZEOF_SHORT_NAME);
      ...
      ImageStream.Position := NewSection^.PointerToRawData;
...
begin
  ...
  begin
    ...
    for I := 1 to NtHeaders^.FileHeader.NumberOfSections do
      if _StrLComp(PAnsiChar(@Header^.Name), P, IMAGE_SIZEOF_SHORT_NAME) = 0 then
      begin
        Result := Header;
        Break;
      end
...
begin
  ...
  begin
    ...
    for Result := Low(ImageSectionHeaders) to High(ImageSectionHeaders) do
      if _StrLComp(PAnsiChar(@ImageSectionHeaders[Result].Name), P, IMAGE_SIZEOF_SHORT_NAME) = 0 then
        Exit;
  end;
  Result := -1;
end;
...
begin
  ...
  begin
    ...
    if _StrIComp(CurrName, RefName) = 0 then
    begin
      {$IFDEF CPU32}
...
var
  ...
  procedure ReadRTTI;
  begin
    if _StrLComp(NameP, '$xp$', 4) = 0 then
    begin
      Inc(NameP, 4);
...
begin
  ...
  NameU^ := #0;
  SetLength(UTF8Unmangled, _StrLen(PAnsiChar(UTF8Unmangled)));
