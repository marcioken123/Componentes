...
const
  ...
  NativeTab = Char(#9);
  NativeLineFeed = IWJclBase.NativeLineFeed;
  NativeVerticalTab = Char(#11);
  NativeFormFeed = Char(#12);
  NativeCarriageReturn = IWJclBase.NativeCarriageReturn;
  NativeCrLf = IWJclBase.NativeCrLf;
  NativeSo = Char(#14);
  NativeSi = Char(#15);
  NativeDle = Char(#16);
  NativeDc1 = Char(#17);
  ...
  NativeLineBreak = IWJclBase.NativeLineBreak;
...
// IW: compatibility with AnsiStrings unit -> Lots of functions moved to AnsiStrings and deprecated under SysUtils, XE4+
function _StrPLCopy(Dest: PAnsiChar; const Source: AnsiString; MaxLen: Cardinal): PAnsiChar;
...
function _StrLen(const Str: PAnsiChar): Cardinal;
...
uses
  ...
  IWJclResources, {IWJclStreams,} IWJclSynch, IWJclSysUtils;

// IW: compatibility with AnsiStrings unit -> Lots of functions moved to AnsiStrings and deprecated under SysUtils, XE4+
function _StrPLCopy(Dest: PAnsiChar; const Source: AnsiString; MaxLen: Cardinal): PAnsiChar;
begin
  Result := {$IFDEF HAS_ANSISTRINGS_UNIT}AnsiStrings.{$ENDIF}StrPLCopy(Dest, Source, MaxLen);
end;

function _StrLIComp(const Str1, Str2: PAnsiChar; MaxLen: Cardinal): Integer;
begin
  Result := {$IFDEF HAS_ANSISTRINGS_UNIT}AnsiStrings.{$ENDIF}StrLIComp(Str1, Str2, MaxLen);
end;

function _StrLComp(const Str1, Str2: PAnsiChar; MaxLen: Cardinal): Integer;
begin
  Result := {$IFDEF HAS_ANSISTRINGS_UNIT}AnsiStrings.{$ENDIF}StrLComp(Str1, Str2, MaxLen);
end;

function _StrLen(const Str: PAnsiChar): Cardinal;
begin
  Result := {$IFDEF HAS_ANSISTRINGS_UNIT}AnsiStrings.{$ENDIF}StrLen(Str);
end;

function _StrIComp(const Str1, Str2: PAnsiChar): Integer;
begin
  Result := {$IFDEF HAS_ANSISTRINGS_UNIT}AnsiStrings.{$ENDIF}StrIComp(Str1, Str2);
end;
...
begin
  ...
  try
    ...
    Start := Pointer(S);
    repeat
      Done := IWJclStrings.StrWord(Start, Token);
      if Token <> '' then
        List.Add(Token);
    until Done;
  finally
