...
type
{$HPPEMIT 'namespace IWJclSysUtils'}
{$HPPEMIT '{'}
...
//  IWJclConsole,
  ...
  {$ENDIF HAS_UNIT_ANSISTRINGS}
  {$ENDIF ~HAS_UNITSCOPE}
  IWJclFileUtils, IWJclResources, IWJclStrings,
  IWJclStringConversions, IWJclSysInfo, IWJclWin32;
...
var
  NullDWORD: {$IFDEF RTL250_UP} PCardinal {$ELSE} PDWORD {$ENDIF};
  Res: DWORD;
begin
  NullDWORD := nil;
...
begin
  if Assigned(FOutputCallback) then
    Result := IWJclSysUtils.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutputCallback) = 0
  else
    Result := IWJclSysUtils.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutput) = 0;
end;
