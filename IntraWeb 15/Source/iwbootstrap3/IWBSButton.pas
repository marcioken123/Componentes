...
uses
  SysUtils, Classes, db, StrUtils, Controls, IWRenderContext, IWHTMLTag,
  IWBaseInterfaces, IWRegion, IWBSCustomControl, IWBSCommon,
...
uses
  ...
  IWBSInputCommon, IWBSInputForm, IWBSModal, IWBSCustomRegion, IWBSNavbar,
  IWBSList, IWBSRegion, IWBSButtonGroup;
...
procedure TIWBSButton.Dispose(ADispose: Boolean);
begin
  FDataParent := nil;
  FDataTarget := nil;
  inherited;
...
begin
  ...
  else if (DataTarget <> nil) and (DataTarget is TIWBSNavBarCollapse) then
  begin
...
begin
  ...
  try
    ...
    begin
      if lAnchor then
        AHTMLTag.AddStringParam('href', '#' + DataTarget.HTMLName)
      else
        AHTMLTag.AddStringParam('data-target', '#' + DataTarget.HTMLName);
      if DataParent <> nil then
        AHTMLTag.AddStringParam('data-parent', '#' + DataParent.HTMLName);
...
begin
  if (Parent is TIWBSRegion) and (TIWBSRegion(Parent).BSRegionType = bsrtDropDown) then
    Result := 'dropdown'
  else if (DataTarget = nil) then
    Result := ''
  else if (DataTarget is TIWBSModal) then
    Result := 'modal'
  else if (DataTarget is TIWBSCustomRegion) and TIWBSCustomRegion(DataTarget).Collapse then
    Result := 'collapse'
  else if (DataTarget is TIWBSNavBarCollapse) then
    Result := 'collapse';
end;
...
procedure TIWBSButton.SetDataParent(const Value: TIWCustomRegion);
begin
...
procedure TIWBSButton.SetDataTarget(const Value: TIWCustomRegion);
begin
