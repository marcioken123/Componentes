...
uses
  ...
  IWCGJQRegion, IWBSCommon, IWBSRegion, IWBSRegionCommon;
...
type
  ...
  published
    ...
    property BSRegionType: TIWBSRegionType read FRegionType write SetRegionType default bsrtContainer; {changed by fduenas}
...
begin
  ...
  // bootstrap settings
  FGridOptions := TIWBSGridOptions.Create(self); {changed by fduenas}
  FButtonGroupOptions := TIWBSButonGroupOptions.Create(Self);
  FPanelStyle := bspsDefault;
  FRegionType := bsrtContainer; {changed by fduenas}
