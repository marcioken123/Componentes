...
uses
  Classes, SysUtils, StrUtils, DesignEditors, DesignIntf, IWDsnPaintHandlers,
  StrEdit;
...
uses
  Windows, Forms, Dialogs, Graphics, glyphicons, IWBaseControl, IWBSLayoutMgr,
  IWBSControls, IWBSCustomInput, IWBSCustomRegion, IWBSRegion, IWBSInputForm,
  IWBSModal, IWBSInput, IWBSButton, IWBSDropDown, IWBSTabControl, IWBSNavBar,
  IWBSList, IWBSDeprecated, IWBSCommon, IWBSCustomControl, IWBSImage,
  IWBSMemoHTML, IWBSButtonGroup, IWDsnControlAbout;
...
begin
  ...
  with TIWBSCustomRegion(Control) do
  begin
    ...
    // draw css classes
    s := GetCssString;
    if Control is TIWBSFormControl then
    begin
      if s <> '' then
        s := ' ' + s;
      s := 'form-control' + s;
    ...
    begin
      ...
      if Control is TIWBSRegion then
        case TIWBSRegion(Control).TagType of
          bsttDiv, bsttP:
            ControlCanvas.Font.Height := -12;
          bsttH1:
            ControlCanvas.Font.Height := -36;
          bsttH2:
            ControlCanvas.Font.Height := -30;
          bsttH3:
            ControlCanvas.Font.Height := -24;
          bsttH4:
            ControlCanvas.Font.Height := -18;
          bsttH5:
...
begin
  ...
  if Control is TIWBSCustomInput then
  begin
    LMultiLine := False;
...
begin
  ...
  if not (Control is TIWBSCustomButton) then
    Exit;

  with TIWBSCustomButton(Control) do
  begin
...
begin
  ...
  if Control is TIWBSGlyphicon then
  begin
    LRect := Rect(0, 0, Control.Width, Control.Height);
...
var
  ...
  w: integer;
begin
  if Control is TIWBSRadioGroup then
  begin
    LRect := Rect(0, 0, Control.Width, Control.Height);
    ControlCanvas.Brush.Color := clWhite;
...
begin
  ...
  RegisterComponentEditor(TIWBSRegion, TIWControlTabOrderEditor);
  RegisterComponentEditor(TIWBSCustomDbControl, TIWControlTabOrderEditor);
end;
